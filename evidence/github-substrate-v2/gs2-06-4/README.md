...
empty. The complete tracked tree is checked for Dependabot, every official Renovate
repository filename (JSON, JSONC, JSON5, `.renovaterc`, GitHub and GitLab locations),
the root `package.json` `renovate` section, and custom config selectors in executable
workflow/script surfaces. Every discovered identity and content digest is sealed;
alternate/extra/custom configuration, non-Renovate authority, and branch/direct-push
automerge semantics are rejected. This evidence grants no workflow publication or
GitHub write.
