...
4. Commit the candidate and require a clean worktree. Run `roadmap-work manifest` with the candidate's tracked `--index`, explicit `--repo`, canonical UTC `--created-at YYYY-MM-DDTHH:MM:SSZ`, one or more tracked `--artifact name=path` values, and an ignored `--output artifacts/roadmap-work/<unit>/candidate.json`. The output state is `candidate`, not qualified or accepted.
5. Run `roadmap-work gates` with the exact manifest and reviewed catalog. Before starting any process, it requires the tracked candidate index and each selected catalog entry to match the unit's ordered command ID, Q-gate, and executable-plus-arguments digest. It stops on the first failure, rechecks the candidate/artifact bindings, and writes results beneath `artifacts/roadmap-work/`.
