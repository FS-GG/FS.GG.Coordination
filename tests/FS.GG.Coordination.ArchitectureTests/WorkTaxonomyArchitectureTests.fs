...
let ``work taxonomy signature precedes implementation and stays in pure core`` () =
    ...
    Assert.True(
        project.IndexOf("WorkTaxonomy.fsi", StringComparison.Ordinal) <
            project.IndexOf("WorkTaxonomy.fs\"", StringComparison.Ordinal)
    )

    let source =
        File.ReadAllText(Path.Combine(root, "src/FS.GG.Coordination.Core/WorkTaxonomy.fs"))
...
let ``frozen taxonomy corpus is complete and registered validator is offline`` () =
    let corpusPath =
        Path.Combine(root, "evidence/github-substrate-v2/gs2-05-1/corpus.json")
    ...
    let validator =
        File.ReadAllText(Path.Combine(root, "eng/validate-github-work-taxonomy.fsx"))
    ...
    Assert.Contains(
        "\"args\": [\"fsi\", \"eng/validate-github-work-taxonomy.fsx\", \"--\", \".\"]",
        gates,
...
let ``registered work taxonomy Q2 command passes repository local evidence`` () =
    ...
    for argument in [ "fsi"; "eng/validate-github-work-taxonomy.fsx"; "--"; "." ] do
    ...
    Assert.Contains(
        "github-work-taxonomy-contract OK cases=32 accepted=18 refused=14 q=Q2 network=offline inversions=17",
        output,
        StringComparison.Ordinal
    )
