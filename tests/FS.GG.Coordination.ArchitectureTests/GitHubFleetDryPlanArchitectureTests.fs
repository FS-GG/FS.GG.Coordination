...
let ``Q5 inventory is complete ordered and independently doubled`` () =
    ...
    let independent: GitHubFleetControlResult list =
        controls
        |> List.map (fun value ->
            {
                Control = value
...
let ``fleet dry plan contract exposes no production mutation boundary`` () =
    ...
    let signature =
        File.ReadAllText(
            Path.Combine(root, "src/FS.GG.Coordination.Qualification.Contracts/GitHubFleetDryPlanQualification.fsi")
        )
...
let ``registered Q5 command remains exact and final`` () =
    ...
    Assert.Contains(
        "\"args\": [\"fsi\", \"eng/validate-github-fleet-dry-plans.fsx\", \"--\", \".\"]",
        catalog,
    ...
    let previous =
        index.IndexOf("github-workflow-selection-supply-chain-contract", StringComparison.Ordinal)

    let fleet =
        index.IndexOf("github-fleet-dry-plans-contract", StringComparison.Ordinal)
