...
let ``retained qualification boundary produces the closed mutation matrix`` () =
    ...
    let observations =
        rootElement.GetProperty("observations").EnumerateArray() |> Seq.toList

    Assert.All(observations, fun item -> Assert.Equal("rejected", item.GetProperty("outcome").GetString()))

    let forged =
        observations
        |> List.filter (fun item -> item.GetProperty("mutationKind").GetString() = "forged")
    ...
    Assert.All(
        ...
        fun item ->
            Assert.Contains(
                item.GetProperty("diagnostics").EnumerateArray(),
                fun code -> code.GetString() = "HMP-FORGED-FINGERPRINT"
            )
    )
...
let ``proof coverage candidate and independent inputs cannot be asserted or substituted`` () =
    ...
    Assert.True(
        HarnessMutationProof.validate
            value
            (ReadOnlyMemory<byte>(inventory))
            (ReadOnlyMemory<byte>(changedBaseline))
            (ReadOnlyMemory<byte>(proof))
        |> Result.isError
    )
...
let ``accepted GS2-03.9 proof reproduces from exact protected evidence`` () =
    ...
    for argument in [ "fsi"; "work/119-gs2-03-9-mutation-proof/acceptance/validate.fsx" ] do
    ...
    Assert.Contains("proof=b4f6187b606e99e5bf787cf0dd94aa66c650c25c5f86a074c842ebbfd4cc995b", output)
    Assert.Contains("validatorBoundary=64e7fd87f4cb8e911c0c44384cb4277d9ce0b3a6ad3a2765cb4caf08d7a548c2", output)
    Assert.Equal("", error)
