...
let ``GS2-06-3 registration binds accepted predecessor and exact Q3 gate`` () =
    ...
    let unitValue =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "GS2-06.3")
    ...
    Assert.Equal(
        "c469645aeabd48bf3ca4fc9fc7ab4d054fd61869696d9416ca669c88c184a84d",
        unitValue.GetProperty("contractSha256").GetString()
    )

    let command =
        gates.RootElement.GetProperty("commands").EnumerateArray()
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "github-ruleset-plan-contract")

    Assert.Equal("Q3", command.GetProperty("qGate").GetString())

    Assert.Equal<string list>(
        [ "fsi"; "eng/validate-github-ruleset-plans.fsx"; "--"; "." ],
    ...
    use receipt =
        JsonDocument.Parse(read "evidence/github-substrate-v2/accepted/GS2-06.2.json")
...
let ``retained ruleset target is exact secure and census gated`` () =
    use corpus =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-3/corpus.json")

    use expected =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-3/independent-expectations.json")
...
let ``ruleset plan Q3 validator rejects its closed mutation inventory`` () =
    ...
    for argument in [ "fsi"; "eng/validate-github-ruleset-plans.fsx" ] do
    ...
    Assert.Contains(
        "GITHUB_RULESET_PLANS_OK repository=FS-GG/FS.GG.Coordination checks=6 mergeQueue=false bypass=0 exceptions=0 controls=26 seal=f16a8e3cde6e2a6e607e53b4e97c4165e895113aab9353bb1edb95a4fb5ffaea",
        output
    )
