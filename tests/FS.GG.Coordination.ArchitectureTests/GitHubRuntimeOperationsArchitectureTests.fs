...
let ``qualification adds no callable production command or network host`` () =
    let source =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubRuntimeOperationsQualification.fs"

    let signature =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubRuntimeOperationsQualification.fsi"
...
let ``Q3 and Q6 commands execute the retained qualification`` () =
    let q3, q3out, q3err =
        run "dotnet" [ "fsi"; "eng/validate-github-runtime-operations.fsx"; "--"; root ]
    ...
    let q6, q6out, q6err =
        run "dotnet" [ "fsi"; "eng/validate-github-runtime-recovery.fsx"; "--"; root; "--skip-cold" ]
...
let ``retained evidence records limitations and independent controls`` () =
    use report =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-07-8/qualification-report.json")

    Assert.Equal(
        "no-host-scheduled-audit-authoritative",
        report.RootElement.GetProperty("runtimeDisposition").GetString()
    )
    ...
    use generated =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-07-8/generated-controls.json")

    use independent =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-07-8/independent-controls.json")
    ...
    Assert.NotEqual(
        generated.RootElement.GetProperty("cases").GetRawText(),
        independent.RootElement.GetProperty("cases").GetRawText()
    )
