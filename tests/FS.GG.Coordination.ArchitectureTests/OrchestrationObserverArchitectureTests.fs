...
let ``observer assembly is inert and has no writer runner or App edge`` () =
    ...
    let references =
        ...
        |> Seq.map (fun value -> value.Attribute(XName.Get "Include").Value.Replace('\\', '/'))
        |> Seq.toList

    Assert.Equal<string list>(
        [
            "../FS.GG.Coordination.Core/FS.GG.Coordination.Core.fsproj"
            "../FS.GG.Coordination.GitHub/FS.GG.Coordination.GitHub.fsproj"
        ],
        references
    )
...
let ``observer composition public dependencies are read plan readback and typed journal only`` () =
    ...
    for forbidden in [ "IRunnerDispatcher"; "IProviderMutation"; "IWriterCredential" ] do
