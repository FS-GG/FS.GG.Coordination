...
let ``provider qualification is registered as the GS2-08-2 Q4 continuation`` () =
    ...
    Assert.Equal<string list>(["Q3";"Q4";"Q6"], unitValue.GetProperty("qGates").EnumerateArray() |> Seq.map _.GetString() |> Seq.toList)
    Assert.Equal<string list>(["github-ledger-protection-contract";"github-ledger-protection-provider-contract";"github-ledger-operational-contract"], unitValue.GetProperty("gateCommands").EnumerateArray() |> Seq.map _.GetString() |> Seq.toList)
