...
let ``provider adapter is pure read model with no transport credential or apply surface`` () =
    let text =
        read "src/FS.GG.Coordination.GitHub/LedgerProtectionProviderAdapter.fsi"
        + read "src/FS.GG.Coordination.GitHub/LedgerProtectionProviderAdapter.fs"
    ...
    for required in
        [
            ...
            "/orgs/FS-GG/installations"
            "/user/installations/"
            "FS-GG/.github/environments"
            "FS.GG.Coordination.Authority/issues"
        ] do
...
let ``provider corpus is explicitly sanitized and non operational`` () =
    use corpus =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-08-2/provider-observation-corpus.json")
...
let ``capture transport is fixed read only and retained evidence is sanitized`` () =
    ...
    for name, pass, continuity in
        [
            ("live-capture-pass1.json", 1, "uninitialized")
            ("live-capture-pass2.json", 2, "matched")
        ] do
        use document =
            JsonDocument.Parse(read ("evidence/github-substrate-v2/gs2-08-2/" + name))
        ...
        Assert.NotEqual(
            value.GetProperty("rawSetSha256").GetString(),
            value.GetProperty("normalizedSetSha256").GetString()
        )
...
let ``App identities are distinct and capture tests prove the private transport boundary`` () =
    use desired =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-08-2/desired-policy.json")
...
let ``independent provider validator passes from a fresh process`` () =
    let info =
        ProcessStartInfo("dotnet", "fsi eng/validate-github-ledger-protection-provider.fsx -- .")
...
let ``correspondence fixture binds real identities and no operational claim`` () =
    use fixture =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-08-2/provider-correspondence-fixtures.json")
...
let ``provider qualification is registered as the GS2-08-2 Q4 continuation`` () =
    ...
    let unitValue =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "GS2-08.2")
    ...
    Assert.Equal<string list>(
        [
            "github-ledger-protection-contract"
            "github-ledger-protection-provider-contract"
            "github-ledger-operational-contract"
        ],
        unitValue.GetProperty("gateCommands").EnumerateArray()
        |> Seq.map _.GetString()
        |> Seq.toList
    )
    ...
    let command =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "github-ledger-protection-provider-contract")

    Assert.Equal("Q4", command.GetProperty("qGate").GetString())

    Assert.Equal<string list>(
        [ "fsi"; "eng/validate-github-ledger-protection-provider.fsx"; "--"; "." ],
...
let ``post-install conformance remains sealed symbolic preparation`` () =
    ...
    use evidence =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-08-2/post-install-conformance.json")

    Assert.Equal("CurrentPreInstall", evidence.RootElement.GetProperty("currentState").GetString())

    Assert.False(
        evidence.RootElement.GetProperty("administrativePreparation").GetProperty("applyAuthorized").GetBoolean()
    )

    Assert.Equal(
        0,
        evidence.RootElement.GetProperty("administrativePreparation").GetProperty("providerWritesAttempted").GetInt32()
    )
