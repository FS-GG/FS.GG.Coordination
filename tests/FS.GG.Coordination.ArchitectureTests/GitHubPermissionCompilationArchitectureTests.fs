...
let private validator =
    Path.Combine(root, "eng/validate-github-permission-compilation.fsx")

let private runValidator candidateRoot =
    let info = ProcessStartInfo("dotnet")

    for argument in [ "fsi"; validator; "--"; candidateRoot ] do
...
let ``production Q3 rejects an independently isolated release crossover`` () =
    let temporary =
        Path.Combine(Path.GetTempPath(), "fsgg-permission-compilation-" + Guid.NewGuid().ToString("n"))
    ...
    try
            ...
            if
                not (
                    ...
                    |> Array.exists (fun value ->
                        value = ".git" || value = "bin" || value = "obj" || value = "artifacts")
                )
            then
        ...
        let corpus =
            Path.Combine(temporary, "evidence/github-substrate-v2/gs2-06-5/corpus.json")

        let changed =
            File.ReadAllText(corpus).Replace("\"environment\": \"release\"", "\"environment\": \"coordination\"")
...
let ``production Q3 rejects divergence from the canonical permission producer`` () =
    let temporary =
        Path.Combine(Path.GetTempPath(), "fsgg-permission-producer-" + Guid.NewGuid().ToString("n"))
    ...
    try
        ...
        let census =
            Path.Combine(temporary, "src/FS.GG.Coordination.Protocol/Generated/compiled-outputs/permission-census.json")
...
let ``accepted provider evidence is durable in the candidate Git tree`` () =
    ...
    let qualification =
        "artifacts/test-results/246-permission-compilation/qualification.trx"
    ...
    let evidence =
        File.ReadAllText(Path.Combine(root, "work/246-permission-compilation/evidence.yml"))
    ...
    Assert.Equal(
        5,
        evidence
            .Split("sha256:766aba973103662273625dca96ba9d0639d4a40c16840a5e51fe6a2dc31d1493", StringSplitOptions.None)
            .Length
        - 1
    )
