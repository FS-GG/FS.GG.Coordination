...
let ``lifecycle projection composes derived facts through the existing Project adapter only`` () =
    ...
    let implementation =
        read "src/FS.GG.Coordination.GitHub/LifecycleProjectionAdapter.fs"
...
let ``GS2-05-7 registration binds accepted predecessor and exact gate`` () =
    ...
    let unitValue =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "GS2-05.7")
    ...
    Assert.Equal<string list>(
        [ "github-lifecycle-projection-contract" ],
    ...
    let command =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "github-lifecycle-projection-contract")

    Assert.Equal("Q3", command.GetProperty("qGate").GetString())

    Assert.Equal<string list>(
        [ "fsi"; "eng/validate-github-lifecycle-projection.fsx"; "--"; "." ],
    ...
    for required in
        [
            "accepted GS2-05.6 receipt"
            "sole human or policy input"
...
let ``lifecycle projection evidence has independent closed controls`` () =
    use corpus =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-05-7/corpus.json")

    use independent =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-05-7/independent-expectations.json")
