...
let ``writer census qualification remains pure and claims no fence or receiver`` () =
    let signature =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubV1WriterCensusQualification.fsi"

    let implementation =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubV1WriterCensusQualification.fs"
...
let ``retained producer bytes and historical authority bindings are exact`` () =
    use binding =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-08-3/source-binding.json")
    ...
    Assert.Equal(
        value.GetProperty("commandContractSha256").GetString(),
        digest "evidence/github-substrate-v2/gs2-08-3/producer-command-contract.json"
    )

    Assert.Equal(
        "49c70359ebfbc00331ba90c7c5b100a292efa4cc95a5dfa8007867ceceec5c31",
        value.GetProperty("acceptedEpochReceiptDigest").GetString()
    )
...
let ``independent Q3 validator closes the writer census mutation inventory`` () =
    ...
    for argument in [ "fsi"; "eng/validate-github-v1-writer-census.fsx"; "--"; "." ] do
...
let ``retained command contract and census join by exact ordered name and write class`` () =
    use census =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-08-3/producer-v1-writer-census.json")

    use contract =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-08-3/producer-command-contract.json")
...
let ``GS2-08-3 registration preserves accepted epoch prerequisite and writer Q3 gate`` () =
    ...
    Assert.Equal<string list>(
        [ "fsi"; "eng/validate-github-v1-writer-census.fsx"; "--"; "." ],
