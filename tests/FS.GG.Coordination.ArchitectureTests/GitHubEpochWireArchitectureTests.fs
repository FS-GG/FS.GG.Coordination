...
let ``native unit binds the exact landed roadmap prerequisite and Q3 command`` () =
    ...
    Assert.Equal("2e553e41e58ee2f5e27aedcffc7403ce50e7cdd4", roadmap.GetProperty("revision").GetString())
    ...
    Assert.Equal<string list>(
        [ "fsi"; "eng/validate-github-epoch-wire.fsx"; "--"; "." ],
...
let ``wire qualification has no network writer or orchestration service`` () =
    ...
    Assert.DoesNotContain(
        "epoch-wire",
        read "src/FS.GG.Coordination.Cli/Program.fs",
...
let ``generated and independently authored controls qualify the same contract`` () =
    let code, output, error =
        run "dotnet" [ "fsi"; "eng/validate-github-epoch-wire.fsx"; "--"; root ]
    ...
    use generated =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-08-1/generated-controls.json")

    use independent =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-08-1/independent-controls.json")

    Assert.Equal(
        generated.RootElement.GetProperty("controls").GetRawText(),
        independent.RootElement.GetProperty("controls").GetRawText()
    )
