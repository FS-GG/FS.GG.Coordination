...
let private time minute =
    DateTimeOffset(2026, 8, 31, 0, minute, 0, TimeSpan.Zero)
...
let ``critique bundle binds five phases to one owner candidate and evidence set`` () =
    ...
    let candidateFingerprint =
        bundle.GetProperty("candidate").GetProperty("fingerprintSha256").GetString()
    ...
    Assert.Equal(
        5,
        findings
        |> List.map (fun item -> item.GetProperty("phaseId").GetString())
        |> List.distinct
        |> List.length
    )
...
let ``review schema evolves additively and storage policy executes v2`` () =
    let v1 =
        Path.Combine(root, "evidence/github-substrate-v2/schemas/v1/reviews.schema.json")

    let v2 =
        Path.Combine(root, "evidence/github-substrate-v2/schemas/v2/reviews.schema.json")

    Assert.True(File.Exists v1)
    Assert.True(File.Exists v2)

    let v1Digest =
        SHA256.HashData(File.ReadAllBytes v1)
        |> Convert.ToHexString
        |> _.ToLowerInvariant()
    ...
    use policy =
        JsonDocument.Parse(File.ReadAllBytes(Path.Combine(root, "evidence/github-substrate-v2/storage-policy.json")))
    ...
    Assert.Equal(
        "https://fs-gg.github.io/schemas/evidence/reviews/v2",
...
let ``critique architecture documents evidence authority and frozen diagnostics boundaries`` () =
    let text =
        File.ReadAllText(Path.Combine(root, "docs/architecture/critique-evidence.md"))
    ...
    for required in
        [
            ...
            "Neither layer consults GitHub approval counts"
        ] do
...
let ``accepted GS2-03.8 bundle reproduces from exact protected evidence`` () =
    ...
    for argument in [ "fsi"; "work/115-gs2-03-8-critique-evidence/acceptance/validate.fsx" ] do
