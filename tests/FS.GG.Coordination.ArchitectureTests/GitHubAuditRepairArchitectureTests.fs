...
let ``audit repair surface remains pure and repository local`` () =
    let source =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubAuditRepairQualification.fs"

    let signature =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubAuditRepairQualification.fsi"
...
let ``retained audit writer classification and control inventories are exact`` () =
    use contract =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-07-3/contract.json")
    ...
    Assert.Equal<string list>(
        GitHubAuditRepairQualification.requiredClassifications,
        strings "classifications" contract.RootElement
    )

    Assert.Equal<string list>(
        GitHubAuditRepairQualification.writerBoundary,
    ...
    let generatedIds, generatedCases, generatedContract =
        controls "evidence/github-substrate-v2/gs2-07-3/generated-controls.json"

    let independentIds, independentCases, independentContract =
        controls "evidence/github-substrate-v2/gs2-07-3/independent-controls.json"

    let expected =
        GitHubAuditRepairQualification.requiredControls
        |> List.map GitHubAuditRepairQualification.controlId
...
let ``Q3 audit repair validator executes all controls`` () =
    ...
    for argument in [ "fsi"; "eng/validate-github-audit-repair.fsx"; "--"; root ] do
