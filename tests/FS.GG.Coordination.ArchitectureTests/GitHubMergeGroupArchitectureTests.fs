...
let ``merge-group surface is pure and repository local`` () =
    ...
    let signature =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubMergeGroupQualification.fsi"
...
let ``retained merge-group control inventories are exact and independent`` () =
    ...
    let generatedIds, generatedCases, generatedContract =
        inventory "evidence/github-substrate-v2/gs2-07-5/generated-controls.json"

    let independentIds, independentCases, independentContract =
        inventory "evidence/github-substrate-v2/gs2-07-5/independent-controls.json"

    let expected =
        GitHubMergeGroupQualification.requiredControls
        |> List.map GitHubMergeGroupQualification.controlId
...
let ``Q3 merge-group validator executes every control`` () =
    ...
    for argument in [ "fsi"; "eng/validate-github-merge-group-support.fsx"; "--"; root ] do
    ...
    Assert.Contains(
        "GITHUB_MERGE_GROUP_OK disposition=merge-group-qualified controls=31",
        output,
