...
let ``release hardening public surface is pure and mutation free`` () =
    let surface =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubReleaseHardeningQualification.fsi"
...
let ``retained release corpus carries exact hardening inventory`` () =
    use corpus =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-6/corpus.json")
    ...
    use expectations =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-6/independent-expectations.json")
...
let ``roadmap and gate catalog register exact GS2-06.6 command`` () =
    ...
    let unitValue =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "GS2-06.6")

    let command =
        catalog.RootElement.GetProperty("commands").EnumerateArray()
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "github-release-hardening-contract")
...
let ``exact release hardening Q3 validator passes`` () =
    ...
    for argument in [ "fsi"; "eng/validate-github-release-hardening.fsx"; "--"; "." ] do
        startInfo.ArgumentList.Add(argument)
...
let ``accepted release hardening provider evidence is durable in the candidate Git tree`` () =
    ...
    Assert.Equal(
        10,
        evidence
            .Split("sha256:892127d103cd4cad4e6f253ba8363bbdf6be29df57a0b5975583d798bc20ce7d", StringSplitOptions.None)
            .Length
        - 1
    )
