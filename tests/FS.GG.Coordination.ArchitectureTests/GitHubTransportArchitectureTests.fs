...
let ``transport and qualification signatures precede implementations`` () =
    ...
    Assert.True(
        githubProject.IndexOf("Transport.fsi", StringComparison.Ordinal) <
            githubProject.IndexOf("Transport.fs\"", StringComparison.Ordinal)
    )
    ...
    Assert.True(
        qualificationProject.IndexOf("GitHubTransportQualification.fsi", StringComparison.Ordinal) <
            qualificationProject.IndexOf("GitHubTransportQualification.fs\"", StringComparison.Ordinal)
    )
...
let ``independent control inventory is closed and mutations really red`` () =
    ...
    | Error findings ->
        Assert.Contains(
            findings,
            fun finding -> finding.Code = "GTQ-INDEPENDENT-NOT-RED" && finding.ControlId = "truncation"
        )
...
let ``registered Q3 validator is offline and runs both producers`` () =
    let validator =
        File.ReadAllText(Path.Combine(root, "eng/validate-github-transport.fsx"))
    ...
    Assert.Contains(
        "\"args\": [\"fsi\", \"eng/validate-github-transport.fsx\", \"--\", \".\"]",
        catalog,
