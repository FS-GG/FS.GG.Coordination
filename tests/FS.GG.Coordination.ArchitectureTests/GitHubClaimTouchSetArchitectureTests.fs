...
let ``claim adapter is additive pure and composes canonical journal primitives`` () =
    ...
    for required in
        [
            ...
            "ShardedJournalAdapter.planConflict"
        ] do
...
let ``claim authority and projection hints are structurally separate`` () =
    ...
    let authorityStart =
        signature.IndexOf("type ClaimAuthorityObservation", StringComparison.Ordinal)

    let projectionStart =
        signature.IndexOf("type ClaimProjectionHints", StringComparison.Ordinal)

    let acquireStart =
        signature.IndexOf("type ClaimAcquireIntent", StringComparison.Ordinal)

    Assert.True(
        authorityStart >= 0
        && projectionStart > authorityStart
        && acquireStart > projectionStart
    )

    let projectionBlock =
        signature.Substring(projectionStart, acquireStart - projectionStart)
...
let ``claim qualification evidence has independent closed controls`` () =
    use corpus =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-05-5/corpus.json")

    use independent =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-05-5/independent-expectations.json")
