...
let ``receiver census qualification is pure source evidence and cannot apply`` () =
    let signature =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubV1ReceiverCensusQualification.fsi"

    let implementation =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubV1ReceiverCensusQualification.fs"
...
let ``retained landed producer evidence is byte exact`` () =
    use binding =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-08-3/receiver-source/source-binding.json")
    ...
    Assert.Equal(
        value.GetProperty("censusSha256").GetString(),
        digest "evidence/github-substrate-v2/gs2-08-3/receiver-source/producer-v1-writer-receiver-census.json"
    )

    Assert.Equal(
        value.GetProperty("sourceManifestsSha256").GetString(),
        digest "evidence/github-substrate-v2/gs2-08-3/receiver-source/producer-source-manifests.json.gz"
    )

    Assert.Equal(
        value.GetProperty("sourceBlobsSha256").GetString(),
        digest "evidence/github-substrate-v2/gs2-08-3/receiver-source/producer-source-blobs.json.gz"
    )
...
let ``independent Q3 validator closes receiver-source controls`` () =
    ...
    for argument in [ "fsi"; "eng/validate-github-v1-receiver-census.fsx"; "--"; "." ] do
...
let ``GS2-08-3 registration adds receiver census Q3 without changing prerequisite`` () =
    ...
    Assert.Equal<string list>(
        [ "github-v1-writer-census-contract"; "github-v1-receiver-census-contract" ],
    ...
    let gateContract =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "github-v1-receiver-census-contract")

    Assert.Equal(
        "fbff7ac12feadedf1fc5a440848bfecfd4ac799e1c8bc45cc15fed666b68d0e9",
        gateContract.GetProperty("commandSha256").GetString()
    )
    ...
    Assert.Equal<string list>(
        [ "fsi"; "eng/validate-github-v1-receiver-census.fsx"; "--"; "." ],
