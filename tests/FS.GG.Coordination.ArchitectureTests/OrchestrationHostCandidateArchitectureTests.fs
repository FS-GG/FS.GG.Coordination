...
let ``host project is a locked Linux x64 self contained single file`` () =
    let project =
        File.ReadAllText(
            Path.Combine(root, "src/FS.GG.Coordination.Orchestration.Host/FS.GG.Coordination.Orchestration.Host.fsproj")
        )
    ...
    let lockText =
        File.ReadAllText(Path.Combine(root, "src/FS.GG.Coordination.Orchestration.Host/packages.lock.json"))
...
let ``runner client candidate workflow and archive controls pass mutation self test`` () =
    ...
    for argument in [ "eng/orchestration-runner-client-candidate.py"; "self-test"; "--repo"; "." ] do
        start.ArgumentList.Add argument
...
let ``runner client is separately locked and composes only the selected executor provider`` () =
    ...
    let source =
        File.ReadAllText(Path.Combine(root, "src/FS.GG.Coordination.Orchestration.Runner.Client/Program.fs"))
    ...
    Assert.Contains(
        "net10.0/linux-x64",
        File.ReadAllText(Path.Combine(root, "src/FS.GG.Coordination.Orchestration.Runner.Client/packages.lock.json")),
...
let ``runner candidate media type is identical across protocol host and durable store`` () =
    ...
    for relative in
        [
            "src/FS.GG.Coordination.Core/Orchestration.fs"
            "src/FS.GG.Coordination.Orchestration.Host/RunnerWireRuntime.fs"
            "src/FS.GG.Coordination.Orchestration.PostgreSql/PostgreSqlStore.fs"
        ] do
        let source = File.ReadAllText(Path.Combine(root, relative))
        Assert.Contains(expected, source, StringComparison.Ordinal)

        for value in legacy do
...
let ``combined orchestration bundle binds both protected main executables`` () =
    ...
    for argument in [ "eng/orchestration-container-bundle.py"; "self-test" ] do
        start.ArgumentList.Add argument
    ...
    let workflow =
        File.ReadAllText(Path.Combine(root, ".github/workflows/orchestration-container-bundle.yml"))

    for binding in
        [
            "github.ref == 'refs/heads/main'"
            "orchestration-host-candidate.py prepare"
            "orchestration-runner-client-candidate.py prepare"
            "orchestration-container-bundle.py assemble"
            "orchestration-container-bundle.py verify"
            "compression-level: 0"
        ] do
        Assert.Contains(binding, workflow, StringComparison.Ordinal)

    let documentation =
        File.ReadAllText(Path.Combine(root, "docs/architecture/orchestration-container-bundle.md"))

    for binding in
        [
            "two rootless Podman containers"
            "host/fsgg-coord-orchestration-host"
            "runner/fsgg-coord-orchestration-runner"
            "no Podman socket"
            "no HTTP executor relay"
            "`podman exec`"
            "development-container dependency"
            "actor remoting"
        ] do
        Assert.Contains(binding, documentation, StringComparison.Ordinal)

    let program =
        File.ReadAllText(Path.Combine(root, "src/FS.GG.Coordination.Orchestration.Host/Program.fs"))

    let transport =
        File.ReadAllText(Path.Combine(root, "src/FS.GG.Coordination.Orchestration.Host/LocalExecutorTransport.fs"))

    for binding in [ "LocalExecutorTransport"; "serveMainLocal" ] do
        Assert.Contains(binding, program, StringComparison.Ordinal)
