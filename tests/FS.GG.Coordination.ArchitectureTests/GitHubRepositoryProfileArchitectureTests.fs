...
let ``repository profiles are a pure desired-state compiler with no apply path`` () =
    ...
    let implementation =
        read "src/FS.GG.Coordination.GitHub/RepositoryProfileAdapter.fs"
    ...
    for required in
        [
            ...
            "fsgg_role"
            "fsgg_owner_scope"
            "fsgg_coordination_mode"
            "canonicalRosterDigest"
            "SHA256.HashData"
        ] do
...
let ``GS2-06-1 registration binds terminal prerequisites and exact Q3 gate`` () =
    ...
    let unitValue =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "GS2-06.1")
    ...
    let command =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "github-repository-profile-contract")

    Assert.Equal("Q3", command.GetProperty("qGate").GetString())

    Assert.Equal<string list>(
        [ "fsi"; "eng/validate-github-repository-profiles.fsx"; "--"; "." ],
    ...
    Assert.Equal(
        "6de48b57654910625bf65f0bc4d30a8b399dbf0aa9e6d9e6c79b7a209f53a89e",
...
let ``repository profile evidence preserves all reviewed rows and the external boundary`` () =
    use roster =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-1/roster.json")

    use corpus =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-1/corpus.json")

    use expected =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-1/independent-expectations.json")
    ...
    Assert.Equal(
        ...
        roster.RootElement.GetProperty("source").GetProperty("artifactSha256").GetString()
    )

    let external =
        rows.EnumerateArray()
        |> Seq.find (fun value -> value.GetProperty("fullName").GetString() = "EHotwagner/S.I.R.")
...
let ``repository profile Q3 validator rejects its closed mutation inventory`` () =
    ...
    for argument in [ "fsi"; "eng/validate-github-repository-profiles.fsx" ] do
    ...
    Assert.Contains(
        "GITHUB_REPOSITORY_PROFILES_OK repositories=10 organization=9 external=1 properties=27 controls=16 seal=f3524e8edbd6b88b0783551c14377881dee5dd958ebd4835d77a57913d30d74b",
        output
    )
