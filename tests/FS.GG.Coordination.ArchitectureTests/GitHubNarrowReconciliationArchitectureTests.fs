...
let ``narrow reconciliation surface remains pure and repository local`` () =
    let source =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubNarrowReconciliationQualification.fs"

    let signature =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubNarrowReconciliationQualification.fsi"
...
let ``retained event writer and control inventories are exact`` () =
    use contract =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-07-2/contract.json")

    let strings (property: string) (node: JsonElement) =
        node.GetProperty(property).EnumerateArray()
        |> Seq.map _.GetString()
        |> Seq.toList

    Assert.Equal<string list>(
        GitHubNarrowReconciliationQualification.supportedEventKinds,
        strings "eventKinds" contract.RootElement
    )

    Assert.Equal<string list>(
        GitHubNarrowReconciliationQualification.writerBoundary,
        strings "writerBoundary" contract.RootElement
    )
    ...
    let controls relative =
        ...
        document.RootElement.GetProperty("caseContract").GetString()

    let generatedIds, generatedCases, generatedContract =
        controls "evidence/github-substrate-v2/gs2-07-2/generated-controls.json"

    let independentIds, independentCases, independentContract =
        controls "evidence/github-substrate-v2/gs2-07-2/independent-controls.json"

    let expected =
        GitHubNarrowReconciliationQualification.requiredControls
        |> List.map GitHubNarrowReconciliationQualification.controlId
...
let ``Q3 narrow reconciliation validator executes all controls`` () =
    ...
    for argument in [ "fsi"; "eng/validate-github-narrow-reconciliation.fsx"; "--"; root ] do
