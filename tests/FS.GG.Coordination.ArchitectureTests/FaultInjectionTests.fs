...
let private artifactRelative =
    "src/FS.GG.Coordination.Qualification.Contracts/Generated/fault-injection.json"

let private artifactPath = Path.Combine(root, artifactRelative)

let private parseArtifact () =
    JsonNode.Parse(File.ReadAllBytes artifactPath).AsObject()
...
let ``committed matrix is deterministic complete and source bound`` () =
    ...
    Assert.Equal("14367ea444517c31d2af0a5ba244f91d6a9b73ed06f52b89c5efae0a0df68646", summary.SelfSha256)
...
let ``every modeled external step has before and after convergence`` () =
    ...
    let steps =
        document["externalSteps"].AsArray()
...
let ``independent oracle accepts the executed subject and rejects every subject defect`` () =
    let executions =
        FaultInjection.execute root FaultInjection.SubjectDefect.None
...
let ``semantic oracle rejects duplicate and reorder defects even with healthy labels restored`` () =
    ...
    let healthyById =
        healthy |> List.map (fun execution -> execution.Id, execution) |> Map.ofList
    ...
    for defect, id, expected in probes do
        let forged =
            ...
            |> List.map (fun execution ->
                if execution.Id = id then
                    { execution with
                        Trace = healthyById[id].Trace
                    }
                else
                    execution)

        Assert.NotEqual(
            healthyById[id].FinalStateSha256,
            (forged |> List.find (fun execution -> execution.Id = id)).FinalStateSha256
        )
