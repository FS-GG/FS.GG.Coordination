...
let ``required check census is pure and exposes no plan or apply path`` () =
    ...
    let implementation =
        read "src/FS.GG.Coordination.GitHub/RequiredCheckCensusAdapter.fs"
...
let ``GS2-06-2 registration binds accepted predecessor and exact Q3 gate`` () =
    ...
    let unitValue =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "GS2-06.2")
    ...
    Assert.Equal(
        "2c27bbe68be5ce3767bb87bfaa1c42c01290a3d79b041f518335fef84daed030",
        unitValue.GetProperty("contractSha256").GetString()
    )

    let command =
        gates.RootElement.GetProperty("commands").EnumerateArray()
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "github-required-check-census-contract")

    Assert.Equal("Q3", command.GetProperty("qGate").GetString())

    Assert.Equal<string list>(
        [ "fsi"; "eng/validate-github-required-check-census.fsx"; "--"; "." ],
    ...
    let receipt =
        JsonDocument.Parse(read "evidence/github-substrate-v2/accepted/GS2-06.1.json")
...
let ``required check census evidence separates complete provenance from stable aggregates`` () =
    use corpus =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-2/corpus.json")

    use expected =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-2/independent-expectations.json")

    use authorities =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-2/authorities.json")
    ...
    Assert.Equal(
        sha256Text (read "evidence/github-substrate-v2/gs2-06-2/authorities.json"),
        corpus.RootElement.GetProperty("authorityEvidenceSha256").GetString()
    )
...
let ``required check census Q3 validator rejects its closed mutation inventory`` () =
    ...
    for argument in [ "fsi"; "eng/validate-github-required-check-census.fsx" ] do
    ...
    Assert.Contains(
        "GITHUB_REQUIRED_CHECK_CENSUS_OK repository=FS-GG/FS.GG.Coordination required=6 classicOnly=0 rulesetOnly=6 dual=0 controls=22 seal=db294ff75dbfb97a81433331ac7d696a0321a3433a8dd6b29694cbebf37396a3",
        output
    )
