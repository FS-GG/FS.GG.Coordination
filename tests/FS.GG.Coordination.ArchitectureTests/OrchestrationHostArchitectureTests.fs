...
let ``administration host is separate and serve path is migration free`` () =
    ...
    Assert.Equal(
        "fsgg-coord-orchestration-host",
        project.Descendants(XName.Get "AssemblyName") |> Seq.exactlyOne |> _.Value
    )
    ...
    let references =
        ...
        |> Seq.map (_.Attribute(XName.Get "Include") >> _.Value.Replace('\\', '/'))
        |> Seq.toList
    ...
    let initialization =
        read "src/FS.GG.Coordination.Orchestration.Host/HostInitialization.fs"
