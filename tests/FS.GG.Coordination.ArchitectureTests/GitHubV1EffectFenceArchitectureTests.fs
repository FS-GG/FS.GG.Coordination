...
let ``common fence has only injected read and effect boundaries`` () =
    ...
    Assert.True(
        implementation.IndexOf("reader.ReadFresh()", StringComparison.Ordinal) <
            implementation.IndexOf("effect.ApplyOnce request", StringComparison.Ordinal)
    )
...
let ``GS2-08-4 registration binds accepted epoch and landed census gates directly`` () =
    ...
    let unitValue =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "GS2-08.4")
    ...
    let command =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "github-v1-effect-fence-contract")
...
let ``retained census dependencies are byte exact`` () =
    use binding =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-08-4/source-binding.json")
    ...
    Assert.Equal(
        writer.GetProperty("sourceBindingSha256").GetString(),
        digest "evidence/github-substrate-v2/gs2-08-3/source-binding.json"
    )

    Assert.Equal(
        writer.GetProperty("censusSha256").GetString(),
        digest "evidence/github-substrate-v2/gs2-08-3/producer-v1-writer-census.json"
    )

    Assert.Equal(
        receiver.GetProperty("sourceBindingSha256").GetString(),
        digest "evidence/github-substrate-v2/gs2-08-3/receiver-source/source-binding.json"
    )

    Assert.Equal(
        receiver.GetProperty("censusSha256").GetString(),
...
let ``independent Q3 validator passes without provider access`` () =
    ...
    for argument in [ "fsi"; "eng/validate-github-v1-effect-fence.fsx"; "--"; "." ] do
