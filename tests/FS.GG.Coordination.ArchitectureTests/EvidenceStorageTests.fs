...
let ``evidence storage contract and all independent negative cases pass`` () =
    ...
    Assert.Contains("EVIDENCE_STORAGE_OK categories=12 entries=93 maxTrackedBytes=65536 frozenCorpusCases=21 observed=2 unobserved=19 aggregate=bf38fc3d426e74237561798d9f3b9fa5dd1b94b487e69f1565cc9cc6ab58c753", output)
...
[<Fact>]
let ``GS2-06.8 acceptance is indexed and accepted by the roadmap prerequisite reader`` () =
    let indexPath = Path.Combine(root, "eng/github-substrate-v2-units.json")
    let index = JsonNode.Parse(File.ReadAllText(indexPath)).AsObject()
    let units = index["units"].AsArray()
    ...
    match
        RoadmapWork.checkPrerequisites
            (ReadOnlyMemory<byte>(indexBytes))
            (ReadOnlyMemory<byte>(roadmapBytes))
            [ receipt "GS2-06.7"; receipt "GS2-06.8" ]
            "GS2-06.8"
    with
    | Ok status ->
        Assert.True(status.Ready)
    ...
    let tampered = JsonNode.Parse(File.ReadAllBytes(receiptPath "GS2-06.8")).AsObject()
    tampered["digest"] <- String.replicate 64 "0"
...
[<Fact>]
let ``GS2-07.1 acceptance is indexed and accepted by the roadmap prerequisite reader`` () =
    let indexPath = Path.Combine(root, "eng/github-substrate-v2-units.json")
    let index = JsonNode.Parse(File.ReadAllText(indexPath)).AsObject()
    let units = index["units"].AsArray()
    ...
    match
        RoadmapWork.checkPrerequisites
            (ReadOnlyMemory<byte>(indexBytes))
            (ReadOnlyMemory<byte>(roadmapBytes))
            [ receipt "GS2-06.8"; receipt "GS2-07.1" ]
            "GS2-07.1"
    with
    | Ok status ->
        Assert.True(status.Ready)
...
[<Fact>]
let ``GS2-07.2 acceptance is indexed and accepted by the roadmap prerequisite reader`` () =
    let indexPath = Path.Combine(root, "eng/github-substrate-v2-units.json")
    let index = JsonNode.Parse(File.ReadAllText(indexPath)).AsObject()
    let units = index["units"].AsArray()
    ...
    match
        RoadmapWork.checkPrerequisites
            (ReadOnlyMemory<byte>(indexBytes))
            (ReadOnlyMemory<byte>(roadmapBytes))
            [ receipt "GS2-07.1"; receipt "GS2-07.2" ]
            "GS2-07.2"
    with
    | Ok status ->
        Assert.True(status.Ready)
