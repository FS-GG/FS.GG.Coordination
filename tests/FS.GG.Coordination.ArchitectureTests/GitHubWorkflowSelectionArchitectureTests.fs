...
let ``workflow selection public surface is pure and mutation free`` () =
    let surface =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubWorkflowSelectionQualification.fsi"
...
let ``production selector CLI handles arbitrary and merge-group inputs and fails stale authority closed`` () =
    let common request currentSettings currentQueuedHead =
        [
            ...
            request
            "--current-base"
            "6d3b7662ac4d9474a9976ac093ec910f55fb6087"
            "--current-settings"
            currentSettings
            "--current-queued-head"
            currentQueuedHead
            "--expected-inventory-version"
            "coordination-workflows/1"
            "--expected-graph-version"
            "fsgg.workflow-impact/1"
            "--expected-seal"
            "2ff268103734c9f14d80302575aea4996c1a040a125b7f4356880efde90b5d5a"
        ]

    let goodCode, goodOutput, goodError =
        runCli (
            common
                "evidence/github-substrate-v2/gs2-06-7/runtime-request-arbitrary.json"
                "5c7cd805ec9924c1895749df66fc0fd49eedbfeadd8721baafd75ced79a89518"
                "none"
        )
    ...
    let mergeCode, mergeOutput, _ =
        runCli (
            common
                "evidence/github-substrate-v2/gs2-06-7/runtime-request-merge-group.json"
                "5c7cd805ec9924c1895749df66fc0fd49eedbfeadd8721baafd75ced79a89518"
    ...
    let inventedVersion =
        common
            ...
            "none"
        |> List.map (fun value ->
            if value = "coordination-workflows/1" then
                "invented-v999"
            else
                value)
    ...
    let duplicateRequest =
        Path.Combine(Path.GetTempPath(), $"fsgg-duplicate-selector-{Guid.NewGuid():N}.json")
    ...
    let authority =
        Path.Combine(Path.GetTempPath(), $"fsgg-selector-authority-{Guid.NewGuid():N}.json")

    File.WriteAllText(
        authority,
        """{"schema":"fsgg.coordination.workflow-selection-authority/1","baseRevision":"6d3b7662ac4d9474a9976ac093ec910f55fb6087","currentRevision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","settingsSha256":"5c7cd805ec9924c1895749df66fc0fd49eedbfeadd8721baafd75ced79a89518","queuedHead":"none"}"""
    )
    ...
    File.WriteAllText(
        authority,
        File.ReadAllText(authority).Replace("6d3b7662ac4d9474a9976ac093ec910f55fb6087", String.replicate 40 "d")
    )
    ...
    File.WriteAllText(
        authority,
        File
            .ReadAllText(authority)
            .Replace(String.replicate 40 "d", "6d3b7662ac4d9474a9976ac093ec910f55fb6087")
            .Replace("5c7cd805ec9924c1895749df66fc0fd49eedbfeadd8721baafd75ced79a89518", String.replicate 64 "e")
    )
...
let ``sentinel authority survives unrelated protected advances and rejects stale paired evidence`` () =
    let temporary =
        Path.Combine(Path.GetTempPath(), $"fsgg-gs267-authority-{Guid.NewGuid():N}")
    ...
    File.WriteAllText(
        inventory,
        (read "evidence/github-substrate-v2/gs2-06-7/runtime-inventory.json").Replace(oldBase, initial)
    )

    let sealCode, sealOutput, sealError =
        runCli [ "workflow-select"; "--seal-inventory"; inventory ]
    ...
    File.WriteAllText(
        ...
        (read "evidence/github-substrate-v2/gs2-06-7/runtime-request-sentinel.json")
            .Replace(oldBase, initial)
            .Replace(oldSeal, inventorySeal)
    )
    ...
    let firstCode, firstOutput, firstError =
        resolve [ "GITHUB_SHA", current; "GITHUB_EVENT_NAME", "schedule" ]
    ...
    Assert.Equal(
        "5c7cd805ec9924c1895749df66fc0fd49eedbfeadd8721baafd75ced79a89518",
        first.RootElement.GetProperty("settingsSha256").GetString()
    )
    ...
    let forgedCode, _, _ =
        resolve [ "GITHUB_SHA", reviewedCommit; "GITHUB_EVENT_NAME", "schedule" ]
    ...
    File.WriteAllText(
        settingsPath,
        File
            .ReadAllText(settingsPath)
            .Replace("5c7cd805ec9924c1895749df66fc0fd49eedbfeadd8721baafd75ced79a89518", "stale")
    )

    let settingsCode, _, _ =
        resolve [ "GITHUB_SHA", current; "GITHUB_EVENT_NAME", "schedule" ]
    ...
    Assert.Contains(
        "tests/Rollover.fs",
        mergeRequest.RootElement.GetProperty("changedPaths").EnumerateArray()
        |> Seq.map _.GetString()
    )
...
let ``repository owns callable reusable composite aggregate and sentinel contracts`` () =
    ...
    let provisionIndex =
        sentinelScript.IndexOf("provision_pinned_quint \"$decision\"", StringComparison.Ordinal)

    let testIndex =
        sentinelScript.IndexOf("find tests -name '*Tests.fsproj'", StringComparison.Ordinal)

    Assert.True(
        provisionIndex < testIndex,
        "the full suite must provision pinned Quint before architecture tests start"
    )

    Assert.Contains("run-private-postgres.sh", sentinelScript)
    Assert.Contains("FSGG_PG_MODE=docker", sentinelScript)
    Assert.Contains("output_root=\"$(cd \"$output_root\" && pwd -P)\"", sentinelScript)
...
let ``sentinel consumes the typed Q7 missed-obligation decision and disables selection`` () =
    let temporary =
        Path.Combine(Path.GetTempPath(), $"fsgg-gs267-sentinel-{Guid.NewGuid():N}")
    ...
    let authorityFailureDecision =
        Path.Combine(authorityFailureRoot, "workflow-selection-sentinel/decision.json")

    let authorityFailureCode, authorityFailureOutput, authorityFailureError =
        runBashAt
            root
            [
                "RUNNER_TEMP", authorityFailureRoot
                "GITHUB_SHA", String.replicate 40 "0"
    ...
    let q7Output =
        q7Child.StandardOutput.ReadToEnd() + q7Child.StandardError.ReadToEnd()
    ...
    Assert.Contains(
        "release",
        decision.RootElement.GetProperty("missedObligations").EnumerateArray()
        |> Seq.map _.GetString()
    )
    ...
    let invalidCases =
        [
            "non-hex-seal",
            """{"schema":"fsgg.coordination.workflow-selection-supply-chain-decision/1","seal":"zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz","missedObligations":[],"fleetSelection":"eligible","productionMutation":false}"""
            "unknown-obligation",
            """{"schema":"fsgg.coordination.workflow-selection-supply-chain-decision/1","seal":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","missedObligations":["invented"],"fleetSelection":"disabled","productionMutation":false}"""
            "duplicate-obligation",
            """{"schema":"fsgg.coordination.workflow-selection-supply-chain-decision/1","seal":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","missedObligations":["release","release"],"fleetSelection":"disabled","productionMutation":false}"""
            "duplicate-member",
            """{"schema":"fsgg.coordination.workflow-selection-supply-chain-decision/1","seal":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","missedObligations":["release"],"missedObligations":[],"fleetSelection":"eligible","productionMutation":false}"""
            "extra-property",
            """{"schema":"fsgg.coordination.workflow-selection-supply-chain-decision/1","seal":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","missedObligations":[],"fleetSelection":"eligible","productionMutation":false,"invented":true}"""
        ]
    ...
    for name, payload in invalidCases do
        ...
        let code, stdout, stderr =
            runBash [ "eng/workflow-selection-sentinel.sh"; "--decision-only"; input; output ]
    ...
    Assert.Contains(
        "release",
        currentMiss.RootElement.GetProperty("missedObligations").EnumerateArray()
        |> Seq.map _.GetString()
    )
...
let ``stable aggregate correlates every child result with its selection disposition`` () =
    let passing =
        [
            "eng/workflow-selection-aggregate.sh"
            "success"
            "selected"
            "success"
            "selected"
    ...
    let skippedSelected =
        passing |> List.mapi (fun index value -> if index = 3 then "skipped" else value)
...
let ``original GS2-06-7 receipt remains byte immutable during repair`` () =
    Assert.Equal(
        "9a98a13213c9a6934b362a6cb75dc3b523800205961e76cd4de984157733dc0b",
        shaFile "evidence/github-substrate-v2/accepted/GS2-06.7.json"
    )

    use receipt =
        JsonDocument.Parse(read "evidence/github-substrate-v2/accepted/GS2-06.7.json")
...
let ``retained corpus carries complete impact sentinel and fleet evidence`` () =
    use corpus =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-7/corpus.json")
    ...
    use deletionLedger =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-7/deletion-ledger.json")
    ...
    Assert.Equal(
        "396645252215e6ff1d904d15cd3667530bf30ce93482df735a9f3ae94c8a5439",
        value.GetProperty("observationSha256").GetString()
    )

    use observations =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-7/observed-workflow-runs.json")

    Assert.Equal(10, observations.RootElement.GetProperty("repositories").GetArrayLength())

    Assert.All(
        observations.RootElement.GetProperty("repositories").EnumerateArray(),
        fun repository -> Assert.Equal(8, repository.GetProperty("runs").GetArrayLength())
    )

    use expectations =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-06-7/independent-expectations.json")
...
let ``roadmap registers distinct exact Q3 and Q7 commands`` () =
    ...
    for id in
        ...
        let command =
            ...
            |> Seq.find (fun value -> value.GetProperty("id").GetString() = id)

        let components =
            command.GetProperty("executable").GetString()
            :: (command.GetProperty("args").EnumerateArray()
                |> Seq.map _.GetString()
                |> List.ofSeq)
        ...
        let digest =
            contracts
            |> Seq.find (fun value -> value.GetProperty("id").GetString() = id)
            |> _.GetProperty("commandSha256").GetString()
...
let ``workflow selection provider evidence is durable in the candidate Git tree`` () =
    let expected =
        [
            "readiness/262-workflow-selection/analysis.json",
            "2dc4c5c43ee8430303a22ceb38c3f33b2bba0d11016b8b5f90d6ada6edf3c83d"
            ...
            "readiness/262-workflow-selection/work-model.json",
            "f22fff614e7d8ee1e7fd024f9f8620068d02e1c13d771ff6a5e940dd56ffc04a"
            "readiness/262-workflow-selection/verify.json",
            "2aecda1b2d1e1b0cb000f82be85e5fb622bef3c07d99e20813c3123a519215e6"
            "readiness/262-workflow-selection/ship.json",
            "0b4e5756cdeefddf934aaacbf8f5683c13e1ad659a443899c87d4870eaec3b72"
            "readiness/262-workflow-selection/ship-verdict.json",
            "b3eb891464341c78f14dfc2a8ee577f825a886e0c3343677bf617478d4280868"
            "readiness/262-workflow-selection/governance-handoff.json",
            "6c4bd08e91b3a573e7d7e50c61ebd423f81a95f7b4e4891d39b5ed4d0f40c6ce"
            "artifacts/test-results/262-workflow-selection/unit-tests.trx",
            "6fbe1892e2d3658297991876d86d55344e08b192e98457ee630543b5e8e4f74d"
        ]
    ...
    use workModel =
        JsonDocument.Parse(read "readiness/262-workflow-selection/work-model.json")

    let modelViews =
        workModel.RootElement.GetProperty("generatedViews").EnumerateArray()
        |> Seq.toList
    ...
    use verification =
        JsonDocument.Parse(read "readiness/262-workflow-selection/verify.json")
    ...
    let verifiedViews =
        verified.GetProperty("generatedViews").EnumerateArray() |> Seq.toList
    ...
    let workModelSource =
        ...
        |> Seq.find (fun source ->
            source.GetProperty("path").GetString() = "readiness/262-workflow-selection/work-model.json")

    Assert.Equal(
        "f22fff614e7d8ee1e7fd024f9f8620068d02e1c13d771ff6a5e940dd56ffc04a",
        workModelSource.GetProperty("digest").GetProperty("value").GetString()
    )
    ...
    Assert.Equal(
        19,
        evidence
            .Split("source: artifacts/test-results/262-workflow-selection/unit-tests.trx", StringSplitOptions.None)
            .Length
        - 1
    )
    ...
    Assert.Equal(
        19,
        evidence
            .Split("sha256:6fbe1892e2d3658297991876d86d55344e08b192e98457ee630543b5e8e4f74d", StringSplitOptions.None)
            .Length
        - 1
    )

    let architectureReport =
        "artifacts/test-results/262-workflow-selection/workflow-selection-architecture.trx"

    let architectureCode, architectureOutput = tracked [ architectureReport ]

    if architectureCode <> 0 then
        failwith architectureOutput
