...
let private createArtifacts root =
    ...
    for relative in paths do
        ...
        if relative = "evidence-manifest/plan.json" then
            ...
            let sourceDigest = "b295b7bc8b75b2f3a8856a9a0c7851a91dcd591b6314e071ded16128b5967570"
            ...
            let resultDigest =
                SHA256.HashData(Encoding.UTF8.GetBytes($"passed|passed|8|61|99|74|14|%s{preparationDigest}|none|none"))
                |> Convert.ToHexString
                |> _.ToLowerInvariant()
            File.WriteAllText(
                target,
                $"{{\"schema\":\"fsgg.coordination.canonical-quint-qualification/1\",\"q1Outcome\":\"passed\",\"q2Outcome\":\"passed\",\"positiveInvariantCount\":8,\"negativeControlCount\":61,\"preparationDurationMs\":100,\"q2DurationMs\":200,\"totalDurationMs\":300,\"processCounts\":{{\"external\":99,\"quintCli\":74,\"apalacheVerify\":14}},\"tools\":{{\"toolchainSha256\":\"%s{toolchainDigest}\",\"quintSha256\":\"%s{quintDigest}\",\"apalacheJarSha256\":\"%s{apalacheDigest}\"}},\"inputs\":{{\"sourceSha256\":\"%s{sourceDigest}\",\"contractSha256\":\"%s{contractDigest}\"}},\"preparationSha256\":\"%s{preparationDigest}\",\"failure\":null,\"resultSha256\":\"%s{resultDigest}\"}}")
        else
...
[<InlineData("\"negativeControlCount\":61", "\"negativeControlCount\":60", "quint-receipt-inventory")>]
[<InlineData("\"totalDurationMs\":300", "\"totalDurationMs\":301", "quint-receipt-timing")>]
[<InlineData("\"external\":99", "\"external\":98", "quint-receipt-process-count")>]
[<InlineData("\"quintCli\":74", "\"quintCli\":73", "quint-receipt-process-count")>]
[<InlineData("\"apalacheVerify\":14", "\"apalacheVerify\":13", "quint-receipt-process-count")>]
[<InlineData("\"quintCli\":74", "\"quintCli\":0", "quint-receipt-process-count")>]
