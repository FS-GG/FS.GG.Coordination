...
let ``formal reuse invalidates when correspondence gates or retained evidence change`` () =
    ...
    let critical =
        [
            ...
            "tests/FS.GG.Coordination.Orchestration.Host.Tests/packages.lock.json"
            "tests/FS.GG.Coordination.Orchestration.Host.Tests/ChoreoTrace.fs"
            "tests/FS.GG.Coordination.QuintReplay.Tests/JournalFencingReplayTests.fs"
...
let ``bootstrap control surface stays typed thin and bounded`` () =
    ...
    // Bounded artifact discovery and resumable recovery add guarded shell paths.
    Assert.InRange(gateLines, 1, 850)
    Assert.InRange(uniqueGateLines, 1, 630)
