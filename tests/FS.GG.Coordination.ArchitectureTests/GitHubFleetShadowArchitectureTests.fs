...
let ``fleet shadow is a pure comparison surface with no apply path`` () =
    ...
    for required in
        [
            ...
            "UnclassifiedFleetDivergence"
            "FleetMutationAttempted"
            "SHA256.HashData"
        ] do
...
let ``GS2-05-8 registration binds accepted predecessor and exact Q4 gate`` () =
    ...
    let unitValue =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "GS2-05.8")
    ...
    let command =
        ...
        |> Seq.find (fun value -> value.GetProperty("id").GetString() = "github-fleet-shadow-contract")

    Assert.Equal("Q4", command.GetProperty("qGate").GetString())

    Assert.Equal<string list>(
        [ "fsi"; "eng/validate-github-fleet-shadow.fsx"; "--"; "." ],
    ...
    for required in
        [
            "accepted GS2-05.7 receipt"
            "zero-item repositories"
            "v1-defect"
...
let ``fleet shadow evidence covers the complete roster and closed control inventory`` () =
    use evidence =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-05-8-fleet-shadow.json")

    use corpus =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-05-8/corpus.json")

    use independent =
        JsonDocument.Parse(read "evidence/github-substrate-v2/gs2-05-8/independent-expectations.json")
    ...
    Assert.Equal(
        "value-independent",
        observation.GetProperty("source").GetProperty("independence").GetProperty("highestReached").GetString()
    )

    Assert.Equal(
        "fsgg-coord ready --all --json",
        observation.GetProperty("source").GetProperty("v1").GetProperty("command").GetString()
    )

    Assert.Equal(
        "gh api graphql --paginate --slurp",
        observation.GetProperty("source").GetProperty("v2").GetProperty("command").GetString()
    )
    ...
    let independentIds =
        independent.RootElement.GetProperty("controls").EnumerateArray()
        |> Seq.map _.GetString()
        |> Seq.toList
