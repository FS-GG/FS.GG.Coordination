...
let ``queue sandbox contract is pure and identity bounded`` () =
    ...
    let signature =
        read "src/FS.GG.Coordination.Qualification.Contracts/GitHubQueueSandbox.fsi"

    let project =
        read "src/FS.GG.Coordination.Qualification.Contracts/FS.GG.Coordination.Qualification.Contracts.fsproj"

    Assert.True(
        project.IndexOf("GitHubQueueSandbox.fsi", StringComparison.Ordinal) <
            project.IndexOf("GitHubQueueSandbox.fs\"", StringComparison.Ordinal)
    )
    ...
    for required in
        [
            "CandidateSha"
            "MergeGroupHeadSha"
            "CurrentBaseObservationRevision"
            "CurrentRequiredChecks"
            "CurrentClaimGeneration"
            "CurrentReviewDigest"
            "CurrentDependencyDigest"
            "CurrentReleaseObligationsMet"
...
let ``retained pilot and recovery inventories are exact and independently authored`` () =
    let inventory relative =
        ...
        let strings (name: string) : string list =
            node.GetProperty(name).EnumerateArray() |> Seq.map _.GetString() |> Seq.toList

        strings "pilotControls",
        strings "recoveryControls",
        strings "cases",
        node.GetProperty("caseContract").GetString()

    let generatedPilot, generatedRecovery, generatedCases, generatedContract =
        inventory "evidence/github-substrate-v2/gs2-07-6/generated-controls.json"

    let independentPilot, independentRecovery, independentCases, independentContract =
        inventory "evidence/github-substrate-v2/gs2-07-6/independent-controls.json"
...
let private isolatedEvidence () =
    ...
    copyTree
        (Path.Combine(evidenceRoot, "gs2-07-6"))
        (Path.Combine(directory.FullName, "evidence/github-substrate-v2/gs2-07-6"))

    let accepted =
        Path.Combine(directory.FullName, "evidence/github-substrate-v2/accepted")
    ...
    let source =
        Path.Combine(directory.FullName, "src/FS.GG.Coordination.Qualification.Contracts")

    Directory.CreateDirectory(source) |> ignore

    File.Copy(
        Path.Combine(root, "src/FS.GG.Coordination.Qualification.Contracts/GitHubQueueSandbox.fs"),
        Path.Combine(source, "GitHubQueueSandbox.fs")
    )
...
let private runGateAt evidenceRoot script =
    ...
    for argument in [ "fsi"; script; "--"; evidenceRoot ] do
        info.ArgumentList.Add argument
...
let ``Q4 and Q6 execute generated and independent controls offline`` () =
    ...
    for script in
        [
            "eng/validate-github-queue-sandbox-pilot.fsx"
            "eng/validate-github-queue-sandbox-recovery.fsx"
            "eng/validate-github-queue-routine-burst.fsx"
        ] do
        let text = read script
    ...
    runGate
        "eng/validate-github-queue-sandbox-pilot.fsx"
        "GITHUB_QUEUE_SANDBOX_PILOT_OK disposition=queue-pilot-qualified controls=24"

    runGate
        "eng/validate-github-queue-sandbox-recovery.fsx"
        "GITHUB_QUEUE_SANDBOX_RECOVERY_OK disposition=queue-sandbox-recovered controls=20"
...
let ``Q4 and Q6 reject contradictory retained hosted evidence`` () =
    let cases: (string * (JsonNode -> unit)) list =
        [
            "candidate head",
            fun node -> (node["candidate"].AsObject())["sha"] <- JsonValue.Create(String.replicate 40 "a")
            "initial base",
            fun node -> (node["initialAdmission"].AsObject())["baseSha"] <- JsonValue.Create(String.replicate 40 "b")
            "dependency authority",
            fun node ->
                (((node["authority"].AsObject())["current"]).AsObject())["dependencyDigest"] <-
                    JsonValue.Create(String.replicate 64 "c")
            "settings authority",
            fun node ->
                (((node["authority"].AsObject())["observed"]).AsObject())["settingsDigest"] <-
                    JsonValue.Create(String.replicate 64 "d")
            "distinct authority observation",
            fun node ->
                (((node["authority"].AsObject())["current"]).AsObject())["observedAtUnixSeconds"] <-
                    JsonValue.Create(1L)
        ]
    ...
    for name, mutation in cases do
        ...
        try
            let hosted =
                Path.Combine(isolated.FullName, "evidence/github-substrate-v2/gs2-07-6/hosted-run.json")
            ...
            Directory.Delete(isolated.FullName, true)
...
let ``Q4 and Q6 reject a retained proof bound to the wrong queue ref`` () =
    ...
    try
        let proofPath =
            Path.Combine(isolated.FullName, "evidence/github-substrate-v2/gs2-07-6/hosted-proof.json")

        mutateJson proofPath (fun node ->
            node.AsObject()["fullRef"] <- JsonValue.Create("refs/heads/gh-readonly-queue/wrong"))
...
let ``Q6 rejects tampered checkpoint journal handoff and expiry records`` () =
    let cases: (string * string * (JsonNode -> unit)) list =
        [
            ...
            fun node -> node.AsObject()["candidateSha"] <- JsonValue.Create(String.replicate 40 "e")
            "journal",
            "final-journal.json",
            fun node ->
                (((node["appliedEffects"].AsArray())[0]).AsObject())["resultDigest"] <-
                    JsonValue.Create(String.replicate 64 "f")
            "handoff",
            "process-handoff.json",
            fun node -> node.AsObject()["resumePid"] <- (node["preparePid"].DeepClone())
            "expiry",
            "expired-admission-refusal.json",
            fun node -> node.AsObject()["decision"] <- JsonValue.Create("accepted-expired-admission")
        ]
    ...
    for name, relative, mutation in cases do
        ...
        try
            let path =
                Path.Combine(isolated.FullName, "evidence/github-substrate-v2/gs2-07-6", relative)

            mutateJson path mutation

            let exitCode, output, error =
                runGateAt isolated.FullName "eng/validate-github-queue-sandbox-recovery.fsx"
...
let ``routine burst gate rejects stale hosted heads and cleanup mismatch`` () =
    let cases: (string * string * (JsonNode -> unit)) list =
        [
            ...
            "routine-burst.json",
            fun node ->
                (((((node["decisions"].AsArray())[0]).AsObject())["currentRun"]).AsObject())["headSha"] <-
                    JsonValue.Create(String.replicate 40 "9")
            "cleanup",
            "burst-cleanup.json",
            fun node -> node.AsObject()["settingsDigest"] <- JsonValue.Create(String.replicate 64 "8")
        ]
    ...
    for name, relative, mutation in cases do
        ...
        try
            ...
            let exitCode, output, error =
                runGateAt isolated.FullName "eng/validate-github-queue-routine-burst.fsx"
...
let ``hosted harness fails closed and retains typed exact-head proof`` () =
    ...
    let workflow =
        read "evidence/github-substrate-v2/gs2-07-6/sandbox-queue-workflow.yml"
    ...
    Assert.True(
        harness.IndexOf("[[ $(ref_status", StringComparison.Ordinal) <
            harness.IndexOf("cleanup_armed=true", StringComparison.Ordinal)
    )
