...
let ``issue field and qualification signatures precede implementations`` () =
    ...
    Assert.True(
        githubProject.IndexOf("IssueFields.fsi", StringComparison.Ordinal) <
            githubProject.IndexOf("IssueFields.fs\"", StringComparison.Ordinal)
    )

    let qualificationProject =
        File.ReadAllText(
            Path.Combine(
                root,
    ...
    Assert.True(
        qualificationProject.IndexOf("GitHubIssueFieldQualification.fsi", StringComparison.Ordinal) <
            qualificationProject.IndexOf("GitHubIssueFieldQualification.fs\"", StringComparison.Ordinal)
    )
...
let ``independent issue field inventory is closed and every mutation turns red`` () =
    ...
    let generated: GitHubIssueFieldControlResult list =
    ...
    let independent: GitHubIssueFieldControlResult list =
        independentInventory
        |> List.map (fun control ->
            {
                Control = control
    ...
    | Error findings ->
        Assert.Contains(
            findings,
            fun finding -> finding.Code = "GIFQ-INDEPENDENT-NOT-RED" && finding.ControlId = "pagination"
        )
...
let ``registered issue field Q3 validator is offline and runs two producers`` () =
    ...
    Assert.Contains(
        "\"args\": [\"fsi\", \"eng/validate-github-issue-field.fsx\", \"--\", \".\"]",
        catalog,
...
let ``registered issue field Q3 command passes against repository local evidence`` () =
    ...
    Assert.Contains(
        "github-issue-field-contract OK controls=7 q=Q3 network=offline provenance=synthetic",
        output,
        StringComparison.Ordinal
    )
