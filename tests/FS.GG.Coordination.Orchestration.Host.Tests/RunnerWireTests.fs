...
let ``runner poll is a closed digest-bound message`` () =
    let request: RunnerPollRequest =
        {
            ...
            CommandId = Guid.Parse("10000000-0000-0000-0000-000000000001")
            WorkItemPersistenceId = "work-item-v1-" + String.replicate 64 "a"
            SessionId = Guid.Parse("20000000-0000-0000-0000-000000000001")
            RunnerId = Guid.Parse("30000000-0000-0000-0000-000000000001")
            PrincipalId = "main-runner"
            FingerprintSha256 = String.replicate 64 "b"
            Generation = 3L
            ExpectedRevision = 14L
            ClientSequence = 1L
            IssuedAt = now
    ...
    let changed =
        Encoding.UTF8
            .GetString(bytes)
            .TrimEnd('}')
            .Insert(Encoding.UTF8.GetString(bytes).TrimEnd('}').Length, ",\"unknown\":true}")
        |> Encoding.UTF8.GetBytes

    Assert.Equal(Error "runner-message-shape-refused", RunnerWire.parsePoll changed)

    let duplicate =
        Encoding.UTF8.GetString(bytes).Replace("\"schema\":", $"\"schema\":\"{RunnerWire.pollSchema}\",\"schema\":")
        |> Encoding.UTF8.GetBytes
    ...
    let reordered =
        ...
        |> String.concat ","
        |> fun properties -> Encoding.UTF8.GetBytes($"{{{properties}}}")
...
let ``assignment digest changes with authority and ignores only its own digest field`` () =
    let assignment: RunnerAssignment =
        {
            ...
            WorkItemPersistenceId = "work-item-v1-" + String.replicate 64 "a"
            RouteId = Guid.Parse("10000000-0000-0000-0000-000000000001")
            AttemptId = Guid.Parse("20000000-0000-0000-0000-000000000001")
            CandidateId = Guid.Parse("30000000-0000-0000-0000-000000000001")
            SessionId = Guid.Parse("40000000-0000-0000-0000-000000000001")
            RunnerId = Guid.Parse("50000000-0000-0000-0000-000000000001")
            PrincipalId = "main-runner"
            FingerprintSha256 = String.replicate 64 "b"
            Generation = 3L
            WorkflowRevision = 14L
            ClientSequence = 1L
            ServerSequence = 1L
            PayloadSha256 = String.replicate 64 "c"
            AssignmentSha256 = ""
    ...
    Assert.False(
        String.Equals(digest, RunnerWire.assignmentDigest { assignment with Generation = 4L }, StringComparison.Ordinal)
    )
    ...
    let extra =
        Encoding.UTF8.GetString(RunnerWire.serialize closed).Replace("}", ",\"extra\":true}")
        |> Encoding.UTF8.GetBytes
...
let ``successful runner receipts are closed and bounded`` () =
    let ack: RunnerAckReceipt =
        {
            Schema = RunnerWire.ackReceiptSchema
            Accepted = true
            WorkflowRevision = 14L
        }

    let candidate: RunnerCandidateReceipt =
        {
            Schema = RunnerWire.candidateReceiptSchema
            Accepted = true
            WorkflowRevision = 17L
        }
    ...
    let malformed =
        Encoding.UTF8.GetBytes(
            $"{{\"schema\":\"{RunnerWire.ackReceiptSchema}\",\"accepted\":true,\"workflowRevision\":14,\"extra\":false}}"
        )
...
let ``event serializer v2 reads retained v1 event bytes`` () =
    ...
    let receipt =
        {
            CommandId = Id.command (Guid.Parse("90000000-0000-0000-0000-000000000001"))
            BodySha256 = String.replicate 64 "a"
