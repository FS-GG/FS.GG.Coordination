...
module private Fixture =
    ...
    let connectionString =
        $"Host={host};Port={port};Database=orchestration_o0;Username={username};Pooling=false"
    ...
    let readbackAt
        (permitValue: PilotPermit)
        (purpose: ReadbackPurpose)
        (operationId: OperationId option)
        (observedAt: DateTimeOffset)
        =
        let input =
            { input purpose operationId observedAt with
                SubjectId = permitValue.SubjectId
                Generation = permitValue.Generation
            }
        ...
        let request =
            {
                ...
                Purpose = purpose
                OperationId = operationId
                NotBefore = observedAt.AddSeconds -1.
            }
    ...
    let readback purpose operationId =
        readbackAt (permit ()) purpose operationId now

    let allocation () =
        {
            O0ReservationId = Id.reservation (Guid.NewGuid())
            O0Deadline = now.AddMinutes 10.
            PlanningBudgetSha256 = Some(sha "c")
            Tokens = 25L
            RuntimeSeconds = 10L
            CostMicros = 100L
        }
    ...
    let envelope (state: PilotState) (command: PilotCommand) : PilotCommandEnvelope =
        let principal =
            match command with
            | IssuePermit _
            | RecordTransferIntent _
            | AcknowledgeReturn _ -> "stable-route"
            | _ -> "pilot-route"
    ...
    let start () =
        if mode = "docker" then
            ...
            run
                "/usr/bin/pg_ctl"
                $"-D {root}/data -l {root}/postgres.log -o \"-k {host} -h '' -p {port} -c fsync=on -c synchronous_commit=on -c full_page_writes=on\" start"
    ...
    let waitReady () =
        task {
            ...
            if not ready then
                failwith "PostgreSQL unavailable"
        }
...
type PostgreSqlPilotStoreTests() =
    ...
    member _.``pilot reducer preserves controls unknown authority and cumulative budgets``() =
        ...
        state <-
            Fixture.advance
                state
                (StartAssignment(op1, attempt1, Fixture.allocation (), Fixture.readback AssignmentAdmission (Some op1)))

        state <-
            Fixture.advance
                state
                (StartAssignment(op2, attempt2, Fixture.allocation (), Fixture.readback AssignmentAdmission (Some op2)))

        state <- Fixture.advance state (MarkOutcomeUnknown(op1, "lost"))
        state <- Fixture.advance state (MarkOutcomeUnknown(op2, "lost"))

        let operationBorrow =
            Fixture.envelope state (ReconcileOutcome(op2, Fixture.readback AssignmentReconciliation (Some op1)))
        ...
        let oldRequest =
            {
                ...
                Purpose = Reconnect
                OperationId = None
                NotBefore = Fixture.now.AddSeconds -1.
            }
        ...
        let oldResult =
            PilotReadback.read
                (Fixture.FixedClock Fixture.now)
                (Fixture.Capability oldInput)
                oldRequest
        ...
        let staleGenerationInput =
            { Fixture.input Reconnect None Fixture.now with
                Generation = Id.generation 5L
            }

        let staleGenerationResult =
            PilotReadback.read
                (Fixture.FixedClock Fixture.now)
                (Fixture.Capability staleGenerationInput)
                oldRequest
                CancellationToken.None
            |> _.GetAwaiter().GetResult()
        ...
        let reused =
            Fixture.envelope
                ...
                (StartAssignment(
                    ...
                    Fixture.allocation (),
                    Fixture.readback AssignmentAdmission (Some op1)
                ))

        Assert.True(Pilot.decide Fixture.now state reused |> Result.isError)

        let forgedPrincipal =
            { Fixture.envelope state (RecordReturnIntent(Fixture.readback ReturnIntent None)) with
                PrincipalId = "borrower"
            }
        ...
        budgetState <-
            Fixture.advance
                ...
                (StartAssignment(
                    ...
                    Id.attempt (Guid.NewGuid()),
                    allocation60,
                    Fixture.readback AssignmentAdmission (Some budgetOp)
                ))

        budgetState <-
            Fixture.advance
                budgetState
                (SettleAssignment(budgetOp, Fixture.readback AssignmentSettlement (Some budgetOp)))
        ...
        let exceeds =
            Fixture.envelope
                ...
                (StartAssignment(
                    ...
                    Id.attempt (Guid.NewGuid()),
                    allocation60,
                    Fixture.readback AssignmentAdmission (Some excessOp)
                ))

        Assert.True(Pilot.decide Fixture.now budgetState exceeds |> Result.isError)

    [<Fact>]
    member _.``closed pilot lifecycle survives store replacement and preserves paused reconciliation``() =
        task {
            ...
            let store =
                PostgreSqlPilotStore(Fixture.options source identity 0L) :> IPilotJournalStore
            ...
            let append command =
                task {
                    ...
                    let! result =
                        store.AppendPilot(
                            PilotJournal.appendRequest Fixture.permitId Fixture.now envelope events,
                            token
                        )

                    match result with
                    | PilotAppended sequence -> Assert.Equal(state.Sequence + int64 events.Length, sequence)
            ...
            let operationId, attemptId =
                Id.operation (Guid.NewGuid()), Id.attempt (Guid.NewGuid())
            ...
            do!
                append (
                    StartAssignment(
                        ...
                        attemptId,
                        Fixture.allocation (),
                        Fixture.readback AssignmentAdmission (Some operationId)
                    )
                )
            ...
            | Ok value -> Assert.Equal<PilotState>(state, value.State)
            | Error failure -> failwithf "%A" failure
        }

    [<Fact>]
    member _.``pilot inbox binds complete envelope and sequence CAS``() =
        task {
            let! source, identity = Fixture.reset ()
            use source = source
            ...
            let events =
                Pilot.decide Fixture.now Pilot.initial envelope |> Result.defaultWith failwith
            ...
            let nextEnvelope =
                { Fixture.envelope nextState (RecordTransferIntent(Fixture.stableEvidence ())) with
                    ExpectedSequence = 0L
                }
            ...
            let! wrong =
                store.AppendPilot(
                    PilotJournal.appendRequest Fixture.permitId Fixture.now nextEnvelope nextEvents,
                    token
                )
    ...
    [<Fact>]
    member _.``process replacement requires fresh trusted reconnect before further assignment``() =
        task {
            let! source, identity = Fixture.reset ()
            use source = source
            ...
            let append (store: IPilotJournalStore) command =
                task {
                    let envelope = Fixture.envelope state command
                    let events = Pilot.decide Fixture.now state envelope |> Result.defaultWith failwith
                    ...
                    match result with
                    | PilotAppended _ -> state <- List.fold Pilot.evolve state events
                    | other -> failwithf "%A" other
                }
            ...
            let recoveredState =
                recovered |> Result.map _.State |> Result.defaultWith (sprintf "%A" >> failwith)
            ...
            let maliciousEvents =
                Pilot.decide Fixture.now maliciousState maliciousEnvelope
                |> Result.defaultWith failwith

            let! refused =
                replacement.AppendPilot(
                    PilotJournal.appendRequest Fixture.permitId Fixture.now maliciousEnvelope maliciousEvents,
                    token
                )
    ...
    [<Fact>]
    member _.``pilot recovery rejects acknowledged tail loss and future codec``() =
        task {
            let! source, identity = Fixture.reset ()
            use source = source
            ...
            let! _ =
                store.AppendPilot(PilotJournal.appendRequest Fixture.permitId Fixture.now envelope events, token)

            let! _ =
                Fixture.sql
                    $"UPDATE fsgg_orchestration.pilot_event SET schema_version=99,serializer_version='future' WHERE permit_id='{Fixture.permitId}'"
            ...
            | Error failures ->
                Assert.Contains(
                    failures,
                    function
                    | PilotUnknownEventVersion(_, 1L, 99) -> true
                    | _ -> false
                )
                ...
                Assert.Contains(
                    failures,
                    function
                    | PilotUnknownSerializerVersion "future" -> true
                    | _ -> false
                )
            ...
            let! _ =
                Fixture.sql
                    $"DELETE FROM fsgg_orchestration.pilot_inbox WHERE permit_id='{Fixture.permitId}'; DELETE FROM fsgg_orchestration.pilot_event WHERE permit_id='{Fixture.permitId}'"
            ...
            | Error failures ->
                Assert.Contains(
                    failures,
                    function
                    | PilotCorruptRecord(_, 1L) -> true
                    | _ -> false
                )
            | Ok _ -> failwith "tail loss recovered"
        }

    [<Fact>]
    member _.``pilot recovery rejects a valid payload whose permit identity mismatches its stream``() =
        task {
            let! source, identity = Fixture.reset ()
            use source = source
            ...
            use command =
                new NpgsqlCommand(
                    "UPDATE fsgg_orchestration.pilot_event SET payload=$1,payload_sha256=$2 WHERE permit_id=$3",
                    connection
                )
            ...
            | Ok _ -> failwith "mismatched permit recovered"
        }

    [<Fact>]
    member _.``pilot metadata generation fence is mandatory``() =
        task {
            let! source, identity = Fixture.reset ()
            use source = source

            let store =
                PostgreSqlPilotStore(Fixture.options source identity 2L) :> IPilotJournalStore
            ...
            | Error failures ->
                Assert.Contains(
                    failures,
                    function
                    | PilotBackupRequiresReconciliation "generation-fence-regressed" -> true
                    | _ -> false
                )
            ...
            match append with
            | PilotAppendUnavailable reason -> Assert.Contains("generation-fence-regressed", reason)
            | other -> failwithf "%A" other
        }

    [<Fact>]
    member _.``expiry stops new assignment but late reconciliation and return remain recoverable``() =
        task {
            let! source, identity = Fixture.reset ()
            use source = source
            ...
            let append (at: DateTimeOffset) (command: PilotCommand) =
                task {
                    ...
                    let! result =
                        store.AppendPilot(PilotJournal.appendRequest Fixture.permitId at envelope events, token)
            ...
            do!
                append
                    Fixture.now
                    (AcknowledgeTransfer(Fixture.readbackAt permit TransferAcknowledgement None Fixture.now))
            ...
            do!
                append
                    ...
                    (StartAssignment(
                        ...
                        Id.attempt (Guid.NewGuid()),
                        { Fixture.allocation () with
                            O0Deadline = permit.ExpiresAt
                        },
                        Fixture.readbackAt permit AssignmentAdmission (Some operationId) Fixture.now
                    ))
            ...
            let newAttempt =
                Fixture.envelope
                    ...
                    (StartAssignment(
                        ...
                        Id.attempt (Guid.NewGuid()),
                        Fixture.allocation (),
                        Fixture.readbackAt permit AssignmentAdmission (Some newOperation) Fixture.now
                    ))
            ...
            Assert.True(
                Pilot.decide
                    ...
                    state
                    { newAttempt with
                        IssuedAt = late.AddSeconds -1.
                        ExpiresAt = late.AddMinutes 1.
                    }
                |> Result.isError
            )
            ...
            do!
                append
                    ...
                    (ReconcileOutcome(
                        operationId,
                        Fixture.readbackAt permit AssignmentReconciliation (Some operationId) late
                    ))
            ...
            match recovered with
            | Ok value -> Assert.Equal(PilotPhase.StableOwned, value.State.Phase)
            | Error failures -> failwithf "%A" failures
        }

    [<Fact>]
    member _.``ordinary host startup refuses interrupted migration without repairing it``() =
        task {
            let! source, identity = Fixture.reset ()
            use source = source

            let! _ =
                Fixture.sql "UPDATE fsgg_orchestration.pilot_store_metadata SET migration_state='applying'"

            let configuration =
                {
                    ConnectionString = Fixture.connectionString
                    Token = String.replicate 32 "x"
                    RunnerToken = Some(String.replicate 32 "r")
                    Prefix = "http://127.0.0.1:5110/"
                    StoreId = "pilot-pg18-lab"
                    BackupIdentity = identity
            ...
            use command =
                new NpgsqlCommand(
                    "SELECT migration_state FROM fsgg_orchestration.pilot_store_metadata WHERE singleton",
                    connection
                )
