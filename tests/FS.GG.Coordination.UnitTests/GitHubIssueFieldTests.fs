...
let private identity kind id value =
    {
        Kind = kind
        Id = liveId id
        Name = name value
...
let ``semantic identity resolves every supported kind only from a unique complete observation`` () =
    ...
    Assert.Equal(
        ...
        IssueFields.resolveIdentity (name "Missing") Field (complete "rev-1" identities)
    )

    Assert.Equal(
        Error(ObservationRefused(ObservationUnauthorized "denied")),
        IssueFields.resolveIdentity (name "Status") Field (Unauthorized "denied")
    )
...
let ``identity ambiguity and duplicate live ids fail closed`` () =
    ...
    Assert.Equal(
        ...
        IssueFields.resolveIdentity (name "Status") Field (complete "rev-1" duplicateName)
    )
...
let ``field validation enforces type and an exact closed option set`` () =
    ...
    Assert.Equal(
        Ok(field [ ready; doneValue ] SingleSelect),
        IssueFields.validateField declaration (complete "rev-1" [ field [ ready; doneValue ] SingleSelect ])
    )

    Assert.Equal(
        Error(FieldTypeDrift(SingleSelect, Text)),
        IssueFields.validateField declaration (complete "rev-1" [ field [] Text ])
    )

    Assert.Equal(
        Error(MissingOption(name "Done")),
        IssueFields.validateField declaration (complete "rev-1" [ field [ ready ] SingleSelect ])
    )

    Assert.Equal(
        Error(UnexpectedOption(name "Parked")),
        IssueFields.validateField
            declaration
            (complete "rev-1" [ field [ ready; doneValue; option "O_3" "Parked" ] SingleSelect ])
    )

    Assert.Equal(
        Error(DuplicateOptionName(name "Ready")),
        IssueFields.validateField declaration (complete "rev-1" [ field [ ready; option "O_3" "Ready" ] SingleSelect ])
    )

    match
        IssueFields.validateField declaration (complete "rev-1" [ field [ ready; option "O_1" "Done" ] SingleSelect ])
    with
    ...
    Assert.Equal(
        ...
        IssueFields.validateField { declaration with DataType = Text } (complete "rev-1" [])
    )
...
let ``current value read preserves revision and terminal page evidence`` () =
    ...
    let current =
        {
            IssueId = issue
            FieldId = field
            Value = SingleSelectValue(name "Ready")
        }
    ...
    Assert.Equal(
        ...
        IssueFields.readCurrentValue issue field broken
    )

    Assert.Equal(
        Error(SchemaObservationRefused(ObservationIncomplete("page missing", Some "cursor"))),
        IssueFields.readCurrentValue issue field (Incomplete("page missing", Some "cursor"))
    )
...
let ``guarded plans are deterministic and distinguish create update clear and no-op`` () =
    ...
    let create =
        IssueFields.plan "rev-9" "cause-1" (CreateIssue(repository, "New issue")) (complete "rev-9" [ IssueAbsent ])
    ...
    let update =
        IssueFields.plan
            "rev-9"
            "cause-1"
            (UpdateField(issue, field, doneValue))
            (complete "rev-9" [ FieldPresent ready ])
    ...
    match
        IssueFields.plan "rev-9" "cause-1" (UpdateField(issue, field, ready)) (complete "rev-9" [ FieldPresent ready ])
    with
...
let ``planning refuses stale incomplete and ambiguous authority`` () =
    ...
    Assert.Equal(
        Error(StaleExpectedRevision "rev-old"),
        IssueFields.plan "rev-new" "cause" intent (complete "rev-old" [ FieldAbsent ])
    )

    Assert.Equal(
        Error(PlanObservationRefused(ObservationIncomplete("truncated", None))),
        IssueFields.plan "rev-new" "cause" intent (Incomplete("truncated", None))
    )

    Assert.Equal(
        Error AmbiguousCurrentState,
        IssueFields.plan "rev-new" "cause" intent (complete "rev-new" [ FieldAbsent; FieldAbsent ])
    )

    Assert.Equal(
        Error InvalidExpectedRevision,
        IssueFields.plan " rev-new" "cause" intent (complete "rev-new" [ FieldAbsent ])
    )

    Assert.Equal(
        Error(PlanObservationRefused InvalidCompletenessEvidence),
        IssueFields.plan "rev-new" "cause" intent Unchecked.defaultof<_>
    )

    Assert.Equal(
        Error InvalidMutationIntent,
        IssueFields.plan "rev-new" "cause" Unchecked.defaultof<_> (complete "rev-new" [ FieldAbsent ])
    )
...
let ``idempotency identities use unambiguous length-prefixed opaque values`` () =
    ...
    let first =
        IssueFields.plan "rev" "cause" (UpdateField(liveId "I|F", liveId "G", desired)) (complete "rev" [ FieldAbsent ])

    let second =
        IssueFields.plan "rev" "cause" (UpdateField(liveId "I", liveId "F|G", desired)) (complete "rev" [ FieldAbsent ])

    match first, second with
    | Ok(Planned left), Ok(Planned right) ->
        Assert.NotEqual<string>(left.IdempotencyIdentity, right.IdempotencyIdentity)
...
let ``qualification requires two complete independently reported control inventories`` () =
    ...
    | Error findings ->
        Assert.Contains(findings, fun finding -> finding.Code = "GIFQ-INVENTORY" && finding.ControlId = "generated")
