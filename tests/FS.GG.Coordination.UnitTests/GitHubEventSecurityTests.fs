    ...
    let unsigned =
        {
            ...
            ExpectedInstallationId = 81234L
            ExpectedRepository = "FS-GG/FS.GG.Coordination"
            ...
            GrantedPermissions = [ "contents:read"; "metadata:read" ]
...
let ``signature authenticates exact raw bytes and canonical header`` () =
    ...
    Assert.Contains(
        GitHubEventSecurityFinding.InvalidSignature,
        compile
            { original with
    ...
    Assert.Contains(
        ...
        compile
            { original with
                Signature = original.Signature.ToUpperInvariant()
            }
        |> findings
    )
    ...
    let malformed =
        Encoding.UTF8.GetBytes
            "{\"installation\":{\"id\":81234},\"repository\":{\"full_name\":\"FS-GG/FS.GG.Coordination\"}}"

    Assert.Contains(
        GitHubEventSecurityFinding.MalformedPayload "missing subject",
        compile
            { original with
                RawPayload = malformed
                Signature = sign secret malformed
            }
        |> findings
    )
...
let ``installation and repository scopes are exact`` () =
    ...
    Assert.Contains(
        GitHubEventSecurityFinding.InstallationScopeMismatch 7L,
        compile
            { original with
                RawPayload = wrongInstallation
                Signature = sign secret wrongInstallation
            }
        |> findings
    )
    ...
    Assert.Contains(
        GitHubEventSecurityFinding.RepositoryScopeMismatch "FS-GG/Outside",
        compile
            { original with
                RawPayload = wrongRepository
                Signature = sign secret wrongRepository
            }
        |> findings
    )
...
let ``replay window is inclusive and duplicates are rejected`` () =
    ...
    Assert.True(
        compile
            { original with
                EventTimestampUnixSeconds = received - 300L
            }
        |> Result.isOk
    )
    ...
    Assert.Contains(
        GitHubEventSecurityFinding.ReplayExpired(received - 301L),
        compile
            { original with
                EventTimestampUnixSeconds = received - 301L
            }
        |> findings
    )

    Assert.Contains(
        GitHubEventSecurityFinding.ReplayFromFuture(received + 301L),
        compile
            { original with
                EventTimestampUnixSeconds = received + 301L
            }
    ...
    Assert.Contains(
        GitHubEventSecurityFinding.MalformedField "replayWindow",
        compile
            { original with
                ReceivedAtUnixSeconds = System.Int64.MaxValue
            }
        |> findings
    )

    Assert.Contains(
        GitHubEventSecurityFinding.DuplicateDelivery original.DeliveryId,
        compile
            { original with
                SeenDeliveryIds = [ original.DeliveryId ]
            }
        |> findings
    )
    ...
    Assert.Contains(
        GitHubEventSecurityFinding.DuplicatePayload baseline.PayloadSha256,
        compile changedDelivery |> findings
    )
...
let ``payload and API authority must agree exactly`` () =
    ...
    Assert.Contains(
        GitHubEventSecurityFinding.PayloadApiDisagreement "subject",
        compile
            { original with
    ...
    Assert.Contains(
        GitHubEventSecurityFinding.PayloadApiDisagreement "revision",
        compile { original with ApiRevision = 8L } |> findings
    )
    ...
    let contradictions =
        compile
            { original with
                RawPayload = contradictory
                Signature = sign secret contradictory
            }
        |> findings
...
let ``least privilege is exact and canonical`` () =
    ...
    Assert.Contains(
        GitHubEventSecurityFinding.MissingPermission "contents:read",
        compile
            { original with
    ...
    Assert.Contains(
        GitHubEventSecurityFinding.ExcessivePermission "issues:write",
        compile
            { original with
    ...
    Assert.Contains(
        GitHubEventSecurityFinding.NonCanonicalPermissions "granted",
        compile
            { original with
                GrantedPermissions = List.rev original.GrantedPermissions
            }
        |> findings
    )
...
let ``direct derived write is refused`` () =
    ...
    Assert.Contains(
        GitHubEventSecurityFinding.DirectWriteAttempt original.DeliveryId,
        compile
            { original with
