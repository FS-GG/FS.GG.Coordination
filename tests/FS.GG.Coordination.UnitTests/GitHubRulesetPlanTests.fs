...
let private repository =
    {
        Id = "example"
        FullName = "FS-GG/example"
        Role = Framework
        Capabilities = [ "labels"; "coordination-kit" ]
        KitDelivery = Some "package"
        AbsenceCover = Some "required"
        Reason = None
    }
...
let private profileReport =
    RepositoryProfileAdapter.compile observedAt maxAge profileSnapshot
    |> Result.defaultWith (failwithf "%A")

let private event declared =
    {
        Declared = declared
        BranchFilters = []
        PathFilters = []
...
let private producer context mergeGroup =
    {
        ...
        PullRequest = event true
        MergeGroup = event mergeGroup
...
let private censusSnapshot mergeGroup =
    {
        SchemaVersion = 1
        Repository = repository.FullName
        ProfileSeal = profileReport.Seal
...
let private censusReport mergeGroup =
    RequiredCheckCensusAdapter.compile observedAt maxAge (censusSnapshot mergeGroup)
    |> Result.defaultWith (failwithf "%A")
...
let private snapshot mergeGroup =
    {
        ...
        ExpectedProfileSeal = profileReport.Seal
        CensusSnapshot = Some(censusSnapshot mergeGroup)
        ExpectedCensusSeal = Some((censusReport mergeGroup).Seal)
        CurrentPolicyRepository = repository.FullName
        CurrentPolicyRevision = revision
        CurrentPolicyEvidenceSha256 = digest
        CurrentPolicyObservedAt = observedAt
        CurrentPolicyComplete = true
        ObservedAt = observedAt
        Complete = true
        ApprovedBypass = []
        RequestedBypass = []
...
let private compile value =
    RulesetPlanAdapter.compile observedAt maxAge value
...
let ``administered profile compiles the complete secure desired target`` () =
    ...
    Assert.True(
        tags.ProtectDeletion
        && tags.BlockNonFastForward
        && tags.BlockUpdate
        && tags.RequireSignatures
    )
...
let ``profile census and current-policy source substitutions fail exact verification`` () =
    ...
    Assert.Equal(
        Error [ AlteredRulesetPlanSeal ],
        RulesetPlanAdapter.verify report.Seal observedAt maxAge changedPolicy
    )

    let changedCensus =
        { baseline with
            ExpectedCensusSeal = Some digest
        }
...
let ``ordering of checks bypass authorities requests and exceptions is stable`` () =
    ...
    let exceptionValue id actor =
        {
            ...
            Scope = BypassPrincipal actor
            ApprovedAt = observedAt.AddDays(-1)
            StartsAt = observedAt.AddHours(-1)
            ExpiresAt = observedAt.AddDays(1)
        }
    ...
    let second =
        { first with
            ...
            CensusSnapshot =
                first.CensusSnapshot
                |> Option.map (fun value ->
                    { value with
                        Requirements = List.rev value.Requirements
                        Producers = List.rev value.Producers
                    })
        }
...
let ``bypass is deny by default and exact kind plus profile class`` () =
    ...
    Assert.Contains(
        UnauthorizedRulesetBypassPrincipal 7L,
        compile
            { snapshot true with
                RequestedBypass = [ requested ]
            }
        |> findings
    )
...
let ``exceptions must be identified current bounded and owned`` mutation =
    let baseline: RulesetPlanException =
        {
            ...
            Owner = "security"
            Rationale = "bounded migration"
            Scope = RulesetExceptionScope.MergeQueue false
            ApprovedAt = observedAt.AddDays(-1)
            StartsAt = observedAt.AddHours(-1)
            ExpiresAt = observedAt.AddDays(1)
        }
    ...
    let changed =
        ...
        | "overlong" ->
            { baseline with
                ExpiresAt = baseline.ApprovedAt.AddDays 31
            }
...
let ``bounded exceptions project into the target and authorization is seal-bound`` () =
    ...
    let duplicate =
        { review with
            Id = "E-2"
            Rationale = "second override"
        }

    Assert.True(
        compile
            { snapshot false with
                Exceptions = [ review; duplicate ]
            }
        |> Result.isError
    )
...
let ``exceptions cannot enable queue before census readiness and authorized bypass scope projects`` () =
    let queue: RulesetPlanException =
        {
            ...
            Scope = RulesetExceptionScope.MergeQueue true
            ApprovedAt = observedAt.AddDays(-1)
            StartsAt = observedAt.AddHours(-1)
            ExpiresAt = observedAt.AddDays(1)
        }
    ...
    let bypass =
        { queue with
            Id = "E-B"
            Rationale = "bounded integration migration"
            Scope = BypassPrincipal principal
        }
...
let ``current policy binding is complete repository exact and fresh`` () =
    ...
    Assert.True(
        compile
            { baseline with
                CurrentPolicyObservedAt = observedAt.AddHours(-2)
            }
        |> Result.isError
    )
...
let ``external profile compiles only an observe-only disposition`` () =
    let external =
        {
            ...
            KitDelivery = None
            AbsenceCover = None
            Reason = Some "external authority"
        }

    let externalRoster = roster [ authority; external ]

    let externalReport =
        RepositoryProfileAdapter.compile observedAt maxAge externalRoster
        |> Result.defaultWith (failwithf "%A")

    let externalSnapshot =
        { snapshot true with
            Repository = external.FullName
            ProfileSnapshot = externalRoster
            ExpectedProfileSeal = externalReport.Seal
            CensusSnapshot = None
    ...
    Assert.True(
        report.DefaultBranch.IsNone
        && report.ReleaseTags.IsNone
        && report.RepositoryPolicy.IsNone
    )
...
let ``partial stale and cross-repository evidence refuses`` () =
    ...
    Assert.True(
        RulesetPlanAdapter.compile (observedAt.AddHours 2) maxAge (snapshot true)
        |> Result.isError
    )
...
let ``qualification requires every generated and independent inversion`` () =
    ...
    let broken =
        passing
        |> List.map (fun value ->
            if value.Control = GitHubRulesetPlanControl.MergeQueue then
