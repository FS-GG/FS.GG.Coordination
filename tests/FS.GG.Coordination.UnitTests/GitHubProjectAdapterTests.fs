...
let private semantic value =
    SemanticName.tryCreate value |> Result.defaultWith failwith
...
let private issue project item repo number content archived =
    {
        ...
        ItemId = liveId item
        Content = RepositoryIssue(repo, number, liveId content)
        Archived = archived
    }
...
let private draft project item content =
    {
        ProjectId = liveId project
        ItemId = liveId item
        Content = DraftIssue(liveId content)
        Archived = false
    }

let private page number terminal items =
    {
        Number = number
...
let private snapshot observation =
    ProjectAdapter.readProject observation |> Result.defaultWith (failwithf "%A")

let private option id name : StatusOptionProjection =
...
let ``Project reads require terminal pagination and reject duplicate item and content identities`` () =
    ...
    Assert.Equal(
        Error(DuplicateProjectItemId first.ItemId),
        ProjectAdapter.readProject (complete "rev" [ page 1 true [ first; first ] ])
    )

    let duplicateContent = { first with ItemId = liveId "I3" }

    Assert.Equal(
        Error(DuplicateProjectContent(liveId "C2")),
        ProjectAdapter.readProject (complete "rev" [ page 1 true [ first; duplicateContent ] ])
    )
...
let ``membership preserves archived external draft redacted unknown missing and unreadable outcomes`` () =
    ...
    let redacted =
        {
            ProjectId = liveId "P"
            ItemId = liveId "I4"
            Content = RedactedContent(liveId "RED")
            Archived = false
        }
    ...
    let unknown =
        {
            ProjectId = liveId "P"
            ItemId = liveId "I5"
            Content = UnknownContent("future", liveId "UNK")
            Archived = false
        }

    let observed =
        snapshot (complete "rev" [ page 1 true [ external; archived; draftItem; redacted; unknown ] ])

    Assert.Equal(
        Ok(ExternalRepositoryMembership external),
        ProjectAdapter.resolveMembership expected (liveId "EXT") observed
    )
    ...
    Assert.Equal(
        Error(ProjectObservationUnreadable "timeout"),
        ProjectAdapter.readProject (ProjectUnreadable "timeout")
    )
...
let ``Status reads are explicitly projection-only complete and duplicate-safe`` () =
    ...
    Assert.Equal(
        Error(DuplicateStatusOptionId ready.Id),
        ProjectAdapter.readStatus (liveId "P") (liveId "I") (status "rev" duplicate)
    )

    Assert.Equal(
        Error(StatusObservationUnreadable "denied-read"),
        ProjectAdapter.readStatus (liveId "P") (liveId "I") (StatusUnreadable "denied-read")
    )
...
let ``membership planning is deterministic idempotent typed for no-ops and blocks archived items`` () =
    ...
    let add =
        ProjectAdapter.planMembership "rev" "cause" repo (EnsureMember(liveId "P", liveId "C2")) before
...
let ``Status planning validates options and produces mutation-free no-ops`` () =
    ...
    let before =
        ProjectAdapter.readStatus
            (liveId "P")
            (liveId "I")
            (status "rev" (statusField "P" "I" (Some "READY") [ ready; backlog ]))
    ...
    Assert.Equal(
        Error(RequestedStatusOptionMissing(liveId "UNKNOWN")),
        ProjectAdapter.planStatus "rev" "cause" (SetStatus(liveId "UNKNOWN")) before
    )
...
let ``Status reread and post-state verification require exact projection delta and advanced revision`` () =
    ...
    let beforeObservation =
        status "rev-1" (statusField "P" "I" (Some "READY") [ ready; backlog ])

    let before =
        ProjectAdapter.readStatus (liveId "P") (liveId "I") beforeObservation
        |> Result.defaultWith (failwithf "%A")

    let plan =
        match ProjectAdapter.planStatus "rev-1" "cause" (SetStatus backlog.Id) before with
    ...
    Assert.Equal(
        Error(StatusReReadRequired("rev-1", "rev-2")),
        ProjectAdapter.checkStatusPreState plan (status "rev-2" (statusField "P" "I" (Some "READY") [ ready; backlog ]))
    )

    Assert.Equal(
        Error ConcurrentStatusChange,
        ProjectAdapter.checkStatusPreState plan (status "rev-1" (statusField "P" "I" None [ ready; backlog ]))
    )

    Assert.Equal(
        Error(StatusResultRevisionDidNotAdvance "rev-1"),
        ProjectAdapter.verifyStatusPostState "rev-1" plan beforeObservation
    )

    Assert.True(
        ProjectAdapter.verifyStatusPostState
            "rev-2"
            plan
            (status "rev-2" (statusField "P" "I" (Some "BACKLOG") [ ready; backlog ]))
        |> Result.isOk
    )

    Assert.Equal(
        Error StatusPostStateMismatch,
        ProjectAdapter.verifyStatusPostState "rev-2" plan (status "rev-2" (statusField "P" "I" None [ ready; backlog ]))
    )
...
let ``mandatory rereads and exact post-state checks reject stale and concurrent changes`` () =
    ...
    let plan =
        match ProjectAdapter.planMembership "rev-1" "cause" repo (EnsureMember(liveId "P", liveId "C2")) before with
        | Ok(MembershipPlanned value) -> value
        | value -> failwithf "%A" value

    Assert.Equal(
        Error(MembershipReReadRequired("rev-1", "rev-2")),
        ProjectAdapter.checkMembershipPreState plan (complete "rev-2" [ page 1 true [ existing ] ])
    )

    let concurrent = issue "P" "IX" repo 9 "CX" false

    Assert.Equal(
        Error ConcurrentMembershipChange,
        ProjectAdapter.checkMembershipPreState plan (complete "rev-1" [ page 1 true [ existing; concurrent ] ])
    )

    let result = issue "P" "I2" repo 2 "C2" false

    Assert.True(
        ProjectAdapter.verifyMembershipPostState
            "rev-2"
            (Some result)
            plan
            (complete "rev-2" [ page 1 true [ existing; result ] ])
        |> Result.isOk
    )

    Assert.Equal(
        Error MembershipPostStateMismatch,
        ProjectAdapter.verifyMembershipPostState
            "rev-2"
            (Some result)
            plan
            (complete "rev-2" [ page 1 true [ result ] ])
    )
...
let ``Project qualification inventory is exact and every mutation must turn red`` () =
    ...
    | Error findings ->
        Assert.Contains(
            findings,
            fun finding -> finding.Code = "GPAQ-INDEPENDENT-NOT-RED" && finding.ControlId = "external-item"
        )
