...
let ``canonical envelope round trips and binds all fields`` () =
    ...
    Assert.True(
        envelope.Cursor =
            [
                "1:110:delivery-17:event-19:receipt-1"
                "1:210:delivery-27:event-29:receipt-2"
            ]
    )
...
let ``conflicting delivery event and cursor identities fail closed`` () =
    ...
    | Error findings ->
        Assert.Contains(GitHubEventEnvelopeFinding.DuplicateDeliveryConflict "delivery-1", findings)
        Assert.Contains(GitHubEventEnvelopeFinding.DuplicateEventConflict "event-1", findings)
        Assert.Contains(GitHubEventEnvelopeFinding.CursorPositionConflict 1L, findings)
...
let ``gaps stale revisions and causal receipt mismatches remain explicit`` () =
    let invalid =
        { second with
            ...
            CorrelationId = second.DeliveryId
            ReceiptId = second.DeliveryId
        }
...
let ``verification refuses noncanonical envelope structure`` () =
    ...
    Assert.Equal(
        ...
        verify envelope.Seal { envelope with SchemaVersion = 2 }
    )
...
let ``unknown malformed source and cross source replay refuse`` () =
    ...
    Assert.Equal(
        Error [ GitHubEventEnvelopeFinding.CrossSource "FS-GG/other" ],
        replay
            envelope
            { source with
