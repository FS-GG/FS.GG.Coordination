...
let private endpoint repository name disposition status =
    let permission =
        if name = "repository" then
            "metadata:read"
        elif name = "workflows" then
            "actions:read"
        elif name = "environments" then
            "environments:read"
        elif name = "releases-and-tags" then
            "contents:read"
        elif
            List.contains
                name
...

    {
        ...
        Pagination = page
        PayloadSha256 = sha $"{repository}:{name}:payload"
        RelevantFingerprint = sha $"{repository}:{name}:state"
    ...
    |> List.map (fun repository ->
        {
            ...
            Endpoints =
                expectedEndpoints
                |> List.map (fun name -> endpoint repository name GitHubFleetDisposition.Supported 200)
        })
...
let private targets () =
    ...
    |> List.map (fun repository ->
        {
            Repository = repository
            ExternalOwner = repository = "EHotwagner/S.I.R."
            Settings =
                expectedEndpoints
                |> List.map (fun setting ->
                    {
                        ...
                        DesiredSha256 =
                            sha (
                                if setting = "repository" then
                                    $"{repository}:desired"
                                else
                                    $"{repository}:{setting}:state"
                            )
                        RequiredPermission =
                            if setting = "workflows" then
                                "actions:write"
                            elif setting = "environments" then
                                "environments:write"
                            elif setting = "releases-and-tags" then
                                "contents:write"
                            elif
                                List.contains
                                    setting
...
let ``fleet dry plan is deterministic canonical and lossless`` () =
    ...
    let reviewed =
        review "plan-author" "independent-reviewer" (instant.AddMinutes 6.) first
        |> acceptReview first
        |> get
    ...
    Assert.All(
        first.Plans.Tail,
        fun plan ->
            Assert.Equal(GitHubFleetDisposition.Supported, plan.Disposition)
            Assert.Single(plan.Operations) |> ignore
    )
...
let ``authority roster and omission fail closed`` () =
    ...
    | Error findings ->
        Assert.Contains(GitHubFleetDryPlanFinding.InvalidFleetAuthority "roadmap", findings)
        Assert.Contains(GitHubFleetDryPlanFinding.InvalidFleetRoster, findings)
...
let ``pagination identity and unsupported permissions stay explicit`` () =
    ...
    let unauthorized =
        { first.Endpoints.Head with
            StatusCode = 403
            Disposition = GitHubFleetDisposition.Unauthorized
        }

    let unsupported =
        { first.Endpoints[1] with
            StatusCode = 404
            Disposition = GitHubFleetDisposition.Unsupported
        }

    let changed =
        { first with
            Endpoints = unauthorized :: unsupported :: first.Endpoints.Tail.Tail
        }
    ...
    match
        compileFixture
            ({ first with
                Endpoints = malformed :: first.Endpoints.Tail
             }
             :: observed.Tail)
            (targets ())
    with
    | Error findings ->
        Assert.Contains(
            GitHubFleetDryPlanFinding.InvalidFleetPagination $"{first.Repository}:{malformed.Endpoint}",
            findings
        )
...
let ``reinspection is independent and relevant drift stales`` () =
    ...
    let decision =
        review "plan-author" "independent-reviewer" (instant.AddMinutes 6.) plan
        ...
        |> List.map (fun value ->
            {
                ...
                ObservedAt = instant.AddMinutes 2.
                RelevantFingerprint = value.PreStateSha256
                Complete = true
                Authoritative = true
            })
    ...
    let drifted =
        { same.Head with
            RelevantFingerprint = sha "relevant-drift"
        }
        :: same.Tail
    ...
    match
        reinspect
            { reviewed with
                Review = { decision with Independent = false }
            }
            same
    with
...
let ``all explicit dispositions compile without invented operations`` () =
    let variants =
        [
            GitHubFleetDisposition.Unsupported, 404
            GitHubFleetDisposition.Unauthorized, 403
            GitHubFleetDisposition.Unavailable, 503
            GitHubFleetDisposition.Unreadable, 0
            GitHubFleetDisposition.Stale, 200
            GitHubFleetDisposition.Indeterminate, 200
    ...
    for disposition, status in variants do
        ...
        let changedEndpoints =
            ...
            |> List.map (fun endpoint ->
                if endpoint.Endpoint = "repository" then
                    let fingerprint =
                        if disposition = GitHubFleetDisposition.NoOp then
                            sha $"{repository.Repository}:desired"
                        else
        ...
        let plan =
            compileFixture (observed.Head :: changed :: observed.Tail.Tail) (targets ())
            |> get
    ...
    let changed =
        { repository with
            Complete = false
            Endpoints = incompleteEndpoint :: repository.Endpoints.Tail
        }
...
let ``generated and independent Q5 inventories are exact`` () =
    let side: GitHubFleetControlResult list =
        requiredControls
...
let ``serialization refuses extra fields and altered seals`` () =
    ...
    let altered =
        bytes.Replace($"\"seal\":\"{reviewed.Seal}\"", $"\"extra\":true,\"seal\":\"{reviewed.Seal}\"")
...
let ``freshness complete desired inventory and least permission fail closed`` () =
    ...
    Assert.True(
        Result.isError (
            compileFixture
                observed
                ({ desired.Head with
                    Settings = desired.Head.Settings.Tail
                 }
                 :: desired.Tail)
        )
    )
    ...
    let excessive =
        { desired.Head.Settings.Head with
    ...
    Assert.True(
        Result.isError (
            compileFixture
                observed
                ({ desired.Head with
                    Settings = excessive :: desired.Head.Settings.Tail
                 }
                 :: desired.Tail)
        )
    )
    ...
    let environment =
        desired.Head.Settings
        |> List.find (fun setting -> setting.Setting = "environments")

    Assert.Equal("environments:write", environment.RequiredPermission)

    let wrongEnvironment =
        desired.Head.Settings
        |> List.map (fun setting ->
            if setting.Setting = "environments" then
                { setting with
        ...
        { reviewed with
            Plan =
                { reviewed.Plan with
                    Plans =
                        { firstPlan with
                            Settings = changedSetting :: firstPlan.Settings.Tail
                        }
                        :: reviewed.Plan.Plans.Tail
                }
        }
