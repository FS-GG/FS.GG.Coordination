...
let ``selection controls are complete ordered and independently required`` () =
    ...
    let ids =
        controls |> List.map GitHubWorkflowSelectionQualification.selectionControlId
    ...
    let independentRed =
        ...
        |> List.mapi (fun index value ->
            if index = 7 then
                { value with ControlPassed = false }
            else
                value)

    Assert.True(
        GitHubWorkflowSelectionQualification.validateSelection passing independentRed
        |> Result.isError
    )
...
let ``supply-chain controls are distinct complete and independently required`` () =
    ...
    let ids =
        controls |> List.map GitHubWorkflowSelectionQualification.supplyChainControlId
    ...
    let generatedRed =
        ...
        |> List.mapi (fun index value ->
            if index = 10 then
                { value with BaselineGreen = false }
            else
                value)

    Assert.True(
        GitHubWorkflowSelectionQualification.validateSupplyChain generatedRed passing
        |> Result.isError
    )
    ...
    let unsigned =
        {
                ...
                [
                    {
                        ...
                        Pattern = "src/"
                        Match = Prefix
                        Roots = [ WorkflowObligation.Build ]
                    }
                    {
                        Id = "tests"
                        Pattern = "tests/"
                        Match = Prefix
                        Roots = [ WorkflowObligation.Test ]
                    }
                    {
                        Id = "workflow"
                        Pattern = ".github/"
                        Match = Prefix
                        Roots = [ WorkflowObligation.Policy ]
                    }
                    {
                        Id = "release"
                        Pattern = "eng/release"
                        Match = Prefix
                        Roots = [ WorkflowObligation.Packaging ]
                    }
                    {
                        Id = "docs"
                        Pattern = "docs/"
                        Match = Prefix
                        Roots = [ WorkflowObligation.Coordination ]
                    }
                ]
            NonFileRules =
                [
                    {
                        Id = "settings"
                        Pattern = "repository-settings"
                        Match = Exact
                        Roots = [ WorkflowObligation.Policy ]
                    }
                    {
                        Id = "dependency"
                        Pattern = "dependency-revision"
                        Match = Exact
                        Roots = [ WorkflowObligation.Build; WorkflowObligation.Packaging ]
                    }
                ]
            Dependencies =
                [
                    {
                        Source = WorkflowObligation.Build
                        Target = WorkflowObligation.Test
                    }
                    {
                        Source = WorkflowObligation.Test
                        Target = WorkflowObligation.Policy
                    }
                    {
                        Source = WorkflowObligation.Policy
                        Target = WorkflowObligation.Coordination
                    }
                    {
                        Source = WorkflowObligation.Packaging
                        Target = WorkflowObligation.Release
                    }
                ]
            ...
            Expensive =
                [
                    WorkflowObligation.Test
                    WorkflowObligation.Packaging
                    WorkflowObligation.Release
                ]
            Seal = ""
        }

    { unsigned with
        Seal = WorkflowSelection.computeInventorySeal unsigned
    }
...
let private decision result : WorkflowSelectionDecision =
    match result with
    | Ok value -> value
    | Error findings -> failwith $"unexpected refusal: {findings}"

[<Fact>]
let ``runtime selector derives arbitrary mixed closure and stable outcomes`` () =
    let selected =
        WorkflowSelection.select (inventory ()) (request [ "src/NewFeature.fs"; "eng/release-candidate.json" ] [])
        |> decision
    ...
    Assert.Equal<WorkflowObligation list>(
        [
            WorkflowObligation.Build
            WorkflowObligation.Test
            WorkflowObligation.Policy
            WorkflowObligation.Coordination
            WorkflowObligation.Packaging
            WorkflowObligation.Release
        ],
        selected.Closure
    ...
    selected.Aggregates
    |> List.iter (fun value -> Assert.Equal("resolved", value.Status))

[<Fact>]
let ``runtime selector handles non-file input and does not provision not-applicable jobs`` () =
    let selected =
        WorkflowSelection.select (inventory ()) (request [] [ "repository-settings" ])
        |> decision
    ...
    let packaging =
        selected.Children
        |> List.find (fun value -> value.Obligation = WorkflowObligation.Packaging)
...
let ``runtime selector recomputes merge group against queued head current base and settings`` () =
    let merge: MergeGroupSelectionInput =
        {
            QueuedHead = String.replicate 40 "c"
            CurrentQueuedHead = String.replicate 40 "c"
            CurrentBaseRevision = baseRevision
            CurrentSettingsSha256 = settings
            Recomputed = true
        }

    let selected =
        WorkflowSelection.select
            (inventory ())
            { request [ "src/Queue.fs" ] [] with
                MergeGroup = Some merge
            }
        |> decision
    ...
    let stale =
        { merge with
            CurrentSettingsSha256 = String.replicate 64 "d"
        }
    ...
    let staleHead =
        { merge with
            QueuedHead = String.replicate 40 "d"
        }
...
let ``runtime selector fails closed on unknown ambiguous stale incomplete and forged inventory`` () =
    ...
    Assert.True(
        WorkflowSelection.select current (request [ "assets/unknown.bin" ] [])
        |> Result.isError
    )
    ...
    Assert.True(
        WorkflowSelection.select
            current
            { request [ "src/X.fs" ] [] with
                BaseRevision = String.replicate 40 "e"
            }
        |> Result.isError
    )
    ...
    let inventedUnsigned =
        { current with
            InventoryVersion = "invented-v999"
            Seal = ""
        }

    let invented =
        { inventedUnsigned with
            Seal = WorkflowSelection.computeInventorySeal inventedUnsigned
        }
    ...
    Assert.True(
        WorkflowSelection.select
            ...
            { request [ "src/X.fs" ] [] with
                InventoryVersion = "invented-v999"
                ExpectedInventorySeal = invented.Seal
            }
        |> Result.isError
    )
            ...
            PathRules =
                {
                    ...
                    Pattern = ".fs"
                    Match = Suffix
                    Roots = [ WorkflowObligation.Release ]
                }
                :: current.PathRules
            Seal = ""
        }
    ...
    let ambiguous =
        { ambiguousUnsigned with
            Seal = WorkflowSelection.computeInventorySeal ambiguousUnsigned
        }
    ...
    match
        WorkflowSelection.select
            ambiguous
            { request [ "src/X.fs" ] [] with
                ExpectedInventorySeal = ambiguous.Seal
            }
    with
    | Error findings ->
        Assert.Contains(
            findings,
            function
            | AmbiguousChangedPath("src/X.fs", _) -> true
            | _ -> false
        )
    ...
    let sameRoot =
        { sameRootUnsigned with
            Seal = WorkflowSelection.computeInventorySeal sameRootUnsigned
        }
    ...
    match
        WorkflowSelection.select
            sameRoot
            { request [ "src/X.fs" ] [] with
                ExpectedInventorySeal = sameRoot.Seal
            }
    with
    | Error findings ->
        Assert.Contains(
