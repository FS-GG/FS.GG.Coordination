...
let private write name =
    { Name = name; Level = PermissionWrite }
...
let private registration id operation principal app environment appPermissions workflowPermissions =
    {
        ...
        DeclaredAppPermissions = appPermissions
        DeclaredWorkflowPermissions = workflowPermissions
    }
...
let ``complete interpreter inventory compiles least privilege into isolated classes`` () =
    ...
    Assert.Equal<string list>(
        [ "coordination"; "coordination"; "coordination-admin"; "release" ],
        report.Interpreters |> List.map _.Environment |> List.sort
    )
...
let ``ordering is stable and exact replay is seal bound`` () =
    ...
    Assert.Equal(
        ...
        compile
            { baseline with
                Registrations = List.rev baseline.Registrations
            }
    )
    ...
    Assert.Equal(
        Error [ AlteredPermissionCompilationSeal ],
        GitHubPermissionCompilationQualification.verify (String.replicate 64 "0") baseline
    )

[<Fact>]
let ``missing duplicate and incomplete interpreter inventories fail closed`` () =
    Assert.Contains(
        MissingInterpreterOperation ApplyRepositoryCutover,
        compile
            { baseline with
                Registrations = baseline.Registrations.Tail
            }
        |> findings
    )
    ...
    Assert.Contains(
        DuplicateInterpreterId duplicate.Id,
        compile
            { baseline with
                Registrations = duplicate :: baseline.Registrations
            }
        |> findings
    )
...
let ``wildcard undeclared elevated and missing permissions fail closed`` () =
    ...
    let wildcard =
        { head with
            DeclaredAppPermissions = write "*" :: head.DeclaredAppPermissions
        }

    Assert.True(
        compile
            { baseline with
                Registrations = wildcard :: baseline.Registrations.Tail
            }
        |> Result.isError
    )
    ...
    let extra =
        { head with
            DeclaredWorkflowPermissions = write "packages" :: head.DeclaredWorkflowPermissions
        }

    Assert.Contains(
        UndeclaredOrOverprivilegedPermission(head.Id, "workflow:packages:write"),
        compile
            { baseline with
                Registrations = extra :: baseline.Registrations.Tail
            }
        |> findings
    )
    ...
    let missing =
        { head with
            DeclaredAppPermissions = head.DeclaredAppPermissions.Tail
        }

    Assert.True(
        compile
            { baseline with
                Registrations = missing :: baseline.Registrations.Tail
            }
        |> Result.isError
    )
...
let ``normal admin and release principal or environment crossover fails closed`` () =
    let normal =
        ...
        |> List.find (fun value -> value.PrincipalClass = NormalCoordination)

    let crossedPrincipal =
        { normal with
            AppPrincipal = "release-app"
        }

    Assert.Contains(
        InvalidPrincipalBinding normal.Id,
        compile
            { baseline with
                Registrations =
                    crossedPrincipal
                    :: (baseline.Registrations |> List.filter (fun value -> value.Id <> normal.Id))
            }
        |> findings
    )

    let crossedEnvironment =
        { normal with
            Environment = "coordination-admin"
        }

    Assert.Contains(
        InvalidEnvironmentBinding normal.Id,
        compile
            { baseline with
                Registrations =
                    crossedEnvironment
...
let ``qualification requires independent and generated controls`` () =
    ...
    let broken =
        passing
        |> List.map (fun value ->
            if value.Control = LeastPrivilegeWorkflow then
                { value with ControlPassed = false }
            else
                value)

    Assert.True(
        GitHubPermissionCompilationQualification.validate passing broken
        |> Result.isError
    )
