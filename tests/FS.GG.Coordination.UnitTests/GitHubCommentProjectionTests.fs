...
let private instant minute =
    DateTimeOffset(2026, 8, 31, 10, minute, 0, TimeSpan.Zero)
...
let private page number terminal cursor comments : CommentPage =
    {
        Number = number
...
let private request digest human : ProjectionRequest =
    {
        ...
        HumanBody = human
        CausationIdentity = "issue-149"
    }

let private render digest human =
    CommentProjectionAdapter.renderProjection (request digest human)
...
let private expected (value: CommentObservation) : ExpectedProjection =
    {
...
let ``comment reads preserve terminal pages and reject duplicate reordered incomplete unauthorized and unreadable observations``
    ...
    match
        CommentProjectionAdapter.readComments (
            complete "rev-1" [ page 1 false (Some "cursor-1") [ first ]; page 2 true None [ second ] ]
        )
    with
    | Ok observed ->
    ...
    Assert.Equal(
        Error(DuplicateCommentDatabaseId 10L),
        CommentProjectionAdapter.readComments (
            complete
                "rev"
                [
    ...
    Assert.Equal(
        ...
        CommentProjectionAdapter.readComments (complete "rev" [ page 1 true None [ second; first ] ])
    )

    Assert.Equal(
        Error(CommentObservationIncomplete("truncated", Some "cursor")),
        CommentProjectionAdapter.readComments (CommentsIncomplete("truncated", Some "cursor"))
    )

    Assert.Equal(
        Error(CommentObservationUnauthorized "denied"),
        CommentProjectionAdapter.readComments (CommentsUnauthorized "denied")
    )

    Assert.Equal(
        Error(CommentObservationUnreadable "transport"),
        CommentProjectionAdapter.readComments (CommentsUnreadable "transport")
    )
...
let ``canonical marker parsing and durable authority trust reject malformed tampered edited and deleted projections distinctly``
    ...
    match
        CommentProjectionAdapter.evaluateTrust
            (Some(expected malformed))
            (authority digest)
            (snapshot (complete "rev" [ page 1 true None [ malformed ] ]))
    with
    ...
    Assert.Equal(
        Error(ProjectionEdited current.Identity),
        CommentProjectionAdapter.evaluateTrust (Some editedExpected) (authority digest) observed
    )

    Assert.Equal(
        Error(ProjectionDeleted current.Identity),
        CommentProjectionAdapter.evaluateTrust
            (Some(expected current))
            (authority digest)
            (snapshot (complete "rev" [ page 1 true None [] ]))
    )
...
let ``projection planning is byte deterministic idempotent and distinguishes create replace and no-op`` () =
    ...
    | Ok(ProjectionPlanned plan) ->
        ...
        | CreateProjection bytes -> Assert.True((desired = bytes))
        | _ -> failwith "expected create"
    | value -> failwithf "%A" value
    ...
    match
        CommentProjectionAdapter.planProjection (Some(expected existing)) (request digest "Projection body") before
    with
    ...
    | Ok(ProjectionPlanned {
                               Operation = ReplaceProjection(identity, _, _, _)
...
let ``mandatory reread and exact post-state verification refuse stale authority and unrelated change`` () =
    ...
    let beforeObservation =
        complete "rev-1" [ page 1 true None [ existing; unrelated ] ]

    let before = snapshot beforeObservation

    let plan =
        match CommentProjectionAdapter.planProjection (Some(expected existing)) (request digest "New") before with
        | Ok(ProjectionPlanned value) -> value
        | value -> failwithf "%A" value

    Assert.True(
        CommentProjectionAdapter.checkPreState (authority digest) plan beforeObservation
        |> Result.isOk
    )

    Assert.Equal(
        Error DurableAuthorityChanged,
        CommentProjectionAdapter.checkPreState (authority (String.replicate 64 "f")) plan beforeObservation
    )

    Assert.Equal(
        Error(ProjectionReReadRequired("rev-1", "rev-2")),
        CommentProjectionAdapter.checkPreState
            (authority digest)
            plan
            (complete "rev-2" [ page 1 true None [ existing; unrelated ] ])
    )

    let result =
        { existing with
            UpdatedAt = instant 5
            Body = plan.DesiredBody
        }

    Assert.True(
        CommentProjectionAdapter.verifyPostState
            "rev-2"
            result
            plan
            (complete "rev-2" [ page 1 true None [ result; unrelated ] ])
        |> Result.isOk
    )

    let changedUnrelated =
        { unrelated with
    ...
    Assert.Equal(
        ...
        CommentProjectionAdapter.verifyPostState
            ...
            result
            plan
            (complete "rev-2" [ page 1 true None [ result; changedUnrelated ] ])
    )
...
let ``comment projection qualification inventory is closed and every mutation must turn red`` () =
    ...
    | Error findings ->
        Assert.Contains(
            findings,
            fun finding ->
                finding.Code = "GCPQ-INDEPENDENT-NOT-RED"
                && finding.ControlId = "tampered-marker"
        )
