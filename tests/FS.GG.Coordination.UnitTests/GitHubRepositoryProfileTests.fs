...
let private rows =
    [
        ...
        {
            Id = "sir"
            FullName = "EHotwagner/S.I.R."
            Role = NonParticipant
            Capabilities = []
            KitDelivery = None
            AbsenceCover = None
            Reason = Some "external owner"
        }
    ]
...
let ``repository profiles retain rich authority and never plan writes for external rows`` () =
    let first =
        RepositoryProfileAdapter.compile reviewedAt (TimeSpan.FromHours 1) (snapshot rows)
        |> Result.defaultWith (failwithf "%A")

    let second =
        RepositoryProfileAdapter.compile reviewedAt (TimeSpan.FromHours 1) (snapshot (List.rev rows))
        |> Result.defaultWith (failwithf "%A")
...
let ``repository profile compiler fails closed on identity source freshness and vocabulary defects`` () =
    ...
    let duplicateFindings =
        RepositoryProfileAdapter.compile reviewedAt (TimeSpan.FromHours 1) (snapshot duplicate)
        |> findings
    ...
    let unsupported =
        ...
        |> List.map (fun row ->
            if row.Id = "sdd" then
                { row with
                    Capabilities = "unknown" :: row.Capabilities
                }
            else
                row)

    Assert.Contains(
        UnsupportedRepositoryCapability("FS-GG/FS.GG.SDD", "unknown"),
        RepositoryProfileAdapter.compile reviewedAt (TimeSpan.FromHours 1) (snapshot unsupported)
        |> findings
    )
    ...
    Assert.Equal(
        Error [ StaleRoster ],
        RepositoryProfileAdapter.compile (reviewedAt.AddHours 2) (TimeSpan.FromHours 1) stale
    )

    Assert.Contains(
        InvalidSourceBinding "sourceRevision",
        RepositoryProfileAdapter.compile reviewedAt (TimeSpan.FromHours 1) { stale with SourceRevision = "main" }
        |> findings
    )

    Assert.Contains(
        InvalidSourceBinding "canonicalRosterSha256",
        RepositoryProfileAdapter.compile
            reviewedAt
            (TimeSpan.FromHours 1)
            { stale with
                CanonicalRosterSha256 = String.replicate 64 "0"
            }
        |> findings
    )

    let baseline =
        RepositoryProfileAdapter.compile reviewedAt (TimeSpan.FromHours 1) stale
        |> Result.defaultWith (failwithf "%A")
    ...
    let crossed =
        ...
        |> List.map (fun row ->
            if row.Id = "sdd" then
                { row with
                    FullName = "someone/FS.GG.SDD"
                }
            else
                row)

    Assert.Contains(
        UnsupportedRepositoryRole "someone/FS.GG.SDD",
        RepositoryProfileAdapter.compile reviewedAt (TimeSpan.FromHours 1) (snapshot crossed)
        |> findings
    )
...
let ``repository profile qualification requires both complete mutation inventories`` () =
    ...
    let broken =
        passing
        |> List.map (fun result ->
            if result.Control = GitHubRepositoryProfileControl.ExternalObserveOnly then
