...
let private fact authority outcome =
    {
        Subject = subject
...
let private status =
    {
        ...
        ProjectId = live "PROJECT_1"
        ItemId = live "ITEM_1"
        FieldId = live "FIELD_1"
        FieldName = name "Status"
        Options = options
        SelectedOptionId = Some options[0].Id
    }
...
let private observation =
    {
        ...
        Hold = hold FactProvenAbsent
        Dependency = dependency FactProvenAbsent
        Claim = claim FactProvenAbsent
        PullRequest = pullRequest FactProvenAbsent
        Review = review FactProvenAbsent
        Delivery = delivery FactProvenAbsent
        DeliveryProtected = false
...
let ``formal lifecycle precedence is preserved and status remains a projection`` () =
    let cases =
        [
            ...
            { observation with
                Intent = IntentPaused
                Hold = hold FactObserved
            },
            StageBlocked,
            "Blocked"
            { observation with
                Intent = IntentReady
                Claim = claim FactObserved
            },
            StageClaimed,
            "In progress"
            { observation with
                Intent = IntentReady
                PullRequest = pullRequest FactObserved
            },
            StageInReview,
            "In review"
            { observation with
            ...
            { observation with
                ...
                PullRequest = pullRequest FactObserved
                Review = review FactObserved
                Delivery = delivery FactObserved
    ...
    for value, expectedStage, expectedStatus in cases do
        let stage =
            LifecycleProjectionAdapter.derive value |> Result.defaultWith (failwithf "%A")
    ...
    let forgedStatus =
        { observation with
            Status =
                { status with
                    SelectedOptionId = Some options[5].Id
                }
        }
...
let ``unknown historical contradictory and unprotected facts fail closed`` () =
    Assert.Equal(
        Error [ LifecycleFactNotKnowledge("claim", FactUnreadable) ],
        LifecycleProjectionAdapter.derive
            { observation with
                Claim = claim FactUnreadable
            }
    )

    Assert.Equal(
        Error [ HistoricalLifecycleFact "claim" ],
        LifecycleProjectionAdapter.derive
            { observation with
                Claim =
                    { claim FactObserved with
                        Current = false
                    }
            }
    )
    ...
    Assert.Equal(
        Error [ WrongLifecycleAuthority("claim", ReviewJournalAuthority) ],
        LifecycleProjectionAdapter.derive
            { observation with
                Claim = review FactObserved
            }
    )

    Assert.Equal(
        Error [ InvalidLifecycleFactRevision ],
        LifecycleProjectionAdapter.derive
            { observation with
                Claim =
                    { claim FactObserved with
    ...
    Assert.Equal(
        Error
            [
                ContradictoryLifecycleFacts "accepted review requires a current pull request until delivery"
            ],
        LifecycleProjectionAdapter.derive
            { observation with
                Review = review FactObserved
    ...
    Assert.Equal(
        Error [ UnprotectedLifecycleDelivery ],
        LifecycleProjectionAdapter.derive
            { observation with
                Review = review FactObserved
                Delivery = delivery FactObserved
    ...
    Assert.Equal(
        Error [ ClosedIssueWithoutTerminalAuthority ],
        LifecycleProjectionAdapter.derive
            { observation with
                IssueState = IssueClosed
            }
...
let ``exact plan fences source and status revisions and verifies authoritative poststate`` () =
    ...
    let plan =
        LifecycleProjectionAdapter.plan "cause-1" requested
    ...
    Assert.True(
        LifecycleProjectionAdapter.authorize plan requested (statusObservation status)
        |> Result.isOk
    )

    Assert.Equal(
        Error [ AlteredLifecyclePlan ],
        LifecycleProjectionAdapter.authorize
            plan
            { requested with
                Intent = IntentBacklog
            }
    ...
    let applied =
        { status with
            Revision = "status-2"
            SelectedOptionId = Some options[1].Id
        }

    Assert.True(
        LifecycleProjectionAdapter.verify "status-2" plan requested (statusObservation applied)
        |> Result.isOk
    )

    let alteredCost =
        { plan with
            Cost = { plan.Cost with MaximumEffects = 0 }
        }

    Assert.Equal(
        Error [ AlteredLifecyclePlan ],
        LifecycleProjectionAdapter.authorize alteredCost requested (statusObservation status)
    )
        ...
        | StatusPlanned value ->
            { plan with
                StatusDecision =
                    StatusPlanned
                        { value with
                            Operation = SetStatusOperation options[5].Id
                        }
            }
    ...
    Assert.Equal(
        Error [ AlteredLifecyclePlan ],
        LifecycleProjectionAdapter.authorize alteredDecision requested (statusObservation status)
    )
    ...
    Assert.Equal(
        Error [ AlteredLifecyclePlan ],
        LifecycleProjectionAdapter.verify "status-2" alteredVerification requested (statusObservation applied)
    )
...
let ``exact equality is a zero-effect replay and qualification inventory is closed`` () =
    ...
    let first =
        LifecycleProjectionAdapter.plan "cause-1" current
        |> Result.defaultWith (failwithf "%A")

    Assert.Equal(0, first.Cost.MaximumEffects)

    match first.StatusDecision with
    | StatusNoOp _ -> ()
    | _ -> failwith "expected no-op"
