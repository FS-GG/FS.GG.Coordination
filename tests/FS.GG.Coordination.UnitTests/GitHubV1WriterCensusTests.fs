        ...
        for index in 0..53 ->
            {
                ...
                Writes =
                    if index < 20 then "always"
                    elif index < 26 then "conditional"
                    else "never"
            }
    ]
...
let private snapshot =
    {
        ...
        ProducerRevision = revision 'a'
        ProducerTree = revision 'b'
        RoadmapRevision = revision 'c'
        RoadmapSha256 = digest 'd'
        AcceptedEpochReceiptSha256 = digest 'e'
        SourceBaseRevision = revision 'f'
        Q0EvidenceSha256 = digest '1'
        Q0CorpusSha256 = digest '2'
        CensusSha256 = digest '3'
        CommandContractSha256 = digest '4'
        CheckerSha256 = digest '5'
        FixtureRunnerSha256 = digest '6'
        Commands = commands
        Sources = sources
    }
...
let ``unknown command and changed write class refuse`` () =
    let unknown =
        { snapshot with
            Commands = snapshot.Commands @ [ { Name = "new"; Writes = "always" } ]
        }
...
let ``dynamic source omission and malformed source identity refuse`` () =
    Assert.True(
        GitHubV1WriterCensusQualification.validateSnapshot { snapshot with Sources = sources.Tail }
        |> Result.isError
    )
    ...
    Assert.True(
        GitHubV1WriterCensusQualification.validateSnapshot
            { snapshot with
                Sources = malformed :: sources.Tail
            }
        |> Result.isError
    )
...
let ``non-writer sink requires an explicit justification`` () =
    ...
    let changed =
        sources
        |> List.map (fun source -> if source.Path = fixture.Path then unjustified else source)

    Assert.True(
        GitHubV1WriterCensusQualification.validateSnapshot { snapshot with Sources = changed }
        |> Result.isError
    )
...
let ``generated and independent controls must both close every mutation`` () =
    ...
    let broken =
        passing
        |> List.map (fun value ->
            if value.Control = DynamicWriterRefusal then
                { value with MutationRed = false }
            else
                value)

    Assert.True(
        GitHubV1WriterCensusQualification.validateControls passing broken
        |> Result.isError
    )
