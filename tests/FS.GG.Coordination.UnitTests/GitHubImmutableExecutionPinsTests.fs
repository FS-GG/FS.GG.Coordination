...
let private action workflow =
    {
        ...
        TargetRepository = "actions/checkout"
        TargetPath = None
        Revision = revision
        Kind = ThirdPartyAction
    }

let private workflow path =
    {
        Path = path
        Sha256 = digest
        References = [ action path ]
    }
...
let private renovate =
    {
        ...
        PolicyPath = "renovate.json"
        PolicySha256 = digest
        OwnedManagers = [ "github-actions" ]
    }
...
let private snapshot =
    {
        ...
        Complete = true
        Workflows = [ workflow ".github/workflows/ci.yml" ]
        Publications = []
        RequiredUpdaterConfigurationPaths = [ ".github/dependabot.yml"; "renovate.json" ]
        RequiredUpdaterInvocationSelectors = [ "RENOVATE_CONFIG_FILE" ]
        UpdaterConfigurations = []
        Updaters = [ renovate ]
        RequiredManagers = [ "github-actions" ]
    }

let private compile value =
    GitHubImmutableExecutionPinsQualification.compile value
...
let ``Renovate is sole PR-only automated authority with complete manager ownership`` () =
    Assert.True(
        compile
            { snapshot with
                Updaters = [ { renovate with Name = "dependabot" } ]
            }
        |> Result.isError
    )
    ...
    Assert.True(
        compile
            { snapshot with
                Updaters = [ renovate; { renovate with Name = "custom" } ]
            }
        |> Result.isError
    )
    ...
    Assert.Contains(
        ...
        compile
            { snapshot with
                UpdaterConfigurations = [ dependabot ]
            }
        |> errors
    )
    ...
    Assert.Contains(
        ...
        compile
            { snapshot with
                UpdaterConfigurations = [ branchAutomerge ]
            }
        |> errors
    )
...
let ``qualification requires every generated and independent control`` () =
    ...
    let broken =
        passing |> List.filter (fun value -> value.Control <> RenovateSoleUpdater)
