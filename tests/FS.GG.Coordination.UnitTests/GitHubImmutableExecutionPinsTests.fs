...
let private snapshot =
    { SchemaVersion = 1; Repository = "FS-GG/example"; SourceRevision = revision
      ...
      UpdaterConfigurations = []; Updaters = [ renovate ]; RequiredManagers = [ "github-actions" ] }
...
let ``Renovate is sole PR-only automated authority with complete manager ownership`` () =
    ...
    let branchAutomerge =
        { dependabot with Path = "renovate.jsonc"; Authority = "renovate"
                          PullRequestOnly = false; DirectPush = true }
    Assert.Contains(InvalidUpdaterConfiguration, compile { snapshot with UpdaterConfigurations = [ branchAutomerge ] } |> errors)
