...
let ``intent is authoritative and status is its derived projection`` () =
    ...
    for intent, status, hold, expectedIntent, expectedStatus in cases do
        match
            OrganizationIssueFields.validate
                { valid intent with
                    SchedulingIntent = Some intent
                    LifecycleStatus = Some status
                    HoldReason = hold
                }
        with
        | Ok fields ->
            Assert.Equal(expectedIntent, fields.SchedulingIntent)
            Assert.Equal(expectedStatus, fields.LifecycleStatus)
...
let ``contract and touch set projections require exact authoritative bindings`` () =
    ...
    let bound =
        { valid "bound" with
            ...
            ContractAuthorityDigest = Some digest
            TouchSet = paths
    ...
    Assert.Contains(
        ...
        codes
            { bound with
                TouchSetAuthorityDigest = Some(String.replicate 64 "b")
            }
    )
...
let ``all registered refusal families are stable and corpus plans all or nothing`` () =
    ...
    for observation, expected in cases do
        Assert.Contains(expected, codes observation)
    ...
    | Ok dispositions ->
        ...
        let reversed =
            OrganizationIssueFields.plan [ valid "a"; valid "b" ]
    ...
    | Error refusals ->
        Assert.All(
            refusals,
            fun refusal -> Assert.Contains(OrganizationIssueFieldDiagnostic.DuplicateStableRowId, refusal.Diagnostics)
        )
