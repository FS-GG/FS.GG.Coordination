...
let decision raw normalized revision =
    {
        Raw = raw
...
let equalItem repo item value =
    {
        Repository = repo
        Item = item
        V1 = decision value value "v1-r1"
        V2 = decision value value "v2-r1"
        Divergence = None
    }
...
let divergentItem repo item classification =
    {
        Repository = repo
        Item = item
        V1 = decision "legacy-ready" "ready" "v1-r1"
        V2 = decision "native-ready" "native-ready" "v2-r1"
        Divergence =
            Some
                {
                    Classification = classification
                    AccountableAgent = "critic-1"
...
let refusals =
    function
    | Ok report -> failwithf "expected refusal, got %A" report
    | Error values -> values

let observation =
    {
        Complete = true
        RosterRevision = "roster-r1"
        Roster = [ "fs-gg/.github"; "fs-gg/fs.gg.coordination" ]
        WindowStartedAt = start
        WindowEndedAt = finish
        Capabilities = FleetShadowAdapter.requiredCapabilities
        MutationAttempts = []
        Repositories =
            [
                {
...
let ``coverage divergence and subject defects fail closed`` () =
    let missingRepo =
        { observation with
            Repositories = observation.Repositories |> List.tail
        }

    Assert.Contains(
        MissingFleetRepository "fs-gg/.github",
        FleetShadowAdapter.compare asOf (TimeSpan.FromHours 1) missingRepo |> refusals
    )
        ...
        { observation with
            Repositories =
                [
                    ...
                    observation.Repositories[1]
                ]
        }

    Assert.Contains(
        IncompleteFleetRepository "fs-gg/.github",
        FleetShadowAdapter.compare asOf (TimeSpan.FromHours 1) incomplete |> refusals
    )

    let unclassifiedItem =
        { divergentItem "FS-GG/.github" "FS-GG/.github#1" V1Defect with
            Divergence = None
        }
    ...
    Assert.Contains(
        UnclassifiedFleetDivergence "FS-GG/.github#1",
        FleetShadowAdapter.compare asOf (TimeSpan.FromHours 1) unclassified |> refusals
    )

    let crossedItem =
        { observation.Repositories.Head.Items.Head with
    ...
    Assert.Contains(
        CrossRepositoryFleetItem "FS-GG/.github#1",
        FleetShadowAdapter.compare asOf (TimeSpan.FromHours 1) crossed |> refusals
    )
        ...
        { observation with
            Repositories =
                [
                    ...
                    { observation.Repositories[1] with
                        Items =
                            [
                                { observation.Repositories[1].Items.Head with
                                    Item = "FS-GG/.github#1"
                                }
                            ]
                    }
    ...
    Assert.Contains(
        DuplicateFleetItem "FS-GG/.github#1",
        FleetShadowAdapter.compare asOf (TimeSpan.FromHours 1) duplicateAcrossRepositories
        |> refusals
    )
...
let ``permissions attempts freshness and seals are independent fences`` () =
    let permission =
        { observation with
            Capabilities = observation.Capabilities @ [ MutationCapability "issues:write" ]
        }

    let permissionErrors =
        FleetShadowAdapter.compare asOf (TimeSpan.FromHours 1) permission |> refusals
    ...
    Assert.Equal(
        Error [ FleetMutationAttempted "PATCH /issues/1" ],
        FleetShadowAdapter.compare
            asOf
            (TimeSpan.FromHours 1)
            { observation with
                MutationAttempts = [ "PATCH /issues/1" ]
            }
    )

    Assert.Equal(
        Error [ StaleFleetObservation ],
        FleetShadowAdapter.compare (finish.AddHours 2) (TimeSpan.FromHours 1) observation
    )
    ...
    let changed =
        { observation with
            RosterRevision = "roster-r2"
        }

    Assert.Equal(
        Error [ AlteredFleetShadowSeal ],
        FleetShadowAdapter.verify report.Seal asOf (TimeSpan.FromHours 1) changed
    )
...
let ``generated and independent Q4 control inventories are exact`` () =
    let passing: GitHubFleetShadowControlResult list =
        ...
        |> List.map (fun control ->
            ...
            : GitHubFleetShadowControlResult))
