...
let private digest =
    "9e6528221e1b8440f19495a2783ef70872c2058060f89a08d37cf4438afeb5f3"
...
let private baseline =
    {
        ...
        RequiredReviewers = 1
        OidcProvider = "github-actions-oidc"
        OidcAudience = "nuget.org"
        LongLivedCredential = false
        ReleaseImmutable = true
        TagImmutable = true
        PackCount = 1
        PackageId = "FS.GG.Coordination.Protocol"
        PackageVersion = "2.0.0-candidate"
        ...
        Recovery =
            {
                FailureAfterFeed = "github-packages"
                ResumeFeed = "nuget-org"
                SourcePackageSha256 = digest
                Repack = false
            }
        SbomFormat = "spdx-2.3"
        SbomSubjectSha256 = digest
        AttestationPredicate = "slsa-provenance-v1"
        AttestationSubjectSha256 = digest
        DependencySubmission = true
        DependencyReview = true
        PublicDownloadAnonymous = true
        PublicDownloadStatus = 200
        PublicDownloadSha256 = digest
    }

[<Fact>]
let ``complete protected OIDC plan compiles and replays exactly`` () =
    let report =
        GitHubReleaseHardeningQualification.compile baseline
...
let ``environment identity and immutable surfaces fail closed`` () =
    Assert.True(
        GitHubReleaseHardeningQualification.compile
            { baseline with
...
let ``one pack and ordered dual feed identity fail closed`` () =
    Assert.True(
        GitHubReleaseHardeningQualification.compile { baseline with PackCount = 2 }
        |> Result.isError
    )
    ...
    Assert.True(
        GitHubReleaseHardeningQualification.compile
            { baseline with
                FeedPublications = List.rev baseline.FeedPublications
            }
        |> Result.isError
    )

    let divergent =
        baseline.FeedPublications
        |> List.map (fun value ->
            if value.Feed = "nuget-org" then
                { value with
                    PackageSha256 = String.replicate 64 "0"
                }
            else
                value)
...
let ``SBOM attestation dependency and public verification fail closed`` () =
    Assert.True(
        GitHubReleaseHardeningQualification.compile
            { baseline with
                SbomSubjectSha256 = String.replicate 64 "0"
            }
        |> Result.isError
    )
...
let ``stage ordering and no-repack recovery are exact`` () =
    Assert.True(
        GitHubReleaseHardeningQualification.compile
            { baseline with
                Stages = List.rev baseline.Stages
            }
        |> Result.isError
    )
    ...
    Assert.True(
        GitHubReleaseHardeningQualification.compile
            { baseline with
                Recovery = { baseline.Recovery with Repack = true }
            }
        |> Result.isError
    )
...
let ``generated and independent control inventories must both pass`` () =
    let passing: GitHubReleaseHardeningControlResult list =
    ...
    let broken =
        passing
        |> List.map (fun value ->
            if value.Control = PublicDownloadControl then
