...
let private edge kind source target =
    {
        Kind = kind
...
let private complete revision scope pages =
    RelationsComplete(revision, scope, pages)

let private page number terminal edges =
    {
        Number = number
...
let ``complete reads require a contiguous terminal page chain and sort all edges`` () =
    ...
    let observed =
        complete "rev-1" ParentChild [ page 1 false [ first ]; page 2 true [ second ] ]
    ...
    Assert.Equal(
        ...
        NativeRelations.read (complete "rev-1" ParentChild [ page 2 true [ first ] ])
    )

    Assert.Equal(
        Error InvalidRelationPageChain,
        NativeRelations.read (complete "rev-1" ParentChild [ page 1 false [ first ] ])
    )
...
let ``reads preserve refusals and reject duplicate self and wrong-kind edges`` () =
    ...
    Assert.Equal(
        Error(RelationObservationRefused(ObservationUnauthorized "denied")),
        NativeRelations.read (RelationsUnauthorized "denied")
    )

    Assert.Equal(
        Error(DuplicateRelationEdge relation),
        NativeRelations.read (complete "rev" Blocks [ page 1 true [ relation; relation ] ])
    )
    ...
    Assert.Equal(
        Error(RelationKindMismatch(ParentChild, Blocks)),
        NativeRelations.read (complete "rev" ParentChild [ page 1 true [ relation ] ])
    )
...
let ``planning is edge-local deterministic collision-safe and typed for no-ops`` () =
    ...
    let snapshot =
        NativeRelations.read (complete "rev" Blocks [ page 1 true [ unrelated; existing ] ])
    ...
    let empty =
        NativeRelations.read (complete "rev" Blocks [ page 1 true [] ])
        |> Result.defaultWith (failwithf "%A")

    let key intent =
        match NativeRelations.plan "rev" "cause" intent empty with
        | Ok(RelationPlanned value) -> value.IdempotencyIdentity
        | value -> failwithf "%A" value
...
let ``stale reread and concurrent pre-state changes refuse before execution`` () =
    ...
    let before =
        NativeRelations.read (complete "rev-1" ParentChild [ page 1 true [] ])
        |> Result.defaultWith (failwithf "%A")

    let plan =
        match NativeRelations.plan "rev-1" "cause" (AddEdge target) before with
        | Ok(RelationPlanned value) -> value
        | value -> failwithf "%A" value

    Assert.Equal(
        Error(ReReadRequired("rev-1", "rev-2")),
        NativeRelations.checkPreState plan (complete "rev-2" ParentChild [ page 1 true [] ])
    )

    match
        NativeRelations.checkPreState plan (complete "rev-1" ParentChild [ page 1 true [ edge ParentChild "P2" "C2" ] ])
    with
...
let ``post-state verification requires exact delta revision and unrelated-edge preservation`` () =
    ...
    let before =
        NativeRelations.read (complete "rev-1" Blocks [ page 1 true [ unrelated ] ])
        |> Result.defaultWith (failwithf "%A")
    ...
    Assert.Equal(
        Error(ResultRevisionDidNotAdvance "rev-1"),
        NativeRelations.verifyPostState "rev-1" plan (complete "rev-1" Blocks [ page 1 true [ target; unrelated ] ])
    )

    Assert.True(
        NativeRelations.verifyPostState "rev-2" plan (complete "rev-2" Blocks [ page 1 true [ target; unrelated ] ])
        |> Result.isOk
    )

    Assert.Equal(
        Error(ResultRevisionMismatch("rev-2", "rev-3")),
        NativeRelations.verifyPostState "rev-2" plan (complete "rev-3" Blocks [ page 1 true [ target; unrelated ] ])
    )
...
let ``native relation qualification inventory is exact and every mutation must turn red`` () =
    ...
    | Error findings ->
        Assert.Contains(
            findings,
            fun finding ->
                finding.Code = "GNRQ-INDEPENDENT-NOT-RED"
                && finding.ControlId = "reversed-endpoint"
        )
