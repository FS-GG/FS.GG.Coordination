...
let private plan =
    {
        ...
        Authority =
            {
                Actor = "sandbox-app[bot]"
                ActorId = "A_sandbox"
                Credential = NonProductionApp
                Purpose = "isolated qualification"
                Permissions = Set [ "contents:write" ]
            }
        Targets =
            {
                Owner = "FS-GG-Sandbox"
                Repository = "disposable"
                RepositoryNodeId = "R_sandbox"
                ProjectNodeId = "PVT_sandbox"
                PurposeMarker = "fsgg-sandbox-unit"
                Production = false
            }
        Quota =
            {
    ...
    |> List.map (fun operation ->
        {
            ...
            AuthoritativeRevision = Some operation.ExpectedRevision
...
let private receipt () =
    let unsigned =
        {
            ...
            WorkflowRunId = "1"
            ArtifactName = "github-sandbox-closure"
            CreatedAt = now
    ...
    { unsigned with
        ResultDigest = GitHubSandboxClosure.resultDigest unsigned
    }

let private sign (value: SandboxClosureReceipt) =
    { value with
...
let ambiguousEffectResidualCleanupAndWarmReuseFailIndependently () =
    ...
    let repeated =
        sign
            { baseline with
                ChildGates =
                    { baseline.ChildGates.[1] with
                        ProcessId = baseline.ChildGates.Head.ProcessId
                    }
                    :: baseline.ChildGates.Tail
            }
...
let qualificationInventoryRequiresEveryIndependentControl () =
    let passing: GitHubSandboxClosureControlResult list =
    ...
    Assert.True(
        GitHubSandboxClosureQualification.validate passing passing.Tail
        |> Result.isError
    )
