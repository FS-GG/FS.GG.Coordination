    ...
    let input =
        {
            ...
            FleetId = "fs-gg-production"
            Ref = "refs/heads/fsgg/v2/journal/cutover/d5"
            Tag = "refs/tags/fsgg/v2/fleet-cutover/operating-v1/test"
            ManifestSha256 = digest "1"
            TrustAnchorSha256 = digest "2"
            SourceSha256 = digest "3"
            DesiredPolicySha256 = digest "4"
            FirstCaptureSha256 = digest "5"
            SecondCaptureSha256 = digest "6"
            AuthorizationKeyId = "test"
            AuthorizationKeySpkiSha256 = keyDigest
            AuthorizationWorkflowRevision = String.replicate 40 "a"
            AuthorizationWorkflowSha256 = digest "8"
            CutoverAppId = 4882399L
            CutoverInstallationId = 160261436L
            ControlIssueNumber = 2L
            ExpectedRef = ExpectedAbsent
            CreatedAt = DateTimeOffset.Parse("2026-09-09T10:00:00Z")
            AuthorName = "FS.GG cutover"
            AuthorEmail = "cutover@fs.gg"
        }
    ...
    let authority =
        {
            KeyId = "test"
            PublicKeyPem = pem
            PublicKeySpkiSha256 = keyDigest
            Payload = payload
            Signature = rsa.SignData(payload, HashAlgorithmName.SHA256, RSASignaturePadding.Pss)
            AuthorizedAt = input.CreatedAt.AddMinutes(-1.)
            ExpiresAt = input.CreatedAt.AddMinutes(30.)
        }

    input,
    authority,
    LedgerInitializationAdapter.plan input.CreatedAt authority input
...
let ``apply is idempotent and refuses competing leases`` () =
    ...
    let port =
        {
            ...
            PutObject =
                fun value ->
                    ...
                    Ok()
            CreateRef =
                fun name oid expected ->
                    match expected, Map.tryFind name refs with
                    | ExpectedAbsent, None ->
                        refs <- Map.add name oid refs
                        Ok()
                    | _ -> Error "lease"
        }
    ...
    let readback =
        {
            Ref = RefAt plan.Commit.Oid
            Tag = RefAt plan.Commit.Oid
...
let ``authorization expiry and changed binding refuse planning`` () =
    ...
    Assert.True(
        LedgerInitializationAdapter.plan authority.ExpiresAt authority input
        |> Result.isError
    )

    Assert.True(
        LedgerInitializationAdapter.plan input.CreatedAt authority { input with ControlIssueNumber = 3L }
        |> Result.isError
    )

    Assert.True(
        LedgerInitializationAdapter.plan
            input.CreatedAt
            authority
            { input with
                AuthorizationKeySpkiSha256 = digest "9"
            }
        |> Result.isError
    )
    ...
    Assert.True(
        LedgerInitializationAdapter.plan
            ...
            authority
            { input with
                AuthorizationWorkflowRevision = "main"
            }
        |> Result.isError
    )
...
let private operationalFixture () =
    ...
    let context =
        {
            RepositoryId = 1351660651L
            FleetRef = "refs/heads/fsgg/v2/journal/cutover/d5"
            ControlIssueNumber = 2L
            DesiredPolicySha256 = digest "4"
            ProviderObservationSha256 = digest "5"
            InitializationSeal = Some(digest "6")
            MonitorStoreId = Some "private-store-1"
            SignerPublicKeySpkiSha256 = keyDigest
        }
        ...
        let unsigned =
            {
                ...
                Context = context
                RunId = "run-17"
                InputSha256 = digest "7"
                ObservedAt = observed
                ExpiresAt = observed.AddMinutes(15.)
                Authority = "FS-GG/fleet-cutover"
                SignerKeyId = "test-operational"
                PublicKeyPem = pem
                PublicKeySpkiSha256 = keyDigest
                Payload = [||]
                Signature = [||]
    ...
    observed,
    [
        SettingsAppliedEvidence
        AppCustodyEvidence
        FleetInitializedEvidence
        MonitoringEvidence
    ]
    |> List.map signed
...
let ``operational state is derived only from complete signed evidence`` () =
    ...
    let state =
        LedgerOperationalEvidence.derive (observed.AddMinutes(1.)) context evidence
...
let ``missing duplicate and stale operational evidence remain explicit`` () =
    ...
    let duplicated =
        LedgerOperationalEvidence.derive (observed.AddMinutes(1.)) context duplicate

    Assert.Equal(LedgerObservation.Unknown "invalid-or-duplicate-operational-evidence", duplicated.SettingsApplied)

    let missing =
        LedgerOperationalEvidence.derive
            (observed.AddMinutes(1.))
            context
            (evidence |> List.filter (fun item -> item.Dimension <> MonitoringEvidence))

    Assert.Equal(LedgerObservation.Unknown "missing-operational-evidence", missing.MonitoringReady)

    let stale =
        LedgerOperationalEvidence.derive (observed.AddMinutes(16.)) context evidence
    ...
    let wrongSigner =
        LedgerOperationalEvidence.derive
            (observed.AddMinutes(1.))
            { context with
                SignerPublicKeySpkiSha256 = digest "9"
            }
            evidence
