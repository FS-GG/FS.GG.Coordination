...
let ``authorization expiry and changed binding refuse planning`` () =
    ...
    Assert.True(LedgerInitializationAdapter.plan input.CreatedAt authority {input with AuthorizationKeySha256=digest "9"}|>Result.isError)
...
let private operationalFixture () =
    ...
    let pem=rsa.ExportSubjectPublicKeyInfoPem()
    let keyDigest=SHA256.HashData(System.Text.Encoding.UTF8.GetBytes pem)|>Convert.ToHexString|>_.ToLowerInvariant()
    let context={RepositoryId=1351660651L;FleetRef="refs/heads/fsgg/v2/journal/cutover/d5";ControlIssueNumber=2L;DesiredPolicySha256=digest "4";ProviderObservationSha256=digest "5";InitializationSeal=Some(digest "6");MonitorStoreId=Some "private-store-1";SignerPublicKeySha256=keyDigest}
    let signed dimension =
        let unsigned={Dimension=dimension;Context=context;RunId="run-17";InputSha256=digest "7";ObservedAt=observed;ExpiresAt=observed.AddMinutes(15.);Authority="FS-GG/fleet-cutover";SignerKeyId="test-operational";PublicKeyPem=pem;PublicKeySha256=keyDigest;Payload=[||];Signature=[||]}
...
let ``missing duplicate and stale operational evidence remain explicit`` () =
    ...
    let wrongSigner=LedgerOperationalEvidence.derive (observed.AddMinutes(1.)) {context with SignerPublicKeySha256=digest "9"} evidence
