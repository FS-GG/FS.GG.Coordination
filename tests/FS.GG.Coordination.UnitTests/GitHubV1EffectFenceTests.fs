...
let private expectation effectClass eligible =
    {
        ...
        EpochCommit = commit
        EpochGeneration = 7L
        ExpectedClaimGeneration = Some 3L
        CurrentClaimGeneration = Some 3L
        ExpectedOperationGeneration = 5L
        CurrentOperationGeneration = 5L
    }
    ...
    let effect =
        {
            ApplyOnce =
                fun observed ->
                    events.Add $"effect:{observed.OperationId}"
                    attempt
        }
...
let ``OperatingV1 reads verifies then applies exactly once`` () =
    let outcome, events =
        run (evidence "OperatingV1" true false "") (expectation NewV1Admission false) (EffectApplied(sha '1'))
...
let ``Preparing admits only eligible incumbent already admitted`` () =
    let allowed, allowedEvents =
        run (evidence "Preparing" true false "") (expectation EligibleIncumbentV1 true) EffectProvenAbsent
    ...
    for expected in [ expectation NewV1Admission false; expectation EligibleIncumbentV1 false ] do
        let refused, events =
            run (evidence "Preparing" true false "") expected (EffectApplied(sha '1'))

        Assert.Equal<string list>([ "read" ], events)

        match refused with
        | V1RefusedBeforeEffect reasons -> Assert.Contains("phase-refuses-writer", reasons)
...
let ``freeze switch open rollback and v2 phases refuse before effect`` () =
    for phase in
        ...
        let outcome, events =
            run (evidence phase true false "") (expectation EligibleIncumbentV1 true) (EffectApplied(sha '1'))

        Assert.Equal<string list>([ "read" ], events)

        match outcome with
        | V1RefusedBeforeEffect reasons -> Assert.Contains("phase-refuses-writer", reasons)
        | _ -> Assert.Fail $"{phase} must refuse"
...
let ``stale cache manifest and generation mismatches refuse with zero effects`` () =
    let cases =
        [
            ...
            evidence "OperatingV1" true false "",
            { expectation NewV1Admission false with
            ...
            evidence "OperatingV1" true false "",
            { expectation NewV1Admission false with
                CurrentClaimGeneration = Some 4L
            },
            "stale-claim-generation"
            evidence "OperatingV1" true false "",
            { expectation NewV1Admission false with
                CurrentOperationGeneration = 6L
            },
    ...
    for bytes, expected, reason in cases do
        ...
        match outcome with
        | V1RefusedBeforeEffect reasons -> Assert.Contains(reason, reasons)
        | _ -> Assert.Fail reason
...
let ``strict evidence rejects unknown duplicate missing and unreadable fields`` () =
    ...
    for bytes, reason in
        [
            ...
            duplicate, "duplicate-field:phase"
            unreadable, "unreadable-json"
        ] do
        let outcome, events =
            run bytes (expectation NewV1Admission false) (EffectApplied(sha '1'))
...
let ``one effect result is projected without blind retry`` () =
    for attempt, expected in
        [
            ...
            EffectOutcomeIndeterminate "lost-response", V1Indeterminate "lost-response"
        ] do
        let actual, events =
            run (evidence "OperatingV1" true false "") (expectation NewV1Admission false) attempt
