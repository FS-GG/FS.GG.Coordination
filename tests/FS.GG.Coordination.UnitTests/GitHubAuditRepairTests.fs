...
let private get =
    function
    | Ok value -> value
    | Error findings -> failwithf "unexpected findings: %A" findings

let private findings =
    function
    | Error values -> values
    | Ok value -> failwithf "expected refusal, received %A" value
...
let ``complete audit remains authoritative and converges with event history`` () =
    ...
    let dropped =
        plan.Entries |> List.find (fun entry -> entry.Subject.EndsWith("issue:304"))
    ...
    let schema =
        plan.Entries
        |> List.find (fun entry -> entry.Classifications = [ "schema-drift" ])
...
let ``audit and accepted narrow reconciliation share queued subject and scheduling key`` () =
    ...
    let narrow =
        GitHubNarrowReconciliationQualification.compile repository revision [ event ]
        |> get
...
let ``scope cursor and pages fail closed`` () =
    Assert.Contains(
        GitHubAuditRepairFinding.IncompleteAuditScope,
        compile repository revision [ repository; externalRepository ] cursor histories observations
        |> findings
    )

    let missingRepository =
        observations |> List.filter (fun row -> row.Repository <> externalRepository)

    Assert.Contains(
        GitHubAuditRepairFinding.IncompleteAuditScope,
        compile repository revision scope cursor histories missingRepository |> findings
    )

    let alteredScope =
        { observations.Head with
            AuditScope = [ repository ]
        }

    Assert.Contains(
        GitHubAuditRepairFinding.AlteredScope repository,
        compile repository revision scope cursor histories (alteredScope :: observations.Tail)
        |> findings
    )

    Assert.Contains(
        GitHubAuditRepairFinding.StaleCursor "stale",
        compile repository revision scope "stale" histories observations |> findings
    )

    let partial = observations |> List.filter (fun row -> row.Page <> 2)

    Assert.Contains(
        GitHubAuditRepairFinding.PartialPage repository,
        compile repository revision scope cursor histories partial |> findings
    )
...
let ``observation identity revision and classification fail closed`` () =
    ...
    Assert.Contains(
        GitHubAuditRepairFinding.StaleRevision "0",
        compile repository revision scope cursor histories ({ row with SubjectRevision = 0L } :: observations.Tail)
        |> findings
    )

    Assert.Contains(
        GitHubAuditRepairFinding.AlteredClassification "unknown",
        compile
            repository
            revision
            scope
            cursor
            histories
            ({ row with Classification = "unknown" } :: observations.Tail)
        |> findings
    )

    Assert.Contains(
        GitHubAuditRepairFinding.AlteredObservation "audit-drop-1",
        compile repository revision scope cursor histories ({ row with Origin = "event" } :: observations.Tail)
        |> findings
    )
    ...
    Assert.Contains(
        GitHubAuditRepairFinding.StaleRevision(String.replicate 40 "a"),
        compile repository revision scope cursor (staleHistory :: histories.Tail) observations
        |> findings
    )
    ...
    Assert.Contains(
        GitHubAuditRepairFinding.UnknownSubjectKind "mystery",
...
let ``every repair classification is mandatory`` () =
    for classification in requiredClassifications do
        ...
        Assert.Contains(
            GitHubAuditRepairFinding.OmittedClassification classification,
            compile repository revision scope cursor [] reduced |> findings
        )
...
let ``audit discovery schedules reconciler and cannot write`` () =
    ...
    Assert.Contains(
        GitHubAuditRepairFinding.AlteredRouting "reconcile/release",
        compile
            repository
            revision
            scope
            cursor
            histories
            ({ row with Route = "reconcile/release" } :: observations.Tail)
        |> findings
    )

    Assert.Contains(
        GitHubAuditRepairFinding.DirectWrite "audit-drop-1",
        compile
            repository
            revision
            scope
            cursor
            histories
            ({ row with AttemptsDerivedWrite = true } :: observations.Tail)
        |> findings
    )
...
let ``exact replay is byte identical and changed replay refuses`` () =
    ...
    Assert.Contains(
        ...
        replay plan histories (changed :: observations.Tail) |> findings
    )
