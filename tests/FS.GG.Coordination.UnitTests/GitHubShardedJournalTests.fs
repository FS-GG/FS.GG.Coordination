...
let private address kind id =
    ShardedJournalAdapter.address kind id |> Result.defaultWith (failwithf "%A")
...
let private eventBlob generation =
    blob $"{{\"generation\":{generation},\"payload\":\"event\"}}"
...
let ``canonical json recursively sorts keys and terminates with LF`` () =
    let actual =
        ShardedJournalAdapter.canonicalJson "{\"z\":2,\"a\":{\"y\":1,\"b\":0}}"
...
let ``journal validation requires one-parent monotonic append-only ancestry`` () =
    ...
    let next =
        commit location 2L false (Some root.CommitOid) (Some root.Head.HeadDigest) "next" (oid "c")

    let observation = JournalComplete("rev-2", [ root; next ])

    let snapshot =
        ShardedJournalAdapter.validate location observation
    ...
    Assert.Equal(
        ...
        ShardedJournalAdapter.validate
            location
            (JournalComplete("bad", [ root; { next with ParentOid = Some(oid "d") } ]))
    )
        ...
        ShardedJournalAdapter.validate
            ...
            (JournalComplete(
                ...
                [
                    root
                    { next with
                        Head = { next.Head with Generation = 1L }
                    }
                ]
            ))
    )
    ...
    Assert.Equal(
        ...
        ShardedJournalAdapter.validate location (JournalComplete("bad", [ terminalRoot; next ]))
    )

    let wrongDigest =
        { next with
            Event = { next.Event with Digest = digest "0" }
        }

    match ShardedJournalAdapter.validate location (JournalComplete("bad", [ root; wrongDigest ])) with
...
let ``CAS uses exact old oid and ambiguous results require exact authoritative reread`` () =
    ...
    let before =
        ShardedJournalAdapter.validate location (JournalComplete("rev-1", [ root ]))
        |> Result.defaultWith (failwithf "%A")

    let next =
        commit location 2L false (Some root.CommitOid) (Some root.Head.HeadDigest) "op-42" (oid "c")

    let proposal =
        ShardedJournalAdapter.planCas "op-42" before next
        |> Result.defaultWith (failwithf "%A")
    ...
    Assert.Equal(
        Accepted,
        ShardedJournalAdapter.reconcile proposal ReceiveResponseUnknown (JournalComplete("rev-2", [ root; next ]))
    )

    Assert.Equal(
        ResponseUnknownRequiresReread,
        ShardedJournalAdapter.reconcile proposal ReceiveResponseUnknown (JournalComplete("rev-1", [ root ]))
    )

    Assert.Equal(
        ParentConflict,
        ShardedJournalAdapter.reconcile proposal ReceiveParentConflict (JournalComplete("rev-1", [ root ]))
    )
...
let ``effect fencing fails closed on stale terminal and incomplete authority`` () =
    ...
    Assert.Equal(
        ...
        ShardedJournalAdapter.authorizeEffect { grant with Generation = 2L } observation
    )

    Assert.Equal(
        Error(EffectAuthorityUnavailable(IncompleteJournal "partial")),
        ShardedJournalAdapter.authorizeEffect grant (JournalIncomplete "partial")
    )
...
let ``saga acquisition is globally sorted and compensation is reverse append-only`` () =
    let first =
        {
            Address = address Review "z"
            ExpectedGeneration = 3L
        }

    let second =
        {
            Address = address Claim "a"
            ExpectedGeneration = 2L
        }

    let plan =
        ShardedJournalAdapter.planSaga "saga-1" [ first; second ]
        |> Result.defaultWith failwith

    let expected: SagaTouch list =
        List.sortBy
            (fun (touch: SagaTouch) ->
                ShardedJournalAdapter.journalKind touch.Address.Kind, touch.Address.Shard, touch.Address.Digest)
            [ first; second ]
    ...
    let conflict =
        ShardedJournalAdapter.planConflict plan plan.AcquisitionOrder [ plan.AcquisitionOrder.Head ]
        |> Result.defaultWith failwith
    ...
    Assert.Equal(
        ...
        ShardedJournalAdapter.planConflict plan [ first ] [ second ]
    )
...
let ``protection binds exact repository rulesets target and bypass split`` () =
    ...
    Assert.Equal(
        ...
        ShardedJournalAdapter.validateProtection
            { observed with
                EffectiveRules = [ CreationRestricted ]
            }
    )

[<Fact>]
let ``journal qualification inventory is closed and every mutation must be red`` () =
    let passing: GitHubShardedJournalControlResult list =
    ...
    | Error findings ->
        Assert.Contains(
            findings,
            fun finding ->
                finding.Code = "GSJQ-INDEPENDENT-NOT-RED"
