...
let private packageAttrs =
    Map [ "owner", "FS-GG"; "name", "pkg"; "version", "1.0.0"; "feed", "github" ]
...
let private surfaceMap =
    ...
    |> List.map (fun surface -> surface, ArtifactSurfaceObservation.Supported(revision, true, 2, grouped.[surface]))
    |> Map.ofList
...
let private observed surfaces =
    {
        ...
        Surfaces = surfaces
        Fingerprint = ActionsReleaseFeedAdapter.fingerprint repository "R_fixture" revision surfaces
    }
...
let completeExactObservationsValidate () =
    ...
    Assert.Equal(
        Error ArtifactFailure.InvalidFingerprint,
        ActionsReleaseFeedAdapter.validate { baseline with Fingerprint = "bad" }
    )
...
let attemptPaginationAndAvailabilityFailClosed () =
    ...
    let invalid =
        ...
        |> Map.add
            ActionsRuns
            (ArtifactSurfaceObservation.Supported(revision, true, 2, [ { run with Attempt = Some 0 } ]))
        |> observed

    Assert.Equal(
        Error(ArtifactFailure.InvalidLifecycle(ActionsRuns, run.Identity)),
        ActionsReleaseFeedAdapter.validate invalid
    )

    let partial =
        surfaceMap
        |> Map.add Checks (ArtifactSurfaceObservation.Supported(revision, false, 1, [ items.[1] ]))
        |> observed

    Assert.Equal(
        Error(ArtifactFailure.PartialSurface(Checks, "pagination incomplete")),
        ActionsReleaseFeedAdapter.validate partial
    )

    let denied =
        surfaceMap
        |> Map.add Feeds (ArtifactSurfaceObservation.Unauthorized "denied")
        |> observed
...
let attestationPackageAndSecretCoordinatesAreExact () =
    ...
    let invalid =
        ...
        |> Map.add
            Attestations
            (ArtifactSurfaceObservation.Supported(
                revision,
                true,
                1,
                [
    ...
    Assert.Equal(
        Error(ArtifactFailure.InvalidAttestation attestation.Identity),
        ActionsReleaseFeedAdapter.validate invalid
    )
    ...
    let secret =
        ...
        |> Map.add
            ...
            (ArtifactSurfaceObservation.Supported(
                ...
                [
                    { package with
                        Attributes = package.Attributes |> Map.add "token" "forbidden"
                    }
                ]
            ))
        |> observed
...
let servedBytesBindRedirectsAndRetrievalClass () =
    ...
    Assert.True(
        ActionsReleaseFeedAdapter.validateStages [ PublicServedBytes content ]
        |> Result.isOk
    )
...
let qualificationInventoryIsMutationSensitive () =
    let passing: GitHubActionsReleaseFeedControlResult list =
    ...
    | Error findings ->
        Assert.Contains(
            findings,
            fun finding -> finding.ControlId = "byte-digest" && finding.Code = "GARFQ-INDEPENDENT-NOT-RED"
        )
