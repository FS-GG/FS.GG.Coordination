...
let private bytes (value: string) =
    Encoding.UTF8.GetBytes value |> ReadOnlyMemory<byte>
...
let private unit6 =
    sprintf
        """{"exitGate":"stop here","gateCommands":["unit-tests"],"gateContracts":[{"commandSha256":"%s","id":"unit-tests","qGate":"Q7"}],"id":"GS2-01.6","owner":"FS.GG.Coordination","permissionCeiling":["local only"],"prerequisites":["GS2-01.5"],"qGates":["Q7"],"title":"Create the work skill"}"""
        gateDigest
    |> unitWithContract
...
let ``known unit inspection is bound to exact roadmap bytes`` () =
    ...
    | Ok inspection ->
        ...
        Assert.True(inspection.Unit.QGates = [ "Q7" ])
...
let ``accepted exact prerequisite is ready`` () =
    ...
    | Ok status ->
        ...
        Assert.Single(status.AcceptedReceiptDigests) |> ignore
    | Error findings -> Assert.Fail(String.concat "," (findings |> List.map _.Code))
...
let ``candidate manifest is deterministic and never claims acceptance`` () =
    ...
    | Error findings, _
    | _, Error findings -> Assert.Fail(String.concat "," (findings |> List.map _.Code))
...
let ``manifest refuses traversal and validation refuses changed candidate`` () =
    ...
    let manifest =
        RoadmapWork.createManifest
            ...
            "2026-08-27T01:00:00Z"
            [ artifact ]
        |> Result.defaultWith (fun _ -> failwith "fixture")
...
let ``unknown qualification gate and JSON members are refused`` () =
    ...
    let unknownMember =
        indexText.Replace("\"schema\":", "\"surprise\":true,\"schema\":")
...
let ``gate catalog arguments are pinned independently and list-only mutation is refused`` () =
    ...
    let catalog arguments =
        sprintf
            """{"schema":"fsgg.coordination.gate-catalog/1","commands":[{"id":"unit-tests","qGate":"Q7","executable":"dotnet","args":%s}]}"""
            arguments
        |> bytes

    match RoadmapWork.validateGateCatalog expected (catalog "[\"test\",\"tests.fsproj\"]") with
    | Ok commands -> Assert.Single(commands) |> ignore
    | Error findings -> Assert.Fail(String.concat "," (findings |> List.map _.Code))

    match RoadmapWork.validateGateCatalog expected (catalog "[\"test\",\"tests.fsproj\",\"--list-tests\"]") with
    | Ok _ -> Assert.Fail("list-only mutation was accepted")
    | Error findings -> Assert.Contains("RW-GATE-CONTRACT-MISMATCH", codes findings)
