...
let private metric value =
    {
        Value = Some value
        UnknownReason = None
    }

let private attempt number request workload =
    {
        Attempt = number
        Request = request
        ResponseStatus = Some 200
        RateOutcome = "remaining=4999"
...
let private hint id subject revision kind operation state sourceId =
    {
        ...
        ArrivedAt = $"2026-09-08T00:0{revision}:00Z"
        State = state
        SourceId = sourceId
    }
...
let private baselineFacts () =
    let current1 =
        { source
              ...
              1
              2
              "page=1;native=delivered"
              [ attempt 1 "GET /actions/runs/34176802894/attempts/1" false ] with
            Workflow = Some "bootstrap-qualification"
            TestedHead = Some(String.replicate 40 "a")
            RunId = Some 34176802894L
            RunAttempt = Some 1
            AttemptCount = Some 1
            EventAt = Some "2026-09-08T00:00:00Z"
            IngestedAt = Some "2026-09-08T00:00:02Z"
            QueuedAt = Some "2026-09-08T00:00:03Z"
            StartedAt = Some "2026-09-08T00:00:04Z"
            EndedAt = Some "2026-09-08T00:01:00Z"
        }

    let current2 =
        { current1 with
            Page = 2
            RawPayload = "page=2;native=delivered"
            RawPayloadSha256 = sha "page=2;native=delivered"
        }
    ...
    let historical =
        source
            ...
            1
            1
            "routine-burst;native=delivered"
            [ attempt 1 "GET retained routine-burst" false ]
    ...
    let replay =
        { source
              ...
              1
              1
              "same-workload;native=refused"
              [ attempt 1 "replay reconcile A" true; attempt 2 "replay reconcile B" true ] with
            TestedHead = Some(String.replicate 40 "a")
        }

    let injected =
        { source "withheld-C" "injected-negative-control" 1 1 "withheld=C;native=unknown" [] with
            TestedHead = Some(String.replicate 40 "a")
        }

    {
        ...
        CandidateHead = String.replicate 40 "a"
            ...
            [
                {
                    ...
                    Subject = "issue:C"
                    Revision = 1L
                    ScheduledAt = "2026-09-08T00:30:00Z"
                    DiscoveredAt = "2026-09-08T00:31:00Z"
                    ConvergedAt = "2026-09-08T00:32:00Z"
                    SourceId = "withheld-C"
...
let ``canonical report round trips and replay is exact`` () =
    ...
    Assert.True(
        replay
            report
            { facts with
                FullScanSchedules = metric 4L
            }
        |> has EventBenefitFinding.ReplayConflict
    )
...
let ``population window pages attempts digests and provider accounting fail closed`` () =
    ...
    Assert.True(
        compile
            { facts with
                Sources = facts.Sources |> List.filter (fun source -> source.Page <> 2)
            }
    ...
    Assert.True(
        compile
            { facts with
                Sources = { first with AttemptCount = Some 2 } :: facts.Sources.Tail
            }
        |> has (EventBenefitFinding.IncompleteRunAttempt first.SourceId)
    )
    ...
    Assert.True(
        ...
        |> has (EventBenefitFinding.AlteredSourceDigest first.SourceId)
    )
    ...
    Assert.True(
        compile
            { facts with
                Sources =
                    { first with
                        TestedHead = Some(String.replicate 40 "b")
                    }
                    :: facts.Sources.Tail
            }
        |> has EventBenefitFinding.SubstitutedHead
    )
    ...
    Assert.True(
        compile
            { facts with
                Sources =
                    { first with
                        EndedAt = Some "2026-09-08T01:00:01Z"
                    }
                    :: facts.Sources.Tail
            }
        |> has (EventBenefitFinding.OutsideWindow $"{first.SourceId}:endedAt")
    )

    Assert.True(
        compile
            { facts with
                Sources = { first with ApiAttempts = [] } :: facts.Sources.Tail
            }
        |> has (EventBenefitFinding.MissingCallAttempt first.SourceId)
    )

    let noRate =
        { first.ApiAttempts.Head with
            RateOutcome = ""
        }
    ...
    Assert.True(
        compile
            { facts with
                Sources = { first with ApiAttempts = [ noRate ] } :: facts.Sources.Tail
            }
        |> has (EventBenefitFinding.MissingRateOutcome first.SourceId)
    )
...
let ``unsupported contradictory and unrepaired hints refuse`` () =
    ...
    Assert.True(
        compile
            { facts with
                Hints = unsupported :: facts.Hints.Tail
            }
        |> has (EventBenefitFinding.UnsupportedHint unsupported.HintId)
    )
    ...
    Assert.True(
        compile
            { facts with
                Hints = contradiction :: facts.Hints
            }
        |> has (EventBenefitFinding.ContradictoryRevision contradiction.Subject)
    )
...
let ``missing metrics and native observations remain unknown not zero`` () =
    ...
    let unknownCalls =
        {
            Value = None
            UnknownReason = Some "provider counter unavailable"
        }

    let report =
        compile
            { facts with
                FullScanApiCalls = unknownCalls
            }
        |> get
    ...
    let hosted =
        {
            ...
            RunId = 1L
            RunAttempt = 1
            TestedHead = facts.CandidateHead
        ...
        compile
            { facts with
                HostedClaim =
                    Some
                        { hosted with
                            TestedHead = String.replicate 40 "b"
                        }
            }
...
let ``tampered policy and seal cannot be accepted`` () =
    ...
    Assert.True(
        verify report.Seal { report with InstalledBenefit = true }
        |> has EventBenefitFinding.ReplayAsInstalled
    )

    Assert.True(
        verify report.Seal { report with ProductionBenefit = true }
    ...
    Assert.True(
        ...
        |> has EventBenefitFinding.AlteredSeal
    )
