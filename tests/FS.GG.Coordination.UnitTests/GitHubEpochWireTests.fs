...
let ``fleet epoch ref is derived by the canonical Cutover journal address`` () =
    ...
    let address =
        ShardedJournalAdapter.address JournalKind.Cutover aggregateId
...
let ``OperatingV1 and Preparing implement the incumbent admission table`` () =
    ...
    Assert.True(
        Q.admit AuthorityObserved (authority Preparing) (fence NewOrdinaryV1 true)
        |> refused
    )

    Assert.True(
        Q.admit AuthorityObserved (authority Preparing) (fence IncumbentV1Effect false)
        |> refused
    )

    Assert.True(
        Q.admit AuthorityObserved (authority RollingBack) (fence IncumbentV1Effect true)
        |> refused
    )
...
let ``freeze and every later phase fence ordinary v1 at the effect boundary`` () =
    for phase in
        ...
        Assert.True(
            Q.admit AuthorityObserved (authority phase) (fence NewOrdinaryV1 true)
            |> refused,
            Q.phaseName phase
        )

        Assert.True(
            Q.admit AuthorityObserved (authority phase) (fence IncumbentV1Effect true)
            |> refused,
            Q.phaseName phase
        )
...
let ``fresh exact content addressed authority is mandatory`` () =
    ...
    Assert.True(
        Q.admit AuthorityObserved { current with Fresh = false } (fence NewOrdinaryV1 true)
        |> refused
    )
    ...
    Assert.True(
        Q.admit
            ...
            (fence NewOrdinaryV1 true)
        |> refused
    )

    Assert.True(
        Q.admit AuthorityObserved { current with Parent = None } (fence NewOrdinaryV1 true)
        |> refused
    )

    Assert.True(
        Q.admit AuthorityObserved { current with Tag = "" } (fence NewOrdinaryV1 true)
        |> refused
    )
    ...
    Assert.True(
        match Q.admit AuthorityUnreadable current (fence NewOrdinaryV1 true) with
    ...
    let applied =
        {
            ...
            OperationId = Some expected.OperationId
            EpochCommit = Some expected.ExpectedEpochCommit
            EpochGeneration = Some expected.ExpectedEpochGeneration
...
let ``issue projection is exact and never authority`` () =
    ...
    Assert.False(
        Q.validateProjection
            current
            { projection with
                Generation = projection.Generation - 1L
            }
    )
