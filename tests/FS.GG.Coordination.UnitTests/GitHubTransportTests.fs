...
let private request idempotency =
    Rest
        {
            Method = Get
            Uri = Uri("https://api.github.test/items")
            Headers = Map.empty
            Body = None
...
let ``REST Link parsing preserves next relation and rejects ambiguous mapping`` () =
    ...
    Assert.Equal(
        Ok(Some next),
        Transport.tryNextLink
            $"<{next.AbsoluteUri}>; rel=\"next\", <https://api.github.test/items?page=5>; rel=\"last\""
    )
    ...
    Assert.Equal(
        ...
        Transport.tryNextLink
            $"<{next.AbsoluteUri}>; rel=\"next\", <https://api.github.test/items?page=3>; rel=\"next\""
    )
...
let ``fixture projection is stable allow-listed and leak rejecting`` () =
    ...
    Assert.Equal(
        Error(InvalidFixtureField "<request>"),
        Transport.projectFixture
            allowed
            { fixture with
...
let ``qualification rejects producer gaps and accepts two closed inventories`` () =
    ...
    | Error findings ->
        Assert.Contains(findings, fun finding -> finding.Code = "GTQ-INVENTORY" && finding.ControlId = "generated")
