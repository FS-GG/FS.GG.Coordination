...
let private touch repository path =
    { Repository = repository; Path = path }

let private claimCommit
    (authority: ClaimAuthorityRecord)
    (generation: int64)
...
let private baseGrant =
    {
        ...
        Touches =
            baseAuthority.Touches
            |> List.map (fun value ->
                { value with
                    Repository = value.Repository.ToLowerInvariant()
                })
...
let private domainCommit (address: AggregateAddress) commitOid =
    let bytes =
        ClaimTouchSetAdapter.authorityBytes baseAuthority
        |> Result.defaultWith (failwithf "%A")
    ...
    let head =
        { unsigned with
            HeadDigest = ShardedJournalAdapter.sha256 (ShardedJournalAdapter.journalHeadBytes unsigned)
        }
...
let ``touches are canonical repository-scoped and ancestry conflicts`` () =
    ...
    Assert.True(
        ClaimTouchSetAdapter.touchesConflict
            [ touch "fs-gg/repo" "src/Claims" ]
            [ touch "FS-GG/Repo" "src/Claims/Lease.fs" ]
    )

    Assert.False(
        ClaimTouchSetAdapter.touchesConflict [ touch "fs-gg/repo" "src/Claims" ] [ touch "fs-gg/other" "src/Claims" ]
    )
    ...
    Assert.True(
        ClaimTouchSetAdapter.normalizeTouches [ touch "repo" "src"; touch "REPO" "src" ]
        |> Result.isError
    )
...
let ``expiry grants successor eligibility but never changes current authority`` () =
    Assert.Equal(
        Ok(SuccessorEligibility.BlockedUntil 100L),
        ClaimTouchSetAdapter.successorEligibility 99L "worker-b" baseObservation
    )

    Assert.Equal(
        Ok SuccessorEligibility.EligibleAfterExpiry,
        ClaimTouchSetAdapter.successorEligibility 100L "worker-b" baseObservation
    )

    Assert.Equal(
        Ok SuccessorEligibility.CurrentOwner,
        ClaimTouchSetAdapter.successorEligibility 10L "worker-a" baseObservation
    )

    Assert.True(ClaimTouchSetAdapter.authorizeEffect baseGrant baseObservation |> Result.isOk)

    Assert.Equal(
        Error WrongClaimOwner,
        ClaimTouchSetAdapter.authorizeEffect { baseGrant with Owner = "worker-b" } baseObservation
    )
...
let ``successor acquisition is expected-parent CAS with a new fencing generation`` () =
    ...
    let plan =
        ClaimTouchSetAdapter.planAcquire
            intent
            baseObservation
            {
                CommitOid = oid "c"
    ...
    Assert.Equal(
        $"--force-with-lease={baseCommit.Head.Address.Ref}:{baseCommit.CommitOid}",
        plan.Proposal.ForceWithLease
    )
    ...
    Assert.Equal(
        Error(ClaimEffectRefused EffectRefusal.StaleFence),
        ClaimTouchSetAdapter.authorizeEffect staleGrant baseObservation
    )
...
let ``multi-touch plan persists the full ordered acquisition and compensates in reverse`` () =
    ...
    let plan =
        ClaimTouchSetAdapter.planMultiTouch "operation-42" "worker-a" touches domains
        |> Result.defaultWith (failwithf "%A")
    ...
    let conflict =
        ClaimTouchSetAdapter.planConflict plan plan.Saga.AcquisitionOrder plan.Saga.AcquisitionOrder
        |> Result.defaultWith failwith

    Assert.Equal<SagaTouch list>([], conflict.ReleaseUnconsumed)

    Assert.Equal<SagaTouch list>(
        plan.Saga.AcquisitionOrder |> List.rev,
        conflict.CompensateApplied
        |> List.map (fun value ->
            {
    ...
    Assert.Equal(
        Error [ PersistedPlanMissing ],
        ClaimTouchSetAdapter.authorizeMultiTouchEffects plan None (baseGrant, baseObservation) []
    )
...
let ``multi-touch effects require every distinct persisted domain proof`` () =
    ...
    let plan =
        ClaimTouchSetAdapter.planMultiTouch "operation-43" "worker-a" touches domains
        |> Result.defaultWith (failwithf "%A")

    let persisted = ClaimTouchSetAdapter.persistPlan plan

    let primaryAuthority =
        { baseAuthority with
            Touches = touches
            OperationId = "multi-root"
        }
    ...
    let proofs =
        ...
        |> List.mapi (fun index domain ->
            ...
            ({
                Touch = domain.Touch
                JournalCommit = commit.CommitOid
                Generation = domain.ExpectedGeneration
             },
    ...
    match
        ClaimTouchSetAdapter.authorizeMultiTouchEffects
            plan
            (Some persisted)
            (primaryGrant, primaryObservation)
            duplicate
    with
    | Error failures ->
        Assert.Contains(
    ...
    match
        ClaimTouchSetAdapter.authorizeMultiTouchEffects plan (Some persisted) (primaryGrant, primaryObservation) stale
    with
    | Error failures ->
        Assert.Contains(
            failures,
            function
            | WrongDomainGeneration _ -> true
            | _ -> false
        )
...
let ``claim qualification inventory is closed and every mutation must be red`` () =
    ...
    | Error findings ->
        Assert.Contains(
            findings,
            fun finding -> finding.Code = "GCTQ-INDEPENDENT-NOT-RED" && finding.ControlId = "stale-fence"
        )
