...
let private compile value =
    RequiredCheckCensusAdapter.compile observedAt (TimeSpan.FromHours 1) value

[<Fact>]
let ``census unions classic and ruleset authorities while retaining provenance`` () =
    let report =
        compile snapshot |> Result.defaultWith (failwithf "baseline refused: %A")
...
let ``census exact seal rejects changed source bindings`` () =
    ...
    let changed =
        { snapshot with
            SourceRevision = String.replicate 40 "c"
        }

    Assert.Equal(
        Error [ AlteredRequiredCheckCensusSeal ],
        RequiredCheckCensusAdapter.verify report.Seal observedAt (TimeSpan.FromHours 1) changed
    )
...
let ``mixed integration identities for one context refuse`` () =
    let changed =
        { snapshot with
            Requirements = requirement ClassicProtection "build" None :: snapshot.Requirements
        }
...
let ``complete noncompliant producer routes are classified not refused`` mutation =
    ...
    let changedProducer =
        ...
        | "pull-request" ->
            { first with
                PullRequest = { event with Declared = false }
            }
        | "merge-group" ->
            { first with
    ...
    let changed =
        { snapshot with
            Producers = changedProducer :: snapshot.Producers.Tail
        }

    let report =
        compile changed
        |> Result.defaultWith (failwithf "complete observation refused: %A")

    if mutation = "merge-group" then
...
let ``missing duplicate orphan and cross-repository producers refuse`` () =
    ...
    Assert.True(
        compile
            { snapshot with
                Producers = snapshot.Producers @ [ snapshot.Producers.Head ]
            }
        |> Result.isError
    )
    ...
    Assert.True(
        compile
            { snapshot with
                Producers = producer "orphan" None :: snapshot.Producers
            }
        |> Result.isError
    )

    let cross =
        { snapshot.Producers.Head with
    ...
    Assert.True(
        compile
            { snapshot with
                Producers = cross :: snapshot.Producers.Tail
            }
        |> Result.isError
    )
...
let ``partial stale and invalid binding observations refuse`` () =
    ...
    Assert.True(
        RequiredCheckCensusAdapter.compile (observedAt.AddHours 2) (TimeSpan.FromHours 1) snapshot
        |> Result.isError
    )
    ...
    Assert.True(
        compile
            { snapshot with
                AuthorityEvidenceSha256 = "not-a-digest"
            }
        |> Result.isError
    )

    let invalidWorkflowDigest =
        { snapshot.Producers.Head with
    ...
    Assert.True(
        compile
            { snapshot with
                Producers = invalidWorkflowDigest :: snapshot.Producers.Tail
            }
        |> Result.isError
    )
...
let ``exact verification rejects a producer rename at the same source revision`` () =
    ...
    let changed =
        { snapshot with
            Producers = renamed :: snapshot.Producers.Tail
        }
...
let ``qualification requires complete generated and independent mutation inventories`` () =
    ...
    let broken =
        passing
        |> List.map (fun result ->
            if result.Control = MergeGroupProduction then
