...
let private read name =
    File.ReadAllBytes(Path.Combine(root, "evidence/github-substrate-v2/gs2-08-2", name))
    |> ReadOnlyMemory<byte>

[<Fact>]
let ``first capture is explicitly uninitialized and second binds its actual bytes`` () =
    let first =
        LedgerProtectionProviderCodec.decode (read "live-capture-pass1.json")
        |> Result.defaultWith (failwithf "%A")

    let second =
        LedgerProtectionProviderCodec.decode (read "live-capture-pass2.json")
        |> Result.defaultWith (failwithf "%A")
    ...
    Assert.Equal(
        CurrentPreInstall,
        LedgerProtectionConformance.classify second.CapturedAt (TimeSpan.FromMinutes 5.0) second.Conformance
    )
...
let ``matched live capture compiles through the provider adapter without apply authority`` () =
    ...
    let observation =
        LedgerProtectionProviderCodec.qualifiedObservation capture
        |> Result.defaultWith (failwithf "%A")
    ...
    let normalized =
        LedgerProtectionProviderAdapter.normalize capture.CapturedAt (TimeSpan.FromMinutes 5.0) observation
        |> Result.defaultWith (failwithf "%A")
    ...
    let plan =
        LedgerProtectionProviderAdapter.compile capture.CapturedAt (TimeSpan.FromMinutes 5.0) observation
        |> Result.defaultWith (failwithf "%A")
...
let ``normalized tamper refuses before provider compilation`` () =
    let bytes =
        File.ReadAllText(Path.Combine(root, "evidence/github-substrate-v2/gs2-08-2/live-capture-pass2.json"))

    let tampered =
        bytes.Replace("v2-journal-writer", "v2-journal-writer-tampered")
...
let ``declared raw-set tamper refuses even when normalized content is unchanged`` () =
    ...
    let tampered =
        bytes.Replace("2684f8a6d3e2530d62a62f8d49127c7e61e9f389cf6c416edd8a80ba2d9cdfe5", String.replicate 64 "0")
        |> Text.Encoding.UTF8.GetBytes
        |> ReadOnlyMemory<byte>
...
let ``capture gaps refuse qualification`` () =
    ...
    let tampered =
        bytes.Replace("\"gaps\":[]", "\"gaps\":[\"provider-read-refused\"]")
        |> Text.Encoding.UTF8.GetBytes
        |> ReadOnlyMemory<byte>
...
let ``bound App identities refuse when installation identities are absent or wrong`` () =
    ...
    let bindings =
        "\"bindings\":{\"ordinaryWriterAppId\":4882140,\"ordinaryWriterInstallationId\":160261608,\"cutoverWriterAppId\":4882399,\"cutoverWriterInstallationId\":160261436,\"controlIssueNumber\":null},"

    let tampered =
        bytes.Replace("{\"applyAuthorized\":false,", "{\"applyAuthorized\":false," + bindings)
        |> Text.Encoding.UTF8.GetBytes
        |> ReadOnlyMemory<byte>

    let errors =
        match LedgerProtectionProviderCodec.decode tampered with
        | Error values -> values
        | Ok _ -> []
