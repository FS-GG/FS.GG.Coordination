...
let ``five perspectives generate one canonical derived green rollup`` () =
    ...
    let reordered =
        generated
            { input with
                Evidence = List.rev input.Evidence
                Findings = List.rev input.Findings
            }
...
let ``changes required remains valid evidence but cannot derive green`` () =
    ...
    let red =
        { input.Findings.Head with
            Decision = CritiqueDecision.ChangesRequired
        }

    let changed =
        { input with
            Findings = red :: input.Findings.Tail
        }
...
let ``invalid authority inventory phase evidence and time are refused before rendering`` () =
    ...
    let cases =
        [
            ...
            { baseline with
                Findings = baseline.Findings.Tail
            }
            "CE-PHASE-IDENTITY",
            { baseline with
                Findings = baseline.Findings |> List.map (fun item -> { item with PhaseId = "same-phase" })
            }
            "CE-AUTHORITY",
            { baseline with
                Findings =
                    { baseline.Findings.Head with
                        Author = "another-authority"
                    }
                    :: baseline.Findings.Tail
            }
...
let ``absent stale duplicate forged truncated and prose only bundles are red`` () =
    ...
    let redInput =
        { input with
            Findings = redFinding :: input.Findings.Tail
        }
...
let ``candidate evidence and finding content change bundle identity`` () =
    ...
    let variants =
        [
            { input with
                Candidate =
                    { input.Candidate with
                        TreeSha256 = digest '9'
                    }
            }
            { input with
                Evidence =
                    { input.Evidence.Head with
                        Sha256 = digest '9'
                    }
                    :: input.Evidence.Tail
            }
    ...
    for variant in variants do
        Assert.NotEqual<byte array>(baseline, generated variant)
