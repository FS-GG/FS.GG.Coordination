...
let ``scheduling key uses length framing and repository scope`` () =
    let left =
        compile repository revision [ event "issue" "a:b" 1L "delivery-1" ] |> get
    ...
    let other =
        compile
            ...
            revision
            [
                { event "issue" "a:b" 1L "delivery-1" with
...
let ``unknown malformed incomplete and stale inputs refuse explicitly`` () =
    ...
    Assert.Contains(
        GitHubNarrowReconciliationFinding.UnknownEventKind "unsupported",
        compile repository revision [ event "unsupported" "1" 1L "delivery" ]
        |> findings
    )

    Assert.Contains(
        GitHubNarrowReconciliationFinding.MissingField "subjectId",
        compile
            repository
            revision
            [
                { event "issue" "1" 1L "delivery" with
                    SubjectId = " "
                }
            ]
        |> findings
    ...
    Assert.Contains(
        GitHubNarrowReconciliationFinding.MalformedField "origin",
        compile
            repository
            revision
            [
    ...
    Assert.Contains(
        GitHubNarrowReconciliationFinding.StaleRevision "0",
        compile repository revision [ event "issue" "1" 0L "delivery" ] |> findings
    )
...
let ``cross scope conflicting subject and altered routing refuse`` () =
    ...
    Assert.Contains(
        GitHubNarrowReconciliationFinding.CrossScope "FS-GG/Other",
        compile repository revision [ { good with Repository = "FS-GG/Other" } ]
        |> findings
    )

    Assert.Contains(
        GitHubNarrowReconciliationFinding.StaleRevision(String.replicate 40 "a"),
        compile
            repository
            revision
            [
    ...
    Assert.Contains(
        GitHubNarrowReconciliationFinding.ConflictingSubject "release:299",
        compile repository revision [ { good with SubjectKind = "release" } ]
        |> findings
    )

    Assert.Equal(
        Error [ GitHubNarrowReconciliationFinding.AlteredRouting "reconcile/release" ],
        compile
            repository
            revision
            [
...
let ``commands and events schedule only and direct writes refuse`` () =
    let command =
        { event "repository" "FS.GG.Coordination" 1L "command-1" with
            Origin = "command"
        }

    Assert.Single((compile repository revision [ command ] |> get).Entries)
    |> ignore

    Assert.Equal(
        Error [ GitHubNarrowReconciliationFinding.DirectWrite "command-1" ],
        compile
            repository
            revision
            [
...
let ``writer boundary and seals fail closed when altered`` () =
    ...
    Assert.Equal(
        ...
        parse ((serialize plan).Replace(plan.Seal, String.replicate 64 "0"))
    )
...
let ``exact replay is byte identical and new work refuses replay`` () =
    ...
    Assert.Contains(
        GitHubNarrowReconciliationFinding.ReplayConflict "new or newer subject requires fresh reconciliation",
        replay
            plan
            [
                { one with
