...
module private Fixture =
    ...
    let databaseTool name arguments =
        if mode = "docker" then
            run "docker" $"exec {container} {name} -U {username} {arguments}"
        else
            run $"/usr/bin/{name}" $"-h {host} -p {port} -U {username} {arguments}"
...
type PostgreSqlObserverStoreTests() =
            ...
            let proposal =
                {
                    ...
                    NarrativeSha256 = Fixture.sha "d"
                    Actions =
                        [
                            {
                                Kind = InspectWorkItem
                                WorkItem = Fixture.workItem
                                ParametersSha256 = Fixture.sha "e"
                            }
                        ]
    ...
    member _.``observer inbox deduplicates conflicts and sequence CAS``() =
        task {
            ...
            let envelope =
                Fixture.envelope Observer.initial (OpenSession(Fixture.sessionId, Fixture.projectId, Fixture.budget))

            let decision = Observer.decide Fixture.now Observer.initial envelope
            let request = ObserverJournal.appendRequest observerId Fixture.now envelope decision

            let mismatched =
                ObserverJournal.appendRequest "observer-session-v1-wrong" Fixture.now envelope decision

            let! mismatchedResult = store.AppendObserver(mismatched, cancellationToken)

            Assert.Equal<ObserverAppendOutcome>(
                ObserverInvalidAppend "observer-session-identity-mismatch",
                mismatchedResult
            )
            ...
            let! duplicate =
                store.AppendObserver({ request with Events = [] }, cancellationToken)
            ...
            let! conflict =
                store.AppendObserver(
                    { request with
                        Command = { envelope with PrincipalId = "other" }
                        Events = []
                    },
                    cancellationToken
                )
            ...
            let stale =
                { ObserverJournal.appendRequest observerId Fixture.now staleEnvelope validDecision with
                    Events =
                        validDecision.Events
            ...
            let second =
                ObserverJournal.appendRequest observerId Fixture.now validEnvelope validDecision
            ...
            let! _ =
                Fixture.sql
                    "orchestration_o0"
                    $"DELETE FROM fsgg_orchestration.observer_inbox WHERE observer_id='{observerId}' AND command_id='{Id.commandValue validEnvelope.CommandId}'"

            let! _ =
                Fixture.sql
                    "orchestration_o0"
                    $"DELETE FROM fsgg_orchestration.observer_event WHERE observer_id='{observerId}' AND sequence_number=2"
            ...
            | Error failures ->
                Assert.Contains(
                    failures,
                    function
                    | ObserverCorruptRecord(_, 2L) -> true
                    | _ -> false
                )
    ...
    member _.``observer recovery refuses unknown versions and corrupt payload``() =
        task {
            ...
            let! _ =
                store.AppendObserver(
                    ObserverJournal.appendRequest observerId Fixture.now envelope decision,
                    cancellationToken
                )
            ...
            let! _ =
                Fixture.sql
                    "orchestration_o0"
                    $"UPDATE fsgg_orchestration.observer_event SET schema_version=99,serializer_version='future',payload=decode('00','hex') WHERE observer_id='{observerId}'"
            ...
            | Error failures ->
                Assert.Contains(
                    failures,
                    function
                    | ObserverUnknownEventVersion(_, 1L, 99) -> true
                    | _ -> false
                )
                ...
                Assert.Contains(
                    failures,
                    function
                    | ObserverUnknownSerializerVersion "future" -> true
                    | _ -> false
                )
                ...
                Assert.Contains(
                    failures,
                    function
                    | ObserverCorruptRecord(_, 1L) -> true
                    | _ -> false
                )
        }
    ...
    member _.``observer metadata fence is mandatory for recovery and append``() =
        task {
            ...
            let! _ =
                Fixture.sql "orchestration_o0" "UPDATE fsgg_orchestration.observer_store_metadata SET schema_version=99"
            ...
            | Error failures ->
                Assert.Contains(
                    failures,
                    function
                    | ObserverIncompatibleDowngrade(99, 1) -> true
                    | _ -> false
                )
        ...
        task {
            ...
            | Error failures ->
                Assert.Contains(
                    failures,
                    function
                    | ObserverStoreUnavailable _ -> true
                    | _ -> false
                )
    ...
    member _.``restored old observer backup refuses recovery and append transactionally``() =
        task {
            ...
            let restoredStore =
                PostgreSqlObserverStore(Fixture.options restored identity 2L) :> IObserverJournalStore
            ...
            | Error failures ->
                Assert.Contains(
                    failures,
                    function
                    | ObserverBackupRequiresReconciliation "generation-fence-regressed" -> true
                    | _ -> false
                )
            ...
            let! write =
                restoredStore.AppendObserver(
                    ObserverJournal.appendRequest observerId Fixture.now next nextDecision,
                    cancellationToken
                )
