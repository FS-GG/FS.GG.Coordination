...
type MemoryJournal() =
    ...
    interface IExecutionSessionJournal with
        member _.ReadAttempt(assignmentId, attemptId, _) =
            task {
                ...
                | true, events ->
                    return
                        Some
                            {
                                Revision = int64 events.Count
                                Events = List.ofSeq events
                            }
                | _ -> return None
            }

        member _.AppendAttempt(assignmentId, attemptId, revision, eventValue, _) =
            task {
                if failNext then
                    failNext <- false
                    ...
                    if int64 events.Count <> revision then
                        return AppendConflict
                    elif events |> Seq.exists ((=) eventValue) then
...
type FakeProvider(style: string, supportsResume: bool, knownUsage: bool) =
    ...
    member _.BeforeLaunch
        with set value = beforeLaunch <- value
    ...
    member private _.Observation(intent, lifecycle) =
...

        {
            ...
            Resolved =
                {
                    Model = (if style = "stream" then Some "stream-model" else None)
                    Effort = None
                }
            Lifecycle = lifecycle
            Output =
                if style = "stream" then
                    ...
                    [
                        {
                            Kind = "final-text"
                            Reference = "memory://result"
                            Digest = Some(String.replicate 64 "a")
                        }
                    ]
            LifecycleReferences =
                if style = "stream" then
            ...
            Usage = usage
            Candidate = None
            ObservedAt = intent.RecordedAt.AddSeconds 1.
        }
    ...
    interface IExecutionProvider with
            ...
            Task.FromResult
                {
                    ...
                    Authentication = Authenticated(style + "-fixture")
                    SupportsResume = supportsResume
                    ObservedAt = DateTimeOffset.UnixEpoch
                }

        member this.Launch(intent, _) =
            task {
                launches <- launches + 1
        ...
        member _.Cancel(session, _) =
            task {
                return
                    if sessions.Values |> Seq.exists (fun value -> value.Session = session) then
                        CancelAccepted
                    else
                        CancelUnknown "session-not-found"
            }
            ...
            Limits =
                {
                    ...
                    MaximumRuntime = TimeSpan.FromMinutes 3.
                    MaximumAttempts = 1
                }
            RecordedAt = now
        }
...
type ExecutionTests() =
    ...
    member _.``intended adapters fit the neutral readiness contract``
        (providerName: string, supportsResume: bool, provenance: string)
        =
        let readiness =
            {
                Identity =
    ...
    member _.``new schema is closed negotiated and byte stable``() =
        ...
        Assert.Equal(
            Ok ExecutionProtocol.launchSchema,
            ExecutionProtocol.negotiate [ "fsgg.orchestration.runner-assignment/1"; ExecutionProtocol.launchSchema ]
        )

        Assert.Equal(
            Error "execution-schema-not-supported",
            ExecutionProtocol.negotiate [ "fsgg.orchestration.runner-assignment/1" ]
        )
        ...
        let changed =
            ("{\"unknown\":true," + json.Substring(1)) |> System.Text.Encoding.UTF8.GetBytes

        Assert.Equal(Error "execution-launch-shape-refused", ExecutionProtocol.decode changed)

    [<Fact>]
    member _.``intent is durable before provider launch and duplicate reconciles without spawning``() =
        task {
            let journal = MemoryJournal()
            let provider = FakeProvider("stream", true, true)

            provider.BeforeLaunch <-
                fun intent ->
                    journal.Events(intent.Key)
                    |> List.exists (function
                        | LaunchIntentRecorded _ -> true
                        | _ -> false)

            let coordinator =
                ExecutionSessionCoordinator(provider, journal, MutableClock(Fixture.now))
    ...
    [<Fact>]
    member _.``crash before spawn launches only after confirmed absence with original budget``() =
        task {
            let journal = MemoryJournal()
            ...
            let! _ =
                (journal :> IExecutionSessionJournal)
                    .AppendAttempt(
                        intent.Key.AssignmentId,
                        intent.Key.AttemptId,
                        0L,
    ...
    [<Fact>]
    member _.``crash after provider start receipt reconciles without duplicate process``() =
        task {
            let journal = MemoryJournal()
            let provider = FakeProvider("stream", true, true)
    ...
    [<Fact>]
    member _.``generation is fenced on the attempt stream``() =
        task {
            let journal = MemoryJournal()
            let provider = FakeProvider("stream", true, true)
    ...
    [<Fact>]
    member _.``deadline cannot renew and unsupported resume stays ambiguous``() =
        task {
            let journal = MemoryJournal()
            ...
            let expired =
                ExecutionSessionCoordinator(provider, journal, MutableClock(intent.Limits.Deadline.AddSeconds 1.))
    ...
    [<Fact>]
    member _.``deadline requests cancellation without claiming observed termination``() =
        task {
            let journal = MemoryJournal()
            let provider = FakeProvider("stream", true, true)
    ...
    [<Fact>]
    member _.``provider differences retain unknown and not applicable instead of zero``() =
        task {
            let journal = MemoryJournal()
            let provider = FakeProvider("aggregate", false, false)
    ...
    [<Fact>]
    member _.``malformed usage and unbounded output are refused``() =
        task {
            let journal = MemoryJournal()
            let provider = FakeProvider("stream", true, true)
            let intent = Fixture.intent 1L
            ...
            let coordinator =
                ExecutionSessionCoordinator(bad, journal, MutableClock(Fixture.now))

            let! launched = coordinator.Launch(intent, CancellationToken.None)
            let state = Fixture.state launched

            let malformed =
                { state.Observation.Value with
                    Usage =
                        {
                            Values = Map["tokens", UsageKnown(-1L, "tokens", "bad")]
                            Cost = CostKnown(-1M, "USD", "bad")
                        }
                }
    ...
    [<Fact>]
    member _.``cancel request is not terminal until provider termination is observed``() =
        task {
            let journal = MemoryJournal()
            let provider = FakeProvider("stream", true, true)
            ...
            let terminated =
                { value.Observation.Value with
                    Lifecycle = Cancelled
                    ObservedAt = Fixture.now.AddMinutes 4.
                }
    ...
    [<Fact>]
    member _.``thin Akka actor delegates launch to neutral coordinator``() =
        task {
            let journal = MemoryJournal()
            let provider = FakeProvider("stream", true, true)
            ...
            let! result =
                actor.Ask<CoordinationResult>(Launch(Fixture.intent 1L), TimeSpan.FromSeconds 3.)
