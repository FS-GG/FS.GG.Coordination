...
The eight positive invariants are checked by one pinned Quint multi-invariant invocation. The runner derives its completed inventory from execution and requires exactly 56 rejected Quint mutation processes before Q2 can pass; this corrects the earlier unmeasured assumption of 51. Removing or unexpectedly greening one control changes the derived count and fails qualification. Server mode and mutation parallelism are not enabled by default: concurrency 1, 2, and 4 must first be compared on hosted runners, and server reuse additionally requires at least a 10% median improvement over five samples with identical outcomes and stable resource use.
