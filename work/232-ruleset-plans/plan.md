...
- VO-001 [PD-001] [PD-002] [PD-003] [PD-004] [PC-001] semanticTest: Run the exact plan validator against digest-bound profile, census, revision-addressed current-policy, bypass, and exception evidence; execute compiled source mutants for constant branch, tag, merge-method, auto-merge, and branch-deletion targets; exercise generated input mutations and independently authored exact receipt/profile/census/source binding, branch target, tag target, required-check retention, reviews, conversation resolution, merge queue, bypass, exception identity/window/scope, observe-only, freshness, ordering, replay, seal, prerequisite, Quint, and no-apply oracles; run a warning-free build, focused and full unit/architecture suites, evidence self-test, and receipt-bound SDD analyze, verify, and ship gates.
