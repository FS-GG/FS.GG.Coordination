...
- PD-002 [AC-001] [FR-002] complete: The script invokes one counted `dotnet pack` operation into private scratch storage, then treats the resulting package path and SHA-256 as immutable inputs for evidence, upload, served-byte comparison, and consumer verification; tests instrument the pack runner and refuse a count other than one.
...
- PD-005 [AC-001] [FR-005] complete: Add an explicit manual post-merge GitHub Actions workflow with `packages: write`, exact expected-SHA input, prerelease-only version validation, and the sole push endpoint `https://nuget.pkg.github.com/FS-GG/index.json`; declare the publication as a post-merge delivery obligation.
...
- PD-008 [AC-001] [FR-008] complete: Add architecture tests for schema, determinism, one-pack accounting, channel/refusal policy, byte substitution, SBOM and attestation tamper, and clean-consumer configuration; integrate hosted artifacts and retained evidence without changing the closed GS2 gate catalog.
