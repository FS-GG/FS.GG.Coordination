...
- PD-002 [AC-001] [FR-002] complete: The script restores and builds only through fresh output-owned intermediates, maps each checkout and isolated-build root to stable compiler paths, invokes one counted `dotnet pack` operation into private scratch storage, canonicalizes that archive's entry order and timestamps without a second project pack, then treats the resulting package path and SHA-256 as immutable inputs for evidence, upload, served-byte comparison, and consumer verification; independent isolated prepares must produce identical compiled payloads and containers, and tests refuse a pack count other than one.
