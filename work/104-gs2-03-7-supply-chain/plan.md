...
- PD-002 [AC-001] [FR-002] complete: The script validates the exact clean candidate, projects only that commit's tracked bytes through `git archive` into output-owned source storage, restores and builds from that immutable projection through fresh output-owned intermediates with build-server reuse and shared compilation disabled, maps the projected source and isolated-build root to stable compiler paths, disables candidate PDB/debug-symbol emission so PE layout cannot retain the pre-PathMap absolute PDB path length, and pins the .NET SDK, runtime, and F# compiler digest in validated provenance. It invokes one counted `dotnet pack` operation into private scratch storage, canonicalizes that archive's entry order and timestamps without a second project pack, then treats the resulting package path and SHA-256 as immutable inputs for evidence, upload, served-byte comparison, and consumer verification; independent isolated prepares in genuinely different worker roots, including clean checkouts contaminated only by ignored project-local `bin` or `obj`, must produce identical compiled payloads and containers, and tests refuse a pack count other than one.
