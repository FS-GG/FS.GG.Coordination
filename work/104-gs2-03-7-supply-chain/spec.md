...
## Scope
- SB-001: GS2-03.7 reproducible package and portable-symbol identity, deterministic SBOM and attestations, one allowed pre-production publication channel, served-byte verification, installed-assembly identity, and clean-consumer execution only
...
- AC-001 [US-001] [FR-001] [FR-002] [FR-003] [FR-004] [FR-005] [FR-006] [FR-007] [FR-008]: Given an exact protected candidate and explicit pre-production authorization, when the supply-chain workflow completes, then one package operation supplies immutable package and canonical Portable-PDB symbol bytes whose retained digests agree across independent roots, whose SPDX SBOM and in-toto attestations are digest-bound and validated, and whose served bytes install the exact retained assembly and execute in isolated clean consumers.
...
- FR-002: Exactly one `dotnet pack` operation MUST create the candidate package and canonical `.snupkg` Portable-PDB symbol package; package, symbol, PDB, and assembly bytes MUST be reproducible across unequal clean roots, and every comparison, evidence document, upload, download, and consumer MUST use those bytes without repacking. (Stories: US-001; Acceptance: AC-001)
...
- FR-004: The candidate MUST carry deterministic in-toto/SLSA-shaped provenance and verification attestations whose subjects name package and symbol SHA-256 values and whose predicates bind the exact build definition, source and lock inputs, builder, channel, and verification outcomes. (Stories: US-001; Acceptance: AC-001)
...
- FR-007: At least two isolated clean consumers MUST restore from the allowed channel, build without warnings, retain the exact installed-assembly SHA-256, and execute the served exact package while an override route proves that the downloaded bytes, not local build outputs, supplied the package. (Stories: US-001; Acceptance: AC-001)
