...
## User Value
Maintainers can invoke one sealed workflow selector on arbitrary changed paths and non-file inputs, consume stable aggregate outcomes, and reproduce real fleet baselines without enabling selection across the fleet.
...
- FR-002: A public pure Core API and CLI MUST validate sealed inventory/version/base/settings, derive roots from arbitrary changed paths and non-file inputs, and compute the smallest sound transitive closure across build, test, policy, coordination, packaging, and release while retaining unconditional policy and core. (Stories: US-001; Acceptance: AC-002)
