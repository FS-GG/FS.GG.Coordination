...
- FR-001: The system MUST bind canonical repository, merge-group identity and head SHA, base repository identity, full base ref and exact base SHA, base observation revision, observed-at and freshness deadline, complete aggregate required-check inventory/results, and current claim generation, review, candidate head, dependency, release, and settings authority into one canonical length-framed seal; accept only `merge_group` checks-requested facts whose complete unique inventory ran successfully for that merge group and whose every authority was freshly re-observed unchanged; explicitly reject every registered missing, malformed, unknown-event, stale/changed-base, incomplete/missing/pending/failed-check, stale claim/review/head/dependency, unmet release, changed settings, conflicting group, direct merge, unsealed/altered seal, ordering, replay, network, production mutation, Quint-change, and GS2-07.6 inversion; and reproduce byte-identical results on exact replay. (Stories: US-001; Acceptance: AC-001)
