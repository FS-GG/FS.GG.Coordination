...
- checklist: work/gs2-04-9-sandbox-qualification-closure/checklist.md sha256:0f35f38d1ecea821e8a44161156b0ca5e9321a22fad10540941d38d84481892f schemaVersion:1
...
- Add a pure synthetic qualification corpus and process orchestrator under repository-owned tests and `eng/`; live GitHub effects stay in an explicit protected workflow route.
...
- PD-008 [AC-008] [FR-008] [DEC-002] [DEC-004] complete: Extend protected Bootstrap qualification with explicit synthetic/live dispatch modes, concurrency isolation, short expiry, bounded quota, always-run cleanup, artifact retention, and non-green live status when authority or cleanup proof is absent.
...
- PC-003 [PD-008] workflowContract: Protected workflow inputs and secrets distinguish synthetic from live mode and name only non-production sandbox configuration; no fallback to `github.token` or ambient human credentials is allowed for live effects.
...
- VO-004 [PD-007] [PD-008] [PC-003] protectedTest: Prove synthetic protected execution and fail-before-write live readiness without a sandbox credential; live green evidence is admitted only from a separately configured non-production identity and full cleanup reread.
...
## Advisory Notes
- Live Q4 acceptance depends on an external non-production credential and isolated targets that are intentionally absent from the repository. Implementation and synthetic/protected fail-closed evidence can proceed without them; a live green receipt cannot be synthesized or inferred.
