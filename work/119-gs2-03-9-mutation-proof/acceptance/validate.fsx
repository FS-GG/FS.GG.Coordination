...
let evidencePath name =
    Path.Combine(acceptance, "evidence", name)

let findingPath name =
    Path.Combine(acceptance, "findings", name)

let proofPath =
    Path.Combine(root, "evidence/github-substrate-v2/mutation-proofs/GS2-03.9.json")

let reviewPath =
    Path.Combine(root, "evidence/github-substrate-v2/reviews/GS2-03.9.json")

let inventoryPath =
    Path.Combine(root, "evidence/github-substrate-v2/qualification-inventories/GS2-03.1.json")

let baselinePath =
    Path.Combine(root, "evidence/github-substrate-v2/qualification-manifests/GS2-03.1.json")
...
let unitContract =
    "acb013dd87697c21886dca39fa9ca97ff48e24402e964000cdc1d4c4645be40b"

let fail message =
    failwith $"GS2_03_9_ACCEPTANCE_INVALID %s{message}"
...
match
    HarnessMutationProof.validate
        proofContext
        (ReadOnlyMemory<byte>(inventory))
        (ReadOnlyMemory<byte>(baseline))
        (ReadOnlyMemory<byte>(File.ReadAllBytes proofPath))
with
...
require
    (proof["controls"].AsArray()
     |> Seq.forall (fun item -> item["outcome"].GetValue<string>() = "passed"))
    "a mutation control is not green"

require
    (proof["observations"].AsArray()
     |> Seq.forall (fun item -> item["outcome"].GetValue<string>() = "rejected"))
    "a negative mutation is not red"
...
require
    (text mainPrior "evidenceSha256" = sha256 (evidencePath "pr-bootstrap-evidence.json"))
    "protected-main prior evidence digest differs"
...
let artifactNames =
    candidate["artifacts"].AsArray()
    |> Seq.map (fun item -> item["name"].GetValue<string>())
    |> Set.ofSeq

for requiredArtifact in
    [
        "formal-validator"
        "formal-observability-tests"
        "proof-contract"
        "sdd-evidence"
        "gate-catalog"
    ] do
...
let formalGate =
    prEvidence["gates"].AsArray()
    |> Seq.find (fun item -> item["id"].GetValue<string>() = "canonical-quint")

require
    (text (formalGate.AsObject()) "sha256" = sha256 (evidencePath "hosted-formal-qualification.json"))
    "terminal evidence does not bind retained formal receipt"
...
require
    (runs
     |> Seq.forall (fun item -> item["conclusion"].GetValue<string>() = "success"))
    "a hosted run is not green"
...
let completed second =
    DateTimeOffset(2026, 8, 31, 2, 3, second, TimeSpan.Zero)
...
let critiqueInput =
    {
        ...
        Findings =
            [
                finding
                    "gs2-03-9-architecture"
                    CritiquePerspective.Architecture
                    "gs2-03-9-architecture-acceptance"
                    "architecture.md"
                    5
...
| Ok summary ->
    ...
    printfn
        "GS2_03_9_ACCEPTANCE_OK candidate=%s proof=%s validatorBoundary=%s critique=%s evidenceSet=%s findingSet=%s"
        merge
        (sha256 proofPath)
        validatorBoundary
        summary.Digest
        summary.EvidenceSetSha256
        summary.FindingSetSha256
