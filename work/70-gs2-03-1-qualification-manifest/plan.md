...
## Source Snapshot
- spec: work/70-gs2-03-1-qualification-manifest/spec.md sha256:f88f47fc59758a692701f7421605e9a5fd7fc2003167131bd2b6476b97210f51 schemaVersion:1
...
- `FS.GG.Coordination.Cli` currently exposes only local `roadmap-work`; the additive `qualification-manifest validate` command will read one caller-named file and emit deterministic JSON/text without network effects.
...
- PD-005 [AC-002] [AC-006] [FR-007] complete: Add `qualification-manifest validate --file FILE` as a read-only deterministic CLI projection, retain one complete example plus schema in evidence storage, and update the canonical evidence index without widening runtime authority.
...
- PC-002 [PD-005] additive command: `qualification-manifest validate --file FILE` returns deterministic pass/finding JSON and has no write, process, filesystem-discovery, or network authority beyond reading the named file.
