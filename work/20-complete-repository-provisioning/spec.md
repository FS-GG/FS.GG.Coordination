...
- AC-002 [US-002] [FR-002]: Given repository security and Actions APIs, when provisioning applies, then every supported scanner is enabled, workflow tokens default read-only, workflow self-approval is off, and only GitHub-owned Actions are allowed.
...
- FR-002: Repository settings MUST preserve public visibility, `main`, Issues, squash-only PR merges, auto-merge, branch deletion, disabled wiki/repository Projects, read-only workflow tokens, and no workflow review approval; the dependency graph, Dependabot alerts and security updates, secret scanning, push protection, validity checks, and non-provider scanning MUST be enabled. Actions MUST use the selected policy with GitHub-owned Actions only. Organization-only SHA enforcement MUST be recorded as unsupported when the current credential receives 403 and MUST NOT be reported compliant. (Stories: US-002, US-004; Acceptance: AC-002, AC-005)
...
- FR-005: A versioned desired-state contract and deterministic validator MUST fail closed against a canonical compact `fsgg.coordination.repository-settings-receipt/1` document. The receipt MUST bind repository id/node, observed UTC instant, exact merge/security/Actions/team/check/ruleset values, per-operation results, unsupported controls, raw-response SHA-256 values, and rollback or forward-repair guidance. (Stories: US-004; Acceptance: AC-005)
