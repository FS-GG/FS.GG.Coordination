...
## User Stories
- US-001 (P1): As a maintainer, I can complete review and merge from one authenticated author when the structured critic decision passes and the six exact qualification checks pass on the current head.
...
- AC-003 [US-001] [FR-003]: Given the exact six successful GitHub Actions check identities, when `main` changes, then an active no-bypass ruleset requires a pull request, resolved conversations, no deletion/force update, and all six current-head checks without a separate native GitHub reviewer.
...
- AC-005 [US-004] [FR-005]: Given a protected write, when it completes, then a canonical compact receipt binds independently reproducible canonical pre-state bytes, desired contract, operation results, rule ids, authoritative post-state, repository and Actions App identities, unsupported organization SHA enforcement, and rollback/forward-repair instructions.
...
- FR-003: One active branch ruleset MUST target `~DEFAULT_BRANCH`, carry no bypass actor, block deletion and non-fast-forward changes, require a pull request with resolved conversations and zero native approvals, disable CODEOWNERS and last-push approval requirements, and require exactly `deterministic-build`, `compiler-and-tests`, `dependency-and-security`, `package-install-smoke`, `bootstrap-recovery`, and `evidence-manifest` from GitHub Actions App id 15368 with strict current-head enforcement. The structured coordination critic decision is the approval record and MAY use the same authenticated author. (Stories: US-001; Acceptance: AC-003)
...
- FR-005: A versioned desired-state contract and deterministic validator MUST fail closed against canonical compact `fsgg.coordination.repository-settings-pre-state/1` and `fsgg.coordination.repository-settings-receipt/2` documents. The validator MUST recompute the receipt's pre-state SHA-256 from the exact pre-state artifact. The receipt MUST bind repository id/node, observed UTC instant, exact merge/security/Actions/team/check/ruleset values, configuration 17 association and policy, CodeQL default setup, the repository workflow-permissions response, the organization Actions-permissions 403, every other per-operation result, unsupported controls, raw-response SHA-256 values, and rollback or forward-repair guidance. (Stories: US-004; Acceptance: AC-005)
