...
## Source Snapshot
- spec: work/20-complete-repository-provisioning/spec.md sha256:e42b6316e671a4166f1524fede3f70cb87fd981ff9cc3aafbb6acea1ec3ef677 schemaVersion:1
...
- PD-002 [AC-002, AC-005] [FR-002] complete: Record the authoritative pre-state, then apply supported missing secret validity/non-provider scanning and replace `allowed_actions=all` with selected GitHub-owned Actions. Preserve the already-correct merge, repository-feature, workflow-token, and self-approval values. Treat organization SHA-policy 403 as an explicit unsupported result, never success.
...
## Contract Impact
- PC-001 [PD-001, PD-002, PD-003, PD-004, PD-005] repository provisioning contract: `.github/CODEOWNERS`, `eng/repository-settings/desired.json`, branch/tag ruleset request JSON, `eng/repository-settings/verify.fsx`, and `fsgg.coordination.repository-settings-receipt/1` become the reviewed source and evidence boundary.
...
## Migration Posture
- PM-001 [PC-001] additiveThenProtect: Merge the validator and CODEOWNERS before enabling enforcement; apply repository settings in small reread-verified batches; create branch protection last; roll forward by reviewed contract correction because removing protection to recover would weaken the accepted boundary.
