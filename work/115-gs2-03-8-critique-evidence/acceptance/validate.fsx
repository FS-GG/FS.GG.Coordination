...
let acceptance =
    Path.Combine(root, "work/115-gs2-03-8-critique-evidence/acceptance")
...
let bundlePath =
    Path.Combine(root, "evidence/github-substrate-v2/reviews/GS2-03.8.json")
...
let unitContract =
    "c38b9ed4a473a99ee1bb6b43dddcafc06c153e6f1b698fbb44f45af018f52920"

let fail message =
    failwith $"CRITIQUE_ACCEPTANCE_INVALID %s{message}"
...
let completed second =
    DateTimeOffset(2026, 8, 31, 0, 5, second, TimeSpan.Zero)
...
let input =
    {
        ...
        Findings =
            [
                finding
                    "gs2-03-8-architecture"
                    CritiquePerspective.Architecture
                    "gs2-03-8-architecture-acceptance"
                    "architecture.md"
                    5
...
| Ok expected when fsi.CommandLineArgs |> Array.contains "--generate" ->
    ...
    printfn
        "CRITIQUE_ACCEPTANCE_GENERATED path=%s sha256=%s"
        (Path.GetRelativePath(root, bundlePath))
        (sha256 bundlePath)
| Ok _ ->
    ...
    | Ok summary ->
        ...
        printfn
            "CRITIQUE_ACCEPTANCE_OK candidate=%s evidenceSet=%s findingSet=%s digest=%s"
            merge
