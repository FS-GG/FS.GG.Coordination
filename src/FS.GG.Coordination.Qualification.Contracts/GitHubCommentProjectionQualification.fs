...
type GitHubCommentProjectionFinding =
    {
        Code: string
        ControlId: string
        Message: string
...
module GitHubCommentProjectionQualification =
    ...
    let controlId =
        ...
        | ReorderedPage -> "reordered-page"
        | EditedProjection -> "edited-projection"
        | DeletedProjection -> "deleted-projection"
        | TamperedMarker -> "tampered-marker"
        | MalformedJson -> "malformed-json"
        | AuthorityDigestMismatch -> "authority-digest-mismatch"
        | IncompleteObservation -> "incomplete-observation"
        | StaleRevision -> "stale-revision"
        | ConcurrentChange -> "concurrent-change"
            ...
            [
                for value in results do
                    ...
                    if not value.MutationRed then
                        yield
                            {
                                Code = $"GCPQ-{producer.ToUpperInvariant()}-NOT-RED"
                                ControlId = id
                    ...
                    if not value.BaselineGreen then
                        yield
                            {
                                Code = $"GCPQ-{producer.ToUpperInvariant()}-BASELINE-NOT-GREEN"
                                ControlId = id
