...
type GitHubShardedJournalControl =
    | WrongShard
    | MissingParent
    | DuplicateGeneration
    | DigestMismatch
...
type GitHubShardedJournalControlResult =
    {
        Control: GitHubShardedJournalControl
        MutationRed: bool
        BaselineGreen: bool
    }

type GitHubShardedJournalFinding =
    {
        Code: string
        ControlId: string
        Message: string
...
module GitHubShardedJournalQualification =
    ...
    let controlId =
        function
        | WrongShard -> "wrong-shard"
        | MissingParent -> "missing-parent"
        | DuplicateGeneration -> "duplicate-generation"
        | DigestMismatch -> "digest-mismatch"
        | UnknownSchema -> "unknown-schema"
        | StaleParent -> "stale-parent"
        | AmbiguousResponse -> "ambiguous-response"
        | Rewind -> "rewind"
        | Deletion -> "deletion"
            ...
            [
                for result in results do
                    if not result.MutationRed then
                        yield
                            {
                                Code = $"GSJQ-{producer.ToUpperInvariant()}-NOT-RED"
                    ...
                    if not result.BaselineGreen then
                        yield
                            {
                                Code = $"GSJQ-{producer.ToUpperInvariant()}-BASELINE-NOT-GREEN"
                                ControlId = controlId result.Control
