...
module QualificationManifest =
    ...
    let private addString (target: JsonObject) (name: string) (value: string) =
        target.Add(name, JsonValue.Create value)
    ...
    let private idArray (values: string list) =
        ...
        values
        |> List.sort
        |> List.iter (fun value -> result.Add(JsonValue.Create value))
    ...
    let private expectedPairs (expected: QualificationManifestExpectedInventory) =
        [
            ...
            "externalFixtures", expected.ExternalFixtures
            "generatedCases", expected.GeneratedCases
            "independentCases", expected.IndependentCases
    ...
    let private expectedIdsNode expected =
        ...
        for name, ids in expectedPairs expected do
            node.Add(name, idArray ids)
    ...
    let private inputSetDigest (root: JsonObject) =
        inputSetNode root |> canonicalBytes |> sha256
    ...
    let private manifestNode (input: QualificationManifestInput) =
        ...
        input.Results
        |> List.sortBy _.Id
        |> List.iter (fun entry -> results.Add(resultNode input.Candidate.CommitSha inputsDigest entry))
        ...
        input.Reviewers
        |> List.sortBy _.Id
        |> List.iter (fun entry -> reviewers.Add(reviewNode input.Candidate.CommitSha inputsDigest entry))
    ...
    let private isId value =
        stringPattern "^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$" value
    ...
    let private exactProperties code path expected (value: JsonElement) =
        if value.ValueKind <> JsonValueKind.Object then
            ...
            if actual = expectedSet then
                ...
                [
                    finding code path (String.concat "," expected) (actual |> Set.toList |> String.concat ",")
                ]
    ...
    let private stringProperty code path name value findings =
        ...
        | Some child ->
            null,
            finding code (path + "/" + name) "string" (child.ValueKind.ToString())
            :: findings
    ...
    let private arrayProperty code path name value findings =
        ...
        | Some child ->
            [],
            finding code (path + "/" + name) "array" (child.ValueKind.ToString())
            :: findings
    ...
    let private parseInventory (inventory: ReadOnlyMemory<byte>) =
        try
            ...
            let schema, next =
                stringProperty "QM-INVENTORY-SCHEMA" "/inventory" "schema" root findings
            ...
            if schema <> "fsgg.coordination.qualification-inventory/1" then
                findings <-
                    finding
                        "QM-INVENTORY-SCHEMA"
                        "/inventory/schema"
                        "fsgg.coordination.qualification-inventory/1"
                        (if isNull schema then "<missing>" else schema)
                    :: findings
            ...
            for name in names |> List.filter ((<>) "schema") do
                let elements, next =
                    arrayProperty "QM-INVENTORY-IDS" "/inventory" name root findings
                ...
                if
                    ids.Length <> elements.Length
                    || List.isEmpty ids
                    || ids |> List.exists (isId >> not)
                    || ids <> List.sort ids
                ...
                then
                    findings <-
                        finding
                            "QM-INVENTORY-IDS"
                            ("/inventory/" + name)
                            "nonempty sorted unique stable identifiers"
                            (String.concat "," ids)
                        :: findings
            ...
            if not (inventory.Span.SequenceEqual(ReadOnlySpan<byte>(canonical))) then
                findings <-
                    finding "QM-INVENTORY-CANONICAL" "/inventory" "canonical JSON bytes" "non-canonical bytes"
                    :: findings
                ...
                Ok(
                    {
                        ...
                        GeneratedCases = values["generatedCases"]
                        IndependentCases = values["independentCases"]
                        ExternalFixtures = values["externalFixtures"]
        ...
        | :? InvalidOperationException as error ->
            Error
                [
                    finding "QM-INVENTORY-TYPE" "/inventory" "valid inventory types" error.Message
                ]
        | :? NullReferenceException as error ->
            Error [ finding "QM-INVENTORY-TYPE" "/inventory" "complete inventory" error.Message ]
    ...
    let private validateContent category kind candidate root findings =
        ...
        if List.isEmpty entries then
            current <-
                finding "QM-CATEGORY-EMPTY" ("/" + category) "at least one entry" "empty"
                :: current
        ...
        |> List.iteri (fun index entry ->
            ...
            let producer, next =
                stringProperty "QM-ENTRY-PRODUCER" path "producer" entry current

            current <- next

            let candidateSha, next =
                stringProperty "QM-CANDIDATE-BINDING" path "candidateSha" entry current
            ...
            | Some value when value.ValueKind = JsonValueKind.Number ->
                ...
                | _ ->
                    current <-
                        finding "QM-ENTRY-BYTES" (path + "/bytes") "positive integer" (value.GetRawText())
                        :: current
            | _ ->
                current <-
                    finding "QM-ENTRY-BYTES" (path + "/bytes") "positive integer" "missing or non-integer"
                    :: current

            if not (isId id) then
                current <-
                    finding "QM-ENTRY-ID" (path + "/id") "stable identifier" (if isNull id then "<missing>" else id)
                    :: current
            elif not (seen.Add id) then
                current <- finding "QM-ENTRY-DUPLICATE" (path + "/id") "unique identifier" id :: current

            if index > 0 && String.CompareOrdinal(previous, id) >= 0 then
                current <- finding "QM-ENTRY-ORDER" (path + "/id") "ordinal id order" id :: current
            ...
            if actualKind <> kind then
                current <-
                    finding
                        ...
                        (path + "/kind")
                        kind
                        (if isNull actualKind then "<missing>" else actualKind)
                    :: current

            if not (isSha digest) then
                current <-
                    finding
                        "QM-ENTRY-DIGEST"
                        (path + "/sha256")
                        "lowercase SHA-256"
                        (if isNull digest then "<missing>" else digest)
                    :: current

            if String.IsNullOrWhiteSpace mediaType then
                current <-
                    finding "QM-ENTRY-MEDIA" (path + "/mediaType") "non-empty media type" "empty"
                    :: current

            if not (isId producer) then
                current <-
                    finding
                        "QM-ENTRY-PRODUCER"
                        (path + "/producer")
                        "stable principal id"
                        (if isNull producer then "<missing>" else producer)
                    :: current

            if candidateSha <> candidate then
                current <-
                    finding
                        "QM-CANDIDATE-BINDING"
                        (path + "/candidateSha")
                        candidate
                        (if isNull candidateSha then "<missing>" else candidateSha)
                    :: current
            ...
            if parsed.IsNone then
                current <-
                    finding
                        ...
                        (if isNull observedAt then "<missing>" else observedAt)
                    :: current
    ...
    let private validateManifest
        ...
        try
            ...
            if schema <> Schema then
                findings <-
                    finding "QM-SCHEMA" "/schema" Schema (if isNull schema then "<missing>" else schema)
                    :: findings
            ...
            if createdAt.IsNone then
                findings <-
                    finding
                        "QM-TIME"
                        "/createdAt"
                        "canonical UTC second"
                        (if isNull createdAtRaw then "<missing>" else createdAtRaw)
                    :: findings

            let candidateElement =
                tryProperty "candidate" root
            ...
            let commit, next =
                stringProperty "QM-CANDIDATE" "/candidate" "commitSha" candidateElement findings

            findings <- next

            let tree, next =
                stringProperty "QM-CANDIDATE" "/candidate" "treeSha256" candidateElement findings

            findings <- next

            let contract, next =
                stringProperty "QM-CANDIDATE" "/candidate" "contractSha256" candidateElement findings

            findings <- next

            let statedInputs, next =
                stringProperty "QM-INPUT-SET" "/candidate" "inputSetSha256" candidateElement findings

            findings <- next

            let statedInventory, next =
                stringProperty "QM-INVENTORY-BINDING" "/candidate" "inventorySha256" candidateElement findings

            findings <- next

            let candidateProducer, next =
                stringProperty "QM-CANDIDATE" "/candidate" "producer" candidateElement findings

            findings <- next

            if not (isRevision commit) then
                findings <-
                    finding
                        "QM-CANDIDATE"
                        "/candidate/commitSha"
                        "lowercase 40-hex revision"
                        (if isNull commit then "<missing>" else commit)
                    :: findings

            if not (isSha tree) then
                findings <-
                    finding
                        "QM-CANDIDATE"
                        "/candidate/treeSha256"
                        "lowercase SHA-256"
                        (if isNull tree then "<missing>" else tree)
                    :: findings

            if not (isSha contract) then
                findings <-
                    finding
                        "QM-CANDIDATE"
                        "/candidate/contractSha256"
                        "lowercase SHA-256"
                        (if isNull contract then "<missing>" else contract)
                    :: findings

            if not (isSha statedInputs) then
                findings <-
                    finding
                        "QM-INPUT-SET"
                        "/candidate/inputSetSha256"
                        "lowercase SHA-256"
                        (if isNull statedInputs then "<missing>" else statedInputs)
                    :: findings

            if statedInventory <> expectedInventoryDigest then
                findings <-
                    finding
                        "QM-INVENTORY-BINDING"
                        "/candidate/inventorySha256"
                        expectedInventoryDigest
                        (if isNull statedInventory then
                             "<missing>"
                         else
                             statedInventory)
                    :: findings

            if not (isId candidateProducer) then
                findings <-
                    finding
                        "QM-CANDIDATE"
                        "/candidate/producer"
                        "stable principal id"
                        (if isNull candidateProducer then
                             "<missing>"
                         else
                             candidateProducer)
                    :: findings
            ...
            let expectedElement =
                tryProperty "expectedIds" candidateElement
                |> Option.defaultValue Unchecked.defaultof<JsonElement>

            findings <-
                exactProperties "QM-EXPECTED-SHAPE" "/candidate/expectedIds" expectedNames expectedElement
                @ findings
            ...
            for name in expectedNames do
                let values, next =
                    arrayProperty "QM-EXPECTED-IDS" "/candidate/expectedIds" name expectedElement findings
                ...
                for index, value in values |> List.indexed do
                    if value.ValueKind <> JsonValueKind.String then
                        findings <-
                            finding
                                "QM-EXPECTED-IDS"
                                $"/candidate/expectedIds/%s{name}/%d{index}"
                                "stable identifier"
            ...
            let expectedByCategory =
                expectedPairs expected
                |> List.map (fun (name, ids) -> name, List.sort ids)
                |> Map.ofList
            ...
            for name in expectedNames do
                ...
                let authoritativeIds =
                    expectedByCategory.TryFind name |> Option.defaultValue [] |> List.sort
                ...
                if embeddedIds <> authoritativeIds then
                    findings <-
                        finding
                            "QM-INVENTORY-BINDING"
                            ("/candidate/expectedIds/" + name)
                            (String.concat "," authoritativeIds)
                            (String.concat "," embeddedIds)
                        :: findings
            ...
            let categories =
                [
                    ...
                    "generatedCases", "generated-case"
                    "independentCases", "independent-case"
                    "externalFixtures", "external-fixture"
                    "packages", "package"
                ]
                ...
                if observedIds <> expectedIds then
                    findings <-
                        finding
                            "QM-CATEGORY-CLOSED"
                            ("/" + category)
                            (String.concat "," expectedIds)
                            (String.concat "," observedIds)
                        :: findings

            let environment =
                tryProperty "environment" root
                |> Option.defaultValue Unchecked.defaultof<JsonElement>

            findings <-
                exactProperties
                    "QM-ENVIRONMENT-SHAPE"
                    "/environment"
                    [
                        "architecture"
                        "candidateSha"
            ...
            for name in environmentNames do
                let value, next =
                    stringProperty "QM-ENVIRONMENT" "/environment" name environment findings
                ...
                if String.IsNullOrWhiteSpace value then
                    findings <-
                        finding "QM-ENVIRONMENT" ("/environment/" + name) "non-empty string" "empty"
                        :: findings

            let environmentCandidate, next =
                stringProperty "QM-CANDIDATE-BINDING" "/environment" "candidateSha" environment findings

            findings <- next

            if environmentCandidate <> commit then
                findings <-
                    finding
                        "QM-CANDIDATE-BINDING"
                        "/environment/candidateSha"
                        commit
                        (if isNull environmentCandidate then
                             "<missing>"
                         else
                             environmentCandidate)
                    :: findings

            let environmentTimeRaw, next =
                stringProperty "QM-TIME" "/environment" "observedAt" environment findings

            findings <- next
            let environmentTime = parseTime environmentTimeRaw

            if environmentTime.IsNone then
                findings <-
                    finding
                        "QM-TIME"
                        "/environment/observedAt"
                        "canonical UTC second"
                        (if isNull environmentTimeRaw then
                             "<missing>"
                         else
                             environmentTimeRaw)
                    :: findings

            if environmentValues.TryFind "networkMode" <> Some "isolated" then
                findings <-
                    finding
                        "QM-ENVIRONMENT"
                        "/environment/networkMode"
                        "isolated"
                        (environmentValues.TryFind "networkMode" |> Option.defaultValue "<missing>")
                    :: findings
            ...
            let computedInputs =
                if inputNames |> List.forall (fun name -> not (isNull node[name])) then
                    inputSetDigest node
                else
                    "<incomplete-input-set>"

            if statedInputs <> computedInputs then
                findings <-
                    finding
                        "QM-INPUT-SET"
                        "/candidate/inputSetSha256"
            ...
            if List.isEmpty resultElements then
                findings <- finding "QM-RESULTS-EMPTY" "/results" "at least one result" "empty" :: findings
            ...
            |> List.iteri (fun index entry ->
                ...
                let producer, next =
                    stringProperty "QM-RESULT-PRODUCER" path "producer" entry findings

                findings <- next

                let candidateSha, next =
                    stringProperty "QM-CANDIDATE-BINDING" path "candidateSha" entry findings

                findings <- next

                let inputs, next =
                    stringProperty "QM-INPUT-SET-BINDING" path "inputSetSha256" entry findings

                findings <- next
                let completedRaw, next = stringProperty "QM-TIME" path "completedAt" entry findings
                findings <- next

                if not (isId id) || not (resultIds.Add id) then
                    findings <-
                        finding
                            "QM-RESULT-ID"
                            (path + "/id")
                            "unique stable identifier"
                            (if isNull id then "<missing>" else id)
                        :: findings

                if index > 0 && String.CompareOrdinal(previousResult, id) >= 0 then
                    findings <- finding "QM-RESULT-ORDER" (path + "/id") "ordinal id order" id :: findings

                previousResult <- if isNull id then previousResult else id

                if not (isNull id) then
                    observedResultIds.Add id

                if not (stringPattern "^Q(?:[0-9]|10)$" gate) then
                    findings <-
                        finding
                            "QM-RESULT-GATE"
                            (path + "/qGate")
                            "Q0..Q10"
                            (if isNull gate then "<missing>" else gate)
                        :: findings
                ...
                if outcome <> "passed" then
                    findings <-
                        finding
                            ...
                            (path + "/outcome")
                            "passed"
                            (if isNull outcome then "<missing>" else outcome)
                        :: findings
                ...
                if not (isId producer) then
                    ...
                    resultProducers.Add producer |> ignore

                if candidateSha <> commit then
                    findings <-
                        finding
                            "QM-CANDIDATE-BINDING"
                            (path + "/candidateSha")
                ...
                if inputs <> computedInputs then
                    findings <-
                        finding
                            "QM-INPUT-SET-BINDING"
                            (path + "/inputSetSha256")
                            computedInputs
                            (if isNull inputs then "<missing>" else inputs)
                        :: findings
                ...
                | None ->
                    findings <-
                        finding
                            ...
                            (path + "/completedAt")
                            "canonical UTC second"
                            (if isNull completedRaw then "<missing>" else completedRaw)
                        :: findings)

            let expectedResultIds =
                expectedByCategory.TryFind "results" |> Option.defaultValue []
            ...
            if List.ofSeq observedResultIds <> expectedResultIds then
                findings <-
                    finding
                        "QM-RESULTS-CLOSED"
                        "/results"
                        (String.concat "," expectedResultIds)
                        (String.concat "," observedResultIds)
                    :: findings

            let reviewElements, next = arrayProperty "QM-REVIEWS" "" "reviewers" root findings
            findings <- next

            if List.isEmpty reviewElements then
                findings <-
                    finding "QM-REVIEWS-EMPTY" "/reviewers" "at least one independent reviewer" "empty"
                    :: findings
            ...
            |> List.iteri (fun index entry ->
                ...
                let principal, next =
                    stringProperty "QM-REVIEW-PRINCIPAL" path "principal" entry findings
                ...
                if not (isId id) || not (reviewIds.Add id) then
                    findings <-
                        finding
                            "QM-REVIEW-ID"
                            (path + "/id")
                ...
                if index > 0 && String.CompareOrdinal(previousReview, id) >= 0 then
                    findings <- finding "QM-REVIEW-ORDER" (path + "/id") "ordinal id order" id :: findings

                previousReview <- if isNull id then previousReview else id

                if not (isNull id) then
                    observedReviewIds.Add id
                ...
                then
                    findings <-
                        finding
                            ...
                            (path + "/role")
                            "closed review role"
                            (if isNull role then "<missing>" else role)
                        :: findings
                ...
                if not (isId principal) then
                    findings <-
                        finding
                            "QM-REVIEW-PRINCIPAL"
                            (path + "/principal")
                            "stable principal id"
                            (if isNull principal then "<missing>" else principal)
                        :: findings
                else
                    reviewPrincipals.Add principal

                    if principal = candidateProducer || resultProducers.Contains principal then
                        findings <-
                            finding
                                "QM-SELF-REVIEW"
                                (path + "/principal")
                                "principal distinct from candidate and result producers"
                                principal
                            :: findings
            ...
            let expectedReviewIds =
                expectedByCategory.TryFind "reviewers" |> Option.defaultValue []
            ...
            if List.ofSeq observedReviewIds <> expectedReviewIds then
                findings <-
                    finding
                        "QM-REVIEWS-CLOSED"
                        "/reviewers"
                        (String.concat "," expectedReviewIds)
                        (String.concat "," observedReviewIds)
                    :: findings

            let generatedProducers =
                observedByCategory.TryFind "generatedCases"
                |> Option.defaultValue []
                |> List.map _.Producer
                |> Set.ofList

            let independent =
                observedByCategory.TryFind "independentCases" |> Option.defaultValue []

            if
                independent
                |> List.exists (fun entry ->
                    entry.Producer = candidateProducer || generatedProducers.Contains entry.Producer)
            then
                findings <-
                    finding
                        "QM-INDEPENDENCE"
                        "/independentCases"
                        "producer distinct from candidate and generated-case producers"
                        "overlap"
                    :: findings
            ...
            for category in
                ...
                if
                    ...
                    && entries |> List.forall (fun entry -> generatedProducers.Contains entry.Producer)
                then
                    findings <-
                        finding
                            "QM-GENERATED-ONLY"
                            ("/" + category)
                            "at least one producer independent of generated cases"
                            "generated producers only"
                        :: findings

            if
                resultProducers.Count > 0
                && resultProducers |> Seq.forall generatedProducers.Contains
            then
                findings <-
                    finding
                        "QM-GENERATED-ONLY"
            ...
            if
                reviewPrincipals.Count > 0
                && reviewPrincipals |> Seq.forall generatedProducers.Contains
            then
                findings <-
                    finding
                        "QM-GENERATED-ONLY"
                        "/reviewers"
                        "at least one principal independent of generated cases"
                        "generated principals only"
                    :: findings
                ...
                [
                    for KeyValue(_, entries) in observedByCategory do
                        for entry in entries do
                            yield! entry.ObservedAt |> Option.toList
                    yield! environmentTime |> Option.toList
                ]

            if
                not (List.isEmpty inputTimes)
                && not (Seq.isEmpty resultTimes)
                && (List.max inputTimes > Seq.min resultTimes)
            then
                findings <-
                    finding "QM-TIME-ORDER" "/results" "results not earlier than inputs" "inverted"
                    :: findings

            if
                not (Seq.isEmpty resultTimes)
                && not (Seq.isEmpty reviewTimes)
                && (Seq.max resultTimes > Seq.min reviewTimes)
            then
                findings <-
                    finding "QM-TIME-ORDER" "/reviewers" "reviews not earlier than results" "inverted"
                    :: findings

            match createdAt with
            | Some created when
                (inputTimes |> List.exists (fun value -> value > created))
                || (resultTimes |> Seq.exists (fun value -> value > created))
                || (reviewTimes |> Seq.exists (fun value -> value > created))
                ->
                findings <-
                    finding "QM-TIME-ORDER" "/createdAt" "not earlier than any entry" createdAtRaw
                    :: findings
            | _ -> ()
            ...
            if statedDigest <> computedDigest then
                findings <-
                    finding
                        "QM-SELF-DIGEST"
                        "/digest"
                        computedDigest
                        (if isNull statedDigest then "<missing>" else statedDigest)
                    :: findings
            ...
            if not (manifest.Span.SequenceEqual(ReadOnlySpan<byte>(canonical))) then
                findings <-
                    finding "QM-CANONICAL" "/" "canonical JSON bytes" "non-canonical bytes"
                    :: findings
