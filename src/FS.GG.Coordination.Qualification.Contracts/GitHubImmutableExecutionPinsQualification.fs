...
type ImmutableExecutionReferenceKind =
    | ThirdPartyAction
    | ReusableWorkflow
...
type GitHubImmutableExecutionPinsFinding =
    {
        Code: string
        ControlId: string
        Message: string
...
module GitHubImmutableExecutionPinsQualification =
        ...
        if String.IsNullOrWhiteSpace literal then
            ...
            if split.Length <> 2 || not (hexLength 40 split[1]) then
                ...
                if identity.Length < 2 then
                    ...
                    let isWorkflow =
                        identity.Length >= 5 && identity[2] = ".github" && identity[3] = "workflows"

                    let targetPath =
                        if isWorkflow then
                            Some(String.concat "/" identity[2..])
                        else
                            None
    ...
    let private canonical snapshot =
        ...
        let publications =
            ...
            |> List.sortBy (fun value -> value.Repository, value.Path, value.Revision)
        ...
        [
            frame (string snapshot.SchemaVersion)
            frame snapshot.Repository
            frame snapshot.SourceRevision
            frame snapshot.PrerequisiteReceiptDigest
                    ...
                    for workflow in workflows do
                        ...
                        for reference in
                            ...
                            |> List.sortBy (fun value ->
                                kind value.Kind, value.TargetRepository, value.TargetPath, value.Revision) do
                            yield
                                frame (kind reference.Kind)
                                + frame reference.WorkflowPath
                                + frame reference.TargetRepository
                                + scalar reference.TargetPath
                                + frame reference.Revision
                ]
            strings
                [
                    for publication in publications do
                        yield
                            frame publication.Repository
                            + frame publication.Path
                            + frame publication.Revision
                            + frame publication.ContentSha256
                            + frame (string publication.WorkflowCall)
                ]
            ...
            strings
                [
                    for configuration in updaterConfigurations do
                        yield
                            frame configuration.Path
                            + frame configuration.Sha256
                            + frame configuration.Authority
                            + frame (string configuration.PullRequestOnly)
                            + frame (string configuration.DirectPush)
                ]
            strings
                [
                    for updater in updaters do
                        yield
                            frame updater.Name
                            + frame (string updater.Automated)
                            + frame (string updater.PullRequestOnly)
                            + frame (string updater.DirectPush)
                            + frame updater.PolicyRepository
                            + frame updater.PolicyRevision
                            + frame updater.PolicyPath
                            + frame updater.PolicySha256
    ...
    let compile snapshot =
        ...
        if snapshot.SchemaVersion <> 1 then
            errors.Add UnsupportedImmutablePinsSchema

        if not (validRepository snapshot.Repository) then
            errors.Add InvalidImmutablePinsRepository

        if not (hexLength 40 snapshot.SourceRevision) then
            errors.Add InvalidImmutablePinsSourceRevision

        if not (hexLength 64 snapshot.PrerequisiteReceiptDigest) then
            errors.Add InvalidImmutablePinsPrerequisite

        if not snapshot.Complete || snapshot.Workflows.IsEmpty then
            errors.Add IncompleteImmutablePinsCorpus
        ...
        then
            errors.Add DuplicateImmutableWorkflow

        for workflow in snapshot.Workflows do
            if not (validPath workflow.Path) || not (hexLength 64 workflow.Sha256) then
                errors.Add InvalidImmutableWorkflowDigest

            for reference in workflow.References do
                if reference.WorkflowPath <> workflow.Path then
                    errors.Add CrossWorkflowReference

                if
                    not (validRepository reference.TargetRepository)
                    || not (hexLength 40 reference.Revision)
                then
                    errors.Add MutableExecutionReference
        ...
        if
            snapshot.Publications
            |> List.countBy (fun value -> value.Repository, value.Path, value.Revision)
            |> List.exists (fun (_, count) -> count <> 1)
        then
            errors.Add DuplicateImmutablePublication

        for publication in snapshot.Publications do
            if
                not (validRepository publication.Repository)
                || not (validPath publication.Path)
                || not (hexLength 40 publication.Revision)
                || not (hexLength 64 publication.ContentSha256)
            then
                errors.Add InvalidImmutablePublication

            if not publication.WorkflowCall then
                errors.Add PublicationIsNotReusableWorkflow

        for reference in
            snapshot.Workflows
            |> List.collect _.References
            |> List.filter (fun value -> value.Kind = ReusableWorkflow) do
            let matches =
                snapshot.Publications
                |> List.filter (fun value ->
                    value.Repository = reference.TargetRepository
                    && Some value.Path = reference.TargetPath
                    && value.Revision = reference.Revision)

            if matches.IsEmpty then
                errors.Add MissingImmutablePublication
            elif matches.Length <> 1 then
                errors.Add ConflictingImmutablePublication
        ...
        let requiredUpdaterSelectors =
            snapshot.RequiredUpdaterInvocationSelectors |> Set.ofList

        if
            requiredUpdaterPaths.IsEmpty
            || requiredUpdaterPaths.Count <> snapshot.RequiredUpdaterConfigurationPaths.Length
            || snapshot.RequiredUpdaterConfigurationPaths |> List.exists (validPath >> not)
            || requiredUpdaterSelectors.IsEmpty
            || requiredUpdaterSelectors.Count
               <> snapshot.RequiredUpdaterInvocationSelectors.Length
            || snapshot.RequiredUpdaterInvocationSelectors
        ...
        for configuration in snapshot.UpdaterConfigurations do
            if
                not (validPath configuration.Path)
                || not (hexLength 64 configuration.Sha256)
                || String.IsNullOrWhiteSpace configuration.Authority
                || not configuration.PullRequestOnly
        ...
        if automated.Length > 1 then
            errors.Add MultipleAutomatedUpdaters

        if automated.Length <> 1 || automated.Head.Name <> "renovate" then
            errors.Add RenovateAuthorityMissing

        for updater in snapshot.Updaters do
            if
                String.IsNullOrWhiteSpace updater.Name
                ...
                || (updater.Automated && (not updater.PullRequestOnly || updater.DirectPush))
            then
                errors.Add InvalidUpdaterAuthority

        let requiredManagers = snapshot.RequiredManagers |> Set.ofList

        if
            requiredManagers.IsEmpty
            || snapshot.RequiredManagers.Length <> requiredManagers.Count
        then
            errors.Add RenovateOwnershipIncomplete

        if
            automated.Length = 1
            && (automated.Head.OwnedManagers |> Set.ofList) <> requiredManagers
        then
            errors.Add RenovateOwnershipIncomplete

        if errors.Count > 0 then
            Error(List.ofSeq errors |> List.distinct)
        else
            let references =
                snapshot.Workflows |> List.sumBy (fun value -> value.References.Length)
            ...
            Ok
                {
                    ...
                    PublicationCount = snapshot.Publications.Length
                    UpdaterConfigurationCount = snapshot.UpdaterConfigurations.Length
                    AutomatedUpdater = automated.Head.Name
                    Managers = snapshot.RequiredManagers |> List.sort
                    Seal = seal snapshot
                }
        ...
        let findingsFor source values =
            ...
            [
                for missing in Set.difference expected (grouped |> List.map fst |> Set.ofList) do
                    {
                        Code = "IEP-CONTROL-MISSING"
                        ControlId = missing
                        Message = $"%s{source} omitted the required control"
                    }
                for control, results in grouped do
                    if results.Length <> 1 then
                        {
                            Code = "IEP-CONTROL-DUPLICATE"
                            ControlId = control
                            Message = $"%s{source} supplied the control more than once"
                        }
                    else
                        ...
                        if not result.BaselineGreen then
                            {
                                Code = "IEP-BASELINE-RED"
                                ControlId = control
                                Message = $"%s{source} baseline is not green"
                            }

                        if not result.ControlPassed then
                            {
                                Code = "IEP-CONTROL-FAILED"
                                ControlId = control
                                Message = $"%s{source} control did not pass"
                            }
            ]
