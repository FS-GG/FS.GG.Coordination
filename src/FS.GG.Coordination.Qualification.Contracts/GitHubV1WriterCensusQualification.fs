...
type GitHubV1WriterCommand = { Name: string; Writes: string }
    ...
    let validateSnapshot snapshot =
        ...
        for label, value in
            [
                ...
                "producerTree", snapshot.ProducerTree
                "roadmapRevision", snapshot.RoadmapRevision
                "sourceBaseRevision", snapshot.SourceBaseRevision
            ] do
            if not (revision value) then
                findings.Add(finding "WC-REVISION" label "expected a full lowercase Git revision")

        for label, value in
            [
                "roadmapSha256", snapshot.RoadmapSha256
                "acceptedEpochReceiptSha256", snapshot.AcceptedEpochReceiptSha256
                "q0EvidenceSha256", snapshot.Q0EvidenceSha256
                "q0CorpusSha256", snapshot.Q0CorpusSha256
                "censusSha256", snapshot.CensusSha256
                "commandContractSha256", snapshot.CommandContractSha256
                "checkerSha256", snapshot.CheckerSha256
                "fixtureRunnerSha256", snapshot.FixtureRunnerSha256
            ] do
            if not (sha256 value) then
                findings.Add(finding "WC-DIGEST" label "expected a lowercase SHA-256")

        let commandNames = snapshot.Commands |> List.map _.Name

        if snapshot.Commands.Length <> 54 then
            findings.Add(finding "WC-COMMAND-COUNT" "commands" "expected all 54 typed command roots")

        if
            commandNames <> List.sort commandNames
            || commandNames.Length <> (commandNames |> List.distinct |> List.length)
        then
        ...
        for command in snapshot.Commands do
            if String.IsNullOrWhiteSpace command.Name then
                findings.Add(finding "WC-COMMAND-NAME" "commands" "blank command root")
        ...
        let count writes =
            snapshot.Commands |> List.filter (_.Writes >> (=) writes) |> List.length
        ...
        if snapshot.Sources.Length <> 62 then
            findings.Add(finding "WC-SOURCE-COUNT" "sources" "expected all 62 discovered source identities")

        if
            sourcePaths <> List.sort sourcePaths
            || sourcePaths.Length <> (sourcePaths |> List.distinct |> List.length)
        then
        ...
        for source in snapshot.Sources do
            ...
            if not (sha256 source.Sha256) then
                findings.Add(finding "WC-SOURCE-DIGEST" source.Path "source identity is not a SHA-256")

            if
                source.SinkKinds <> List.sort source.SinkKinds
                || source.SinkKinds.Length <> (source.SinkKinds |> List.distinct |> List.length)
            then
                findings.Add(finding "WC-SINK-ORDER" source.Path "sink kinds must be unique and ordinally sorted")

            if
                not (
                    Set.contains source.Disposition writerDispositions
                    || Set.contains source.Disposition otherDispositions
                )
            then
                findings.Add(finding "WC-DISPOSITION" source.Path "unknown source disposition")

            if
                not source.SinkKinds.IsEmpty
                && not (
                    Set.contains source.Disposition writerDispositions
                    || source.Disposition = "typed-command-catalogue"
                )
            then
                if
                    source.NonWriterJustification
                    |> Option.forall (fun value -> String.IsNullOrWhiteSpace value || value.Trim().Length < 12)
                then
                    findings.Add(
                        finding "WC-JUSTIFICATION" source.Path "non-writer sink requires a specific justification"
                    )
                ...
                for control, results in groups do
                    if results.Length <> 1 then
                        ...
                        if not results.Head.BaselineGreen then
                            finding "WC-BASELINE-RED" control $"{source} baseline is not green"

                        if not results.Head.MutationRed then
                            finding "WC-MUTATION-SURVIVED" control $"{source} mutation did not fail"
            ]
