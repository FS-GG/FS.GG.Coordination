...
type GitHubPrincipalClass =
    | NormalCoordination
    | AdminCutover
    | Release

type GitHubInterpreterOperation =
    | InspectCoordination
    | CoordinateIssue
...
type GitHubPermissionCompilationControlResult =
    {
        Control: GitHubPermissionCompilationControl
        ControlPassed: bool
        BaselineGreen: bool
    }

type GitHubPermissionCompilationQualificationFinding =
    {
        Code: string
        ControlId: string
        Message: string
...
module GitHubPermissionCompilationQualification =
    ...
    let requiredOperations =
        [ InspectCoordination; CoordinateIssue; ApplyRepositoryCutover; PublishRelease ]
    ...
    let private className =
        function
        | NormalCoordination -> "normal"
        | AdminCutover -> "admin-cutover"
        | Release -> "release"
    ...
    let private validHex count (value: string) =
        value.Length = count && value |> Seq.forall Uri.IsHexDigit

    let private permissionFrames values =
        values
        |> normalize
        |> List.collect (fun value -> [ frame value.Name; frame (levelName value.Level) ])
    ...
    let private compileSeal snapshot registrations =
        [
            ...
            frame snapshot.RoadmapRevision
            frame snapshot.RoadmapSha256
            frame snapshot.PrerequisiteReceiptDigest
            frame snapshot.PermissionCensusPath
            frame snapshot.PermissionCensusSha256
            for value in snapshot.RequiredPermissionFamilies |> List.sort do
                frame value
            frame (string snapshot.Complete)
            for value in registrations |> List.sortBy _.Id do
                frame value.Id
                frame (operationName value.Operation)
                frame (className value.PrincipalClass)
                frame value.AppPrincipal
                frame value.Environment
                yield! permissionFrames value.AppPermissions
                yield! permissionFrames value.WorkflowPermissions
        ]
        |> hash
    ...
    let compile snapshot =
        ...
        if snapshot.SchemaVersion <> 1 then
            findings.Add(InvalidPermissionCompilationField "schemaVersion")

        if String.IsNullOrWhiteSpace snapshot.Repository then
            findings.Add(InvalidPermissionCompilationField "repository")

        if not (validHex 40 snapshot.SourceRevision) then
            findings.Add(InvalidPermissionCompilationField "sourceRevision")

        if not (validHex 40 snapshot.RoadmapRevision) then
            findings.Add(InvalidPermissionCompilationField "roadmapRevision")

        if not (validHex 64 snapshot.RoadmapSha256) then
            findings.Add(InvalidPermissionCompilationField "roadmapSha256")

        if not (validHex 64 snapshot.PrerequisiteReceiptDigest) then
            findings.Add(InvalidPermissionCompilationField "prerequisiteReceiptDigest")

        if
            snapshot.PermissionCensusPath
            ...
            || snapshot.RequiredPermissionFamilies <> requiredPermissionFamilies
        then
            findings.Add CanonicalPermissionCensusMismatch

        if not snapshot.Complete then
            findings.Add IncompleteInterpreterInventory

        snapshot.Registrations
        |> List.groupBy _.Id
        |> List.iter (fun (id, values) ->
            if String.IsNullOrWhiteSpace id then
                findings.Add(InvalidPermissionCompilationField "interpreter.id")

            if values.Length <> 1 then
                findings.Add(DuplicateInterpreterId id))

        snapshot.Registrations
        |> List.groupBy _.Operation
        |> List.iter (fun (operation, values) ->
            if values.Length <> 1 then
                findings.Add(DuplicateInterpreterOperation operation))

        let present = snapshot.Registrations |> List.map _.Operation |> Set.ofList

        for operation in requiredOperations do
            if not (present.Contains operation) then
                findings.Add(MissingInterpreterOperation operation)

        let compiled =
            [
                for registration in snapshot.Registrations do
                    let expectedClass, expectedPrincipal, expectedEnvironment, expectedApp, expectedWorkflow =
                        requirement registration.Operation

                    if
                        registration.PrincipalClass <> expectedClass
                        || registration.AppPrincipal <> expectedPrincipal
                    then
                        findings.Add(InvalidPrincipalBinding registration.Id)

                    if registration.Environment <> expectedEnvironment then
                        findings.Add(InvalidEnvironmentBinding registration.Id)

                    let checkPermissions surface expected actual =
                        actual
                        |> List.groupBy _.Name
                        |> List.iter (fun (name, values) ->
                            if name = "*" || name.Contains("all", StringComparison.OrdinalIgnoreCase) then
                                findings.Add(WildcardPermission $"{registration.Id}:{surface}:{name}")

                            if values.Length <> 1 then
                                findings.Add(DuplicatePermission(registration.Id, $"{surface}:{name}")))

                        let expectedSet, actualSet =
                            expected |> List.map permissionKey |> Set.ofList,
                            actual |> List.map permissionKey |> Set.ofList

                        for name, level in Set.difference actualSet expectedSet do
                            findings.Add(
                                UndeclaredOrOverprivilegedPermission(registration.Id, $"{surface}:{name}:{level}")
                            )

                        for name, level in Set.difference expectedSet actualSet do
                            findings.Add(MissingLeastPrivilegePermission(registration.Id, $"{surface}:{name}:{level}"))
                    ...
                    yield
                        {
                            ...
                            PrincipalClass = registration.PrincipalClass
                            AppPrincipal = registration.AppPrincipal
                            Environment = registration.Environment
                            AppPermissions = normalize expectedApp
                            WorkflowPermissions = normalize expectedWorkflow
                        }
            ]
            ...
            Ok
                {
                    ...
                    InterpreterCount = ordered.Length
                    NormalCount =
                        ordered
                    ...
                    Seal = compileSeal snapshot ordered
                }
            ...
            [
                ...
                for control, results in grouped do
                    if results.Length <> 1 then
                        {
                            Code = "PC-CONTROL-DUPLICATE"
                            ControlId = control
                            Message = $"{source} supplied the control more than once"
                        }
                    else
                        if not results.Head.BaselineGreen then
                            {
                                Code = "PC-BASELINE-RED"
                                ControlId = control
                                Message = $"{source} baseline is not green"
                            }

                        if not results.Head.ControlPassed then
                            {
                                Code = "PC-CONTROL-FAILED"
                                ControlId = control
                                Message = $"{source} control did not pass"
                            }
            ]
