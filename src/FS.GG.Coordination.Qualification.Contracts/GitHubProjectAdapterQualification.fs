...
type GitHubProjectAdapterFinding =
    {
        Code: string
        ControlId: string
        Message: string
...
module GitHubProjectAdapterQualification =
    ...
    let controlId =
        ...
        | UnreadableObservation -> "unreadable-observation"
            ...
            [
                for value in results do
                    ...
                    if not value.MutationRed then
                        yield
                            {
                                Code = $"GPAQ-{producer.ToUpperInvariant()}-NOT-RED"
                                ControlId = id
                    ...
                    if not value.BaselineGreen then
                        yield
                            {
                                Code = $"GPAQ-{producer.ToUpperInvariant()}-BASELINE-NOT-GREEN"
                                ControlId = id
