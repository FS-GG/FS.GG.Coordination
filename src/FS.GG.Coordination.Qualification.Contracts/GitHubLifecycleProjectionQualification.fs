...
type GitHubLifecycleProjectionControl =
    | IntentAuthority
    | CompleteKnowledge
    | FormalPrecedence
    | HoldDependency
...
type GitHubLifecycleProjectionControlResult =
    {
        Control: GitHubLifecycleProjectionControl
        MutationRed: bool
        BaselineGreen: bool
    }

type GitHubLifecycleProjectionFinding =
    {
        Code: string
        ControlId: string
        Message: string
...
module GitHubLifecycleProjectionQualification =
    ...
    let requiredControls =
        [
            GitHubLifecycleProjectionControl.IntentAuthority
            GitHubLifecycleProjectionControl.CompleteKnowledge
            GitHubLifecycleProjectionControl.FormalPrecedence
            GitHubLifecycleProjectionControl.HoldDependency
            GitHubLifecycleProjectionControl.Claim
            GitHubLifecycleProjectionControl.PullRequest
            GitHubLifecycleProjectionControl.Review
            GitHubLifecycleProjectionControl.Delivery
            GitHubLifecycleProjectionControl.IssueState
            GitHubLifecycleProjectionControl.StatusMapping
            GitHubLifecycleProjectionControl.StatusNotIntent
            GitHubLifecycleProjectionControl.HistoricalFact
            GitHubLifecycleProjectionControl.ProtectedDelivery
            GitHubLifecycleProjectionControl.ExactPlan
            ...
            [
                ...
                for value in values do
                    if not value.BaselineGreen then
                        yield
                            {
                                Code = prefix + "-BASELINE"
                                ControlId = controlId value.Control
                                Message = "baseline was not green"
                            }
                        ...
                        yield
                            {
                                Code = prefix + "-NOT-RED"
                                ControlId = controlId value.Control
                                Message = "mutation did not fail closed"
                            }
            ]

        let findings =
            validateSet "GLPQ-GENERATED" generated
            @ validateSet "GLPQ-INDEPENDENT" independent
