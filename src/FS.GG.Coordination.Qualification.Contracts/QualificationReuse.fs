...
let private isLowerSha256 (value: string) =
    ...
    && value.Length = 64
    && value
       |> Seq.forall (fun character -> Char.IsDigit character || character >= 'a' && character <= 'f')
...
let private framedTreeBytes (files: TrackedFile list) =
    ...
    if paths.Length <> (paths |> List.distinct |> List.length) then
        invalidArg (nameof files) "tracked paths must be distinct"
    ...
    for file in ordered do
        if not (allowedModes.Contains file.Mode) then
            invalidArg (nameof files) $"unsupported tracked mode: %s{file.Mode}"

        if
            String.IsNullOrWhiteSpace file.Path
            || Path.IsPathRooted file.Path
            || file.Path.Contains('\\')
            || file.Path.Split('/') |> Array.exists ((=) "..")
        then
...
let private subjectPayloadBytes
    (tree: string)
    (plan: string)
    (workflow: string)
    (toolchain: string)
...
let createSubject
    (files: TrackedFile list)
    (planBytes: byte array)
    (workflowBytes: byte array)
    (environmentBytes: byte array)
    (reviewPolicyBytes: byte array)
    =
    if files.IsEmpty then
        invalidArg (nameof files) "tracked tree must not be empty"

    let tree = framedTreeBytes files |> sha256
    let plan = sha256 planBytes
    let workflow = sha256 workflowBytes

    let subset (predicate: string -> bool) =
        files
        |> List.filter (fun file -> predicate file.Path)
        |> framedTreeBytes
        |> sha256
    ...
    let digest =
        subjectPayloadBytes tree plan workflow toolchain dependencies gateSet environment reviewPolicy
        |> sha256
...
let createFormalSubject (files: TrackedFile list) selectors (policyBytes: byte array) =
    ...
    if List.isEmpty selectors then
        invalidArg (nameof selectors) "formal subject selectors must not be empty"

    let selectorNames = selectors |> List.map selectorText

    if selectorNames.Length <> (selectorNames |> List.distinct |> List.length) then
        invalidArg (nameof selectors) "formal subject selectors must be distinct"
    ...
    for selector in selectors do
        match selector with
        | Exact path when String.IsNullOrWhiteSpace path || Path.IsPathRooted path || path.Contains('\\') ->
            invalidArg (nameof selectors) "formal exact selector is unsafe"
        | Prefix prefix when
            ...
            || prefix.Contains('\\')
            ->
            invalidArg (nameof selectors) "formal prefix selector is unsafe"
        | _ -> ()
    ...
    let selected =
        ...
        |> List.choose (fun file ->
            let count =
                selectors
                |> List.filter (fun selector -> matches selector file.Path)
                |> List.length

            if count > 1 then
                invalidArg (nameof selectors) $"formal subject selector overlap: {file.Path}"
    ...
    {
        FilesSha256 = filesDigest
        SelectorPolicySha256 = selectorPolicy
        FileCount = selected.Length
        SubjectSha256 = sha256 payload
    }
...
let private make (kind: DecisionKind) (reason: string) (candidate: string) (subject: string) (prior: PriorRun option) =
    if not (isHead candidate) then
        invalidArg (nameof candidate) "candidate must be an exact 40-hex SHA"

    if not (isLowerSha256 subject) then
        invalidArg (nameof subject) "subject must be a lowercase SHA-256"

    if String.IsNullOrWhiteSpace reason then
        invalidArg (nameof reason) "decision reason is required"
    ...
    |> Option.iter (fun value ->
        if
            not (isHead value.Head)
            || value.RunId <= 0L
            || value.Attempt <= 0
            || not (isLowerSha256 value.EvidenceSha256)
        then
        ...
        |> Option.iter (fun minutes ->
            if minutes < 0M then
                invalidArg (nameof prior) "prior runner minutes cannot be negative")
...
let decide candidate subjectSha256 prior priorSubjectSha256 =
    ...
    | Some value, Some priorSubject when priorSubject = subjectSha256 ->
        make Reuse "identical-complete-tree" candidate subjectSha256 (Some value)
...
let refuse candidate subjectSha256 reason =
    make Refuse reason candidate subjectSha256 None

let decisionBytes decision =
    let expected =
        payloadBytes decision.Kind decision.Reason decision.Candidate decision.SubjectSha256 decision.Prior
        |> sha256

    if expected <> decision.SelfSha256 then
        invalidArg (nameof decision) "decision self digest is stale"
...
let parseDecision (bytes: byte array) =
    try
        ...
        if properties <> expected then
            Error "reuse receipt properties are not the exact canonical set"
        elif
            stringProperty "schema" root
            <> Some "fsgg.coordination.qualification-reuse-receipt/1"
        then
            Error "reuse receipt schema is unsupported"
        else
            let kind =
            ...
            let reason =
                stringProperty "reason" root
                |> Option.defaultWith (fun () -> failwith "reuse reason is missing")

            let candidate =
                stringProperty "candidate" root
                |> Option.defaultWith (fun () -> failwith "reuse candidate is missing")

            let subject =
                stringProperty "subjectSha256" root
                |> Option.defaultWith (fun () -> failwith "reuse subject is missing")

            let priorElement = root.GetProperty("prior")

            let prior =
                if priorElement.ValueKind = JsonValueKind.Null then
                    None
                    ...
                    Some
                        {
                            Head =
                                stringProperty "head" priorElement
                                |> Option.defaultWith (fun () -> failwith "prior head is missing")
                            RunId = priorElement.GetProperty("runId").GetInt64()
                            Attempt = priorElement.GetProperty("attempt").GetInt32()
                            EvidenceSha256 =
                                stringProperty "evidenceSha256" priorElement
                                |> Option.defaultWith (fun () -> failwith "prior evidence digest is missing")
                            ArtifactExpiresAt =
                                stringProperty "artifactExpiresAt" priorElement
                                |> Option.defaultWith (fun () -> failwith "prior expiry is missing")
                            RunnerMinutes =
                                let minutes = priorElement.GetProperty("runnerMinutes")

                                if minutes.ValueKind = JsonValueKind.Null then
                                    None
                                elif minutes.ValueKind = JsonValueKind.Number then
                                    Some(minutes.GetDecimal())
                                else
                                    failwith "prior runner minutes must be a number or null"
                        }
                else
                    failwith "reuse prior must be an object or null"

            let parsed = make kind reason candidate subject prior

            let self =
                stringProperty "selfSha256" root
                |> Option.defaultWith (fun () -> failwith "reuse self digest is missing")

            let final = { parsed with SelfSha256 = self }

            if self <> parsed.SelfSha256 then
                Error "reuse self digest does not match"
            elif bytes <> decisionBytes final then
                Error "reuse receipt bytes are not canonical"
            else
                Ok final
    with exceptionValue ->
...
let private requireDigest (name: string) (value: string) =
    if not (isLowerSha256 value) then
        invalidArg name $"{name} must be a lowercase SHA-256"
...
let private obligationPayload
    ...
    compactBytes (fun writer ->
        ...
        for name, value in identityFields identity do
            writer.WriteString(name, value)

        writer.WriteEndObject())

let createCandidateObligation
    (candidate: string)
    (baseRevision: string)
    (treeSha256: string)
    (sourceSha256: string)
    (identity: ReuseIdentity)
    =
    if not (isHead candidate) || not (isHead baseRevision) then
        invalidArg (nameof candidate) "candidate and base must be exact 40-hex revisions"

    requireDigest (nameof treeSha256) treeSha256
    requireDigest (nameof sourceSha256) sourceSha256

    identityFields identity
    |> List.iter (fun (name, value) -> requireDigest name value)
...

    {
        ...
        Identity = identity
        ObligationSha256 =
            obligationPayload normalizedCandidate normalizedBase treeSha256 sourceSha256 identity
            |> sha256
    }
...
let candidateObligationBytes (obligation: CandidateObligation) =
    let payload =
        obligationPayload
            ...
            obligation.BaseRevision
            obligation.TreeSha256
            obligation.SourceSha256
            obligation.Identity

    if sha256 payload <> obligation.ObligationSha256 then
        invalidArg (nameof obligation) "candidate obligation digest is stale"
    ...
    Array.append
        (compactBytes (fun writer ->
            ...
            |> Seq.iter (fun property -> property.WriteTo writer)

            writer.WriteString("obligationSha256", obligation.ObligationSha256)
            writer.WriteEndObject()))
        [| byte '\n' |]

let private dispositionText =
    function
    | Current -> "current"
    | Reused -> "reused"
    | Deferred -> "deferred"
...
let private selectionPayload
    (candidate: CandidateObligation)
    (disposition: ReuseDisposition)
    (reason: string)
    (prior: PriorExecution option)
    (semantic: SemanticDelta)
    (correspondence: string option)
    (pending: bool)
    (state: CoherentState)
    =
    compactBytes (fun writer ->
        ...
        | Some value -> writer.WriteString("bindingCorrespondenceSha256", value)
        | None -> writer.WriteNull("bindingCorrespondenceSha256")

        writer.WriteBoolean("coherentRunPending", pending)
        writer.WriteString("coherentState", coherentText state)
        writer.WriteEndObject())

let private makeSelection
    (candidate: CandidateObligation)
    disposition
    reason
    prior
    ...
    : ReuseSelection =
    let payload =
        selectionPayload candidate disposition reason prior semantic correspondence pending state

    {
        ...
        BindingCorrespondenceSha256 = correspondence
        CoherentRunPending = pending
        CoherentState = state
        SelectionSha256 = sha256 payload
    }
...
let selectReusable
    ...
    bindingCorrespondenceSha256
    |> Option.iter (requireDigest "binding correspondence")
    ...
    | Some previous ->
        ...
        let bindingValid =
            identity.BindingSha256 = old.BindingSha256
            || Option.isSome bindingCorrespondenceSha256
        ...
        let receiptValid =
            previous.RunId > 0L
            && previous.Attempt > 0
            && isLowerSha256 previous.ExecutedReceiptSha256
            && previous.Authentic
            && previous.Complete
            && temporal

        if not receiptValid then
            makeSelection
                candidate
                Current
                "prior-execution-incomplete-inauthentic-or-expired"
                None
                semanticDelta
                None
                true
                Pending
        elif not semanticDelta.IsEmpty then
            makeSelection candidate Current "semantic-delta-nonempty" None semanticDelta None true Pending
        elif not sameSemanticIdentity then
            makeSelection
                candidate
                Current
                "semantic-tool-bounds-corpus-or-harness-mismatch"
                None
                semanticDelta
                None
                true
                Pending
        elif not bindingValid then
            makeSelection
                candidate
                Current
                "changed-binding-correspondence-missing"
                None
                semanticDelta
                None
                true
            ...
            makeSelection
                candidate
                Reused
                "independently-equivalent-prior-execution"
                (Some previous)
                semanticDelta
                bindingCorrespondenceSha256
                true
...
let applyCoherentOutcome merged passed (selection: ReuseSelection) =
    if passed then
        makeSelection
            ...
            selection.Reason
            selection.Prior
            selection.SemanticDelta
            selection.BindingCorrespondenceSha256
            false
            Passed
    elif merged then
        makeSelection
...
let selectionBytes (selection: ReuseSelection) =
    let expected =
        selectionPayload
            ...
            selection.SemanticDelta
            selection.BindingCorrespondenceSha256
            selection.CoherentRunPending
            selection.CoherentState
        |> sha256

    if expected <> selection.SelectionSha256 then
        invalidArg (nameof selection) "selection self digest is stale"
...
let parseCandidateObligation (bytes: byte array) =
    try
        ...
        let text name =
            ...
            |> Option.defaultWith (fun () -> failwith $"candidate obligation is missing {name}")

        let identity =
            {
                BehavioralSha256 = text "behavioralSha256"
                CompiledContractSha256 = text "compiledContractSha256"
                ToolchainProfileSha256 = text "toolchainProfileSha256"
                VerificationBoundsSha256 = text "verificationBoundsSha256"
                FormalCorpusSha256 = text "formalCorpusSha256"
                HarnessSha256 = text "harnessSha256"
                BindingSha256 = text "bindingSha256"
            }
        ...
        if text "schema" <> "fsgg.coordination.candidate-obligation/1" then
            Error "candidate obligation schema is unsupported"
        elif text "obligationSha256" <> value.ObligationSha256 then
            Error "candidate obligation digest differs"
        elif bytes <> candidateObligationBytes value then
            Error "candidate obligation bytes are not canonical"
        else
            Ok value
    with error ->
        Error error.Message

let private partitionPayload
    (obligationSha: string)
    (qualificationPlanSha: string)
    (obligations: string list)
    (partitions: (int * string list) list)
    =
    compactBytes (fun writer ->
        writer.WriteStartObject()
        writer.WriteString("schema", "fsgg.coordination.coherent-partition-plan/1")
        ...
        obligations |> List.iter writer.WriteStringValue
...
let createPartitionPlan
    ...
    if maxPartitions < 1 || maxPartitions > 6 then
        invalidArg (nameof maxPartitions) "partition count must be between one and six"

    let ordered = obligations |> List.sort

    if
        ordered.IsEmpty
        || ordered <> List.distinct ordered
        || ordered |> List.exists String.IsNullOrWhiteSpace
    then
        invalidArg (nameof obligations) "obligations must be non-empty and distinct"
        ...
        [
            for index in 0 .. count - 1 ->
                ...
                (ordered
                 ...
                 |> List.choose (fun (i, value) -> if i % count = index then Some value else None))
        ]

    {
        Candidate = candidate
        QualificationPlanSha256 = qualificationPlanSha256
        Obligations = ordered
        PartitionCount = count
        Partitions = partitions
        PlanSha256 =
            partitionPayload candidate.ObligationSha256 qualificationPlanSha256 ordered partitions
            |> sha256
    }

let partitionPlanBytes (plan: PartitionPlan) =
    let payload =
        partitionPayload plan.Candidate.ObligationSha256 plan.QualificationPlanSha256 plan.Obligations plan.Partitions

    if sha256 payload <> plan.PlanSha256 then
        invalidArg (nameof plan) "partition plan digest is stale"
...
let parsePartitionPlan (candidate: CandidateObligation) (bytes: byte array) =
    try
        ...
        let partitions =
            ...
            |> Seq.map (fun item -> item.GetProperty("index").GetInt32(), strings (item.GetProperty "obligations"))
            |> Seq.toList
        ...
        let value =
            {
                Candidate = candidate
                QualificationPlanSha256 = qualificationPlanSha
        ...
        if
            ...
            <> "fsgg.coordination.coherent-partition-plan/1"
        then
            Error "partition plan schema is unsupported"
        elif
            root.GetProperty("candidateObligationSha256").GetString()
            <> candidate.ObligationSha256
        then
            Error "partition plan candidate differs"
        elif
            value.PlanSha256
            <> (partitionPayload candidate.ObligationSha256 qualificationPlanSha obligations partitions
                |> sha256)
        then
            Error "partition plan digest differs"
        elif bytes <> partitionPlanBytes value then
            Error "partition plan bytes are not canonical"
        else
            Ok value
    with error ->
        Error error.Message
...
let private receiptPayload (planSha: string) (partition: int) (obligations: string list) passed =
    compactBytes (fun writer ->
        writer.WriteStartObject()
        writer.WriteString("schema", "fsgg.coordination.coherent-partition-receipt/1")
        writer.WriteString("planSha256", planSha)
        writer.WriteNumber("partition", partition)
        writer.WriteStartArray("obligations")
        obligations |> List.iter writer.WriteStringValue
        writer.WriteEndArray()
...
let createPartitionReceipt (plan: PartitionPlan) partition obligations passed =
    let expected =
        plan.Partitions
        |> List.tryFind (fst >> (=) partition)
        |> Option.map snd
        |> Option.defaultWith (fun () -> invalidArg (nameof partition) "partition is not in plan")

    if obligations <> expected then
        invalidArg (nameof obligations) "partition obligations differ from immutable plan"
...
let resolvePartitionObligation (plan: PartitionPlan) partition =
    ...
    | Some(_, obligations) ->
        Error $"partition {partition} must have exactly one hosted obligation, found {obligations.Length}"

let validatePriorAggregateBinding
    (currentPlan: PartitionPlan)
    (priorPlan: PartitionPlan)
    (receipt: CoherentAggregateReceipt)
    =
    if currentPlan.QualificationPlanSha256 <> priorPlan.QualificationPlanSha256 then
        Error "prior coherent execution plan is stale"
    elif receipt.CandidateObligationSha256 <> priorPlan.Candidate.ObligationSha256 then
        Error "prior aggregate receipt does not bind the prior candidate"
    elif receipt.PlanSha256 <> priorPlan.PlanSha256 then
        Error "prior aggregate receipt does not bind the prior partition plan"
    elif not receipt.Passed then
        Error "prior aggregate receipt did not pass"
    else
        Ok()

let partitionReceiptBytes (receipt: PartitionReceipt) =
    let payload =
        receiptPayload receipt.PlanSha256 receipt.Partition receipt.Obligations receipt.Passed

    if sha256 payload <> receipt.ReceiptSha256 then
        invalidArg (nameof receipt) "partition receipt digest is stale"
        ...
        let receipt =
            {
                ...
                ReceiptSha256 = root.GetProperty("receiptSha256").GetString()
            }

        if
            root.GetProperty("schema").GetString()
            <> "fsgg.coordination.coherent-partition-receipt/1"
        then
            Error "partition receipt schema is unsupported"
        elif
            receipt.ReceiptSha256
            <> (receiptPayload receipt.PlanSha256 receipt.Partition receipt.Obligations receipt.Passed
                |> sha256)
        then
            Error "partition receipt digest differs"
        elif bytes <> partitionReceiptBytes receipt then
            Error "partition receipt bytes are not canonical"
        else
            Ok receipt
    with error ->
        Error error.Message
...
let aggregatePartitions (plan: PartitionPlan) (receipts: PartitionReceipt list) =
    ...
    if ordered.Length <> plan.PartitionCount then
        Error "partition coverage incomplete"
    elif ordered |> List.map _.Partition <> [ 0 .. plan.PartitionCount - 1 ] then
        Error "partition indexes are missing or duplicated"
    else
        let invalid =
            ordered
...
let private aggregatePayload (candidate: string) (plan: string) (receipts: string list) passed =
    compactBytes (fun writer ->
        writer.WriteStartObject()
        writer.WriteString("schema", "fsgg.coordination.coherent-aggregate-receipt/1")
        writer.WriteString("candidateObligationSha256", candidate)
        writer.WriteString("planSha256", plan)
        writer.WriteStartArray("partitionReceiptSha256")
        receipts |> List.iter writer.WriteStringValue
        writer.WriteEndArray()
        writer.WriteBoolean("passed", passed)
        writer.WriteEndObject())
...
let createCoherentAggregateReceipt (plan: PartitionPlan) (receipts: PartitionReceipt list) =
    ...
    | Ok passed ->
        ...
        let payload =
            aggregatePayload plan.Candidate.ObligationSha256 plan.PlanSha256 digests passed

        Ok
            {
                CandidateObligationSha256 = plan.Candidate.ObligationSha256
                PlanSha256 = plan.PlanSha256
                PartitionReceiptSha256 = digests
                Passed = passed
...
let coherentAggregateReceiptBytes (receipt: CoherentAggregateReceipt) =
    let payload =
        aggregatePayload
            receipt.CandidateObligationSha256
            receipt.PlanSha256
            receipt.PartitionReceiptSha256
            receipt.Passed

    if sha256 payload <> receipt.ReceiptSha256 then
        invalidArg (nameof receipt) "aggregate receipt digest is stale"
...
let parseCoherentAggregateReceipt (bytes: byte array) =
    try
        ...
        let receipt =
            {
                ...
                PartitionReceiptSha256 =
                    root.GetProperty("partitionReceiptSha256").EnumerateArray()
                    |> Seq.map (_.GetString())
                    |> Seq.toList
                Passed = root.GetProperty("passed").GetBoolean()
                ReceiptSha256 = root.GetProperty("receiptSha256").GetString()
        ...
        if
            root.GetProperty("schema").GetString()
            <> "fsgg.coordination.coherent-aggregate-receipt/1"
        then
            Error "aggregate receipt schema is unsupported"
        elif
            not receipt.Passed
            || receipt.PartitionReceiptSha256.Length <> 6
            || (receipt.PartitionReceiptSha256 |> List.distinct |> List.length) <> 6
            || receipt.PartitionReceiptSha256 |> List.exists (isLowerSha256 >> not)
        then
            Error "aggregate receipt is incomplete or failed"
        elif
            receipt.ReceiptSha256
            <> (aggregatePayload
                    receipt.CandidateObligationSha256
                ...
                |> sha256)
        then
            Error "aggregate receipt digest differs"
        elif bytes <> coherentAggregateReceiptBytes receipt then
            Error "aggregate receipt bytes are not canonical"
        else
            Ok receipt
    with error ->
        Error error.Message
