...
module GitHubNarrowReconciliationQualification =
    ...
    let private normalizedSubject (event: GitHubReconciliationEvent) =
        $"{event.SubjectKind}:{event.SubjectId}"

    let private schedulingKey repository subject =
        [ repository; subject ] |> List.map frame |> String.concat "" |> hash
    ...
    let private seal repository sourceRevision entries =
        [
            "github-narrow-reconciliation/v1"
            repository
            sourceRevision
    ...
    let compile (repository: string) (sourceRevision: string) (events: GitHubReconciliationEvent list) =
        ...
        let validateText name value =
            if String.IsNullOrWhiteSpace value then
                errors.Add(GitHubNarrowReconciliationFinding.MissingField name)
            elif not (token.IsMatch value) then
                errors.Add(GitHubNarrowReconciliationFinding.MalformedField name)
        ...
        if events.IsEmpty then
            errors.Add GitHubNarrowReconciliationFinding.IncompleteEventInventory
        ...
        for event in events do
            for name, value in
                [
                    ...
                    "event.sourceRevision", event.SourceRevision
        ...
        let validEvents =
            ...
            |> List.filter (fun (event: GitHubReconciliationEvent) ->
                ...
                && event.SubjectKind = event.EventKind
                && event.SubjectRevision > 0L
                && event.Route = expectedRoute event.EventKind
                && not event.AttemptsDerivedWrite)
        ...
        |> List.iter (fun ((subject, _), (rows: GitHubReconciliationEvent list)) ->
            if
                rows
                |> List.map (fun (row: GitHubReconciliationEvent) -> row.EventKind, row.Route, row.Origin)
                |> Set.ofList
                |> Set.count
                    >
                    1
        ...
        if errors.Count > 0 then
            ...
            let entries =
                ...
                |> List.map (fun (subject, (rows: GitHubReconciliationEvent list)) ->
                    let newest =
                        rows
                        |> List.maxBy (fun (event: GitHubReconciliationEvent) -> event.SubjectRevision)

                    let deliveries =
                        rows
                        |> List.map (fun (event: GitHubReconciliationEvent) -> event.DeliveryId)
                        |> List.distinct
                        |> List.sort
                    ...
                    let receipt =
                        [
                            key
                            string newest.SubjectRevision
                            deliveries |> List.map frame |> String.concat ""
                        ]
                        |> List.map frame
                        |> String.concat ""
                        |> hash
                    ...
                    {
                        EventKind = newest.EventKind
                        Subject = subject
                        SubjectRevision = newest.SubjectRevision
                        SchedulingKey = key
    ...
    let serialize (plan: GitHubReconciliationPlan) =
        ...
        JsonSerializer.Serialize(
            {|
                ...
                supportedEventKinds = plan.SupportedEventKinds
    ...
    let verify (expectedSeal: string) (plan: GitHubReconciliationPlan) =
        ...
        if plan.SchemaVersion <> 1 then
            Error [ GitHubNarrowReconciliationFinding.InvalidSerialization "schemaVersion" ]
        elif plan.SupportedEventKinds <> supportedEventKinds then
            Error [ GitHubNarrowReconciliationFinding.IncompleteEventInventory ]
        ...
        then
            ...
            Error [ GitHubNarrowReconciliationFinding.AlteredSeal ]
        else
            Ok plan
    ...
    let parse (value: string) =
        try
            ...
            let plan: GitHubReconciliationPlan =
                {
                    ...
                    SupportedEventKinds =
                        root.GetProperty("supportedEventKinds").EnumerateArray()
                        |> Seq.map _.GetString()
                        |> Seq.toList
                    Entries = entries
                    WriterBoundary =
                        root.GetProperty("writerBoundary").EnumerateArray()
                        |> Seq.map _.GetString()
                        |> Seq.toList
                    Seal = text "seal" root
                }
            ...
            | Ok plan when serialize plan <> value ->
                Error [ GitHubNarrowReconciliationFinding.InvalidSerialization "non-canonical bytes" ]
            | Ok plan -> Ok plan
        with error ->
            Error [ GitHubNarrowReconciliationFinding.InvalidSerialization error.Message ]
    ...
    let replay (prior: GitHubReconciliationPlan) (events: GitHubReconciliationEvent list) =
        ...
        | Ok _, Ok candidate ->
            let priorBySubject =
                prior.Entries
                |> List.map (fun (entry: GitHubReconciliationQueueEntry) -> entry.Subject, entry)
                |> Map.ofList
            ...
            if replayOnly then
                ...
                Error
                    [
                        GitHubNarrowReconciliationFinding.ReplayConflict
                            "new or newer subject requires fresh reconciliation"
                    ]

    let validateControls
        (generated: GitHubNarrowReconciliationControlResult list)
        (independent: GitHubNarrowReconciliationControlResult list)
        =
        ...
        let validate (label: string) (rows: GitHubNarrowReconciliationControlResult list) =
            [
                if
                    rows
                    |> List.map (fun (row: GitHubNarrowReconciliationControlResult) -> controlId row.Control)
                    <> expected
                then
                    yield $"{label} control inventory differs"
                if
                    rows
                    |> List.exists (fun (row: GitHubNarrowReconciliationControlResult) ->
