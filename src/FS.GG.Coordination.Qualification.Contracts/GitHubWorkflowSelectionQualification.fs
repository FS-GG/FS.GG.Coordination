...
type GitHubWorkflowRepositoryPerformance =
    { Repository: string; Baseline: GitHubWorkflowMetrics
      Target: GitHubWorkflowMetrics; Selected: GitHubWorkflowMetrics }
...
type GitHubWorkflowSelectionSnapshot =
    { SchemaVersion: int; Repository: string; SourceRevision: string
      ...
      UnconditionalCore: GitHubWorkflowObligation list; Performance: GitHubWorkflowRepositoryPerformance list
      Sentinel: GitHubWorkflowSentinel; FleetSelectionEnabled: bool
      RemovalLedgerComplete: bool; Removals: GitHubWorkflowRemoval list }
...
module GitHubWorkflowSelectionQualification =
    ...
    let private expectedRemovals =
        [ { Workflow = "legacy-policy.yml"; Obligation = "duplicate dependency review"; Reason = "folded into reusable policy contract" }
          ...
          { Workflow = "legacy-release.yml"; Obligation = "duplicate feed verification"; Reason = "folded into reusable release contract" } ]
    ...
    let private seal (snapshot: GitHubWorkflowSelectionSnapshot) =
        [ string snapshot.SchemaVersion; snapshot.Repository; snapshot.SourceRevision; snapshot.RoadmapRevision
          ...
          String.concat "," snapshot.RequiredAggregates; obligationList snapshot.UnconditionalCore
          snapshot.Performance |> List.map (fun item -> $"{item.Repository}|{metricsText item.Baseline}|{metricsText item.Target}|{metricsText item.Selected}") |> String.concat ";"
    ...
    let compile (snapshot: GitHubWorkflowSelectionSnapshot) =
        ...
        if snapshot.SourceRevision <> "ff65faa6697ea5835de72c1e6fa1b53af0a883e3" then findings.Add(InvalidWorkflowSelectionField "sourceRevisionBinding")
        ...
        let inventoryListsValid row =
            ...
            |> List.forall (fun values -> not (List.isEmpty values) && unique values && ordered values && List.forall validText values)
        if workflowNames <> expectedWorkflows || not (unique workflowNames) || not (List.forall inventoryListsValid snapshot.Workflows) then
