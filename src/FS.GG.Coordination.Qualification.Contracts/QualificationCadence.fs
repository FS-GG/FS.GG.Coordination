...
let evaluate (now: DateTimeOffset) (policy: Policy) (gate: string) (observations: Observation list) =
    ...
    if sample |> List.exists (fun value -> value.RunId < 1L || value.Attempt < 1 || value.DurationSeconds < 0 || value.RunnerMinutes < 0M || value.DetectionDelayHours |> Option.exists (fun delay -> delay < 0M)) then
        invalidArg (nameof observations) "cadence observations must have positive identities and non-negative measurements"
    if sample |> List.map (fun value -> value.RunId, value.Attempt) |> List.distinct |> List.length <> sample.Length then
    ...
    let blastRadius = if protectedGate || gate = "canonical-quint" then "high" else "bounded"
    ...
    let kind, reasons, confidence =
        ...
        elif protectedGate then Retain, [ "minimum-cadence-protected" ], "high"
    ...
    { Gate = gate; Kind = kind; ReasonCodes = reasons; ObservationCount = sample.Length; UniqueDefectCount = defects.Length
      RunnerMinutes = total; CostSavedRunnerMinutes = saved; ExpectedDetectionDelayHours = expectedDelay
      ClosureEquivalent = equivalent; BlastRadius = blastRadius; Confidence = confidence; PolicyVersion = policy.Version }
