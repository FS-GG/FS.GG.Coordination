...
let private safePath (value: string) =
    ...
    && not (value.Contains '\\')
    && value.Split('/') |> Array.forall (fun part -> part <> ".." && part <> "")
...
let parse (bytes: byte array) =
    try
        ...
        if stringProperty "schema" root <> "fsgg.coordination.milestone-qualification/1" then
            ...
            let expectedChildren =
                ...
                |> Seq.map (fun value ->
                    if value.ValueKind <> JsonValueKind.String then
                        failwith "expected child ids must be strings"
                    else
                        value.GetString())
                |> Seq.toList
            ...
            let children =
                ...
                |> Seq.map (fun child ->
                    let acceptance =
                        ...
                        if
                            child.TryGetProperty("acceptance", &value)
                            && value.ValueKind = JsonValueKind.Object
                        then
                            Some
                                {
                                    ReceiptPath = stringProperty "receiptPath" value
                                    ReceiptDigest = stringProperty "receiptDigest" value
                                }
                        elif
                        ...
                        then
                            ...
                            failwith "child acceptance must be an object or null"
            ...
            if String.IsNullOrWhiteSpace policy || String.IsNullOrWhiteSpace parent then
                failwith "policyVersion and parent are required"

            if List.isEmpty expectedChildren then
                failwith "expected milestone children must not be empty"

            if expectedChildren |> List.exists String.IsNullOrWhiteSpace then
                failwith "expected child ids must not be empty"

            if expectedChildren.Length <> (expectedChildren |> List.distinct |> List.length) then
                failwith "expected child ids must be distinct"

            if
                children.Length > expectedChildren.Length
                || (children |> List.map _.Id) <> (expectedChildren |> List.take children.Length)
            then
                failwith "registered milestone children must be an ordered prefix of expectedChildren"

            if
                children
                |> List.exists (fun child -> String.IsNullOrWhiteSpace child.Id || not (isDigest child.ContractSha256))
            then
                failwith "every milestone child requires an id and lowercase contract digest"

            if (children |> List.map _.Id |> List.distinct |> List.length) <> children.Length then
                failwith "milestone child ids must be distinct"
            ...
            if mode = Comprehensive && boundary.IsNone then
                failwith "comprehensive mode requires boundaryKind"
...
let private receiptFacts (bytes: byte array) =
    ...
    if stringProperty "schema" root <> "fsgg.coordination.unit-acceptance/1" then
        failwith "unit acceptance schema is unsupported"
...
let private payloadBytes (state: State) =
    ...
    match state.BoundaryKind with
    | Some value -> writer.WriteString("boundaryKind", value)
    | None -> writer.WriteNull("boundaryKind")

    writer.WriteStartArray("expectedChildren")

    for id in state.ExpectedChildren do
...
let validate state receiptBytes =
    try
        ...
        for child in state.Children do
            ...
            | Some binding ->
                if sawGap then
                    failwith "accepted children must form an ordered prefix"

                let bytes =
                    receiptBytes
                    |> Map.tryFind binding.ReceiptPath
                    |> Option.defaultWith (fun () -> failwith $"missing acceptance receipt: {binding.ReceiptPath}")

                let id, contract, digest = receiptFacts bytes

                if id <> child.Id || digest <> binding.ReceiptDigest then
                    failwith $"acceptance receipt binding differs for {child.Id}"

                if contract <> child.ContractSha256 then
                    drift.Add child.Id

                accepted <- accepted + 1

        if state.Mode = Comprehensive then
            if state.Children.Length <> state.ExpectedChildren.Length then
                failwith "comprehensive closure requires every expected child registered"

            if accepted <> state.ExpectedChildren.Length then
                failwith "comprehensive closure requires every child accepted"
        ...
        Ok
            {
                State = state
                AcceptedPrefixLength = accepted
                ContractDrift = List.ofSeq drift
                SubjectSha256 = payloadBytes state |> sha256
            }
    with exceptionValue ->
        Error exceptionValue.Message
...
let closureSubject state exactHead treeSha256 =
    if state.Mode <> Comprehensive then
        invalidArg (nameof state) "closure subject requires comprehensive mode"

    if not (isHead exactHead) then
        invalidArg (nameof exactHead) "exact head must be 40 hex"

    if not (isDigest treeSha256) then
        invalidArg (nameof treeSha256) "tree digest must be lowercase SHA-256"

    Array.concat
        [
            payloadBytes state
            Encoding.UTF8.GetBytes exactHead
            Encoding.UTF8.GetBytes treeSha256
        ]
    |> sha256
