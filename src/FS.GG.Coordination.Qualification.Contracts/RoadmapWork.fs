...
module RoadmapWork =
    ...
    let private parseGateContract path (element: JsonElement) =
        match strictObject path (Set.ofList [ "id"; "qGate"; "commandSha256" ]) element with
        | Error errors -> Error errors
        | Ok _ ->
            let id = requiredString path "id" element
            let qGate = requiredString path "qGate" element
            let digest = requiredString path "commandSha256" element
    ...
    let private gateContractList path name (element: JsonElement) =
        match property name element with
        | Some value when value.ValueKind = JsonValueKind.Array ->
            let parsed =
                value.EnumerateArray()
                |> Seq.mapi (fun index item -> parseGateContract $"{path}/{name}/{index}" item)
                |> Seq.toList
            ...
            let contracts = parsed |> List.choose Result.toOption
    ...
    let private parseUnit index (element: JsonElement) =
        ...
        | Ok _ ->
            ...
            let gateContracts = gateContractList path "gateContracts" element
        ...
        try
            ...
            | Ok _ ->
                ...
                for id in duplicateIds do
                    errors <- errors @ [ finding "RW-UNIT-DUPLICATE" "/units" $"duplicate unit id: {id}" ]
            ...
            try
                ...
                let missing =
                    ...
                    |> List.choose (fun unitValue ->
                        ...
                        if text.Contains(marker, StringComparison.Ordinal) then
                            None
                        else
                            Some(
                                finding
                                    "RW-ROADMAP-UNIT"
                                    index.RoadmapPath
                                    $"roadmap is missing exact unit heading: {marker}"
                            ))
            ...
            | Ok _ ->
                ...
                | Some value when value.ValueKind = JsonValueKind.Array && value.GetArrayLength() > 0 ->
                    for index, artifact in value.EnumerateArray() |> Seq.indexed do
                        ...
                        | Error values -> errors <- errors @ values
                        | Ok _ ->
                            match requiredString $"/artifacts/{index}" "name" artifact with
                            | Error values -> errors <- errors @ values
                            | Ok _ -> ()
                ...
                if List.isEmpty errors then
                    ...
                    Error errors
        with :? JsonException as error ->
            Error [ finding "RW-RECEIPT-JSON" "/" error.Message ]
    ...
    let private prerequisites index unitValue receiptDocuments =
        ...
        if not (List.isEmpty errors) then
            ...
            let duplicateIds =
                receipts |> List.countBy _.UnitId |> List.filter (fun (_, count) -> count > 1)

            let mutable findings =
                duplicateIds
                |> List.map (fun (id, _) -> finding "RW-RECEIPT-DUPLICATE" "/receipts" $"duplicate receipt for {id}")
            ...
            if not (List.isEmpty findings) then
                ...
                Ok(
                    ...
                    |> List.map (fun prerequisite ->
                        receipts |> List.find (fun receipt -> receipt.UnitId = prerequisite))
                )
    ...
    let private manifestNode
        ...
        let gateContractNodes =
            unitValue.GateContracts
            |> List.map (fun contract ->
                let node = JsonObject()
                node.Add("id", contract.Id)
                node.Add("qGate", contract.QGate)
                node.Add("commandSha256", contract.CommandSha256)
                node :> JsonNode)
            |> List.toArray
        ...
        root.Add("gateContracts", JsonArray(gateContractNodes))
        ...
        | Ok(index, unitValue) ->
            ...
            if not (List.isEmpty validation) then
                ...
                | Ok receipts ->
                    ...
                    let bytes =
                        manifestNode (sha256 indexBytes) index unitValue receipts candidate createdAt artifacts
            ...
            | Ok receipts ->
                try
                    ...
                    | Ok _ ->
                        ...
                        match gateContractList "" "gateContracts" root with
                        | Ok actual when actual = unitValue.GateContracts -> ()
                        | Ok _ ->
                            errors <-
                                errors
                                @ [ finding
                            ...
                            for index, artifact in value.EnumerateArray() |> Seq.indexed do
                                ...
                                | Ok _ ->
                                    ...
                                    | Ok _ ->
                                        errors <-
                                            errors
                                            @ [ finding "RW-SHA256" $"/artifacts/{index}/sha256" "expected SHA-256" ]
                                    | Error values -> errors <- errors @ values

                                    match requiredString $"/artifacts/{index}" "path" artifact with
                                    | Ok path when
                                        not (Path.IsPathRooted path) && not (path.Split('/') |> Array.contains "..")
                                        ->
                                        ()
                                    | Ok _ ->
                                        errors <-
                                            errors
                                            @ [ finding "RW-PATH" $"/artifacts/{index}/path" "path escapes repository" ]
                                    | Error values -> errors <- errors @ values
                        | _ ->
                            errors <-
                                errors
                                @ [ finding "RW-ARTIFACTS" "/artifacts" "required non-empty artifact array" ]

                        if List.isEmpty errors then
                            Ok unitValue.GateContracts
                        else
                            Error errors
                with :? JsonException as error ->
                    Error [ finding "RW-MANIFEST-JSON" "/" error.Message ]

    let commandContracts index roadmap unitId =
        inspect index roadmap unitId
    ...
    let validateGateCatalog (expected: RoadmapGateContract list) (catalog: ReadOnlyMemory<byte>) =
        let identityDigest executable arguments =
            sha256 (ReadOnlyMemory<byte>(Encoding.UTF8.GetBytes(String.concat "\u0000" (executable :: arguments))))

        try
            use document = JsonDocument.Parse catalog
            let root = document.RootElement

            match strictObject "" (Set.ofList [ "schema"; "commands" ]) root with
            | Error errors -> Error errors
            | Ok _ ->
                let mutable errors = []

                match requiredString "" "schema" root with
                | Ok "fsgg.coordination.gate-catalog/1" -> ()
                | Ok value ->
                    errors <-
                        errors
                ...
                let parsed =
                    match property "commands" root with
                    | Some commands when commands.ValueKind = JsonValueKind.Array ->
                        commands.EnumerateArray()
                        |> Seq.mapi (fun index command ->
                            let path = $"/commands/{index}"

                            match strictObject path (Set.ofList [ "id"; "qGate"; "executable"; "args" ]) command with
                            | Error values -> Error values
                            | Ok _ ->
                                let id = requiredString path "id" command
                                let qGate = requiredString path "qGate" command
                                let executable = requiredString path "executable" command
                                let arguments = stringList path "args" false command
                                ...
                                | Error values -> Error values
                                | Ok _ ->
                                    let idValue = Result.defaultValue "" id
                                    let qGateValue = Result.defaultValue "" qGate
                                    let executableValue = Result.defaultValue "" executable
                                    let argumentValues = Result.defaultValue [] arguments
                ...
                let commands = parsed |> List.choose Result.toOption

                for id, count in commands |> List.countBy _.Id do
                    if count > 1 then
                        errors <-
                            errors
                            @ [ finding "RW-JSON-DUPLICATE" "/commands" $"duplicate command id: {id}" ]

                let selected =
                    expected
                    |> List.map (fun contract ->
                        match commands |> List.tryFind (fun command -> command.Id = contract.Id) with
                        | None ->
                            errors <-
                                errors
                                @ [ finding
                ...
                if List.isEmpty errors then Ok selected else Error errors
        with :? JsonException as error ->
            Error [ finding "RW-GATE-CATALOG-JSON" "/" error.Message ]
