...
module HarnessMutationProof =
    ...
    let private clone (node: JsonNode) = JsonNode.Parse(node.ToJsonString())

    let private digestNode (root: JsonObject) =
        let unsigned = clone root |> _.AsObject()
    ...
    let private reseal (root: JsonObject) =
        ...
        for name in [ "results"; "reviewers" ] do
            for entry in root[name].AsArray() do
                entry.AsObject()["inputSetSha256"] <- inputDigest
    ...
    let private mutate (gate: string) (kind: string) (baselineBytes: ReadOnlyMemory<byte>) =
        ...
        | "generated-only" ->
            ...
            match gate with
            | "generatedCases" ->
                for other in GateClasses do
                    if other <> gate then
                        root[other] <- JsonArray()
            | "results" ->
                for entry in entries () do
                    entry.AsObject()["producer"] <- producer
            | "reviewers" ->
                for entry in entries () do
                    entry.AsObject()["principal"] <- producer
            | _ ->
                for entry in entries () do
                    entry.AsObject()["producer"] <- producer
    ...
    let private contextErrors context =
        [
            if not (Text.RegularExpressions.Regex.IsMatch(context.CandidateCommit, "^[0-9a-f]{40}$")) then
                yield finding "HMP-CANDIDATE" "/candidateCommit" "expected lowercase 40-hex revision"
            for path, value in
                [
                    "/candidateTreeSha256", context.CandidateTreeSha256
                    "/unitContractSha256", context.UnitContractSha256
                    "/validatorSha256", context.ValidatorSha256
                ] do
                if not (Text.RegularExpressions.Regex.IsMatch(value, "^[0-9a-f]{64}$")) then
                    yield finding "HMP-FINGERPRINT" path "expected lowercase SHA-256"
        ]
            ...
            Error
                [
                    finding
                        "HMP-BASELINE"
                        "/baseline"
                        (String.concat "," (values |> List.map _.Code |> List.distinct |> List.sort))
                ]
        | _ ->
            let root = JsonObject()
            ...
            for gate in GateClasses do
                for kind in MutationKinds do
                    ...
                    if observed.IsEmpty then
                        unexpected <-
                            finding
                                "HMP-UNEXPECTED-GREEN"
                                ($"/observations/%s{gate}/%s{kind}")
                                "production validator accepted mutation"
                            :: unexpected
                    ...
                    if not (observed |> List.contains required) then
                        ...
                        unexpected <-
                            finding
                                "HMP-WRONG-DIAGNOSTIC"
                                ($"/observations/%s{gate}/%s{kind}")
                                ($"expected %s{required}; observed %s{observedText}")
                            :: unexpected
            ...
            Error
                [
                    finding
                        "HMP-PROOF-MISMATCH"
                        "/"
                        ($"expected canonical proof %s{sha256 expected}, observed %s{sha256 (proof.ToArray())}")
                ]
