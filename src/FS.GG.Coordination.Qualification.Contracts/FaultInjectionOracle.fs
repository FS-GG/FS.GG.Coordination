...
let private acceptedSteps root =
    let path =
        Path.Combine(root, "src/FS.GG.Coordination.Protocol/Generated/compiled-outputs/settings-plans.json")
    ...
    try
        ...
        |> Ok
    with error ->
        Error $"FIO-SOURCE: %s{error.Message}"

let private has kind (execution: FaultInjection.Execution) =
    execution.Trace |> List.exists (fun event -> event.Kind = kind)

let private traceSteps kind (execution: FaultInjection.Execution) =
    execution.Trace
...
let validate (root: string) (executions: FaultInjection.Execution list) =
    ...
    | Ok steps ->
        ...
        if steps <> [ "DSPH-Inspect"; "DSPH-Plan"; "DSPH-Apply"; "DSPH-Verify" ] then
            Error "FIO-STEP-AUTHORITY: accepted phase inventory differs"
        elif observedIds <> expectedIds then
            Error "FIO-COVERAGE: execution inventory differs"
        else
            ...
            for step in steps do
                ...
                require
                    (before.Outcome = "converged" && has "fault-before" before && has "retry" before)
                    $"FIO-BEFORE:%s{step}"
                ...
                require
                    (after.Outcome = "converged"
                     && has "response-lost" after
                     && has "retry" after
                     && has "idempotent" after)
                    $"FIO-AFTER:%s{step}"
            ...
            require
                (lost.Outcome = "converged" && has "response-lost" lost && has "idempotent" lost)
                "FIO-LOST-RESPONSE"
            ...
            require
                (duplicate.Outcome = "converged"
                 && has "duplicate-delivered" duplicate
                 && has "duplicate-discarded" duplicate
                 && duplicate.FinalStateSha256 = healthyState)
                "FIO-DUPLICATE"

            let reordered = byId["reordered-events"]

            require
                (reordered.Outcome = "converged"
                 && has "events-reversed" reordered
                 && has "events-reduced-by-ordinal" reordered
                 && reordered.FinalStateSha256 = healthyState)
                "FIO-REORDER"
            ...
            for id, code in
                ...
                require
                    (execution.Outcome = "refused"
                     && execution.RefusalCode = Some code
                     && has "refused" execution)
                    $"FIO-REFUSAL:%s{id}"

            if findings.Count = 0 then
                Ok()
            else
                Error(String.concat ";" findings)
