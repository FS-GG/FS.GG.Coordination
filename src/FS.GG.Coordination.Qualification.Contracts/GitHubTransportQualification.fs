...
module GitHubTransportQualification =
    ...
    let validate
        (generated: GitHubTransportControlResult list)
        (independent: GitHubTransportControlResult list)
        : Result<unit, GitHubTransportFinding list> =
            ...
            [
                for value in results do
                    ...
                    if not value.MutationRed then
                        yield
                            {
                                Code = $"GTQ-{producer.ToUpperInvariant()}-NOT-RED"
                                ControlId = id
                    ...
                    if not value.BaselineGreen then
                        yield
                            {
                                Code = $"GTQ-{producer.ToUpperInvariant()}-BASELINE-NOT-GREEN"
                                ControlId = id
                                Message = $"{producer} unmutated control did not remain green"
                            }
            ]
