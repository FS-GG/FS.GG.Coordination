...
type private Inputs =
    {
        ...
        MutationOutcomes: Set<string>
        ObservationOutcomes: Set<string>
        Permission: string
    }
...
let private stateSha state =
    String.concat
        ...
        [
            ...
            state.Applied |> Set.toList |> String.concat ","
            state.Events
            |> Map.toList
            |> List.map (fun (ordinal, step) -> $"%d{ordinal}:%s{step}")
            |> String.concat ","
            state.Pages |> Set.toList |> List.map string |> String.concat ","
            string state.TerminalPage
        ]
    |> utf8
    |> sha256
...
let private readJson root relative =
    ...
    if not (File.Exists path) then
        Error $"FI-INPUT-MISSING: %s{relative}"
    else
        try
        ...
        with :? JsonException as error ->
            Error $"FI-INPUT-MALFORMED: %s{relative}: %s{error.Message}"
...
let private loadInputs root =
    let settingsPath =
        "src/FS.GG.Coordination.Protocol/Generated/compiled-outputs/settings-plans.json"

    let mutationPath =
        "src/FS.GG.Coordination.Protocol/Generated/compiled-outputs/mutation-census.json"

    let permissionPath =
        "src/FS.GG.Coordination.Protocol/Generated/compiled-outputs/permission-census.json"

    let contractPath = "src/FS.GG.Coordination.Protocol/Generated/contract.json"

    match
        readJson root settingsPath, readJson root mutationPath, readJson root permissionPath, readJson root contractPath
    with
    | Error error, _, _, _
    | _, Error error, _, _
    | _, _, Error error, _
    ...
    | Ok(settingsBytes, settings), Ok(mutationBytes, mutation), Ok(permissionBytes, permission), Ok(_, contract) ->
        let identities value =
            match
                stringProperty "FI-INPUT-IDENTITY" "sourceSha256" value,
                stringProperty "FI-INPUT-IDENTITY" "behavioralSha256" value,
                stringProperty "FI-INPUT-IDENTITY" "contractSha256" value
            with
        ...
        | Ok(source, behavioral, contractSha), Ok(ms, mb, mc), Ok(ps, pb, pc) when
            (source, behavioral, contractSha) = (ms, mb, mc)
            && (source, behavioral, contractSha) = (ps, pb, pc)
            ->
            let specification =
                tryProperty "content" settings
                |> Option.bind (tryProperty "specification")
                |> Option.bind (tryProperty "value")

            let permissions =
                tryProperty "content" permission
                |> Option.bind (tryProperty "requiredPermissions")

            match
                specification |> Option.bind (fieldString "phaseContract"),
                permissions,
                catalogueIds contract "MOUT-",
                catalogueIds contract "OBS-"
            with
            | Some phaseContract, Some permissionRows, Ok mutationOutcomes, Ok observationOutcomes ->
                ...
                if steps <> [ "DSPH-Inspect"; "DSPH-Plan"; "DSPH-Apply"; "DSPH-Verify" ] then
                    ...
                    not (Set.isSubset requiredOutcomes mutationOutcomes)
                    || not (Set.isSubset requiredObservations observationOutcomes)
                then
                    Error "FI-OUTCOME-INVENTORY: accepted typed outcomes are incomplete"
                elif permissionIds.IsEmpty then
                    Error "FI-PERMISSION-INVENTORY: accepted permission census is empty"
                else
                    Ok
                        {
                            SourceSha256 = source
    ...
    if Set.contains step state.Applied then
        ...
        let next =
            { state with
                ...
                Applied = Set.add step state.Applied
                Events = Map.add state.NextEvent step state.Events
...
let private run inputs defect id fault target =
    ...
    let executeStep step =
        ...
        if fault = "partial-page" && step = "DSPH-Inspect" then
            state <-
                { state with
                    Pages = Set.singleton 1
                    TerminalPage = false
                }

            add "page" step

            if defect = SubjectDefect.AcceptPartialPage then
                state <- { state with TerminalPage = true }
            else
                shouldRefuse "OBS-Incomplete"
        elif
            fault = "rate-budget-exhausted"
            && step = "DSPH-Plan"
            && defect <> SubjectDefect.IgnoreRateBudget
        then
            shouldRefuse "MOUT-RateLimited"
        elif
            fault = "permission-revoked"
            && step = "DSPH-Apply"
            && defect <> SubjectDefect.IgnorePermission
        then
            shouldRefuse "OBS-Unauthorized"
        elif
            fault = "concurrent-revision"
            && step = "DSPH-Apply"
            && defect <> SubjectDefect.IgnoreRevision
        then
            state <-
                { state with
                    Revision = state.Revision + 1
            ...
            add "external-revision" step
            shouldRefuse "MOUT-RevisionConflict"
        elif (fault = "before-step" && step = target) then
            add "fault-before" step

            if defect <> SubjectDefect.SkipRetry then
                add "retry" step
                let next, events = applyStep step state trace in
                state <- next
                trace <- events
        elif
            (fault = "after-step" && step = target)
            || (fault = "lost-response" && step = target)
        then
            let next, events = applyStep step state trace in
            state <- next
            trace <- events
            ...
            if defect <> SubjectDefect.SkipRetry then
                add "retry" step
                let replayed, replayEvents = applyStep step state trace in
                state <- replayed
                trace <- replayEvents
        else
            let next, events = applyStep step state trace in
    ...
    if refusal.IsNone && fault = "duplicate-event" then
        ...
        if defect = SubjectDefect.DuplicateIsApplied then
            state <-
                { state with
                    Revision = state.Revision + 1
                    Events = Map.add state.NextEvent firstStep state.Events
                    NextEvent = state.NextEvent + 1
                }
        else
            state <-
                { state with
                    Events =
                        arrivals
                        |> List.fold (fun reduced (identity, step) -> Map.add identity step reduced) Map.empty
                }
    ...
    if refusal.IsNone && fault = "reordered-events" then
        ...
        if defect = SubjectDefect.PreserveArrivalOrder then
            state <-
                { state with
                    Events = reversed |> List.mapi (fun index (_, step) -> index + 1, step) |> Map.ofList
                }
        else
            state <-
                { state with
                    Events = reversed |> List.sortBy fst |> Map.ofList
                }
...

    {
        ...
        Outcome = outcome
        RefusalCode = refusal
        InitialStateSha256 = stateSha initial
        FinalStateSha256 = stateSha state
        Trace = trace
    }
...
let private sourceNode inputs =
    ...
    for name, value in
        [
            ...
            "mutationSha256", inputs.MutationSha256
            "permissionSha256", inputs.PermissionSha256
        ] do
        node[name] <- value
...
let private executionNode item =
    ...
    node["trace"] <-
        JsonArray(
            item.Trace
            |> List.map (fun event -> traceNode event :> JsonNode)
            |> List.toArray
        )
...
let private render inputs executions =
    ...
    root["externalSteps"] <-
        JsonArray(
            inputs.ExternalSteps
            |> List.map (fun step -> JsonValue.Create(step) :> JsonNode)
            |> List.toArray
        )
    ...
    root["executions"] <-
        JsonArray(
            executions
            |> List.map (fun item -> executionNode item :> JsonNode)
            |> List.toArray
        )

    let converged =
        executions
        |> List.filter (fun item -> item.Outcome = "converged")
        |> List.length
...
let validate root (artifactBytes: byte array) =
    ...
    | Ok inputs, Ok executions ->
        try
            ...
            | Ok _ ->
                let canonical =
                    JsonNode.Parse(ReadOnlySpan<byte>(artifactBytes)).ToJsonString() + "\n" |> utf8

                if canonical <> artifactBytes then
                    Error "FI-ARTIFACT-CANONICAL: artifact is not canonical JSON"
                else
                    let expected, self = render inputs executions

                    if expected <> artifactBytes then
                        match
                            tryProperty "source" observed,
                            tryProperty "externalSteps" observed,
                            tryProperty "executions" observed,
                            tryProperty "selfSha256" observed
                        with
                        | Some source, _, _, _ when source.GetRawText() <> (sourceNode inputs).ToJsonString() ->
                            Error "FI-ARTIFACT-SOURCE: accepted protocol identity differs"
                        | _, Some steps, _, _ when steps.GetRawText() <> JsonSerializer.Serialize(inputs.ExternalSteps) ->
                            Error "FI-STEP-INVENTORY: external-step authority differs"
                        | _, _, Some cases, _ when cases.GetArrayLength() <> executions.Length ->
                            Error "FI-SCENARIO-COUNT: executed fault matrix is incomplete"
                        | _, _, _, Some digest when digest.GetString() <> self ->
                            Error "FI-SELF-DIGEST: artifact self digest differs"
                        | _, _, Some cases, _ ->
                            let ids =
                                cases.EnumerateArray()
                                |> Seq.map (stringProperty "FI-EXECUTION-SHAPE" "id")
                                |> Seq.toList

                            if ids <> (executions |> List.map (fun item -> Ok item.Id)) then
                                Error "FI-SCENARIO-ORDER: execution identity or order differs"
                            else
                                Error "FI-EXECUTION-TRACE: executed outcome or retained trace differs"
                        ...
                        Ok
                            {
                                ...
                                RefusedCount = executions.Length - converged
                                SelfSha256 = self
                            }
        with :? JsonException as error ->
            Error $"FI-ARTIFACT-MALFORMED: %s{error.Message}"
...
let check (root: string) (artifactPath: string) =
    ...
    if File.Exists full then
        validate root (File.ReadAllBytes full)
    else
        Error $"FI-ARTIFACT-MISSING: %s{artifactPath}"
