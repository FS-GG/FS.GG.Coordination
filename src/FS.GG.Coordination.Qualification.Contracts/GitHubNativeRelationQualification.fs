...
type GitHubNativeRelationFinding =
    {
        Code: string
        ControlId: string
        Message: string
...
module GitHubNativeRelationQualification =
    ...
    let controlId =
        ...
        | ReversedEndpoint -> "reversed-endpoint"
            ...
            [
                for value in results do
                    ...
                    if not value.MutationRed then
                        yield
                            {
                                Code = $"GNRQ-{producer.ToUpperInvariant()}-NOT-RED"
                                ControlId = id
                    ...
                    if not value.BaselineGreen then
                        yield
                            {
                                Code = $"GNRQ-{producer.ToUpperInvariant()}-BASELINE-NOT-GREEN"
                                ControlId = id
