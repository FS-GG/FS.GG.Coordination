...
let private renderWorkflow (contract: BootstrapContract) =
    ...
    line "    env:"
    line "      NUGET_PACKAGES: /tmp/fsgg-${{ github.run_id }}-nuget-canonical-quint-prepare"
    line "      FSGG_QUINT_TOOLCHAIN_OUTPUT: /tmp/fsgg-${{ github.run_id }}-canonical-quint-toolchain/toolchain.tar.gz"
    line "    steps:"
    line "      - name: Check out the exact candidate"
    line $"        uses: actions/checkout@%s{contract.Actions.Checkout}"
    line "        with:"
    ...
    line "          dotnet-version: 10.0.400"
    line "          global-json-file: global.json"
    line "      - name: Prepare the pinned canonical toolchain"
    line "        run: bash eng/bootstrap-gates/canonical-quint-prepare.sh"
    line "      - name: Upload the pinned canonical toolchain"
    line $"        uses: actions/upload-artifact@%s{contract.Actions.UploadArtifact}"
    line "        with:"
    line "          name: canonical-quint-toolchain"
    line "          path: ${{ env.FSGG_QUINT_TOOLCHAIN_OUTPUT }}"
    line "          if-no-files-found: error"
    line ""
    line "  canonical-quint-semantic:"
    line "    name: canonical-quint-semantic (${{ matrix.shard }})"
    ...
    line "      matrix:"
    line $"        shard: [%s{renderedFormalIds}]"
    line "    env:"
    line "      NUGET_PACKAGES: /tmp/fsgg-${{ github.run_id }}-nuget-canonical-quint-${{ matrix.shard }}"
    line "      FSGG_QUINT_SHARD: ${{ matrix.shard }}"
    ...
    line "          dotnet-version: 10.0.400"
    line "          global-json-file: global.json"
    line "      - name: Download the pinned canonical toolchain"
    ...
    line $"        uses: actions/upload-artifact@%s{contract.Actions.UploadArtifact}"
    line "        with:"
    line "          name: canonical-quint-shard-${{ matrix.shard }}"
    line "          path: ${{ env.FSGG_QUINT_SHARD_ROOT }}/${{ matrix.shard }}.json"
    line "          if-no-files-found: error"
    line ""
    line "  canonical-quint-performance:"
    line "    name: canonical-quint-performance"
    line "    needs: [reuse-decision, canonical-quint-prepare]"
    line "    if: ${{ needs.canonical-quint-prepare.result == 'success' }}"
    line $"    runs-on: %s{contract.Reuse.Runner}"
    ...
    line "          name: canonical-quint-toolchain"
    line "          path: /tmp/fsgg-${{ github.run_id }}-canonical-quint-toolchain"
    line "      - name: Run the independently bounded epoch measurement"
    line "        run: bash eng/bootstrap-gates/canonical-quint-shard.sh"
    line "      - name: Validate the independent epoch measurement"
    line "        run: bash eng/bootstrap-gates/canonical-quint-performance.sh"
    line "      - name: Upload canonical performance receipt"
    line "        if: ${{ always() }}"
    line $"        uses: actions/upload-artifact@%s{contract.Actions.UploadArtifact}"
    line "        with:"
    line "          name: canonical-quint-performance"
    line "          path: ${{ env.FSGG_QUINT_SHARD_ROOT }}"
    line "          if-no-files-found: error"
    line ""
    ...
    line "        with:"
    line "          ref: ${{ github.event.pull_request.head.sha || github.sha }}"
    line "      - name: Download every available semantic shard"
    line $"        uses: actions/download-artifact@%s{contract.Actions.DownloadArtifact}"
    line "        with:"
    line "          pattern: canonical-quint-shard-*"
    line "          path: ${{ env.FSGG_QUINT_SHARD_ROOT }}"
    line "          merge-multiple: true"
    line "      - name: Download canonical performance receipt"
    line $"        uses: actions/download-artifact@%s{contract.Actions.DownloadArtifact}"
    line "        with:"
    line "          name: canonical-quint-performance"
    line "          path: /tmp/fsgg-${{ github.run_id }}-canonical-quint-performance"
    line "      - name: Aggregate the complete canonical receipt"
    line "        run: bash eng/bootstrap-gates/canonical-quint-aggregate.sh"
    line "      - name: Upload parallel accounting evidence"
    line $"        uses: actions/upload-artifact@%s{contract.Actions.UploadArtifact}"
    line "        with:"
    line "          name: canonical-quint-parallel-accounting"
    line "          path: ${{ env.FSGG_QUINT_ACCOUNTING_RECEIPT }}"
    line "          if-no-files-found: error"
    ...
    try
        ...
        let formalRows =
            ...
            |> Seq.map (fun item ->
                ...
                item.GetProperty("manifestSha256").GetString(),
                item.GetProperty("traceSha256").GetString(),
        ...
        [
            ...
            let expectedInputs =
                [
                    "sourceSha256", "cf464a5206981dbf99b5ece74e284046f19b7f0431087fc38d42963541d892bd"
