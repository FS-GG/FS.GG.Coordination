...
let private readBytes root relative =
    ...
    if File.Exists path then
        ...
        Error $"GST-INPUT-MISSING: %s{relative}"
...
let private arrayProperty code name element =
    ...
    | Some value when value.ValueKind = JsonValueKind.Array ->
        Ok(value.EnumerateArray() |> Seq.map _.Clone() |> Seq.toList)
...
let private requireEqual code label expected actual =
    if String.Equals(expected, actual, StringComparison.Ordinal) then
        ...
        Error $"%s{code}: %s{label} expected %s{expected}, observed %s{actual}"
...
let private observedPermissionInventory permissions =
    match
        tryProperty "content" permissions
        |> Option.bind (tryProperty "requiredPermissions")
    with
    | Some values when values.ValueKind = JsonValueKind.Array ->
        ...
        |> traverse (fun value ->
            if value.ValueKind = JsonValueKind.String then
                ...
                Error "GST-PERMISSION-REGISTRATION: permission census contains a non-string")
...
let private expectedSchemaInventory contract =
    ...
    |> Result.bind (
        traverse (fun item ->
            match
                stringProperty "GST-SCHEMA-REGISTRATION" "kind" item,
                tryProperty "value" item |> Option.bind (tryProperty "fields")
            with
...
let private observedSchemaInventory schemas =
    let schemaRow (value: JsonElement) =
        match
            stringProperty "GST-SCHEMA-REGISTRATION" "kind" value,
            arrayProperty "GST-SCHEMA-REGISTRATION" "fields" value
        with
        | Ok kind, Ok fields ->
            fields
            |> traverse (fun field ->
                if field.ValueKind = JsonValueKind.String then
                    Ok(field.GetString())
                else
                    Error "GST-SCHEMA-REGISTRATION: schema field is not a string")
...
let private compiledOutputFamilies contract =
    ...
    | Ok catalogue ->
        match
            catalogue
            |> List.tryFind (fun item -> stringProperty "GST-INPUT-SCHEMA" "id" item = Ok "COUT-Specification")
        with
        ...
        | Some specification ->
            ...
            | Some fields when fields.ValueKind = JsonValueKind.Array ->
                match
                    fields.EnumerateArray()
                    |> Seq.tryFind (fun field -> stringProperty "GST-INPUT-SCHEMA" "name" field = Ok "familyContract")
                with
...
let private jsonRelative family =
    ...
    | "COUT-PermissionCensus" ->
        Some "src/FS.GG.Coordination.Protocol/Generated/compiled-outputs/permission-census.json"
...
let private loadQualified root =
    ...
    | Ok sourceBytes, Ok contractBytes, Ok manifestBytes ->
        match
            parseJson "GST-INPUT-MALFORMED" contractRelative contractBytes,
            parseJson "GST-INPUT-MALFORMED" manifestRelative manifestBytes
        with
        | Error error, _
        | _, Error error -> Error error
        | Ok contract, Ok manifest ->
            match
                stringProperty "GST-INPUT-SCHEMA" "schema" contract,
                ...
                arrayProperty "GST-INPUT-MANIFEST" "outputs" manifest
            with
            | Ok contractSchema, Ok manifestSchema, Ok sourceSha, Ok behavioralSha, Ok contractSha, Ok outputs ->
                match
                    requireEqual "GST-INPUT-SCHEMA" "contract schema" "fsgg.quint.compiled-contract/v2" contractSchema,
                    requireEqual
                        "GST-INPUT-SCHEMA"
                        "manifest schema"
                    ...
                    uniqueBy
                        "GST-SOURCE-DUPLICATE"
                        "compiled-output family"
                        (fun output -> stringProperty "GST-INPUT-MANIFEST" "family" output |> Result.defaultValue "")
                        outputs,
                    compiledOutputFamilies contract
                with
                ...
                | Ok(), Ok(), Ok(), Ok(), Ok uniqueOutputs, Ok expectedFamilies ->
                    let sortedOutputs =
                        uniqueOutputs
                        |> List.sortBy (fun output ->
                            intProperty "GST-INPUT-MANIFEST" "ordinal" output
                            |> Result.defaultValue Int32.MaxValue)

                    let expectedOrdinals = [ 1 .. sortedOutputs.Length ]

                    let observedOrdinals =
                        sortedOutputs
                        |> List.map (intProperty "GST-INPUT-MANIFEST" "ordinal" >> Result.defaultValue -1)
                    ...
                    if observedOrdinals <> expectedOrdinals || observedFamilies <> expectedFamilies then
                        ...
                        let validateOutput output =
                            match
                                stringProperty "GST-INPUT-MANIFEST" "family" output,
                                ...
                                arrayProperty "GST-INPUT-MANIFEST" "files" output
                            with
                            | Ok family,
                              Ok outputSource,
                              Ok outputBehavioral,
                              ...
                              Ok files ->
                                match
                                    requireEqual "GST-INPUT-IDENTITY" $"%s{family} source" sourceSha outputSource,
                                    requireEqual
                                        "GST-INPUT-IDENTITY"
                                        $"%s{family} behavior"
                                        behavioralSha
                                        outputBehavioral,
                                    requireEqual "GST-INPUT-IDENTITY" $"%s{family} contract" contractSha outputContract
                                with
                                | Error error, _, _
                                | _, Error error, _
                                | _, _, Error error -> Error error
                                | Ok(), Ok(), Ok() when not supported || not complete || not fresh ->
                                    Error $"GST-INPUT-STALE: %s{family} is unsupported, incomplete, or stale"
                                | Ok(), Ok(), Ok() ->
                                    match
                                        uniqueBy
                                            "GST-SOURCE-DUPLICATE"
                                            $"%s{family} file"
                                            (fun file ->
                                                stringProperty "GST-INPUT-MANIFEST" "path" file
                                                |> Result.defaultValue "")
                                            files
                                    with
                                    | Error error -> Error error
                                        ...
                                        |> List.fold
                                            (fun state file ->
                                                ...
                                                | Ok() ->
                                                    match
                                                        stringProperty "GST-INPUT-MANIFEST" "path" file,
                                                        stringProperty "GST-INPUT-MANIFEST" "contentSha256" file
                                                    with
                                                    | Ok path, Ok expectedSha ->
                                                        ...
                                                        then
                                                            Error
                                                                $"GST-INPUT-MANIFEST: unsafe compiled-output path %s{path}"
                                                        else
                        ...
                        | Ok() ->
                            let loadOutput family =
                                ...
                                | Some relative ->
                                    ...
                                    | Ok bytes ->
                                        ...
                                        | Ok output ->
                                            match
                                                stringProperty "GST-INPUT-SCHEMA" "schema" output,
                                            ...
                                            | Ok schema,
                                              ...
                                              Ok fresh ->
                                                ...
                                                | Ok(), Ok(), Ok(), Ok(), Ok() when
                                                    not supported || not complete || not fresh
                                                    ->
                                                    Error
                                                        $"GST-INPUT-STALE: %s{family} typed output is unsupported, incomplete, or stale"
                            ...
                            match
                                loadOutput "COUT-CommandMetadata",
                                loadOutput "COUT-MutationCensus",
                                loadOutput "COUT-PermissionCensus",
                                loadOutput "COUT-Schemas",
                                loadOutput "COUT-ProjectionViews"
                            with
                            | Ok commands, Ok mutations, Ok permissions, Ok schemas, Ok projection ->
                                let sameRaw code label (left: JsonElement) (right: JsonElement) =
                                    if
                                        ReadOnlySpan<byte>(canonicalJson left)
                                            .SequenceEqual(ReadOnlySpan<byte>(canonicalJson right))
                                    then
                                        Ok()
                                    else
                                        Error $"%s{code}: %s{label} does not agree with the qualified contract"

                                let registeredMutations =
                                    match tryProperty "content" mutations |> Option.bind (tryProperty "entries") with
                                    | Some value when value.ValueKind = JsonValueKind.Array ->
                                        value.EnumerateArray() |> Seq.map _.Clone() |> Seq.toList
                                    | _ -> []
                                ...
                                let projectionPair contractName projectionName =
                                    match
                                        tryProperty contractName contract,
                                        tryProperty "content" projection |> Option.bind (tryProperty projectionName)
                                    with
                                    | Some left, Some right ->
                                        sameRaw "GST-PROJECTION-REGISTRATION" projectionName left right
                                ...
                                match
                                    tryProperty "actionEffects" contract,
                                    tryProperty "content" commands |> Option.bind (tryProperty "actions"),
                                    uniqueBy
                                        "GST-SOURCE-DUPLICATE"
                                        "registered mutation"
                                        (fun item ->
                                            stringProperty "GST-INPUT-SCHEMA" "id" item |> Result.defaultValue "")
                                        registeredMutations,
                                    uniqueBy
                                        "GST-SOURCE-DUPLICATE"
                                        "contract mutation"
                                    ...
                                    observedSchemas
                                with
                                | Some contractActions,
                                  Some commandActions,
                                  Ok registered,
                                  Ok expectedMutations,
                                  Ok actualPermissions,
                                  Ok expectedSchemaRows,
                                  Ok actualSchemaRows ->
                                    ...
                                    match
                                        sameRaw
                                            "GST-COMMAND-REGISTRATION"
                                            "command actions"
                                            contractActions
                                            commandActions,
                                        (if expectedMutationIds = registeredMutationIds then
                                             Ok()
                                         else
                                             Error
                                                 "GST-MUTATION-REGISTRATION: mutation census differs from the qualified catalogue"),
                                        (if
                                             not expectedPermissions.IsEmpty && expectedPermissions = actualPermissions
                                         then
                                             Ok()
                                         else
                                             Error
                                                 "GST-PERMISSION-REGISTRATION: permission census differs from the qualified source"),
                                        (if not expectedSchemaRows.IsEmpty && expectedSchemaRows = actualSchemaRows then
                                             Ok()
                                         else
                                             Error
                                                 "GST-SCHEMA-REGISTRATION: schema census differs from the qualified contract"),
                                        ...
                                        projectionPair "compatibility" "compatibility"
                                    with
                                ...
                                | None, _, _, _, _, _, _
                                | _, None, _, _, _, _, _ ->
                                    Error "GST-COMMAND-REGISTRATION: missing action registration"
...
let private deriveForGeneration inputs =
    ...
    let outputPath name =
        "src/FS.GG.Coordination.Protocol/Generated/compiled-outputs/" + name

    let getKey property value =
        stringProperty "GST-INPUT-SCHEMA" property value

    match
        arrayProperty "GST-INPUT-SCHEMA" "catalogue" inputs.Contract,
        ...
        arrayProperty "GST-INPUT-MANIFEST" "outputs" inputs.Manifest
    with
    | Ok catalogue, Ok transitions, Ok commands, Ok mutations, Ok permissions, Ok schemas, Ok projections ->
        let build property category artifact derivation values =
            values
            |> List.map (fun value ->
                getKey property value
                |> Result.map (fun key -> makeCase category key artifact key derivation value))
                ...
                if value.ValueKind = JsonValueKind.String then
                    ...
                    Ok(
                        makeCase
                            "permission"
                            key
                            (outputPath "permission-census.json")
                            key
                            "required-permission-entry"
                            value
                    )
                else
                    Error "GST-INPUT-SCHEMA: permission entry is not a string")
            ...
            |> List.fold
                (fun state value ->
                    ...
                    | Ok() ->
                        ...
                        | Error error -> Error error
                        | Ok key ->
                            cases.Add(makeCase category key artifact key derivation value)
                            Ok())
                (Ok())

    let contractArray name element =
        arrayProperty "GST-INPUT-SCHEMA" name element
    ...
    let steps =
        [
            fun () ->
                appendObjects
                    "vocabulary"
                    "src/FS.GG.Coordination.Protocol/Generated/contract.json"
                    "catalogue-entry"
                    (contractArray "catalogue")
                    "id"
                    inputs.Contract
            fun () ->
                appendObjects
                    "transition"
                    "src/FS.GG.Coordination.Protocol/Generated/contract.json"
                    "action-effect-registration"
                    (contractArray "actionEffects")
                    "actionId"
                    inputs.Contract
            fun () ->
                appendObjects
                    "command"
                    (outputPath "command-metadata.json")
                    "command-action-registration"
                    (outputArray "actions")
                    "actionId"
                    inputs.CommandMetadata
            fun () ->
                appendObjects
                    "mutation"
                    (outputPath "mutation-census.json")
                    "mutation-entry-registration"
                    (outputArray "entries")
                    "id"
                    inputs.MutationCensus
            fun () ->
                ...
                | Ok values ->
                    ...
                    |> List.fold
                        (fun state value ->
                            match state with
                            | Error _ -> state
                            | Ok() when value.ValueKind <> JsonValueKind.String ->
                                Error "GST-INPUT-SCHEMA: permission entry is not a string"
                            | Ok() ->
                                let key = value.GetString()
            ...
            fun () ->
                appendObjects
                    "schema"
                    (outputPath "schemas.json")
                    "record-shape-round-trip"
                    (outputArray "recordShapes")
                    "kind"
                    inputs.Schemas
            fun () ->
                appendObjects
...
let private categoryCounts cases =
    categoryOrder
    |> List.map (fun category -> category, cases |> List.filter (_.Category >> (=) category) |> List.length)
...
let private parseArtifact bytes =
    ...
    | Ok root ->
        match
            stringProperty "GST-ARTIFACT-SCHEMA" "schema" root,
            ...
            stringProperty "GST-SELF-DIGEST" "selfSha256" root
        with
...
let private parseCase element =
    match
        stringProperty "GST-CASE-SOURCE" "category" element,
        ...
        stringProperty "GST-CASE-SOURCE" "sourceSha256" element
    with
...
let validate root artifactBytes =
    ...
    | Ok inputs, Ok(schema, source, behavior, contract, manifest, categoryElements, caseElements, total, selfDigest) ->
        match
            requireEqual "GST-ARTIFACT-SCHEMA" "artifact schema" Schema schema,
        ...
        | Ok(), Ok(), Ok(), Ok(), Ok(), Ok expected ->
            ...
            | None ->
                ...
                | Error error -> Error error
                | Ok actual when actual |> List.exists (_.EvidenceClass >> (<>) EvidenceClass) ->
                    Error "GST-EVIDENCE-CLASS: every case must be generated-structural"
                | Ok actual when total <> actual.Length || actual.Length <> expected.Length ->
                    Error $"GST-CASE-COUNT: expected %d{expected.Length}, observed %d{actual.Length}, total %d{total}"
                | Ok actual ->
                    let expectedKeys = expected |> List.map _.Key
                    let actualKeys = actual |> List.map _.Key

                    if Set.ofList expectedKeys <> Set.ofList actualKeys then
                        Error "GST-CASE-SET: live and generated case keys differ"
                    elif expectedKeys <> actualKeys then
                        Error "GST-CASE-ORDER: case keys are not in canonical order"
                    else
                        let mismatch =
                            List.zip expected actual |> List.tryFind (fun (left, right) -> left <> right)

                        match mismatch with
                        | Some(left, _) ->
                            Error $"GST-CASE-SOURCE: generated case differs from live source at %s{left.Key}"
                        | None ->
                            let parsedCategories =
                                categoryElements
                                |> List.map (fun value ->
                                    match
                                        stringProperty "GST-CATEGORY" "id" value,
                                        intProperty "GST-CATEGORY" "count" value
                                    with
                            ...
                            | None ->
                                ...
                                if actualCategories <> expectedCategories then
                                    Error "GST-CATEGORY: category order or counts differ from live sources"
                                else
                                    let canonicalWithoutDigest = serialize inputs expected ""
                                    let expectedSelf = sha256 canonicalWithoutDigest

                                    if not (String.Equals(expectedSelf, selfDigest, StringComparison.Ordinal)) then
                                        Error "GST-SELF-DIGEST: self digest does not match canonical source-bound bytes"
                                    else
                                        ...
                                        if
                                            not (
                                                ReadOnlySpan<byte>(canonical)
                                                    .SequenceEqual(ReadOnlySpan<byte>(artifactBytes))
                                            )
                                        then
                                            Error "GST-CANONICAL: artifact bytes are not canonical"
                                        else
                                            Ok
                                                {
                                                    SourceSha256 = source
...
let check (root: string) (artifactPath: string) =
    let fullPath =
        if Path.IsPathRooted artifactPath then
            ...
            combine root artifactPath

    if not (File.Exists fullPath) then
        Error $"GST-INPUT-MISSING: %s{artifactPath}"
    else
        validate root (File.ReadAllBytes fullPath)
