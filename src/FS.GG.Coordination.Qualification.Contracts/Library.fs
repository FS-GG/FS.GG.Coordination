    ...
    let private readString (root: JsonElement) (segments: string list) =
        let rec loop (current: JsonElement) (remaining: string list) =
            ...
            | segment :: tail when current.ValueKind = JsonValueKind.Object ->
                ...
                if current.TryGetProperty(segment, &child) then
                    loop child tail
                else
                    None
            | _ -> None
    ...
    let validateManifest (manifest: ReadOnlyMemory<byte>) =
        ...
        let digestFindings =
            if digest = expected.ManifestSha256 then
                ...
                [ finding "KERNEL-MANIFEST-DIGEST" "/" expected.ManifestSha256 (Some digest) ]
        ...
        try
            ...
            let expectedFields =
                [
                    ...
                    "KERNEL-PRODUCER",
                    "/qualification/producerMerge",
                    [ "qualification"; "producerMerge" ],
                    expected.ProducerMerge
                    "KERNEL-CONSUMER",
                    "/qualification/consumerMerge",
                    [ "qualification"; "consumerMerge" ],
                    expected.ConsumerMerge
                    "KERNEL-QUINT-VERSION",
                    "/tools/quint/version",
                    [ "tools"; "quint"; "version" ],
                    expected.QuintVersion
                    "KERNEL-QUINT-BINARY",
                    "/tools/quint/binarySha256",
                    [ "tools"; "quint"; "binarySha256" ],
                    expected.QuintBinarySha256
                    "KERNEL-GUIDANCE-SOURCE", "/guidance/source", [ "guidance"; "source" ], expected.GuidanceSource
                    "KERNEL-GUIDANCE-TREE",
                    "/guidance/trackedTreeSha256",
                    [ "guidance"; "trackedTreeSha256" ],
                    expected.GuidanceTreeSha256
                ]
            ...
            let fieldFindings =
                ...
                |> List.choose (fun (code, path, segments, expectedValue) ->
                    ...
                    if actual = Some expectedValue then
                        None
                    else
                        Some(finding code path expectedValue actual))
