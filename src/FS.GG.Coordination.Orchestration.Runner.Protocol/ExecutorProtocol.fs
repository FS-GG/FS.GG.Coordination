...
module ExecutorWire =
    ...
    let parseResponse bytes=
        ...
        |> Result.bind(fun value->
            let validRefs (items:ExecutorReference array)=not(isNull items)&&items.Length<=64&&items|>Array.forall(fun item->validText 64 item.Kind&&validText 2048 item.Reference&&(isNull item.Digest||RunnerWire.validSha256 item.Digest))
            let validUsage=not(isNull value.Usage)&&value.Usage.Length<=64&&value.Usage|>Array.forall(fun item->validText 64 item.Name&&validText 256 item.Provenance&&((item.State="observed"&&item.Value.HasValue&&item.Value.Value>=0L&&validText 64 item.UnitName)||((item.State="unknown"||item.State="not-applicable")&&not item.Value.HasValue&&isNull item.UnitName)))
