...
module RunnerWire =
    ...
    let deserializeClosed<'T> (expected: Set<string>) maximumBytes (bytes: byte array) =
        if bytes.Length = 0 || bytes.Length > maximumBytes then
            Error "runner-message-size-refused"
        else
            try
                use document =
                ...
                if document.RootElement.ValueKind <> JsonValueKind.Object then
                    Error "runner-message-object-required"
                elif not (duplicateFree document.RootElement) then
                    Error "runner-message-shape-refused"
                else
                    ...
                    if names.Length <> expected.Count || Set.ofList names <> expected then
                        ...
                        if isNull (box value) then
                            Error "runner-message-null"
                        else
                            Ok value
            with :? JsonException ->
                Error "runner-message-json-refused"
    ...
    let parsePoll bytes =
        deserializeClosed<RunnerPollRequest> pollProperties 8192 bytes

    let parseAck bytes =
        deserializeClosed<RunnerAckRequest> ackProperties 8192 bytes

    let parseCandidate bytes =
        deserializeClosed<RunnerCandidateRequest> candidateProperties (140 * 1024 * 1024) bytes
    ...
    let parseAssignment bytes =
        ...
        |> Result.bind (fun value ->
            if
                value.Schema = assignmentSchema
                && validSha256 value.AssignmentSha256
                && assignmentDigest value = value.AssignmentSha256
            then
                Ok value
            else
                Error "runner-assignment-response-refused")

    let parseAckReceipt bytes =
        deserializeClosed<RunnerAckReceipt> receiptProperties 8192 bytes
        |> Result.bind (fun value ->
            if
                value.Schema = ackReceiptSchema
                && value.Accepted
                && value.WorkflowRevision >= 0L
            then
                Ok value
            else
                Error "runner-ack-response-refused")

    let parseCandidateReceipt bytes =
        deserializeClosed<RunnerCandidateReceipt> receiptProperties 8192 bytes
        |> Result.bind (fun value ->
            if
                value.Schema = candidateReceiptSchema
                && value.Accepted
                && value.WorkflowRevision >= 0L
            then
                Ok value
            else
                Error "runner-candidate-response-refused")
