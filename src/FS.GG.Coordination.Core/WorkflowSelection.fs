...
module WorkflowSelection =
    ...
    let private order value =
        obligations |> List.findIndex ((=) value)

    let private ordered values =
        values |> List.distinct |> List.sortBy order

    let private obligationList values =
        values |> ordered |> List.map obligationId |> String.concat ","

    let private ruleText rule =
        [ rule.Id; rule.Pattern; ruleMatchId rule.Match; obligationList rule.Roots ]
        |> List.map frame
        |> String.concat "|"

    let private edgeText edge =
        $"{obligationId edge.Source}>{obligationId edge.Target}"

    let computeInventorySeal inventory =
        [
            string inventory.SchemaVersion
            inventory.InventoryVersion
            inventory.GraphVersion
            inventory.BaseRevision
            inventory.SettingsSha256
    ...
    let private graphClosure edges roots =
        let rec loop seen =
            ...
            if Set.isEmpty next then
                ...
                loop (Set.union seen next)

        roots |> Set.ofList |> loop |> Set.toList |> ordered

    let private inventoryFindings inventory =
        [
            if inventory.SchemaVersion <> 1 then
                UnsupportedSchemaVersion inventory.SchemaVersion
            if not inventory.Complete then
                IncompleteInventory
            if not (validText inventory.InventoryVersion) then
                InvalidInventory "inventoryVersion"
            if not (validText inventory.GraphVersion) then
                InvalidInventory "graphVersion"
            if inventory.InventoryVersion <> supportedInventoryVersion then
                UnsupportedInventoryVersion(supportedInventoryVersion, inventory.InventoryVersion)
            if inventory.GraphVersion <> supportedGraphVersion then
                UnsupportedGraphVersion(supportedGraphVersion, inventory.GraphVersion)
            if not (revision inventory.BaseRevision) then
                InvalidInventory "baseRevision"
            if not (sha256Text inventory.SettingsSha256) then
                InvalidInventory "settingsSha256"
            if inventory.PathRules.IsEmpty then
                InvalidInventory "pathRules"
            if inventory.NonFileRules.IsEmpty then
                InvalidInventory "nonFileRules"
            if
                inventory.Aggregates.IsEmpty
                || inventory.Aggregates |> List.exists (validText >> not)
            then
                InvalidInventory "aggregates"
            if inventory.Aggregates.Length <> (inventory.Aggregates |> Set.ofList |> Set.count) then
                InvalidInventory "duplicate aggregate"
            if
                inventory.PathRules @ inventory.NonFileRules
                |> List.exists (fun rule ->
                    not (validText rule.Id) || not (validText rule.Pattern) || rule.Roots.IsEmpty)
            then
                InvalidInventory "impact rule"
            let ruleIds = inventory.PathRules @ inventory.NonFileRules |> List.map _.Id

            if ruleIds.Length <> (ruleIds |> Set.ofList |> Set.count) then
                InvalidInventory "duplicate rule id"

            if inventory.Dependencies |> List.exists (fun edge -> edge.Source = edge.Target) then
                InvalidInventory "self dependency"

            if inventory.Unconditional.IsEmpty then
                InvalidInventory "unconditional obligations"
        ]
        ...
        let preflight =
            [
                ...
                if not request.Complete then
                    IncompleteRequest
                if request.InventoryVersion <> inventory.InventoryVersion then
                    UnsupportedInventoryVersion(inventory.InventoryVersion, request.InventoryVersion)
                if request.GraphVersion <> inventory.GraphVersion then
                    UnsupportedGraphVersion(inventory.GraphVersion, request.GraphVersion)
                if request.ExpectedInventorySeal <> inventory.Seal then
                    InventorySealMismatch(request.ExpectedInventorySeal, inventory.Seal)
                if request.BaseRevision <> inventory.BaseRevision then
                    StaleBaseRevision(inventory.BaseRevision, request.BaseRevision)
                if request.SettingsSha256 <> inventory.SettingsSha256 then
                    StaleSettings(inventory.SettingsSha256, request.SettingsSha256)
                if inventory.Seal <> expectedSeal then
                    InventorySealMismatch(expectedSeal, inventory.Seal)
                match request.MergeGroup with
                | None -> ()
                | Some merge ->
                    if not (revision merge.QueuedHead) then
                        InvalidMergeGroup "queuedHead"

                    if not (revision merge.CurrentQueuedHead) then
                        InvalidMergeGroup "currentQueuedHead"

                    if merge.QueuedHead <> merge.CurrentQueuedHead then
                        InvalidMergeGroup "queued head differs from current queued head"

                    if not merge.Recomputed then
                        InvalidMergeGroup "selection was not recomputed"

                    if not (revision merge.CurrentBaseRevision) then
                        InvalidMergeGroup "currentBaseRevision"

                    if merge.CurrentSettingsSha256 <> request.SettingsSha256 then
                        InvalidMergeGroup "current settings differ from request settings"
            ]

        if not preflight.IsEmpty then
            Error preflight
        else
            let paths = request.ChangedPaths |> List.distinct
            let nonFiles = request.NonFileInputs |> List.distinct

            if paths.IsEmpty && nonFiles.IsEmpty then
                Error [ IncompleteRequest ]
            else
                let pathRoots =
                    classify
                        UnknownChangedPath
                        (fun (value, ids) -> AmbiguousChangedPath(value, ids))
                        inventory.PathRules
                        paths
                ...
                let nonFileRoots =
                    classify
                        UnknownNonFileInput
                        (fun (value, ids) -> AmbiguousNonFileInput(value, ids))
                        inventory.NonFileRules
                        nonFiles
                        ...
                        |> List.map (fun obligation ->
                            if Set.contains obligation closureSet then
                            ...
                            else
                                {
                                    Obligation = obligation
                                    Disposition =
                                        NotApplicable $"{obligationId obligation} is outside the derived closure"
