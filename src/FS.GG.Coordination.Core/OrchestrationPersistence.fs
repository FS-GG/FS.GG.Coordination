...
module OrchestrationPersistence =
    type EffectChange =
        | NoEffect
        | IntentAdded of EffectIntent
        | Settled of OperationId
    ...
    type AppendOutcome =
        ...
        | Conflict
        | WrongExpectedSequence of actual: int64
        | InvalidAppend of reason: string

    type ReadinessFailure =
        | StoreUnavailable of string
        | ReadOnlyStore
        | CapacityUnavailable
        | CorruptRecord of string * int64
        | UnknownEventVersion of string * int64 * int
        | UnknownSerializerVersion of string
        | SnapshotAheadOfJournal of string
        | MigrationInterrupted of string
        | IncompatibleDowngrade of databaseVersion: int * runtimeVersion: int
    ...
    type CandidatePutOutcome =
        | Existing of CandidateStorageReceipt
        | IdentityConflict
        | DigestConflict
        | InvalidArchive
        | CapacityRefused
    ...
    type IJournalStore =
        ...
        abstract Recover:
            persistenceId: string * CancellationToken -> Task<Result<RecoveryResult, ReadinessFailure list>>

        abstract SaveSnapshot: Snapshot * CancellationToken -> Task<Result<unit, ReadinessFailure>>

        abstract SaveProjectionCheckpoint:
            ProjectionCheckpoint * CancellationToken -> Task<Result<unit, ReadinessFailure>>
    ...
    type IBackupReconciler =
        ...
        abstract ReconcileGenerationsRevocationsAndEffects:
            CancellationToken -> Task<Result<unit, ReadinessFailure list>>
