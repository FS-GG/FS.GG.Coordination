...
module WorkTaxonomy =
    ...
    let private normalize (value: string option) =
        value
    ...
    let private parseNative (value: string) =
        ...
        |> List.tryFind (
            nativeIssueTypeName
            >> fun candidate -> String.Equals(candidate, value, StringComparison.OrdinalIgnoreCase)
        )

    let private legacyTarget
        (legacyClass: string option)
        (legacyKind: string option)
        : Result<NativeIssueType, WorkTaxonomyDiagnostic list> =
    ...
    let private preliminaryDiagnostics (observation: WorkTaxonomyObservation) =
        [
            if not observation.Readable then
                WorkTaxonomyDiagnostic.UnreadableObservation
            if not observation.Complete then
                WorkTaxonomyDiagnostic.IncompleteObservation
            if not observation.Current then
                WorkTaxonomyDiagnostic.StaleObservation
            if String.IsNullOrWhiteSpace observation.StableRowId then
                WorkTaxonomyDiagnostic.MissingStableRowId
            if String.IsNullOrWhiteSpace observation.RepositoryScope then
                WorkTaxonomyDiagnostic.MissingRepositoryScope
            if String.IsNullOrWhiteSpace observation.Revision then
                WorkTaxonomyDiagnostic.MissingRevision
            if observation.HierarchyPresent && not observation.HierarchyPreservable then
                WorkTaxonomyDiagnostic.LossyHierarchy
            if not observation.RepositoryScopePreservable then
                WorkTaxonomyDiagnostic.LossyRepositoryScope
        ]

    let private retiredProjections (observation: WorkTaxonomyObservation) =
        [
            if normalize observation.LegacyClass |> Option.isSome then
                "Class"
            if normalize observation.LegacyKind |> Option.isSome then
                "Kind"
        ]

    let classify
        (observation: WorkTaxonomyObservation)
        : Result<WorkTaxonomyClassification, WorkTaxonomyDiagnostic list> =
        let preliminary = preliminaryDiagnostics observation

        if not (List.isEmpty preliminary) then
            Error preliminary
        else
            ...
            if not (List.isEmpty tokenErrors) then
                Error tokenErrors
            else
                    ...
                    Ok
                        {
                            ...
                            RetiredProjections = retiredProjections observation
                        }
    ...
    let private disposition
        (observation: WorkTaxonomyObservation)
        (classification: WorkTaxonomyClassification)
        : WorkTaxonomyDisposition =
        {
            ...
            NoOp =
                (normalize observation.NativeIssueType |> Option.isSome)
                && List.isEmpty classification.RetiredProjections
        }

    let plan
        (observations: WorkTaxonomyObservation list)
        : Result<WorkTaxonomyDisposition list, WorkTaxonomyRefusal list> =
        let duplicateIds =
            observations
        ...
        let evaluated: Choice<WorkTaxonomyDisposition, WorkTaxonomyRefusal> list =
            ...
            |> List.map (fun observation ->
                ...
                | Ok _ ->
                    Choice2Of2(refusal (Some observation.StableRowId) [ WorkTaxonomyDiagnostic.DuplicateStableRowId ])
                    ...
                    Choice2Of2(
                        refusal
                            (if String.IsNullOrWhiteSpace observation.StableRowId then
                                 ...
                                 Some observation.StableRowId)
                            (diagnostics @ [ WorkTaxonomyDiagnostic.DuplicateStableRowId ])
                    )
                | Error diagnostics ->
                    Choice2Of2(
                        refusal
        ...
        if not (List.isEmpty refusals) then
            refusals
            |> List.sortBy (fun refusal ->
                refusal.StableRowId |> Option.defaultValue "", refusal.Diagnostics |> List.map diagnosticCode)
            |> Error
        else
            evaluated
            |> List.choose (function
