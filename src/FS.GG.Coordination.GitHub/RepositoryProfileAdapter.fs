...
module RepositoryProfileAdapter =
    ...
    let private adminText =
        function
        | OrganizationAdministered -> "organization"
        | ExternalObserveOnly -> "external"
    ...
    let private canonicalRow (row: RepositoryRosterRow) =
        [
            ...
            roleText row.Role
            row.Capabilities
            |> List.map (_.Trim().ToLowerInvariant())
            |> List.sort
            |> String.concat ","
    ...
    let private validateRow (row: RepositoryRosterRow) =
        ...
        let unsupported =
            capabilities
            |> List.filter (fun capability -> not (List.contains capability allowedCapabilities))

        let duplicateCapabilities =
            capabilities |> List.countBy id |> List.exists (fun (_, count) -> count > 1)

        let hasKit = List.contains "coordination-kit" capabilities

        [
            if row.Id.Trim() = "" || row.Id.Trim() <> row.Id || owner.IsNone then
                yield InvalidRepositoryIdentity row.FullName
            for capability in unsupported do
                yield UnsupportedRepositoryCapability(row.FullName, capability)
            if duplicateCapabilities then
                yield InvalidRepositoryMetadata row.FullName
            ...
            | NonParticipant, Some _ when
                capabilities.IsEmpty
                && row.Reason |> Option.exists (String.IsNullOrWhiteSpace >> not)
                ->
                ()
    ...
    let private nativeProperties role administration =
        ...
        [
            ...
            {
                Name = "fsgg_owner_scope"
                Value = adminText administration
            }
            {
                Name = "fsgg_coordination_mode"
                Value = mode
            }
        ]
    ...
    let private profileOf (row: RepositoryRosterRow) : RepositoryProfile =
        ...
        let administration =
            if owner.Equals("FS-GG", StringComparison.OrdinalIgnoreCase) then
                OrganizationAdministered
            else
                ExternalObserveOnly

        let capabilities =
            row.Capabilities |> List.map (_.Trim().ToLowerInvariant()) |> List.sort

        let properties =
            if administration = OrganizationAdministered then
                nativeProperties row.Role administration
            else
                []
    ...
    let private profileSeal (snapshot: RepositoryRosterSnapshot) (profiles: RepositoryProfile list) =
        let profileText (profile: RepositoryProfile) =
            [
                ...
                adminText profile.Administration
                String.concat "," profile.Capabilities
                opt profile.KitDelivery
                opt profile.AbsenceCover
                opt profile.Reason
                profile.NativeProperties
                |> List.map (fun property -> $"{property.Name}={property.Value}")
                |> String.concat ","
        ...
        [
            ...
            snapshot.CanonicalRosterSha256
    ...
    let compile (asOf: DateTimeOffset) (maxAge: TimeSpan) (snapshot: RepositoryRosterSnapshot) =
        ...
        let findings =
            [
                if snapshot.SchemaVersion <> 1 then
                    UnsupportedRosterSchema snapshot.SchemaVersion
                if not snapshot.Complete || rows.IsEmpty then
                    IncompleteRoster
                if snapshot.ReviewedAt > asOf || asOf - snapshot.ReviewedAt > maxAge then
                    StaleRoster
                if not (revisionLike snapshot.SourceRevision) then
                    InvalidSourceBinding "sourceRevision"
                if not (digestLike snapshot.SourceArtifactSha256) then
                    InvalidSourceBinding "sourceArtifactSha256"
                if
                    not (digestLike snapshot.CanonicalRosterSha256)
                    || snapshot.CanonicalRosterSha256 <> computedRosterDigest
                then
                    InvalidSourceBinding "canonicalRosterSha256"
                yield!
                    duplicates (fun (row: RepositoryRosterRow) -> row.Id.ToLowerInvariant()) rows DuplicateRepositoryId
                yield!
                    duplicates
                        (fun (row: RepositoryRosterRow) -> row.FullName.ToLowerInvariant())
                        rows
        ...
        if not findings.IsEmpty then
            ...
            let profiles =
                rows
                |> List.map profileOf
                |> List.sortBy (fun (profile: RepositoryProfile) -> profile.FullName.ToLowerInvariant())

            let retentionFindings =
                List.zip
                    (rows
                     |> List.sortBy (fun (row: RepositoryRosterRow) -> row.FullName.ToLowerInvariant()))
                    profiles
                |> List.choose (fun ((row: RepositoryRosterRow), (profile: RepositoryProfile)) ->
                    if
                        ...
                        || row.KitDelivery <> profile.KitDelivery
                        || row.AbsenceCover <> profile.AbsenceCover
                    then
                        Some(LossyRepositoryProfile row.FullName)
                    else
                        None)
            ...
            let propertyFindings =
                ...
                |> List.collect (fun (profile: RepositoryProfile) ->
                    ...
                    |> List.choose (fun property ->
                        if property.Name.Length > 75 || property.Value.Length > 200 then
                            Some(NativePropertyOverflow(profile.FullName, property.Name))
                        else
                            None))
            ...
            let externalFindings =
                ...
                |> List.choose (fun (profile: RepositoryProfile) ->
                    if
                        profile.Administration = ExternalObserveOnly
                        && (profile.PropertyMutationPermitted || not profile.NativeProperties.IsEmpty)
                    then
                        Some(LossyRepositoryProfile profile.FullName)
                    else
                        None)

            let finalFindings = retentionFindings @ propertyFindings @ externalFindings

            if not finalFindings.IsEmpty then
                Error finalFindings
            else
