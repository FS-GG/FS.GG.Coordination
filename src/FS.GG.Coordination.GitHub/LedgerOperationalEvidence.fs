...
type LedgerOperationalDimension =
...
type LedgerOperationalEvidenceContext =
    {
        ...
        InitializationSeal: string option
        MonitorStoreId: string option
        SignerPublicKeySpkiSha256: string
    }

type LedgerOperationalEvidence =
    {
        Dimension: LedgerOperationalDimension
        Context: LedgerOperationalEvidenceContext
        RunId: string
        InputSha256: string
...
module LedgerOperationalEvidence =
    ...
    let private name =
        function
        | SettingsAppliedEvidence -> "settings"
        | AppCustodyEvidence -> "custody"
        | FleetInitializedEvidence -> "initialization"
    ...
    let private valid asOf (expected: LedgerOperationalEvidenceContext) (evidence: LedgerOperationalEvidence) =
        try
            ...
            && evidence.Payload = canonicalPayload evidence
            && evidence.ObservedAt <= asOf
            && asOf < evidence.ExpiresAt
            && evidence.ExpiresAt - evidence.ObservedAt <= TimeSpan.FromMinutes 15.
            && sha (rsa.ExportSubjectPublicKeyInfo()) = evidence.PublicKeySpkiSha256
            && evidence.PublicKeySpkiSha256 = expected.SignerPublicKeySpkiSha256
            && evidence.Authority = "FS-GG/fleet-cutover"
            && not (String.IsNullOrWhiteSpace evidence.RunId)
            && evidence.InputSha256.Length = 64
    ...
    let derive asOf expected (evidence: LedgerOperationalEvidence list) =
        let one dimension =
            ...
            match matches with
            | [ item ] when valid asOf expected item -> Observed true
            | [] -> Unknown "missing-operational-evidence"
            | _ -> Unknown "invalid-or-duplicate-operational-evidence"

        {
            SettingsApplied = one SettingsAppliedEvidence
            AppCustodyReady = one AppCustodyEvidence
            FleetInitialized = one FleetInitializedEvidence
            MonitoringReady = one MonitoringEvidence
        }
