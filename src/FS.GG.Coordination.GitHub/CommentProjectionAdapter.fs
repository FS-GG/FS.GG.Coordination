...
module CommentProjectionAdapter =
    ...
    let private isLowerHex length (value: string) =
        not (isNull value)
        && value.Length = length
    ...
    let private commentKey (comment: CommentObservation) =
        comment.CreatedAt, comment.Identity.DatabaseId

    let private validComment (comment: CommentObservation) =
        not (obj.ReferenceEquals(comment, null))
        && comment.Identity.DatabaseId > 0L
        && validText comment.Identity.NodeId
        && validText comment.AuthorLogin
        && not (isNull comment.Body)
        && comment.UpdatedAt >= comment.CreatedAt

    let readComments (observation: CommentReadObservation) =
        if obj.ReferenceEquals(observation, null) then
            Error InvalidCommentPageChain
        else
            ...
            | CommentsComplete(_, pages) when obj.ReferenceEquals(pages, null) || List.isEmpty pages ->
                Error InvalidCommentPageChain
            | CommentsComplete(revision, pages) ->
                let validChain =
                    ...
                    |> List.mapi (fun index (page: CommentPage) ->
                        ...
                        && not (obj.ReferenceEquals(page.Comments, null))
                        && page.Number = index + 1
                ...
                if not validChain then
                    ...
                    | None ->
                        match
                            duplicateBy (fun (comment: CommentObservation) -> comment.Identity.DatabaseId) comments
                        with
                        | Some comment -> Error(DuplicateCommentDatabaseId comment.Identity.DatabaseId)
                        | None ->
                            match
                                duplicateBy (fun (comment: CommentObservation) -> comment.Identity.NodeId) comments
                            with
                                ...
                                Ok
                                    {
                                        ...
                                        NodeCount = comments.Length
                                        TerminalCursor = pages |> List.last |> _.EndCursor
                                        Comments = comments
                                    }
    ...
    let private validAuthority (authority: JournalAuthority) =
        not (obj.ReferenceEquals(authority, null))
        && validText authority.Subject
        && validText authority.JournalKind
        && validText authority.JournalShard
        && authority.Generation > 0L
        && isLowerHex 40 authority.JournalCommit
        && isLowerHex 64 authority.AuthorityDigest

    let private markerFromAuthority (digest: string) (authority: JournalAuthority) : ProjectionMarker =
        {
            Schema = MarkerSchema
            Subject = authority.Subject
            JournalKind = authority.JournalKind
            JournalShard = authority.JournalShard
            Generation = authority.Generation
            JournalCommit = authority.JournalCommit
    ...
    let private splitProjection (body: string) =
        ...
        if first < 0 || second < 0 || body.Substring(0, first) <> MarkerPrefix then
            None
        else
            Some(body.Substring(first + 1, second - first - 1), body.Substring(second + 1))

    let private requiredNames =
        [
            "schema"
            "subject"
            "journalKind"
        ...
        try
            ...
            | Some(jsonLine, human) ->
                ...
                if
                    ...
                    || (root.EnumerateObject() |> Seq.map _.Name |> Seq.toList) <> requiredNames
                then
                    Error(MarkerMalformed "non-canonical property inventory")
                else
                    let marker: ProjectionMarker =
                        {
                            Schema = root.GetProperty("schema").GetString()
                            Subject = root.GetProperty("subject").GetString()
                            JournalKind = root.GetProperty("journalKind").GetString()
                            JournalShard = root.GetProperty("journalShard").GetString()
                            Generation = root.GetProperty("generation").GetInt64()
                            JournalCommit = root.GetProperty("journalCommit").GetString()
                            AuthorityDigest = root.GetProperty("authorityDigest").GetString()
                            ProjectionDigest = root.GetProperty("projectionDigest").GetString()
                        }

                    if marker.Schema <> MarkerSchema then
                        Error(MarkerUnsupported marker.Schema)
                    elif
                        not (
                            validText marker.Subject
                            && validText marker.JournalKind
                            && validText marker.JournalShard
                            && marker.Generation > 0L
                            && isLowerHex 40 marker.JournalCommit
                            && isLowerHex 64 marker.AuthorityDigest
                            && isLowerHex 64 marker.ProjectionDigest
                        )
                    then
                        Error(MarkerMalformed "invalid marker value")
                    elif canonicalMarker marker <> jsonLine then
                        Error(MarkerMalformed "marker JSON is not canonical")
                    else
                        let humanBytes = utf8.GetBytes human
                        let observed = sha256 humanBytes

                        if observed <> marker.ProjectionDigest then
                            Error(ProjectionDigestMismatch(marker.ProjectionDigest, observed))
                        else
                            Ok
                                {
                                    Marker = marker
                                    HumanBody = humanBytes
                                    FullBodyDigest = sha256 comment.Body
                                }
        with
    ...
    let private markerMatches (authority: JournalAuthority) (marker: ProjectionMarker) =
        marker.Subject = authority.Subject
        && marker.JournalKind = authority.JournalKind
        && marker.JournalShard = authority.JournalShard
        && marker.Generation = authority.Generation
        && marker.JournalCommit = authority.JournalCommit

    let evaluateTrust (expected: ExpectedProjection option) (authority: JournalAuthority) (snapshot: CommentSnapshot) =
        if not (validAuthority authority) then
            Error(
                ProjectionTampered
                    {
                        DatabaseId = 0L
                        NodeId = "invalid-authority"
                    }
            )
        else
            match expected with
            ...
            | Some expected ->
                let candidates =
                    snapshot.Comments
                    |> List.filter (fun (comment: CommentObservation) -> comment.Identity = expected.Identity)

                match candidates with
                | [] -> Error(ProjectionDeleted expected.Identity)
                | _ :: _ :: _ -> Error(ProjectionAmbiguous expected.Identity)
                | [ comment ] when
                    comment.UpdatedAt <> expected.UpdatedAt
                    || sha256 comment.Body <> expected.BodyDigest
                    ->
                    Error(ProjectionEdited expected.Identity)
                | [ comment ] ->
                    match parseProjection comment with
                    | Error failure -> Error(ProjectionMalformed(expected.Identity, failure))
                    | Ok projection when not (markerMatches authority projection.Marker) ->
                        Error(ProjectionTampered expected.Identity)
                    | Ok projection when projection.Marker.AuthorityDigest <> authority.AuthorityDigest ->
                        Error(
                            ProjectionAuthorityDigestMismatch(
                                authority.AuthorityDigest,
                                projection.Marker.AuthorityDigest
                            )
                        )
                    | Ok projection ->
                        Ok
    ...
    let renderProjection (request: ProjectionRequest) =
        if
            obj.ReferenceEquals(request, null)
            || not (validAuthority request.Authority)
            || obj.ReferenceEquals(request.Policy, null)
            || not (validText request.Policy.Version)
            || isNull request.HumanBody
            || not (validText request.CausationIdentity)
        then
            Error(InvalidProjectionRequest "invalid authority, policy, body, or causation")
        else
    ...
    let private planIdentity
        ...
        let expectedParts =
            ...
            | Some value ->
                [
                    string value.Identity.DatabaseId
                    value.Identity.NodeId
                    value.UpdatedAt.ToUniversalTime().ToString("O")
                    value.BodyDigest
                ]
        ...
        [
            ...
            request.Authority.JournalKind
            request.Authority.JournalShard
            string request.Authority.Generation
            request.Authority.JournalCommit
            request.Authority.AuthorityDigest
            request.Policy.Version
            desiredDigest
            request.CausationIdentity
        ]
        @ expectedParts
        |> List.map frame
        |> String.concat ""
                    ...
                    ProjectionPlanned
                        {
                            ...
                            DesiredBody = desired
                            DesiredBodyDigest = digest
                            IdempotencyIdentity = key
                            Operation = CreateProjection desired
                        }
                )
            | Some expected ->
                match evaluateTrust (Some expected) request.Authority snapshot with
                | Ok trusted when trusted.Projection.FullBodyDigest = digest ->
                    Ok(
                        ProjectionNoOp
                            {
                                ObservedRevision = snapshot.Revision
                                Identity = expected.Identity
                                IdempotencyIdentity = planIdentity "no-op" (Some expected) request digest
                            }
                    )
                | Ok _ ->
                    Ok(
                        ProjectionPlanned
                            {
                                ...
                                DesiredBody = desired
                                DesiredBodyDigest = digest
                                IdempotencyIdentity = planIdentity "replace" (Some expected) request digest
                                Operation =
                                    ReplaceProjection(
                                        expected.Identity,
                                        expected.UpdatedAt,
                ...
                | Error(ProjectionMalformed _)
                | Error(ProjectionTampered _)
    ...
    let checkPreState
        ...
        (plan: ProjectionPlan)
        (observation: CommentReadObservation)
        =
        if currentAuthority <> plan.Authority then
            Error DurableAuthorityChanged
        else
            ...
            | Ok observed when observed.Revision <> plan.Before.Revision ->
                Error(ProjectionReReadRequired(plan.Before.Revision, observed.Revision))
    ...
    let verifyPostState
        ...
        (observation: CommentReadObservation)
        =
        if expectedResultRevision = plan.Before.Revision then
            Error(ProjectionResultRevisionDidNotAdvance expectedResultRevision)
        else
            match readComments observation with
            | Error failure -> Error(ProjectionPostStateReadFailed failure)
            | Ok observed when observed.Revision <> expectedResultRevision ->
                Error(ProjectionResultRevisionMismatch(expectedResultRevision, observed.Revision))
            | Ok observed ->
                let expectedUnrelated =
                    match plan.Operation with
                    | CreateProjection _ -> plan.Before.Comments
                    | ReplaceProjection(identity, _, _, _) ->
                        plan.Before.Comments
                        |> List.filter (fun (comment: CommentObservation) -> comment.Identity <> identity)

                let observedUnrelated =
                    observed.Comments
                    |> List.filter (fun (comment: CommentObservation) -> comment.Identity <> resultingComment.Identity)

                if expectedUnrelated <> observedUnrelated then
                    Error UnrelatedCommentChanged
                elif
                    resultingComment.Body <> plan.DesiredBody
                    || sha256 resultingComment.Body <> plan.DesiredBodyDigest
                then
                    Error ProjectionPostStateMismatch
                else
                    match plan.Operation with
                    | CreateProjection _ when
                        plan.Before.Comments
                        |> List.exists (fun (comment: CommentObservation) ->
                            comment.Identity = resultingComment.Identity)
                        ->
                        Error ProjectionPostStateMismatch
                    | ReplaceProjection(identity, updated, _, _) when
                        resultingComment.Identity <> identity || resultingComment.UpdatedAt <= updated
                        ->
                        Error ProjectionPostStateMismatch
                    | _ when
                        observed.Comments
                        |> List.filter (fun (comment: CommentObservation) ->
                            comment.Identity = resultingComment.Identity)
                        |> List.length
                        <> 1
                        ->
