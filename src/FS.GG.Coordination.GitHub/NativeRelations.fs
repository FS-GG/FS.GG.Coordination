...
module NativeRelations =
    ...
    let private edgeKey edge =
        kindText edge.Kind, LiveId.value edge.Source, LiveId.value edge.Target
    ...
    let read observation =
        if obj.ReferenceEquals(observation, null) then
            Error(RelationObservationRefused InvalidCompletenessEvidence)
        else
            match observation with
            | RelationsIncomplete(reason, cursor) ->
                Error(RelationObservationRefused(ObservationIncomplete(reason, cursor)))
            ...
            | RelationsComplete(revision, _, _) when String.IsNullOrWhiteSpace revision ->
                Error(RelationObservationRefused MissingObservationRevision)
            | RelationsComplete(revision, _, _) when revision <> revision.Trim() ->
                Error(RelationObservationRefused InvalidCompletenessEvidence)
            | RelationsComplete(_, _, pages) when obj.ReferenceEquals(pages, null) || List.isEmpty pages ->
                ...
                if not validChain then
                    ...
                    | None ->
                        ...
                        | None ->
                            match
                                edges
                                |> List.groupBy edgeKey
                                |> List.tryPick (fun (_, values) ->
    ...
    let private canonicalPart (value: string) =
        $"{Encoding.UTF8.GetByteCount value}:{value}"

    let private intentEdge =
        function
        | AddEdge edge
    ...
    let plan expectedRevision causationIdentity intent snapshot =
        ...
        then
            Error InvalidRelationExpectedRevision
        elif
            String.IsNullOrWhiteSpace causationIdentity
            || causationIdentity <> causationIdentity.Trim()
        then
            Error InvalidRelationCausationIdentity
        elif obj.ReferenceEquals(intent, null) || obj.ReferenceEquals(snapshot, null) then
            Error InvalidRelationIntent
        else
            let edge = intentEdge intent

            if not (validEdge edge) then
                Error InvalidRelationIntent
            elif edge.Kind <> snapshot.Scope then
                Error(RelationIntentOutsideScope(snapshot.Scope, edge.Kind))
            elif expectedRevision <> snapshot.Revision then
                Error(RelationStaleExpectedRevision snapshot.Revision)
            else
                        ...
                        RelationPlanned
                            {
                                ...
                                IdempotencyIdentity = key
                                Operation = AddEdgeOperation edge
                            }
                    )
                        ...
                        RelationPlanned
                            {
                                ...
                                CausationIdentity = causationIdentity
                                IdempotencyIdentity = key
                                Operation = RemoveEdgeOperation edge
                            }
                    )
    ...
    let checkPreState mutationPlan observation =
        ...
        | Ok observed when observed.Revision <> mutationPlan.Before.Revision ->
            Error(ReReadRequired(mutationPlan.Before.Revision, observed.Revision))
        | Ok observed when
            observed.Scope <> mutationPlan.Before.Scope
            || observed.Edges <> mutationPlan.Before.Edges
            ->
            Error(ConcurrentPreStateChange(mutationPlan.Before.Edges, observed.Edges))
        | Ok observed -> Ok observed

    let verifyPostState expectedResultRevision mutationPlan observation =
        if
            String.IsNullOrWhiteSpace expectedResultRevision
            || expectedResultRevision <> expectedResultRevision.Trim()
        then
            Error InvalidResultRevision
        elif expectedResultRevision = mutationPlan.Before.Revision then
            Error(ResultRevisionDidNotAdvance expectedResultRevision)
        else
            match read observation with
            | Error failure -> Error(PostStateReadRefused failure)
            | Ok observed when observed.Revision <> expectedResultRevision ->
                Error(ResultRevisionMismatch(expectedResultRevision, observed.Revision))
            | Ok observed ->
                let expected =
                    match mutationPlan.Operation with
                    | AddEdgeOperation edge ->
                        edge :: mutationPlan.Before.Edges |> Set.ofList |> Set.toList |> sortEdges
                    | RemoveEdgeOperation edge -> mutationPlan.Before.Edges |> List.filter ((<>) edge) |> sortEdges

                if observed.Scope <> mutationPlan.Before.Scope || observed.Edges <> expected then
                    Error(PostStateMismatch(expected, observed.Edges))
                else
                    Ok observed
