...
module ActionsReleaseFeedAdapter =
    ...
    let surfaceId =
        ...
        | Packages -> "packages"
        | Feeds -> "feeds"
        | ServedDownloads -> "served-downloads"
    ...
    let private digestValid (value: string) =
    ...
    let private evidenceText evidence =
        let attrs =
            ...
            |> Map.toList
            |> List.sortBy fst
            |> List.map (fun (key, value) -> $"{enc key}={enc value}")
            |> String.concat ","

        String.concat
            "|"
            [
                surfaceId evidence.Surface
                enc evidence.Identity
                enc evidence.Repository
                enc evidence.Subject
                evidence.Attempt |> Option.map string |> Option.defaultValue "-"
                evidence.Lifecycle |> Option.map lifecycleId |> Option.defaultValue "-"
                stateId evidence.State
                attrs
                evidence.Digest |> Option.defaultValue "-"
            ]

    let private observationText =
        function
        | Supported(revision, complete, pages, evidence) ->
            $"supported|{enc revision}|{complete}|{pages}|"
            + (evidence
               |> List.sortBy (fun value -> value.Identity, value.Attempt)
               |> List.map evidenceText
               |> String.concat ";")
    ...
    let private validateEvidence repository surface evidence =
        if evidence.Surface <> surface || evidence.Repository <> repository then
            Error(IdentityDrift(surface, evidence.Identity))
        elif
            [ evidence.Identity; evidence.Repository; evidence.Subject ]
            |> List.exists (validText >> not)
        then
            Error(IdentityDrift(surface, evidence.Identity))
        elif evidence.Attributes |> Map.exists (fun key _ -> forbiddenKey key) then
            Error SecretMaterialForbidden
        elif evidence.Digest |> Option.exists (digestValid >> not) then
            Error(InvalidDigest(surface, evidence.Identity))
        elif (surface = ActionsRuns || surface = Checks) && evidence.Lifecycle.IsNone then
            Error(InvalidLifecycle(surface, evidence.Identity))
        elif surface = ActionsRuns && (evidence.Attempt |> Option.defaultValue 0) < 1 then
            Error(InvalidLifecycle(surface, evidence.Identity))
        elif
            surface = Attestations
            && not (
                evidence.Attributes.ContainsKey "predicate"
                && evidence.Attributes.ContainsKey "subject-digest"
                && evidence.Digest.IsSome
            )
        then
            Error(InvalidAttestation evidence.Identity)
        elif
            (surface = Packages || surface = Feeds)
            && not (
                [ "owner"; "name"; "version"; "feed" ]
                |> List.forall evidence.Attributes.ContainsKey
            )
        then
            Error(InvalidPackageCoordinates evidence.Identity)
        else
            Ok()
    ...
    let validate observation =
        if
            [
                ...
                observation.RepositoryNodeId
                observation.CapturedRevision
            ]
            |> List.exists (validText >> not)
        then
            ...
            let folder state surface =
                ...
                |> Result.bind (fun () ->
                    ...
                    | Some(Supported(revision, true, pages, _)) when
                        revision <> observation.CapturedRevision || pages < 1
                        ->
                        Error(PartialSurface(surface, "revision or page identity drift"))
                    | Some(Supported(_, true, _, evidence)) ->
                        match
                            evidence
                            |> List.groupBy (fun value -> value.Identity, value.Attempt)
                        ...
                        | None ->
                            ...
                            |> List.fold
                                (fun result value ->
                                    result
                                    |> Result.bind (fun () -> validateEvidence observation.Repository surface value))
                                (Ok())
                    ...
                    | Some(StaleSurface(expected, actual)) ->
                        Error(PartialSurface(surface, $"stale {expected} {actual}")))
    ...
    let observeServedContent requestUri redirects finalUri status contentType retrieval (bytes: byte array) =
        ...
        if
            not (absolute requestUri)
            || not (absolute finalUri)
            || redirects |> List.exists (absolute >> not)
        then
            Error(InvalidRedirect finalUri)
        elif status < 200 || status > 299 || not (validText contentType) then
            Error(InvalidRedirect finalUri)
        else
            Ok
                {
    ...
    let validateStages stages =
        let rank =
            ...
            | UploadAccepted _ -> 0
            | DurableMetadata _ -> 1
            | AuthenticatedRetrieval _ -> 2
            | PublicServedBytes _ -> 3

        let ranks = stages |> List.map rank

        if ranks <> List.sort ranks || (ranks |> List.distinct).Length <> ranks.Length then
            Error(InvalidRedirect "evidence stage order")
        elif
            stages
            |> List.exists (function
                | DurableMetadata(_, digest) -> not (digestValid digest)
                | AuthenticatedRetrieval content
                | PublicServedBytes content -> not (digestValid content.Sha256)
                | _ -> false)
        then
            Error(InvalidDigest(ServedDownloads, "evidence-stage"))
        elif
            stages
            |> List.exists (function
                | PublicServedBytes content -> content.Retrieval <> AnonymousPublic
                | AuthenticatedRetrieval content -> content.Retrieval = AnonymousPublic
                | _ -> false)
        then
            Error(InvalidRedirect "retrieval class")
        else
            Ok stages
