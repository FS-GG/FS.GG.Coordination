...
type RulesetPlanProfileClass =
    | AuthorityPlan
    | FrameworkPlan
    | HostedNonParticipantPlan
    | ObserveOnlyPlan

type RulesetBypassActorKind =
    | TeamActor
    | IntegrationActor

type RulesetMergeMethod =
    | Squash
    | MergeCommit
    | Rebase
...
module RulesetPlanAdapter =
    ...
    let private approvedActorText (actor: ApprovedRulesetBypassPrincipal) =
        ...
        let profiles =
            actor.AllowedProfiles |> List.map classText |> List.sort |> String.concat ","
    ...
    let private checkText (value: RulesetRequiredCheck) =
        [
            value.Context
            value.IntegrationId |> Option.map string |> Option.defaultValue ""
        ]
        |> List.map frame
        |> String.concat ""
    ...
    let private seal
        ...
        [
            ...
            snapshot.ApprovedBypass
            |> List.sortBy (fun value -> value.Kind, value.ActorId)
            |> List.map approvedActorText
            |> String.concat ","
    ...
    let private duplicates projection values =
        ...
        |> List.choose (fun (key, rows) -> if rows.Length > 1 then Some key else None)
    ...
    let compile (asOf: DateTimeOffset) (maxAge: TimeSpan) (snapshot: RulesetPlanSnapshot) =
        ...
        let selectedProfile =
            ...
            |> Option.bind (fun report ->
                report.Profiles
                |> List.tryFind (fun value -> value.FullName = snapshot.Repository))

        let selectedClass =
            selectedProfile
            |> Option.map profileClass
            |> Option.defaultValue ObserveOnlyPlan

        let mutationPermitted = selectedClass <> ObserveOnlyPlan

        let censusReport =
            match snapshot.CensusSnapshot, snapshot.ExpectedCensusSeal with
            | Some censusSnapshot, Some expected ->
                RequiredCheckCensusAdapter.verify expected asOf maxAge censusSnapshot
                |> Result.toOption
            | _ -> None
        ...
        let findings =
            [
                if snapshot.SchemaVersion <> 1 then
                    yield UnsupportedRulesetPlanSchema snapshot.SchemaVersion
                if not (repositoryLike snapshot.Repository) then
                    yield InvalidRulesetPlanRepository snapshot.Repository
                if not snapshot.Complete then
                    yield IncompleteRulesetPlanObservation
                if snapshot.ObservedAt > asOf || asOf - snapshot.ObservedAt > maxAge then
                    yield StaleRulesetPlanObservation
                if not (digestLike snapshot.PrerequisiteReceiptDigest) then
                    yield InvalidRulesetPlanBinding "prerequisiteReceiptDigest"
                if not (digestLike snapshot.ExpectedProfileSeal) then
                    yield InvalidRulesetPlanBinding "profileReportSeal"
                if profileReport.IsNone then
                    yield InvalidRulesetPlanBinding "profileReport"
                if not (revisionLike snapshot.CurrentPolicyRevision) then
                    yield InvalidRulesetPlanBinding "currentPolicyRevision"
                if not (digestLike snapshot.CurrentPolicyEvidenceSha256) then
                    yield InvalidRulesetPlanBinding "currentPolicyEvidenceSha256"
                if snapshot.CurrentPolicyRepository <> snapshot.Repository then
                    yield InvalidRulesetPlanBinding "currentPolicyRepository"
                if not snapshot.CurrentPolicyComplete then
                    yield InvalidRulesetPlanBinding "currentPolicyComplete"
                if
                    snapshot.CurrentPolicyObservedAt > asOf
                    || asOf - snapshot.CurrentPolicyObservedAt > maxAge
                then
                    yield InvalidRulesetPlanBinding "currentPolicyObservedAt"
                match selectedProfile with
                | None -> yield CrossRepositoryRulesetProfile snapshot.Repository
                | Some profile when profile.PropertyMutationPermitted <> mutationPermitted ->
                    yield InconsistentRulesetProfileAdministration profile.FullName
                | Some _ -> ()
                ...
                | _, Some _, Some expected, Some census ->
                    if not (digestLike expected) then
                        yield InvalidRulesetPlanBinding "censusSeal"

                    if census.Repository <> snapshot.Repository then
                        yield CrossRepositoryRequiredCheckCensus census.Repository

                    if census.ProfileSeal <> snapshot.ExpectedProfileSeal then
                        yield AlteredRulesetProfileBinding
                | _, _, _, _ -> yield InvalidRulesetPlanBinding "censusReport"
                for principal in snapshot.ApprovedBypass do
                    if
                        principal.ActorId <= 0L
                        || principal.AllowedProfiles.IsEmpty
                        || principal.AllowedProfiles |> List.distinct |> List.length
                           <> principal.AllowedProfiles.Length
                    then
                        yield InvalidRulesetBypassPrincipal principal.ActorId
                for _, actorId in snapshot.ApprovedBypass |> duplicates approvedKey do
                    yield DuplicateRulesetBypassPrincipal actorId
                for principal in snapshot.RequestedBypass do
                    if principal.ActorId <= 0L then
                        yield InvalidRulesetBypassPrincipal principal.ActorId
                for _, actorId in snapshot.RequestedBypass |> duplicates requestedKey do
                    yield DuplicateRulesetBypassPrincipal actorId
                let approved =
                    snapshot.ApprovedBypass
                    |> List.map (fun value -> approvedKey value, value)
                    |> Map.ofList
                ...
                for id in snapshot.Exceptions |> duplicates _.Id do
                    yield DuplicateRulesetException id

                for scope in snapshot.Exceptions |> duplicates exceptionScopeKey do
                    yield InvalidRulesetException scope

                for value in snapshot.Exceptions do
                    if
                        not (validText value.Id)
                        || not (validText value.Owner)
                        || not (validText value.Rationale)
                    then
                        yield InvalidRulesetException value.Id
                    ...
                    if
                        value.ApprovedAt > value.StartsAt
                        || value.StartsAt > asOf
                        || value.ExpiresAt <= asOf
                    then
                        yield InactiveRulesetException value.Id

                    if
                        exceptionWindow value <= TimeSpan.Zero
                        || exceptionWindow value > TimeSpan.FromDays 30
                    then
                        yield OverlongRulesetException value.Id
                ...
                if
                    selectedClass = ObserveOnlyPlan
                    && (not snapshot.RequestedBypass.IsEmpty || not snapshot.Exceptions.IsEmpty)
                then
            ...
            let bypass =
                ...
                @ (exceptions
                   |> List.choose (fun value ->
                       match value.Scope with
                       | BypassPrincipal principal -> Some principal
                       | _ -> None))
            ...
            let defaultBranch, releaseTags, repositoryPolicy =
                ...
                | _, Some census ->
                    ...
                    let mergeQueueEnabled =
                        census.Aggregate.PullRequestReady && census.Aggregate.MergeGroupReady

                    let disabledReason =
                        if mergeQueueEnabled then
                            None
                        else
                            Some "required-check-census-not-merge-group-ready"

                    let requiredApprovals =
                        exceptions
                        |> List.tryPick (fun value ->
                            match value.Scope with
                            | RequiredReviewCount count -> Some count
                            | _ -> None)
                    ...
                    let conversationResolution =
                        ...
                        |> List.tryPick (fun value ->
                            match value.Scope with
                            | ConversationResolution required -> Some required
                            | _ -> None)
                        |> Option.defaultValue true

                    let mergeQueueEnabled =
                        if exceptions |> List.exists (fun value -> value.Scope = MergeQueue false) then
                            false
                        else
                            mergeQueueEnabled

                    let disabledReason =
                        if mergeQueueEnabled then None
                        elif disabledReason.IsSome then disabledReason
                        else Some "bounded-exception-disabled"
