...
type LedgerInitializationInput =
    {
        ...
        AuthorizationKeySpkiSha256: string
        AuthorizationWorkflowRevision: string
        AuthorizationWorkflowSha256: string
        CutoverAppId: int64
        CutoverInstallationId: int64
        ControlIssueNumber: int64
        ExpectedRef: ExpectedLedgerRef
...
type LedgerRefRead =
    | RefAbsent
    | RefAt of string
    | RefUnknown of string

type LedgerInitializationPort =
    {
        ReadRef: string -> LedgerRefRead
        PutObject: LedgerObject -> Result<unit, string>
        CreateRef: string -> string -> ExpectedLedgerRef -> Result<unit, string>
    }

type LedgerInitializationOutcome =
    | Initialized
    | AlreadyInitialized
    | InitializationRefused of string list
    | InitializationIndeterminate of string

type LedgerInitializationReadback =
    {
        Ref: LedgerRefRead
        Tag: LedgerRefRead
        Objects: LedgerObject list
    }
...
module LedgerInitializationAdapter =
    ...
    let canonicalInput input =
        json
            [
                ...
                "authorizationKeyId", JsonValue.Create input.AuthorizationKeyId
                "authorizationKeySpkiSha256", JsonValue.Create input.AuthorizationKeySpkiSha256
                ...
                "expectedRef", JsonValue.Create(lease input.ExpectedRef)
                "firstCaptureSha256", JsonValue.Create input.FirstCaptureSha256
                "fleetId", JsonValue.Create input.FleetId
                "manifestSha256", JsonValue.Create input.ManifestSha256
                "ref", JsonValue.Create input.Ref
                "repository", JsonValue.Create input.Repository
                "repositoryId", JsonValue.Create input.RepositoryId
                "secondCaptureSha256", JsonValue.Create input.SecondCaptureSha256
                "sourceSha256", JsonValue.Create input.SourceSha256
    ...
    let private authorityValid asOf (authority: LedgerInitializationAuthority) expected =
        try
            ...
            && sha256 (rsa.ExportSubjectPublicKeyInfo()) = authority.PublicKeySpkiSha256
            && authority.Payload = expected
            && authority.AuthorizedAt <= asOf
            && asOf < authority.ExpiresAt
            && authority.ExpiresAt - authority.AuthorizedAt <= TimeSpan.FromHours 2.
    ...
    let plan asOf authority input =
        ...
        let errors =
            [
                if
                    input.RepositoryId <> 1351660651L
                    || input.Repository <> "FS-GG/FS.GG.Coordination.Authority"
                then
                    "authority-repository"
                if input.FleetId <> "fs-gg-production" then
                    "fleet"
                match address with
                | Ok a when a.Ref = input.Ref -> ()
                | _ -> "ref"
                if
                    not (
                        input.Tag.StartsWith("refs/tags/fsgg/v2/fleet-cutover/operating-v1/", StringComparison.Ordinal)
                    )
                then
                    "tag"
                for value in
                    [
                        ...
                        input.TrustAnchorSha256
                        input.SourceSha256
                        input.DesiredPolicySha256
                        input.FirstCaptureSha256
                        input.SecondCaptureSha256
                    ] do
                    if not (digestLike value) then
                        "digest"
                if
                    input.CutoverAppId <= 0L
                    || input.CutoverInstallationId <= 0L
                    || input.ControlIssueNumber <> 2L
                then
                    "binding"
                if
                    authority.KeyId <> input.AuthorizationKeyId
                    || authority.PublicKeySpkiSha256 <> input.AuthorizationKeySpkiSha256
                    || not (digestLike input.AuthorizationKeySpkiSha256)
                then
                    "authorization-key"
                if
                    input.AuthorizationWorkflowRevision.Length <> 40
                    || not (
                        input.AuthorizationWorkflowRevision
                        |> Seq.forall (fun c -> c >= '0' && c <= '9' || c >= 'a' && c <= 'f')
                    )
                    || not (digestLike input.AuthorizationWorkflowSha256)
                then
                    "authorization-workflow"
                match input.ExpectedRef with
                | ExpectedAbsent -> ()
                | ExpectedParent _ -> "genesis-must-expect-absence"
                if not (authorityValid asOf authority expected) then
                    "authorization"
            ]
            |> List.distinct
        ...
        if not errors.IsEmpty then
            ...
            let eventBytes =
                json
                    [
                        "fleetId", JsonValue.Create input.FleetId
                        "manifestSha256", JsonValue.Create input.ManifestSha256
                        "phase", JsonValue.Create "OperatingV1"
                        "schema", JsonValue.Create "fsgg.github-substrate.epoch-event/1"
                        "trustAnchorSha256", JsonValue.Create input.TrustAnchorSha256
                    ]
            ...
            let address =
                match address with
                | Ok value -> value
                | Error failure -> invalidOp (string failure)
            ...
            let tz =
                input.CreatedAt.ToString("zzz", CultureInfo.InvariantCulture).Replace(":", "")
            ...
            let order =
                [
                    ...
                    "put-commit"
                    "reread-objects"
                    "create-ref-expected-absent"
                    "reread-ref"
                    "create-tag-expected-absent"
                    "reread-tag"
                ]
    ...
    let verify (plan: LedgerInitializationPlan) (readback: LedgerInitializationReadback) =
        ...
        match readback.Ref with
        | RefAt oid when oid = plan.Commit.Oid -> ()
        | RefAt _ -> errors.Add "ref-conflict"
        | RefAbsent -> errors.Add "ref-absent"
        | RefUnknown why -> errors.Add("ref-unknown:" + why)

        match readback.Tag with
        | RefAt oid when oid = plan.Commit.Oid -> ()
        | RefAt _ -> errors.Add "tag-conflict"
        | RefAbsent -> errors.Add "tag-absent"
        | RefUnknown why -> errors.Add("tag-unknown:" + why)

        for expected in [ plan.Event; plan.Head; plan.Tree; plan.Commit ] do
            match
                readback.Objects
                |> List.tryFind (fun value -> value.Kind = expected.Kind && value.Oid = expected.Oid)
            with
    ...
    let apply (port: LedgerInitializationPort) (plan: LedgerInitializationPlan) =
        ...
        | RefUnknown why, _
        | _, RefUnknown why -> InitializationIndeterminate why
        | RefAt _, _
        | _, RefAt _ -> InitializationRefused [ "expected-ref-lease-conflict" ]
        | RefAbsent, RefAbsent ->
            match
                [ plan.Event; plan.Head; plan.Tree; plan.Commit ]
                |> List.tryPick (fun o ->
                    match port.PutObject o with
                    | Error why -> Some why
                    | Ok() -> None)
            with
    ...
    let observationEnvelope (plan: LedgerInitializationPlan) =
        json
            [
                "commit", JsonValue.Create plan.Commit.Oid
                "complete", JsonValue.Create true
                "fleetId", JsonValue.Create "fs-gg-production"
                "generation", JsonValue.Create 1L
                "genesisCommit", JsonValue.Create plan.Commit.Oid
                "parent", null
                "ref", JsonValue.Create plan.Ref
                "schema", JsonValue.Create "fsgg.github-substrate.epoch-wire/1"
                "tag", JsonValue.Create plan.Tag
            ]
