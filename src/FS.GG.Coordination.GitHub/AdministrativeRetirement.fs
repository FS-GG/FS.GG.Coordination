...
type RetirementIdentity =
    { Repository: string; RepositoryId: int64; IssueNumber: int; PullRequestNumber: int; BranchRef: string; ProtectedBaseRef: string
      CandidateHead: string; CandidateTree: string; CandidateParent: string
      RetirementHead: string; RetirementTree: string; RetirementParent: string; AcceptedClientCommit: string
...
type RetirementObservation =
    { ObservedAt: DateTimeOffset; CompleteNativeCensus: bool; PullRequestDisposition: RetirementPullRequestDisposition
      ...
      ObservedBranchHead: string option; RetirementCommitObserved: bool
      ...
      BranchFenceDigest: string option; BranchFenceRef: string option; BranchFenceRules: Set<string>
      BranchFenceActive: bool; BranchFenceHasBypass: bool
      TemporaryMainRuleId: int64 option; TemporaryMainRuleDigest: string option; TemporaryMainRuleRef: string option
      TemporaryMainRuleRules: Set<string>; TemporaryMainRuleActive: bool
...
type AdministrativeRetirementReceipt =
    { Schema: string; IdentityDigest: string; NativeCensusDigest: string; ObservationDigest: string; Disposition: RetirementPullRequestDisposition
      CandidateHead: string; CandidateArchiveDigest: string; RetirementHead: string; FrozenBranchHead: string option
      BranchFenceRuleId: int64; SubjectExcluded: bool
...
type AdministrativeRetirementFailure =
    ...
    | PullRequestMutationStillEnabled | BranchFenceMissing | BranchFenceScopeMismatch | BranchFenceHasBypass | SubjectExclusionMissing
    | RetirementHeadFenceMissing
    | IssueDispositionMissing | TemporaryMainHoldStillActive | TemporaryMainHoldScopeMismatch | FabricatedOriginalHistory | ReceiptDigestMismatch
...
module AdministrativeRetirement =
    ...
    let identityDigest value =
        [ value.Repository; string value.RepositoryId; string value.IssueNumber; string value.PullRequestNumber; value.BranchRef; value.ProtectedBaseRef
          value.CandidateHead; value.CandidateTree; value.CandidateParent; value.RetirementHead; value.RetirementTree; value.RetirementParent
          value.AcceptedClientCommit; value.AcceptedClientArtifactDigest; value.OperationAuthorityDigest ] |> join |> sha256
    ...
    let observationDigest identity value =
        [ identityDigest identity; value.ObservedAt.ToUniversalTime().ToString("O"); string value.CompleteNativeCensus
          ...
          option value.ObservedBranchHead; string value.RetirementCommitObserved
          ...
          option value.BranchFenceRef; value.BranchFenceRules |> Set.toList |> List.sort |> String.concat ","; string value.BranchFenceHasBypass
          value.TemporaryMainRuleId |> Option.map string |> option; option value.TemporaryMainRuleDigest
          option value.TemporaryMainRuleRef; value.TemporaryMainRuleRules |> Set.toList |> List.sort |> String.concat ","
          string value.TemporaryMainRuleActive; string value.SubjectExcluded
    ...
    let receiptDigest value =
        [ value.Schema; value.IdentityDigest; value.NativeCensusDigest; value.ObservationDigest; disposition value.Disposition; value.CandidateHead
          value.CandidateArchiveDigest; value.RetirementHead; option value.FrozenBranchHead; string value.BranchFenceRuleId; string value.SubjectExcluded
    ...
    let private identityFailures value =
        [ if String.IsNullOrWhiteSpace value.Repository || not(value.Repository.Contains('/')) then InvalidIdentity "repository"
          ...
          if String.IsNullOrWhiteSpace value.ProtectedBaseRef || not(value.ProtectedBaseRef.StartsWith("refs/heads/",StringComparison.Ordinal)) || value.ProtectedBaseRef=value.BranchRef then InvalidIdentity "protectedBaseRef"
          ...
          if not(oidLike value.RetirementHead) || value.RetirementHead=value.CandidateHead then InvalidIdentity "retirementHead"
          if value.RetirementTree<>value.CandidateTree then InvalidIdentity "retirementTree"
          if value.RetirementParent<>value.CandidateHead then InvalidIdentity "retirementParent"
    ...
    let validatePreflight now maxAge identity observation =
        let findings =
            [ yield! identityFailures identity
              ...
              if observation.PullRequestHead <> identity.CandidateHead && observation.PullRequestHead <> identity.RetirementHead then CandidateIdentityMismatch
              if observation.ObservedBranchHead |> Option.exists(fun head -> head<>identity.CandidateHead && head<>identity.RetirementHead) then RetirementHeadFenceMissing
              if observation.ObservedBranchHead=Some identity.RetirementHead && not observation.RetirementCommitObserved then RetirementHeadFenceMissing
              ...
              | MergedExactBeforeRetirement when observation.MergeCommit.IsNone || observation.MergedCandidateHead<>Some identity.CandidateHead || observation.MergedCandidateTree<>Some identity.CandidateTree || observation.MergedCandidateParent<>Some identity.CandidateParent || observation.ProtectedBaseRef<>Some identity.ProtectedBaseRef || observation.DeliveredPathDigest |> Option.exists (digestLike >> not) || observation.DeliveredPathDigest.IsNone -> UnexpectedMergeOutcome
              | ClosedUnmerged when observation.MergeCommit.IsSome -> UnexpectedMergeOutcome
              | _ -> ()
              if observation.BranchFenceRuleId.IsSome && (observation.BranchFenceRef<>Some identity.BranchRef || observation.BranchFenceRules<>Set ["creation";"deletion";"update:no-fetch-and-merge"]) then BranchFenceScopeMismatch
              if observation.TemporaryMainRuleId.IsSome && (observation.TemporaryMainRuleRef<>Some identity.ProtectedBaseRef || observation.TemporaryMainRuleRules<>Set ["update:no-fetch-and-merge"]) then TemporaryMainHoldScopeMismatch
              if observation.BranchFenceDigest |> Option.exists (digestLike >> not) then BranchFenceMissing
              if observation.TemporaryMainRuleDigest |> Option.exists (digestLike >> not) then TemporaryMainHoldScopeMismatch
    ...
    let validatePreview now maxAge identity observation =
        let findings =
            [ match validatePreflight now maxAge identity observation with Error values -> yield! values | Ok _ -> ()
              if observation.PullRequestDisposition=Open then PullRequestOutcomeUnresolved
              match observation.PullRequestDisposition with
              | ClosedUnmerged when observation.PullRequestHead<>identity.RetirementHead || observation.ObservedBranchHead<>Some identity.RetirementHead || not observation.RetirementCommitObserved -> RetirementHeadFenceMissing
              | MergedExactBeforeRetirement when observation.PullRequestHead<>identity.CandidateHead -> RetirementHeadFenceMissing
              | MergedExactBeforeRetirement when observation.ObservedBranchHead=Some identity.RetirementHead && not observation.RetirementCommitObserved -> RetirementHeadFenceMissing
              | _ -> ()
              if observation.AutoMergeEnabled || observation.MergeQueueEntry.IsSome then PullRequestMutationStillEnabled
              if not observation.BranchFenceActive || observation.BranchFenceRuleId |> Option.exists ((>=) 0L) || observation.BranchFenceRuleId.IsNone || observation.BranchFenceDigest.IsNone then BranchFenceMissing
              ...
              if observation.TemporaryMainRuleActive || observation.TemporaryMainRuleId.IsSome || observation.TemporaryMainRuleDigest.IsSome || observation.TemporaryMainRuleRef.IsSome || not observation.TemporaryMainRuleRules.IsEmpty then TemporaryMainHoldStillActive ] |> List.distinct
        if findings.IsEmpty then Ok observation else Error findings
        ...
        |> Result.map(fun value ->
            let receipt =
                { Schema=ReceiptSchema; IdentityDigest=identityDigest identity; NativeCensusDigest=value.NativeCensusDigest; ObservationDigest=observationDigest identity value
                  ...
                  RetirementHead=identity.RetirementHead; FrozenBranchHead=value.ObservedBranchHead
