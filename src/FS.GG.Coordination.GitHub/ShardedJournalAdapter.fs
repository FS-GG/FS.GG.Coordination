...
type JournalSnapshot =
    {
        Revision: string
        Current: JournalCommit
        Commits: JournalCommit list
    }
...
type ReceivePackOutcome =
    ...
    | ReceiveDefiniteRefusal of string
...
type EffectRefusal =
    | StaleFence
    | TerminalAuthority
    | EffectAuthorityUnavailable of JournalFailure
...
type SagaPlan =
    {
        OperationId: string
        PersistBeforeEffects: SagaTouch list
        AcquisitionOrder: SagaTouch list
    }
...
type Compensation =
    {
        ...
        Address: AggregateAddress
        Generation: int64
        OriginalResultRetained: bool
    }

type SagaConflictPlan =
    {
        ReleaseUnconsumed: SagaTouch list
        CompensateApplied: Compensation list
    }

type EffectiveBranchRule =
    | CreationRestricted
    | UpdateRestricted
    | DeletionRejected
...
type ProtectionFailure =
    | IncompleteProtectionObservation
    | AuthorityRepositoryDrift
    | WriterRulesetDrift
...
module ShardedJournalAdapter =
    ...
    let address kind aggregateId =
        if not (validText aggregateId) then
            ...
            let prefix =
                Encoding.ASCII.GetBytes(payload.Length.ToString(CultureInfo.InvariantCulture) + ":")
    ...
    let canonicalJson (json: string) =
        let rec write (writer: Utf8JsonWriter) (value: JsonElement) =
            ...
            | JsonValueKind.Array ->
                ...
                value.EnumerateArray() |> Seq.iter (write writer)
    ...
    let journalHeadBytes (head: JournalHead) =
        ...
        match head.PriorHeadDigest with
        | Some value -> root.Add("priorHeadDigest", value)
        | None -> root.Add("priorHeadDigest", null)

        root.Add("schemaVersion", head.SchemaVersion)
        root.Add("shard", head.Address.Shard)

        match head.SnapshotDigest with
        | Some value -> root.Add("snapshotDigest", value)
        | None -> root.Add("snapshotDigest", null)
    ...
    let private validBlob (blob: CanonicalBlob) =
        ...
        && (try
                Encoding.UTF8.GetString blob.Bytes |> canonicalJson = Ok blob.Bytes
            with _ ->
                false)
    ...
    let private validateCommit address index previous eventDigests (commit: JournalCommit) =
        ...
        if
            obj.ReferenceEquals(commit, null)
            || obj.ReferenceEquals(head, null)
            || not (validOid commit.CommitOid)
            || not (validOid commit.TreeOid)
            || not (validText commit.OperationId)
        then
            Error InvalidJournalCommit
        elif head.SchemaVersion <> 1 then
            Error(UnknownJournalSchema head.SchemaVersion)
        elif head.Address <> address || head.Address.Shard <> address.Shard then
            Error WrongJournalShard
        elif
            not (validDigest head.EventDigest)
            || not (validDigest head.HeadDigest)
            || (head.SnapshotDigest |> Option.exists (validDigest >> not))
            || (head.PriorHeadDigest |> Option.exists (validDigest >> not))
        then
            Error(InvalidDigest head.HeadDigest)
        elif not (validBlob commit.Event) || commit.Event.Digest <> head.EventDigest then
            Error(InvalidDigest head.EventDigest)
        elif
            obj.ReferenceEquals(commit.HeadBytes, null)
            || commit.HeadBytes <> journalHeadBytes head
            || sha256 commit.HeadBytes <> head.HeadDigest
        then
            Error(InvalidDigest head.HeadDigest)
        elif
            ...
            | _ -> true
        then
            Error(InvalidDigest(head.SnapshotDigest |> Option.defaultValue "missing-checkpoint"))
        elif head.Generation <> int64 (index + 1) then
            Error NonMonotonicJournalGeneration
        else
            match previous, commit.ParentOid with
            | None, None when index = 0 -> Ok()
            | Some prior, Some parent when
                parent = prior.CommitOid
                && head.PriorHeadDigest = Some prior.Head.HeadDigest
                && not prior.Head.Terminal
                ->
                Ok()
    ...
    let validate address observation =
        ...
        | JournalComplete(revision, commits) when
            not (validText revision)
            || obj.ReferenceEquals(commits, null)
            || List.isEmpty commits
            ->
            Error MissingJournalParent
        | JournalComplete(revision, commits) ->
            match
                commits
                |> List.groupBy (fun commit -> commit.Head.Generation)
            ...
            | None ->
                let folder state (index, commit) =
                    state
                    |> Result.bind (fun (previous, eventDigests) ->
                ...
                | Ok _ ->
                    Ok
                        {
                            Revision = revision
                            Current = List.last commits
                            Commits = commits
                        }

    let planCas (operationId: string) (snapshot: JournalSnapshot) (proposed: JournalCommit) =
        if
            not (validText operationId)
            || proposed.OperationId <> operationId
            || proposed.ParentOid <> Some snapshot.Current.CommitOid
            || proposed.Head.Generation <> snapshot.Current.Head.Generation + 1L
            || proposed.Head.Address <> snapshot.Current.Head.Address
        then
            Error InvalidJournalCommit
        elif snapshot.Current.Head.Terminal then
            ...
            Ok
                {
                    Address = snapshot.Current.Head.Address
                    ObservedObjectId = snapshot.Current.CommitOid
                    ProposedCommit = proposed
                    OperationId = operationId
                    Refspec = $"{proposed.CommitOid}:{refName}"
                    ForceWithLease = $"--force-with-lease={refName}:{snapshot.Current.CommitOid}"
                }
    ...
    let private exactProposal (proposal: CasProposal) (observation: JournalObservation) =
        ...
        | Ok snapshot ->
            ...
            current.CommitOid = proposal.ProposedCommit.CommitOid
            && current.TreeOid = proposal.ProposedCommit.TreeOid
            && current.OperationId = proposal.OperationId
            && current.Head.HeadDigest = proposal.ProposedCommit.Head.HeadDigest
            && current.Head.Generation = proposal.ProposedCommit.Head.Generation
        | Error _ -> false
    ...
    let authorizeEffect (grant: FencedGrant) (observation: JournalObservation) =
        ...
        | Ok snapshot when
            snapshot.Current.CommitOid <> grant.JournalCommit
            || snapshot.Current.Head.Generation <> grant.Generation
            || snapshot.Current.Head.Address <> grant.Address
            ->
            Error StaleFence
        | Ok snapshot -> Ok snapshot.Current

    let private touchKey (touch: SagaTouch) =
        journalKind touch.Address.Kind, touch.Address.Shard, touch.Address.Digest

    let planSaga (operationId: string) (touches: SagaTouch list) =
        if
            not (validText operationId)
            || obj.ReferenceEquals(touches, null)
            || List.isEmpty touches
            || touches |> List.exists (fun touch -> touch.ExpectedGeneration < 0L)
        then
            Error "invalid saga"
        elif
            touches
            |> List.groupBy (fun touch -> touch.Address.Kind, touch.Address.Digest)
            |> List.exists (fun (_, values) -> values.Length > 1)
        then
            Error "duplicate aggregate"
        else
            let ordered = List.sortBy touchKey touches in

            Ok
                {
                    OperationId = operationId
                    PersistBeforeEffects = ordered
                    AcquisitionOrder = ordered
                }

    let planConflict (plan: SagaPlan) (acquired: SagaTouch list) (applied: SagaTouch list) =
        let prefix length values =
            plan.AcquisitionOrder |> List.truncate length = values

        if
            obj.ReferenceEquals(plan, null)
            || not (prefix acquired.Length acquired)
            || not (prefix applied.Length applied)
            || applied.Length > acquired.Length
        then
            Error "conflict state is not a persisted acquisition prefix"
        else
            ...
            let compensation =
                ...
                |> List.map (fun touch ->
                    {
                        OperationId =
                            $"{plan.OperationId}:compensate:{journalKind touch.Address.Kind}:{touch.Address.Digest}:{touch.ExpectedGeneration}"
            ...
            Ok
                {
                    ReleaseUnconsumed = release
                    CompensateApplied = compensation
                }
    ...
    let validateProtection (observation: ProtectionObservation) =
        if obj.ReferenceEquals(observation, null) || not observation.Complete then
            Error IncompleteProtectionObservation
        elif observation.RepositoryId <> 1351660651L then
            Error AuthorityRepositoryDrift
        elif
            observation.Writer.Id <> 21872113L
            || observation.Writer.Name <> "v2-journal-writer"
            || not observation.Writer.Active
            || not observation.Writer.RestrictsCreationAndUpdate
        then
            Error WriterRulesetDrift
        elif
            observation.Integrity.Id <> 21872115L
            || observation.Integrity.Name <> "v2-journal-integrity"
            || not observation.Integrity.Active
            || not observation.Integrity.RejectsDeletionAndNonFastForward
        then
            Error IntegrityRulesetDrift
        elif
            observation.Writer.Target <> "refs/heads/fsgg/v2/journal/**/*"
            || observation.Integrity.Target <> "refs/heads/fsgg/v2/journal/**/*"
        then
            Error TargetPatternDrift
        elif
            observation.Writer.BypassAppIds <> [ 4166418L ]
            || not (List.isEmpty observation.Integrity.BypassAppIds)
        then
            Error BypassDrift
        elif
            not observation.EffectiveRulesComplete
