...
module ClaimTouchSetAdapter =
    ...
    let private normalizeTouch (touch: ClaimTouch) =
        if
            obj.ReferenceEquals(touch, null)
            || not (validIdentity touch.Repository)
            || not (validIdentity touch.Path)
        then
            Error(
                InvalidTouch(
                    if obj.ReferenceEquals(touch, null) then
                        "null"
                    else
                        touch.Path
                )
            ...
            let unsafeSyntax =
                ...
                || parts.Length = 0
                || parts
                   |> Array.exists (fun part -> part = "" || part = "." || part = ".." || part <> part.Trim())
                || path = "."

            if unsafeSyntax then
                Error(InvalidTouch path)
            else
                Ok { Repository = repository; Path = path }
    ...
    let normalizeTouches (touches: ClaimTouch list) =
        if obj.ReferenceEquals(touches, null) || List.isEmpty touches then
            ...
            let values =
                ...
                |> List.sortBy (fun touch -> touch.Repository, touch.Path)

            let duplicates =
                values
                |> List.countBy (fun touch -> touch.Repository, touch.Path)
                |> List.choose (fun ((repository, path), count) ->
                    if count > 1 then
                        Some(DuplicateTouch $"{repository}:{path}")
                    else
                        None)
        ...
        | Ok canonicalLeft, Ok canonicalRight ->
            ...
            |> List.exists (fun a ->
                ...
                |> List.exists (fun b ->
                    ...
                    && (pathContains a.Path b.Path || pathContains b.Path a.Path)))
        | _ -> false

    let private canonicalSubject (subject: string) =
        if validIdentity subject then
            Ok(subject.ToLowerInvariant())
        else
            Error InvalidSubject

    let private canonicalOwner (owner: string) =
        if validIdentity owner then
            Ok(owner.ToLowerInvariant())
        else
            Error InvalidOwner
    ...
    let authorityBytes (authority: ClaimAuthorityRecord) =
        let failures =
            [
                if authority.SchemaVersion <> 1 then
                    yield ClaimPayloadMismatch
                if canonicalSubject authority.Subject |> Result.isError then
                    yield InvalidSubject
                if canonicalOwner authority.Owner |> Result.isError then
                    yield InvalidOwner
                if authority.LeaseExpiresAt < 0L then
                    yield InvalidLease
                if not (validIdentity authority.OperationId) then
                    yield ClaimPayloadMismatch
            ]
        ...
        | Ok touches ->
            ...
            |> Result.mapError (fun _ -> [ ClaimPayloadMismatch ])
                ...
                |> Result.bind (fun snapshot ->
                    ...
                    |> Result.bind (fun bytes ->
                        if
                            ...
                            && snapshot.Current.Head.Address = address
                        then
                            Ok snapshot
                        else
                            Error ClaimPayloadMismatch)))
    ...
    let private sealPlan (operationId: string) (authority: ClaimAuthorityRecord) (proposal: CasProposal) =
        ...
        sha256Text
            $"{operationId}|{authority.Subject}|{authority.Owner}|{touches}|{authority.LeaseExpiresAt}|{proposal.ObservedObjectId}|{proposal.ProposedCommit.CommitOid}|{proposal.ProposedCommit.Head.Generation}"

    let planAcquire
        (intent: ClaimAcquireIntent)
        (observation: ClaimAuthorityObservation)
        (material: ClaimCommitMaterial)
        =
        ...
        let failures =
            [
                match normalizedSubject with
                | Error failure -> yield failure
                | _ -> ()
                match normalizedOwner with
                | Error failure -> yield failure
                | _ -> ()
                match normalizedTouches with
                | Error values -> yield! values
                | _ -> ()
                match snapshot with
                | Error failure -> yield failure
                | _ -> ()
                if intent.LeaseExpiresAt <= intent.Now then
                    yield InvalidLease
                if not (validOid material.CommitOid && validOid material.TreeOid) then
                    yield InvalidCommitMaterial
            ]
        ...
        if not (List.isEmpty failures) then
            ...
            if current.Subject.ToLowerInvariant() <> subject then
                Error [ ClaimPayloadMismatch ]
                        ...
                        let grant =
                            {
                                ...
                                Owner = owner
                                Touches = touches
                                JournalCommit = material.CommitOid
                        ...
                        Ok
                            {
                                ...
                                Proposal = proposal
                                Grant = grant
                                Seal = sealPlan operationId authority proposal
                                Cost =
                                    {
                                        AuthorityReads = 2
                                        MaximumEffects = 1
    ...
    let confirmAcquire (plan: ClaimAcquirePlan) (outcome: ReceivePackOutcome) (reread: ClaimAuthorityObservation) =
        ...
        if expectedSeal <> plan.Seal then
            ClaimAcquireRefused AlteredClaimPlan
        else
    ...
    let authorizeEffect (grant: ClaimGrant) (observation: ClaimAuthorityObservation) =
        ...
        | Ok _ ->
            ...
            | Error _, _
            | _, Error _ -> Error WrongClaimTouches
            | _ ->
    ...
    let planMultiTouch
        ...
        (owner: string)
        (touches: ClaimTouch list)
        (domains: ClaimDomainObservation list)
        =
        ...
        let failures =
            [
                if not (validIdentity operationId) then
                    yield AlteredClaimPlan
                match canonicalOwnerResult with
                | Error failure -> yield failure
                | _ -> ()
                match canonicalTouchesResult with
                | Error values -> yield! values
                | _ -> ()
            ]
        ...
        if not (List.isEmpty failures) then
            ...
            let normalizedDomains =
                domains
                |> List.map (fun domain ->
                    normalizeTouch domain.Touch
                    |> Result.map (fun touch -> touch, domain.ExpectedGeneration, domain.ActiveGrant))
            ...
            let domainValues =
                normalizedDomains
                |> List.choose Result.toOption
                |> List.sortBy (fun (touch, _, _) -> touch.Repository, touch.Path)

            let domainTouches = domainValues |> List.map (fun (touch, _, _) -> touch)

            let generationFailures =
                domainValues
                |> List.choose (fun (_, generation, _) -> if generation < 1L then Some AlteredClaimPlan else None)
            ...
            if domainTouches <> canonicalTouches then
                Error(AlteredClaimPlan :: allFailures)
            elif not (List.isEmpty allFailures) then
                Error allFailures
            else
                let sagaTouchesResult =
                    domainValues
                    |> List.map (fun (touch, generation, _) ->
                        conflictAddress touch
                ...
                if not (List.isEmpty addressFailures) then
                    Error addressFailures
                else
                        ...
                        Ok
                            {
                                ...
                                Saga = saga
                                Seal = multiSeal operationId canonicalOwner expectations
                                Cost =
                                    {
                                        AuthorityReads = canonicalTouches.Length + 1
                                        MaximumEffects = canonicalTouches.Length * 2 + 1
                                    }
                            }
    ...
    let authorizeMultiTouchEffects
        (plan: ClaimMultiTouchPlan)
        (persisted: ClaimPersistedPlan option)
        ...
        | Some receipt ->
            ...
            if
                receipt <> expected
                || multiSeal plan.OperationId plan.Owner receipt.Domains <> plan.Seal
            then
                ...
                let extraFailures =
                    ...
                    |> List.choose (fun (touch, _, _) ->
                        if plan.Domains |> List.exists (fun expectedDomain -> expectedDomain.Touch = touch) then
                            None
                        else
                            Some(WrongClaimTouches))
                ...
                if List.isEmpty failures then
                    Ok(results |> List.choose Result.toOption)
                else
                    Error failures
