...
module RequiredCheckCensusAdapter =
    ...
    let private sourceOrder =
        function
        | ClassicProtection -> 0L, 0L
        | Ruleset id -> 1L, id
    ...
    let private eventUnconditional event =
        event.Declared
        && event.BranchFilters.IsEmpty
        && event.PathFilters.IsEmpty
        && event.ActivityTypes.IsEmpty
    ...
    let private seal
        (snapshot: RequiredCheckCensusSnapshot)
        (entries: RequiredCheckCensusEntry list)
        (aggregate: RequiredCheckCensusAggregate)
        =
        [
    ...
    let compile (asOf: DateTimeOffset) (maxAge: TimeSpan) (snapshot: RequiredCheckCensusSnapshot) =
        ...
        let key (requirement: RequiredCheckRequirement) =
            requirement.Context, requirement.IntegrationId

        let producerKey (producer: RequiredCheckProducer) =
            producer.Context, producer.IntegrationId

        let findings =
            [
                if snapshot.SchemaVersion <> 1 then
                    yield UnsupportedCensusSchema snapshot.SchemaVersion
                if not (repositoryLike snapshot.Repository) then
                    yield InvalidCensusRepository snapshot.Repository
                if
                    not snapshot.Complete
                    || not snapshot.ClassicComplete
                    || not snapshot.RulesetsComplete
                    || not snapshot.ProducersComplete
                then
                    yield IncompleteCensusObservation
                if snapshot.ObservedAt > asOf || asOf - snapshot.ObservedAt > maxAge then
                    yield StaleCensusObservation
                if not (digestLike snapshot.ProfileSeal) then
                    yield InvalidCensusBinding "profileSeal"
                if not (digestLike snapshot.PrerequisiteReceiptDigest) then
                    yield InvalidCensusBinding "prerequisiteReceiptDigest"
                if not (digestLike snapshot.AuthorityEvidenceSha256) then
                    yield InvalidCensusBinding "authorityEvidenceSha256"
                if not (revisionLike snapshot.SourceRevision) then
                    yield InvalidCensusBinding "sourceRevision"
                for requirement in requirements do
                    if requirement.Repository <> snapshot.Repository then
                        yield CrossRepositoryRequirement requirement.Repository

                    if not (validText requirement.Context) then
                        yield InvalidRequiredCheckContext requirement.Context

                    if requirement.IntegrationId |> Option.exists ((>=) 0L) then
                        yield InvalidRequiredCheckIntegration requirement.Context

                    match requirement.Source with
                    | Ruleset id when id <= 0L -> yield InvalidCensusBinding "rulesetId"
                    | _ -> ()
                for _, (context, _) in
                    requirements
                    |> duplicates (fun (requirement: RequiredCheckRequirement) ->
                        sourceText requirement.Source, key requirement) do
                    yield DuplicateRequiredCheck context
                for context, rows in requirements |> List.groupBy _.Context do
                    if rows |> List.map _.IntegrationId |> Set.ofList |> Set.count > 1 then
                        yield AmbiguousRequiredCheckContext context
                for producer in producers do
                    if producer.Repository <> snapshot.Repository then
                        yield CrossRepositoryProducer producer.Repository

                    if
                        not (validText producer.Context)
                        || not (validText producer.Workflow)
                        || not (validText producer.Job)
                        || not (revisionLike producer.WorkflowRevision)
                        || not (digestLike producer.WorkflowSha256)
                    then
                        yield InvalidRequiredCheckProducer(identity producer.Context producer.IntegrationId)

                    if producer.IntegrationId |> Option.exists ((>=) 0L) then
                        yield InvalidRequiredCheckIntegration producer.Context
                for context, _ in producers |> duplicates producerKey do
                    yield DuplicateRequiredCheckProducer context
                let requiredKeys = requirements |> List.map key |> Set.ofList
                let producerKeys = producers |> List.map producerKey |> Set.ofList

                for context, integrationId in Set.difference requiredKeys producerKeys do
                    yield MissingRequiredCheckProducer(identity context integrationId)

                for context, integrationId in Set.difference producerKeys requiredKeys do
                    yield OrphanRequiredCheckProducer(identity context integrationId)
            ]
        ...
        if not findings.IsEmpty then
            ...
            let producerByKey =
                producers
                |> List.map (fun producer -> producerKey producer, producer)
                |> Map.ofList
            ...
            let entries =
                ...
                |> List.map (fun ((context, integrationId), rows) ->
...

                    {
                        ...
                        PullRequestUnconditional =
                            pullRequest
                            && producer.DependenciesComplete
                            && not producer.Conditional
                            && not producer.ContinueOnError
                        MergeGroupUnconditional =
                            mergeGroup
                            && producer.DependenciesComplete
                            && not producer.Conditional
                            && not producer.ContinueOnError
            ...
            let aggregate =
                {
                    ...
                    ClassicOnlyCount =
                        entries
                        |> List.filter (fun entry -> sourceShape entry = Set.singleton 1)
                        |> List.length
                    RulesetOnlyCount =
                        entries
                        |> List.filter (fun entry -> sourceShape entry = Set.singleton 2)
                        |> List.length
                    DualSourceCount =
                        entries
                        |> List.filter (fun entry -> sourceShape entry = Set [ 1; 2 ])
                        |> List.length
