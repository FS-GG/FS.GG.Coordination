...
module V1EffectFenceAdapter =
    let private required =
        Set.ofList
            [
                ...
                "complete"
                "fresh"
                "cacheUsedAsAuthority"
            ]

    let private phase =
        function
        | "OperatingV1" -> Ok V1OperatingV1
        | "Preparing" -> Ok V1Preparing
        | "FreezeRequested" -> Ok V1FreezeRequested
        | "Frozen" -> Ok V1Frozen
        | "SwitchedV2" -> Ok V1SwitchedV2
        | "VerifiedV2" -> Ok V1VerifiedV2
        | "OpenV2" -> Ok V1OpenV2
        | "ObservingV2" -> Ok V1ObservingV2
        | "ContractingV1" -> Ok V1ContractingV1
        | "OperatingV2" -> Ok V1OperatingV2
        | "RollingBack" -> Ok V1RollingBack
    ...
    let parseStrict (bytes: byte array) =
        try
            ...
            if root.ValueKind <> JsonValueKind.Object then
                ...
                if not shapeErrors.IsEmpty then
                    Error shapeErrors
                else
                    try
    ...
    let private refusalReasons
        (evidence: V1EpochEvidence)
        ...
        [
            if evidence.Schema <> "fsgg.github-substrate.epoch-wire/1" then
                "wrong-schema"
            if evidence.FleetId <> "fs-gg-production" then
                "wrong-fleet"
            if
                evidence.Repository <> "FS-GG/FS.GG.Coordination.Authority"
                || evidence.RepositoryId <> 1351660651L
            then
                "wrong-authority-repository"
            match address with
            | Ok value when evidence.Ref <> value.Ref -> "wrong-ref"
            | Error _ -> "canonical-address-unavailable"
            | _ -> ()
            if not (evidence.Tag.StartsWith("refs/tags/fsgg/v2/fleet-cutover/", StringComparison.Ordinal)) then
                "missing-or-wrong-tag"
            if not evidence.Complete then
                "authority-partial"
            if not evidence.Fresh then
                "authority-stale"
            if evidence.CacheUsedAsAuthority then
                "cache-cannot-authorize"
            if evidence.Generation < 1L then
                "invalid-generation"
            if not (validDigest evidence.GenesisCommit) || not (validDigest evidence.Commit) then
                "missing-parent-or-genesis"
            match evidence.Generation, evidence.Parent with
            | 1L, None -> ()
            | generation, Some parent when generation > 1L && validDigest parent -> ()
            | _ -> "missing-parent-or-genesis"
            if not (validDigest evidence.TrustAnchorSha256) then
                "wrong-trust-anchor"
            if
                not (validDigest evidence.ManifestSha256)
                || evidence.ManifestSha256 <> expectation.ManifestSha256
            then
                "manifest-mismatch"
            if
                evidence.Commit <> expectation.EpochCommit
                || evidence.Generation <> expectation.EpochGeneration
            then
                "stale-epoch-generation"
            if expectation.ExpectedClaimGeneration <> expectation.CurrentClaimGeneration then
                "stale-claim-generation"
            if
                expectation.ExpectedOperationGeneration
                <> expectation.CurrentOperationGeneration
            then
                "stale-operation-generation"
            if String.IsNullOrWhiteSpace request.OperationId then
                "missing-operation-id"
            if not (validDigest request.PayloadDigest) then
