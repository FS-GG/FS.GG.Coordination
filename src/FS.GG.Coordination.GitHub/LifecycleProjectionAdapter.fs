...
type LifecycleAuthority =
    | HoldAuthority
    | DependencyAuthority
    | ClaimJournalAuthority
    | PullRequestAuthority
    | ReviewJournalAuthority
    | DeliveryJournalAuthority
...
type LifecycleIssueState =
    | IssueOpen
    | IssueClosed

type DerivedLifecycleStage =
    | StageBacklog
    | StageReady
    | StagePaused
    | StageCancelled
...
module LifecycleProjectionAdapter =
    ...
    let private canonicalSubject (value: string) =
    ...
    let private hashParts values =
        values |> List.map frame |> String.concat "" |> sha256

    let private outcomeName =
        function
        | FactObserved -> "observed"
        | FactProvenAbsent -> "proven-absent"
        | FactIncomplete -> "incomplete"
        | FactUnauthorized -> "unauthorized"
    ...
    let statusName =
        ...
        | StagePaused
        | StageBlocked -> "Blocked"
        | StageClaimed -> "In progress"
        | StageInReview
        | StageAccepted -> "In review"
        | StageCancelled
        | StageDelivered -> "Done"
    ...
    let derive (observation: LifecycleProjectionObservation) =
        if obj.ReferenceEquals(observation, null) then
            Error [ LifecycleObservationIncomplete ]
        else
            ...
            let structural =
                [
                    if not observation.Complete then
                        LifecycleObservationIncomplete
                    if not (validText observation.Subject) then
                        InvalidLifecycleSubject
                    if not (validText observation.Revision) then
                        InvalidLifecycleRevision
                    if
                        facts
                        |> List.exists (fun (_, _, fact) -> canonicalSubject fact.Subject <> expectedSubject)
                    then
                        LifecycleFactSubjectMismatch
                    if
                        facts
                        |> List.exists (fun (_, _, fact) ->
                            not (validText fact.Revision) || fact.Revision <> observation.Revision)
                    then
                        InvalidLifecycleFactRevision
                    for name, expectedAuthority, fact in facts do
                        if fact.Authority <> expectedAuthority then
                            WrongLifecycleAuthority(name, fact.Authority)
                ]
            ...
            if not structural.IsEmpty then
                ...
                if not refusals.IsEmpty then
                    ...
                    let present name =
                        results
                        |> List.find (fun (factName, _) -> factName = name)
                        |> snd
                    ...
                    let contradictions =
                        [
                            if review && not pullRequest && not delivery then
                                ContradictoryLifecycleFacts
                                    "accepted review requires a current pull request until delivery"
                            if delivery && not review then
                                ContradictoryLifecycleFacts "delivery requires accepted review authority"
                            if delivery && not observation.DeliveryProtected then
                                UnprotectedLifecycleDelivery
                            if
                                observation.IssueState = IssueClosed
                                && observation.Intent <> IntentCancelled
                                && not delivery
                            then
                                ClosedIssueWithoutTerminalAuthority
                        ]
                    ...
                    if not contradictions.IsEmpty then
                        ...
                        let stage =
                            if observation.Intent = IntentCancelled then
                                StageCancelled
                            elif delivery then
                                StageDelivered
                            elif review then
                                StageAccepted
                            elif hold || dependency then
                                StageBlocked
                            elif pullRequest then
                                StageInReview
                            elif claim then
                                StageClaimed
                            else
                                match observation.Intent with
                                | IntentReady -> StageReady
                                | IntentPaused -> StagePaused
                                | IntentBacklog -> StageBacklog
                                | IntentCancelled -> StageCancelled

                        Ok stage

    let private optionParts (option: StatusOptionProjection) =
        [ LiveId.value option.Id; SemanticName.value option.Name ]
    ...
    let private decisionParts =
        ...
        | StatusNoOp value ->
            [ "no-op"; value.ObservedRevision; value.IdempotencyIdentity ]
            @ intentParts value.Intent

    let private expectedCost =
        function
        | StatusPlanned _ ->
            {
                AuthorityReads = 8
    ...
    let private seal observation stage statusNameValue decision cost =
        let facts =
            ...
            |> List.collect (fun (name, authority, fact) ->
                [
                    name
                    string authority
                    fact.Revision
                    outcomeName fact.Outcome
                    string fact.Current
                ])

        hashParts (
            [
                canonicalSubject observation.Subject
    ...
    let plan causationIdentity observation =
        ...
        | Ok stage ->
            ...
            match
                ...
                |> List.tryFind (fun option -> SemanticName.value option.Name = desiredName)
            with
            | None -> Error [ LifecycleStatusOptionMissing desiredName ]
            | Some option ->
                match
                    ProjectAdapter.planStatus
                        observation.Status.Revision
                        causationIdentity
                        (SetStatus option.Id)
                        observation.Status
                with
                ...
                | Ok decision ->
                    ...
                    Ok
                        {
                            Subject = canonicalSubject observation.Subject
                            SourceRevision = observation.Revision
                            Stage = stage
                            StatusName = desiredName
                            StatusDecision = decision
                            Seal = seal observation stage desiredName decision cost
                            Cost = cost
                        }
    ...
    let private validatePlan plan observation =
        ...
        | Ok stage ->
            ...
            if
                canonicalSubject observation.Subject <> plan.Subject
    ...
    let authorize plan observation statusObservation =
        ...
        | Ok _ ->
            ...
            | StatusNoOp receipt ->
                match
                    ProjectAdapter.readStatus observation.Status.ProjectId observation.Status.ItemId statusObservation
                with
                | Ok snapshot when snapshot = observation.Status && snapshot.Revision = receipt.ObservedRevision ->
                    Ok snapshot
