...
module ProjectAdapter =
    ...
    let private validRepo repo =
        not (obj.ReferenceEquals(repo, null))
        && validText repo.Owner
        && validText repo.Name

    let private contentId =
        function
        | RepositoryIssue(_, _, id)
        | PullRequest(_, _, id)
        | DraftIssue id
        | RedactedContent id
        | UnknownContent(_, id) -> Some id

    let private contentKey =
        function
    ...
    let private itemKey (item: ProjectItem) =
        LiveId.value item.ProjectId, LiveId.value item.ItemId, contentKey item.Content, item.Archived

    let private validItem (item: ProjectItem) =
        if obj.ReferenceEquals(item, null) || obj.ReferenceEquals(item.Content, null) then
            false
        else
            match item.Content with
            | RepositoryIssue(repo, number, _)
            | PullRequest(repo, number, _) -> validRepo repo && number > 0
            | DraftIssue _
            | RedactedContent _ -> true
            | UnknownContent(kind, _) -> validText kind

    let private duplicateBy key values =
        values
        |> List.groupBy key
        |> List.tryPick (fun (_, xs) -> if xs.Length > 1 then Some xs.Head else None)

    let readProject (observation: ProjectObservation) =
        if obj.ReferenceEquals(observation, null) then
            Error(ProjectObservationRefused InvalidCompletenessEvidence)
        else
            match observation with
            | ProjectIncomplete(reason, cursor) ->
                Error(ProjectObservationRefused(ObservationIncomplete(reason, cursor)))
            ...
            | ProjectComplete(revision, _) when not (validText revision) ->
                Error(ProjectObservationRefused MissingObservationRevision)
            | ProjectComplete(_, pages) when obj.ReferenceEquals(pages, null) || List.isEmpty pages ->
                Error InvalidProjectPageChain
            | ProjectComplete(revision, pages) ->
                let validChain =
                    pages
                    |> List.mapi (fun i p ->
                        not (obj.ReferenceEquals(p, null))
                        && not (obj.ReferenceEquals(p.Items, null))
                        && p.Number = i + 1
                        && p.TerminalPage = (i = pages.Length - 1))
                    |> List.forall id
                ...
                if not validChain then
                    ...
                    | None ->
                        ...
                        | None ->
                            let addressable =
                                items
                                |> List.choose (fun (item: ProjectItem) ->
                                    contentId item.Content |> Option.map (fun id -> id, item))
                                ...
                                Ok
                                    {
                                        ...
                                        Items = List.sortBy itemKey items
                                    }
    ...
    let resolveMembership
        ...
        =
        if not (validRepo expectedRepository) then
            Error InvalidExpectedRepository
        elif obj.ReferenceEquals(snapshot, null) then
            Error InvalidTargetContentIdentity
        else
            match
                snapshot.Items
                |> List.tryFind (fun item -> contentId item.Content = Some targetContentId)
            with
            | None -> Ok MissingMembership
            | Some item when item.Archived -> Ok(ArchivedMembership item)
            | Some({
                       Content = RepositoryIssue(repo, _, _) | PullRequest(repo, _, _)
                   } as item) when repo <> expectedRepository -> Ok(ExternalRepositoryMembership item)
    ...
    let readStatus (projectId: LiveId) (itemId: LiveId) (observation: StatusObservation) =
        if obj.ReferenceEquals(observation, null) then
            Error(StatusObservationRefused InvalidCompletenessEvidence)
        else
            match observation with
            ...
            | StatusComplete(revision, _, _) when not (validText revision) ->
                Error(StatusObservationRefused MissingObservationRevision)
            | StatusComplete(_, evidence, fields) when
                obj.ReferenceEquals(evidence, null)
                || obj.ReferenceEquals(fields, null)
                || evidence.PageCount <= 0
                || evidence.NodeCount < 0
                || not evidence.TerminalPage
                || evidence.NodeCount <> fields.Length
                ->
                Error InvalidStatusCompletenessEvidence
            ...
            | StatusComplete(revision, _, [ field ]) ->
                if
                    obj.ReferenceEquals(field, null)
                    || field.ProjectId <> projectId
                    || field.ItemId <> itemId
                    || SemanticName.value field.FieldName <> "Status"
                    || obj.ReferenceEquals(field.Options, null)
                    || List.isEmpty field.Options
                then
                    Error InvalidStatusField
                else
                    ...
                    | None ->
                        ...
                        | None ->
                            ...
                            | Some selected when
                                field.Options |> List.exists (fun option -> option.Id = selected) |> not
                                ->
                                Error(UnknownSelectedStatusOption selected)
                            | _ ->
                                Ok
                                    {
                                        Revision = revision
                                        Nature = ProjectionOnly
                                        ProjectId = field.ProjectId
                                        ItemId = field.ItemId
                                        FieldId = field.FieldId
                                        FieldName = field.FieldName
                                        Options = field.Options |> List.sortBy (fun option -> LiveId.value option.Id)
                                        SelectedOptionId = field.SelectedOptionId
                                    }
    ...
    let private membershipIntentText =
        function
        | EnsureMember(project, content) -> [ "ensure-member"; LiveId.value project; LiveId.value content ]
        | EnsureNotMember(item, content) -> [ "ensure-not-member"; LiveId.value item; LiveId.value content ]

    let private membershipIdentity revision causation repository intent =
        hash (
            [ revision; causation; repository.Owner; repository.Name ]
            @ membershipIntentText intent
        )
    ...
    let planMembership
        ...
        (snapshot: ProjectSnapshot)
        =
        if not (validText expectedRevision) then
            Error InvalidMembershipExpectedRevision
        elif not (validText causationIdentity) then
            Error InvalidMembershipCausationIdentity
        elif
            not (validRepo repository)
            || obj.ReferenceEquals(snapshot, null)
            || obj.ReferenceEquals(intent, null)
        then
            Error InvalidMembershipIntent
        elif expectedRevision <> snapshot.Revision then
            Error(MembershipStaleExpectedRevision snapshot.Revision)
        else
            let target =
                match intent with
                | EnsureMember(_, id)
                | EnsureNotMember(_, id) -> id
            ...
            | Ok(ActiveMembership item) ->
                match intent with
                | EnsureMember(projectId, _) when item.ProjectId = projectId ->
                    Ok(
                        MembershipNoOp
                            {
                ...
                | EnsureNotMember(itemId, content) when item.ItemId = itemId ->
                    Ok(
                        MembershipPlanned
                            {
                                Before = snapshot
                                Repository = repository
                                CausationIdentity = causationIdentity
                                IdempotencyIdentity = key
                                Operation = RemoveMembershipOperation(item.ProjectId, itemId, content)
                            }
                    )
                | _ -> Error InvalidMembershipIntent
            | Ok MissingMembership ->
                match intent with
                | EnsureMember(projectId, content) ->
                    Ok(
                        MembershipPlanned
                            {
                                Before = snapshot
                                ...
                                CausationIdentity = causationIdentity
                                IdempotencyIdentity = key
                                Operation = AddMembershipOperation(projectId, content)
                            }
                    )
                | EnsureNotMember _ ->
                    Ok(
    ...
    let checkMembershipPreState (plan: MembershipPlan) observation =
        ...
        | Ok observed when observed.Revision <> plan.Before.Revision ->
            Error(MembershipReReadRequired(plan.Before.Revision, observed.Revision))
        | Ok observed when observed <> plan.Before -> Error ConcurrentMembershipChange
        | Ok observed -> Ok observed

    let verifyMembershipPostState
        expectedResultRevision
        (resultingItem: ProjectItem option)
        (plan: MembershipPlan)
        observation
        =
        if not (validText expectedResultRevision) then
            Error InvalidMembershipResultRevision
        elif expectedResultRevision = plan.Before.Revision then
            Error(MembershipResultRevisionDidNotAdvance expectedResultRevision)
        else
            match readProject observation with
            | Error failure -> Error(MembershipPostStateReadRefused failure)
            | Ok observed when observed.Revision <> expectedResultRevision ->
                Error(MembershipResultRevisionMismatch(expectedResultRevision, observed.Revision))
            | Ok observed ->
                let expectedItems =
                    match plan.Operation, resultingItem with
                    | AddMembershipOperation(projectId, content), Some item when
                        item.ProjectId = projectId
                        && contentId item.Content = Some content
                        && not item.Archived
                        ->
                        Some(item :: plan.Before.Items |> List.sortBy itemKey)
                    | RemoveMembershipOperation(_, itemId, _), None ->
                        Some(
                            plan.Before.Items
                            |> List.filter (fun item -> item.ItemId <> itemId)
                            |> List.sortBy itemKey
                        )
                    | _ -> None
    ...
    let private statusIntentText =
        function
        | SetStatus id -> [ "set-status"; LiveId.value id ]
        | ClearStatus -> [ "clear-status" ]

    let private statusIdentity revision causation (snapshot: StatusSnapshot) intent =
        hash (
            [
                revision
                causation
                LiveId.value snapshot.ProjectId
                LiveId.value snapshot.ItemId
                LiveId.value snapshot.FieldId
            ]
            @ statusIntentText intent
        )

    let planStatus expectedRevision causationIdentity (intent: StatusIntent) (snapshot: StatusSnapshot) =
        if not (validText expectedRevision) then
            Error InvalidStatusExpectedRevision
        elif not (validText causationIdentity) then
            Error InvalidStatusCausationIdentity
        elif obj.ReferenceEquals(snapshot, null) || obj.ReferenceEquals(intent, null) then
            Error InvalidStatusIntent
        elif expectedRevision <> snapshot.Revision then
            Error(StatusStaleExpectedRevision snapshot.Revision)
        else
            let key = statusIdentity snapshot.Revision causationIdentity snapshot intent

            match intent with
            | SetStatus option when snapshot.Options |> List.exists (fun current -> current.Id = option) |> not ->
                Error(RequestedStatusOptionMissing option)
            | SetStatus option when snapshot.SelectedOptionId = Some option ->
                Ok(
                    StatusNoOp
                        {
                            ObservedRevision = snapshot.Revision
            ...
            | ClearStatus when snapshot.SelectedOptionId.IsNone ->
                Ok(
                    StatusNoOp
                        {
                            ObservedRevision = snapshot.Revision
                    ...
                    StatusPlanned
                        {
                            ...
                            CausationIdentity = causationIdentity
                            IdempotencyIdentity = key
                            Operation = SetStatusOperation option
                        }
                )
            | ClearStatus ->
                Ok(
                    StatusPlanned
                        {
                            ...
                            CausationIdentity = causationIdentity
                            IdempotencyIdentity = key
                            Operation = ClearStatusOperation
                        }
                )
    ...
    let checkStatusPreState (plan: StatusPlan) observation =
        ...
        | Ok observed when observed.Revision <> plan.Before.Revision ->
            Error(StatusReReadRequired(plan.Before.Revision, observed.Revision))
    ...
    let verifyStatusPostState expectedResultRevision (plan: StatusPlan) observation =
        if not (validText expectedResultRevision) then
            Error InvalidStatusResultRevision
        elif expectedResultRevision = plan.Before.Revision then
            Error(StatusResultRevisionDidNotAdvance expectedResultRevision)
        else
            match readStatus plan.Before.ProjectId plan.Before.ItemId observation with
            | Error failure -> Error(StatusPostStateReadRefused failure)
            | Ok observed when observed.Revision <> expectedResultRevision ->
                Error(StatusResultRevisionMismatch(expectedResultRevision, observed.Revision))
            | Ok observed ->
                let expectedSelected =
                    match plan.Operation with
                    | SetStatusOperation option -> Some option
                    | ClearStatusOperation -> None

                let expected =
                    { plan.Before with
                        Revision = expectedResultRevision
                        SelectedOptionId = expectedSelected
                    }

                if observed = expected then
                    Ok observed
                else
                    Error StatusPostStateMismatch
