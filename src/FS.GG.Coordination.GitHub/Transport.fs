...
module ApiVersion =
    ...
    let tryCreate value =
        if value = "2022-11-28" then
            Ok required
        else
            Error "GitHub REST requests require API version 2022-11-28"
...
module Transport =
    ...
    let validateRequest request =
        ...
        [
            if ApiVersion.value version <> ApiVersion.value ApiVersion.required then
                MissingApiVersion
            if isNull uri || not uri.IsAbsoluteUri then
                InvalidUri
            match document with
    ...
    let decideRetry maxAttempts attempt request outcome =
        ...
        if not (transient outcome) then
            Stop NotTransient
        elif not (replayPermitted idempotency) then
            Stop ReplayForbidden
        elif maxAttempts <= 0 || attempt >= maxAttempts then
            Stop AttemptsExhausted
        else
    ...
    let schedule now cost budget =
        if cost < 0 then
            Refused InvalidRateFacts
        else
            match budget.Limit, budget.Remaining, budget.ResetAt, budget.Cost with
            | Some limit, Some remaining, Some resetAt, Some observedCost when
                limit >= 0 && remaining >= 0 && observedCost >= 0 && resetAt >= now
                ->
    ...
    let tryNextLink (linkHeader: string) =
        if String.IsNullOrWhiteSpace linkHeader then
            ...
            let parse (segment: string) =
                ...
                if not (value.StartsWith("<", StringComparison.Ordinal)) || close <= 1 then
                    Error MalformedContinuation
                else
                    let rawUri = value.Substring(1, close - 1)
                    let mutable uri = Unchecked.defaultof<Uri>

                    if not (Uri.TryCreate(rawUri, UriKind.Absolute, &uri)) then
                        Error MalformedContinuation
                    else
                        let relations =
                            value
                            ...
                            |> Array.choose (fun parameter ->
                                ...
                                if parts.Length = 2 && parts.[0].Equals("rel", StringComparison.OrdinalIgnoreCase) then
                                    Some(
                                        parts.[1].Trim().Trim('"').Split(' ', StringSplitOptions.RemoveEmptyEntries)
                                        |> Set.ofArray
                                    )
                                else
                                    None)
            ...
            let parsed =
                linkHeader.Split(',', StringSplitOptions.RemoveEmptyEntries)
                |> Array.map parse
                |> Array.toList
    ...
    let collectRest (start: Uri) (pages: RestPage<'item> list) =
        ...
        let entries =
            if malformed then
                ...
                pages |> List.map (fun page -> page.Uri.AbsoluteUri, page)

        let ambiguous =
            entries |> List.countBy fst |> List.exists (fun (_, count) -> count <> 1)

        let indexed = entries |> Map.ofList

        let rec loop seen current collected =
            if Set.contains current seen then
                Error RepeatedContinuation
            else
        ...
        elif isNull start || not start.IsAbsoluteUri then
            Error MissingPage
        else
            loop Set.empty start.AbsoluteUri []
    ...
    let collectGraphQL (pages: GraphQLPage<'item> list) =
        ...
        let entries =
            if malformed then
                ...
                pages |> List.map (fun page -> page.Cursor, page)
        ...
        let rec loop seen cursor collected =
            if Set.contains cursor seen then
                Error RepeatedContinuation
            else
    ...
    let projectFixture (allowList: Set<string>) (fixture: CapturedFixture) =
        let project (prefix: string) (fields: FixtureField list) =
            if isNull (box fields) then
                Error(InvalidFixtureField $"<{prefix}>")
            else
                fields
                |> List.fold
                    (fun state field ->
                        ...
                        | Ok values ->
                            if isNull (box field) then
                                Error(InvalidFixtureField $"<{prefix}-field>")
                            elif String.IsNullOrWhiteSpace field.Path || isNull field.Value then
                                Error(InvalidFixtureField(if isNull field.Path then "<null>" else field.Path))
                            else
                                match field.Classification with
                                | Unclassified -> Error(UnclassifiedField field.Path)
                                | Public when looksSensitive field.Path field.Value ->
                                    Error(SensitiveFieldMisclassified field.Path)
                                | Public when allowList.Contains field.Path ->
                                    Ok($"{prefix}.{field.Path}={field.Value}" :: values)
                                | Secret when allowList.Contains field.Path ->
                                    Ok($"{prefix}.{field.Path}=[REDACTED]" :: values)
                                | Public
                                | Secret
                                | Private
                                | Unstable -> Ok values)
        ...
        if isNull (box allowList) then
            Error(InvalidFixtureField "<allow-list>")
        elif isNull (box fixture) then
            Error(InvalidFixtureField "<fixture>")
        else
