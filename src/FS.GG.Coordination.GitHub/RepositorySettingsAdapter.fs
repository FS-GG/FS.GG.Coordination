...
module RepositorySettingsAdapter =
    ...
    let surfaceId =
        ...
        | BranchRulesets -> "branch-rulesets"
        | TagRulesets -> "tag-rulesets"
        | MergePolicy -> "merge-policy"
        | ActionsPolicy -> "actions-policy"
        | Environments -> "environments"
        | ReleasesAndTags -> "releases-tags"
        | CodeSecurity -> "code-security"
        | DependencyControls -> "dependency-controls"
    ...
    let private identityText (identity: RepositoryIdentity) =
        String.concat
            "|"
            [
                escape identity.NodeId
                string identity.DatabaseId
                escape identity.Owner
                escape identity.Name
                escape identity.DefaultBranch
                identity.SourceRepositoryNodeId |> Option.map escape |> Option.defaultValue "-"
            ]
    ...
    let private settingKey (setting: RepositorySetting) =
        setting.Surface, setting.Subject, setting.Name
    ...
    let private observationText =
        function
        | Supported(revision, complete, settings) ->
            let encodedSettings =
                settings |> List.sortBy settingKey |> List.map settingText |> String.concat ";"

            $"supported|{escape revision}|{complete}|{encodedSettings}"
        | Unsupported reason -> $"unsupported|{escape reason}"
        | Unauthorized reason -> $"unauthorized|{escape reason}"
        | Unavailable reason -> $"unavailable|{escape reason}"
        | Incomplete reason -> $"incomplete|{escape reason}"
        | Unreadable reason -> $"unreadable|{escape reason}"

    let observationDigest
        (identity: RepositoryIdentity)
        (capturedRevision: string)
        (surfacesMap: Map<SettingsSurface, SurfaceObservation>)
        =
        let surfaceText =
            surfaces
    ...
    let private validateSettings (settings: RepositorySetting list) =
        match
            ...
            |> List.tryFind (fun setting ->
                not (validText setting.Subject)
                || not (validText setting.Name)
                || isSecretName setting.Name)
        with
    ...
    let validate (observation: RepositorySettingsObservation) =
        if
            not (validIdentity observation.Identity)
            || not (validText observation.CapturedRevision)
        then
            ...
            let folder state surface =
                ...
                |> Result.bind (fun () ->
                    ...
                    | Some(Supported(revision, true, settings)) when
                        revision = observation.CapturedRevision
                        && settings |> List.forall (fun item -> item.Surface = surface)
                        ->
                        validateSettings settings
                    | Some(Supported(_, false, _)) -> Error(PartialSurface(surface, "pagination incomplete"))
                    | Some(Supported(_, true, _)) ->
                        Error(PartialSurface(surface, "revision or surface identity drift"))
            ...
            | Ok() when
                observation.Digest
                <> observationDigest observation.Identity observation.CapturedRevision observation.Surfaces
                ->
                Error InvalidObservationDigest
            | Ok() -> Ok observation

    let private permission =
        function
        | ActionsPolicy -> "actions:write"
        | Environments -> "environments:write"
        | CodeSecurity
        | DependencyControls -> "security_events:write"
        | ReleasesAndTags
        | ImmutableReleases -> "contents:write"
    ...
    let plan
        ...
        (observation: RepositorySettingsObservation)
        (desired: DesiredRepositorySettings)
        =
        validate observation
        |> Result.bind (fun valid ->
            if valid.Identity <> desired.Identity then
                Error IdentityDrift
            elif valid.CapturedRevision <> expectedRevision then
                Error(StaleObservation(expectedRevision, valid.CapturedRevision))
            elif desired.Digest <> desiredDigest desired.Identity desired.Settings then
                Error InvalidDesiredDigest
            else
                validateSettings desired.Settings
                |> Result.bind (fun () ->
                    match
                        desired.Settings
                        |> List.tryFind (fun setting ->
                            match Map.find setting.Surface valid.Surfaces with
                            | Unsupported _ -> true
                            | _ -> false)
                    with
                    ...
                    | None ->
                        let current =
                            ...
                            |> List.collect (fun (_, value) ->
                                match value with
                                | Supported(_, _, settings) -> settings
                                | _ -> [])
                            |> List.map (fun setting -> settingKey setting, setting)
                            |> Map.ofList
                        ...
                        let keys =
                            Set.union (current |> Map.keys |> Set.ofSeq) (wanted |> Map.keys |> Set.ofSeq)
                            |> Set.toList
                        ...
                        let operations =
                            ...
                            |> List.choose (fun key ->
                                ...
                                if before |> Option.map _.Value = (after |> Option.map _.Value) then
                                    None
                                else
                                    ...
                                    Some
                                        {
                                            OperationId =
                                                operationId
                                                    ...
                                                    desired.Digest
                                                    exemplar
                                                    (after |> Option.map _.Value)
                                            Surface = exemplar.Surface
                                            Subject = exemplar.Subject
                                            Name = exemplar.Name
                                            Before = before |> Option.map _.Value
                                            After = after |> Option.map _.Value
                                            RequiredPermission = permission exemplar.Surface
                                            ObservationDigest = valid.Digest
                                            DesiredDigest = desired.Digest
                                        })
                            |> List.sortBy (fun operation ->
                                surfaces |> List.findIndex ((=) operation.Surface),
                                operation.Subject,
                                operation.Name,
                        ...
                        Ok
                            {
                                ...
                                ObservationRevision = valid.CapturedRevision
                                ObservationDigest = valid.Digest
                                DesiredDigest = desired.Digest
    ...
    let private exactPoststate (plan: RepositorySettingsPlan) (observation: RepositorySettingsObservation) =
        ...
        | Ok valid ->
            ...
            let changedKeys =
                plan.Operations
                |> List.map (fun operation -> operation.Surface, operation.Subject, operation.Name)
                |> Set.ofList

            let unchanged =
                actual
                |> List.filter (fun setting -> not (Set.contains (settingKey setting) changedKeys))

            let expected = unchanged @ desiredFromPlan plan
            Ok(desiredDigest plan.Identity expected = desiredDigest plan.Identity actual)

    let reconcile
        (plan: RepositorySettingsPlan)
        (outcome: SettingsTransportOutcome)
        (poststate: RepositorySettingsObservation)
        =
        match outcome with
        ...
        | _ ->
            ...
            | Ok false ->
                match outcome with
                | SettingsAccepted
                | SettingsResponseUnknown -> SettingsRereadAndReplan
                | SettingsPartiallyApplied ids ->
                    let applied, remaining =
                        plan.Operations
                        |> List.partition (fun operation -> List.contains operation.OperationId ids)

                    if List.isEmpty remaining then
                        SettingsRereadAndReplan
                    elif applied.Length * 2 > plan.Operations.Length then
                        SettingsForwardRepair remaining
                    else
                        SettingsRollback(List.rev applied)
