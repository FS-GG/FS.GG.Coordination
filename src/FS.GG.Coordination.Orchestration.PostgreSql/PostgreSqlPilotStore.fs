...
module private PilotStoreSupport =
    ...
    let mapRootFailure =
        ...
        | IncompatibleDowngrade(databaseVersion, runtimeVersion) ->
            PilotIncompatibleDowngrade(databaseVersion, runtimeVersion)
    ...
    let classify (exceptionValue: exn) =
        ...
        | :? PostgresException as pg when pg.SqlState.StartsWith("53", StringComparison.Ordinal) ->
            PilotStoreUnavailable $"postgres-sqlstate-{pg.SqlState}"
...
WHERE r.singleton FOR SHARE OF r,o
            ...
            if not found then
                return [ PilotMigrationInterrupted "pilot-or-root-metadata-missing" ]
            else
                let failures = ResizeArray<PilotRecoveryFailure>()
                let rootVersion = reader.GetInt32 0

                if rootVersion > options.RuntimeSchemaVersion then
                    failures.Add(PilotIncompatibleDowngrade(rootVersion, options.RuntimeSchemaVersion))
                elif rootVersion < options.RuntimeSchemaVersion then
                    failures.Add(
                        PilotMigrationInterrupted $"root-v{rootVersion}-runtime-v{options.RuntimeSchemaVersion}"
                    )

                if reader.GetString 1 <> "ready" then
                    failures.Add(PilotMigrationInterrupted(reader.GetString 1))

                let rootIdentity = reader.GetString 2

                if not (String.Equals(rootIdentity, options.BackupIdentity, StringComparison.OrdinalIgnoreCase)) then
                    failures.Add(PilotBackupRequiresReconciliation "backup-identity-mismatch")

                if reader.GetInt64 3 < options.MinimumGenerationFence then
                    failures.Add(PilotBackupRequiresReconciliation "generation-fence-regressed")

                let observerVersion = reader.GetInt32 4

                if observerVersion > 1 then
                    failures.Add(PilotIncompatibleDowngrade(observerVersion, 1))
                elif observerVersion < 1 then
                    failures.Add(PilotMigrationInterrupted $"pilot-v{observerVersion}-runtime-v1")

                if reader.GetString 5 <> "ready" then
                    failures.Add(PilotMigrationInterrupted(reader.GetString 5))

                if not (String.Equals(reader.GetString 6, rootIdentity, StringComparison.OrdinalIgnoreCase)) then
                    failures.Add(PilotBackupRequiresReconciliation "pilot-backup-identity-mismatch")

                if reader.GetString 7 = "on" then
                    failures.Add PilotStoreReadOnly
    ...
    let loadEvents
        ...
        task {
            use command =
                new NpgsqlCommand(
                    "SELECT sequence_number,event_id,schema_version,serializer_version,payload,payload_sha256,recorded_at FROM fsgg_orchestration.pilot_event WHERE permit_id=$1 ORDER BY sequence_number",
                    connection,
                    transaction
                )
            ...
            while reading do
                ...
                if more then
                    ...
                    if sequence <> expected || PilotStoreSupport.hash payload <> reader.GetString 5 then
                        failures.Add(PilotCorruptRecord(permitId, sequence))

                    if schemaVersion <> PilotCodec.schemaVersion then
                        failures.Add(PilotUnknownEventVersion(permitId, sequence, schemaVersion))

                    if serializerVersion <> PilotCodec.serializerVersion then
                        failures.Add(PilotUnknownSerializerVersion serializerVersion)
            ...
            use headCommand =
                new NpgsqlCommand(
                    "SELECT last_sequence FROM fsgg_orchestration.pilot_stream WHERE permit_id=$1",
                    connection,
                    transaction
                )
            ...
            if isNull headValue then
                if events.Count > 0 then
                    failures.Add(PilotCorruptRecord(permitId, tail))
            ...
            if failures.Count > 0 then
                ...
                with _ ->
                    return Error [ PilotCorruptRecord(permitId, max 1L (expected - 1L)) ]
        }
    ...
    interface IPilotJournalStore with
        member _.AppendPilot(request, cancellationToken) =
            task {
                if
                    request.PermitId = Guid.Empty
                    || request.Command.CommandId = Guid.Empty
                    || request.Command.ExpectedSequence < 0L
                then
                    return PilotInvalidAppend "invalid-pilot-command"
                else
                    ...
                    try
                        ...
                        if not gateFailures.IsEmpty then
                            raise (PilotGateException gateFailures)

                        use ensure =
                            new NpgsqlCommand(
                                "INSERT INTO fsgg_orchestration.pilot_stream(permit_id,last_sequence) VALUES($1,0) ON CONFLICT DO NOTHING",
                                connection,
                                transaction
                            )
                        ...
                        use duplicate =
                            new NpgsqlCommand(
                                "SELECT body_sha256,terminal_sequence FROM fsgg_orchestration.pilot_inbox WHERE permit_id=$1 AND command_id=$2",
                                connection,
                                transaction
                            )
                        ...
                        let duplicateResult =
                            if found then
                                ...
                                then
                                    Some(PilotDuplicate(duplicateReader.GetInt64 1))
                                else
                                    Some PilotConflict
                            else
                                None
                        ...
                        | Some result ->
                            ...
                            | Ok events when terminal > int64 events.Length ->
                                raise (PilotGateException [ PilotCorruptRecord(request.PermitId, terminal) ])
                            | Ok _ -> ()
                        ...
                        | None ->
                            use sequence =
                                new NpgsqlCommand(
                                    "SELECT last_sequence FROM fsgg_orchestration.pilot_stream WHERE permit_id=$1 FOR UPDATE",
                                    connection,
                                    transaction
                                )
                            ...
                            if actual <> request.Command.ExpectedSequence then
                                ...
                                let state =
                                    priorEvents
                                    |> List.map _.Event
                                    |> Pilot.validateReplay request.PermitId
                                    |> Result.defaultWith (fun _ ->
                                        raise (
                                            PilotGateException [ PilotCorruptRecord(request.PermitId, max 1L actual) ]
                                        ))

                                if state.Sequence <> actual then
                                    raise (PilotAppendException "pilot-state-head-mismatch")

                                let decisionState =
                                    if reconnectRequired.ContainsKey request.PermitId then
                                ...
                                | Ok events when events <> requestedEvents ->
                                    raise (PilotAppendException "events-do-not-match-command-decision")
                                | Ok _ -> ()
                                ...
                                |> List.iteri (fun index value ->
                                    if
                                        value.PermitId <> request.PermitId
                                        ...
                                        || value.RecordedAt <> request.ReceivedAt
                                    then
                                ...
                                for eventValue in request.Events do
                                    ...
                                    use insert =
                                        new NpgsqlCommand(
                                            "INSERT INTO fsgg_orchestration.pilot_event(permit_id,sequence_number,event_id,schema_version,serializer_version,payload,payload_sha256,recorded_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8)",
                                            connection,
                                            transaction
                                        )
                                ...
                                use inbox =
                                    new NpgsqlCommand(
                                        "INSERT INTO fsgg_orchestration.pilot_inbox(permit_id,command_id,body_sha256,terminal_sequence,received_at) VALUES($1,$2,$3,$4,$5)",
                                        connection,
                                        transaction
                                    )
                                ...
                                for value in
                                    [
                                        request.PermitId :> obj
                                        request.Command.CommandId :> obj
                                        bodyHash :> obj
                                        terminal :> obj
                                        request.ReceivedAt.ToUniversalTime() :> obj
                                    ] do
                                    PilotStoreSupport.add inbox value

                                let! _ = inbox.ExecuteNonQueryAsync cancellationToken

                                use update =
                                    new NpgsqlCommand(
                                        "UPDATE fsgg_orchestration.pilot_stream SET last_sequence=$2 WHERE permit_id=$1",
                                        connection,
                                        transaction
                                    )
                    ...
                    | :? PostgresException as pg when pg.SqlState = "40001" || pg.SqlState = "23505" ->
                        return PilotAppendUnavailable "concurrent-append-retry"
                    | exceptionValue ->
                        return
                            PilotAppendUnavailable(
                                match PilotStoreSupport.classify exceptionValue with
                                | PilotStoreUnavailable reason -> reason
        ...
        member _.RecoverPilot(permitId, cancellationToken) =
            task {
                if permitId = Guid.Empty then
                    return Error [ PilotStoreUnavailable "permit-id-required" ]
                else
                    try
                        use! connection = options.DataSource.OpenConnectionAsync cancellationToken
                        ...
                        | Ok events ->
                            try
                                let state =
                                    events
                                    |> List.map _.Event
                                    |> Pilot.validateReplay permitId
                                    |> Result.defaultWith (fun _ ->
                                        raise (
                                            PilotGateException
                                                [ PilotCorruptRecord(permitId, max 1L (int64 events.Length)) ]
                                        ))
                            ...
                            with _ ->
                                return Error [ PilotCorruptRecord(permitId, max 1L (int64 events.Length)) ]
                    with
