...
module private ObserverStoreSupport =
    ...
    let mapRootFailure =
        ...
        | IncompatibleDowngrade(databaseVersion, runtimeVersion) ->
            ObserverIncompatibleDowngrade(databaseVersion, runtimeVersion)
...
WHERE r.singleton FOR SHARE OF r,o
            ...
            if not found then
                return [ ObserverMigrationInterrupted "observer-or-root-metadata-missing" ]
            else
                ...
                if rootVersion > options.RuntimeSchemaVersion then
                    failures.Add(ObserverIncompatibleDowngrade(rootVersion, options.RuntimeSchemaVersion))
                elif rootVersion < options.RuntimeSchemaVersion then
                    failures.Add(
                        ObserverMigrationInterrupted $"root-v{rootVersion}-runtime-v{options.RuntimeSchemaVersion}"
                    )

                if reader.GetString 1 <> "ready" then
                    failures.Add(ObserverMigrationInterrupted(reader.GetString 1))
                ...
                if not (String.Equals(rootIdentity, options.BackupIdentity, StringComparison.OrdinalIgnoreCase)) then
                    failures.Add(ObserverBackupRequiresReconciliation "backup-identity-mismatch")

                if reader.GetInt64 3 < options.MinimumGenerationFence then
                    failures.Add(ObserverBackupRequiresReconciliation "generation-fence-regressed")

                let observerVersion = reader.GetInt32 4

                if observerVersion > 1 then
                    failures.Add(ObserverIncompatibleDowngrade(observerVersion, 1))
                elif observerVersion < 1 then
                    failures.Add(ObserverMigrationInterrupted $"observer-v{observerVersion}-runtime-v1")

                if reader.GetString 5 <> "ready" then
                    failures.Add(ObserverMigrationInterrupted(reader.GetString 5))

                if not (String.Equals(reader.GetString 6, rootIdentity, StringComparison.OrdinalIgnoreCase)) then
                    failures.Add(ObserverBackupRequiresReconciliation "observer-backup-identity-mismatch")

                if reader.GetString 7 = "on" then
                    failures.Add ObserverStoreReadOnly
    ...
    let loadEvents
        ...
        task {
            use command =
                new NpgsqlCommand(
                    "SELECT sequence_number,event_id,schema_version,serializer_version,payload,payload_sha256,recorded_at FROM fsgg_orchestration.observer_event WHERE observer_id=$1 ORDER BY sequence_number",
                    connection,
                    transaction
                )
            ...
            while reading do
                ...
                if more then
                    ...
                    if sequence <> expected || ObserverStoreSupport.hash payload <> reader.GetString 5 then
                        failures.Add(ObserverCorruptRecord(observerId, sequence))

                    if schemaVersion <> ObserverEventCodec.schemaVersion then
                        failures.Add(ObserverUnknownEventVersion(observerId, sequence, schemaVersion))

                    if serializerVersion <> ObserverEventCodec.serializerVersion then
                        failures.Add(ObserverUnknownSerializerVersion serializerVersion)
            ...
            use headCommand =
                new NpgsqlCommand(
                    "SELECT last_sequence FROM fsgg_orchestration.observer_stream WHERE observer_id=$1",
                    connection,
                    transaction
                )
            ...
            if isNull headValue then
                if events.Count > 0 then
                    failures.Add(ObserverCorruptRecord(observerId, tail))
            ...
            if failures.Count > 0 then
                ...
                with _ ->
                    return Error [ ObserverCorruptRecord(observerId, max 1L (expected - 1L)) ]
        }
    ...
    interface IObserverJournalStore with
        member _.AppendObserver(request, cancellationToken) =
            task {
                if
                    String.IsNullOrWhiteSpace request.ObserverId
                    || Id.commandValue request.Command.CommandId = Guid.Empty
                ...
                then
                    ...
                    try
                        ...
                        if not gateFailures.IsEmpty then
                            raise (ObserverGateException gateFailures)

                        use ensure =
                            new NpgsqlCommand(
                                "INSERT INTO fsgg_orchestration.observer_stream(observer_id,last_sequence) VALUES($1,0) ON CONFLICT DO NOTHING",
                                connection,
                                transaction
                            )
                        ...
                        use duplicate =
                            new NpgsqlCommand(
                                "SELECT body_sha256,terminal_sequence FROM fsgg_orchestration.observer_inbox WHERE observer_id=$1 AND command_id=$2",
                                connection,
                                transaction
                            )
                        ...
                        let duplicateResult =
                            if found then
                                ...
                                then
                                    Some(ObserverDuplicate(duplicateReader.GetInt64 1))
                                else
                                    Some ObserverConflict
                            else
                                None
                        ...
                        | Some result ->
                            ...
                            | Ok events when terminal > int64 events.Length ->
                                raise (ObserverGateException [ ObserverCorruptRecord(request.ObserverId, terminal) ])
                            | Ok _ -> ()
                        ...
                        | None ->
                            use sequence =
                                new NpgsqlCommand(
                                    "SELECT last_sequence FROM fsgg_orchestration.observer_stream WHERE observer_id=$1 FOR UPDATE",
                                    connection,
                                    transaction
                                )
                            ...
                            if actual <> request.Command.ExpectedSequence then
                                ...
                                if
                                    ...
                                    || decision.Events <> requestedEvents
                                then
                                    raise (ObserverAppendException "events-do-not-match-command-decision")

                                match requestedEvents with
                                | SessionOpened(sessionId, _, _) :: _ when
                                    ObserverJournal.observerId sessionId <> request.ObserverId
                                    ->
                                ...
                                |> List.iteri (fun index value ->
                                    if
                                        value.ObserverId <> request.ObserverId
                                ...
                                for eventValue in request.Events do
                                    ...
                                    use insert =
                                        new NpgsqlCommand(
                                            "INSERT INTO fsgg_orchestration.observer_event(observer_id,sequence_number,event_id,schema_version,serializer_version,payload,payload_sha256,recorded_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8)",
                                            connection,
                                            transaction
                                        )
                                ...
                                use inbox =
                                    new NpgsqlCommand(
                                        "INSERT INTO fsgg_orchestration.observer_inbox(observer_id,command_id,body_sha256,terminal_sequence,received_at) VALUES($1,$2,$3,$4,$5)",
                                        connection,
                                        transaction
                                    )
                                ...
                                for value in
                                    [
                                        request.ObserverId :> obj
                                        Id.commandValue request.Command.CommandId :> obj
                                        bodyHash :> obj
                                        terminal :> obj
                                        request.ReceivedAt.ToUniversalTime() :> obj
                                    ] do
                                    ObserverStoreSupport.add inbox value

                                let! _ = inbox.ExecuteNonQueryAsync cancellationToken

                                use update =
                                    new NpgsqlCommand(
                                        "UPDATE fsgg_orchestration.observer_stream SET last_sequence=$2 WHERE observer_id=$1",
                                        connection,
                                        transaction
                                    )
                    ...
                    | :? PostgresException as pg when pg.SqlState = "40001" || pg.SqlState = "23505" ->
                        return ObserverAppendUnavailable "concurrent-append-retry"
                    | exceptionValue ->
                        return
                            ObserverAppendUnavailable(
                                match ObserverStoreSupport.classify exceptionValue with
                                | ObserverStoreUnavailable reason -> reason
        ...
        member _.RecoverObserver(observerId, cancellationToken) =
            task {
                if String.IsNullOrWhiteSpace observerId then
                    return Error [ ObserverStoreUnavailable "observer-id-required" ]
                else
                    try
                        use! connection = options.DataSource.OpenConnectionAsync cancellationToken
                        ...
                        | Ok events ->
                            ...
                            with _ ->
                                return Error [ ObserverCorruptRecord(observerId, max 1L (int64 events.Length)) ]
                    with
