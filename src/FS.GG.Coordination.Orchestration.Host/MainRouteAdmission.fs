...
type MainRouteAdmissionWire =
    { Schema:string; CommandId:Guid; ProjectId:Guid; WorkflowRevision:int64; CanonicalSha256:string; CapturedAt:DateTimeOffset
      ...
      ReservationId:Guid; ReservationExpiresAt:DateTimeOffset; BudgetMaximumRuntimeSeconds:int64; BudgetExecutionDeadline:DateTimeOffset; BudgetDeliveryDeadline:DateTimeOffset
...
module MainRouteAdmission =
    ...
    let commandId (value:MainRoutePreparation) =
        let seed=$"{WorkItemIdentity.persistenceId value.Route.WorkItemId}:{value.Route.RouteId:D}:{value.Binding.BindingSha256}"
    ...
    let encode commandId (value:MainRoutePreparation) =
        let wire =
            { Schema=schema; CommandId=commandId; ProjectId=Id.projectValue value.Snapshot.ProjectId
              ...
              BudgetExecutionDeadline=value.Budget.ExecutionDeadline; BudgetDeliveryDeadline=value.Budget.DeliveryDeadline
    ...
    let decode (workItemId:WorkItemId) (principal:string) (bytes:byte array) =
        try
            ...
            if isNull(box wire)||wire.Schema<>schema||wire.CommandId=Guid.Empty||wire.ProjectId=Guid.Empty||wire.RouteId=Guid.Empty||wire.AttemptId=Guid.Empty||wire.CandidateId=Guid.Empty||wire.Generation<0L||wire.WorkflowRevision<1L
               ...
               ||wire.BudgetMaximumRuntimeSeconds<1L||wire.BudgetMaximumRuntimeSeconds>1800L||wire.BudgetExecutionDeadline<=wire.SelectedAt
               ||wire.BudgetDeliveryDeadline<wire.BudgetExecutionDeadline||wire.BudgetDeliveryDeadline>wire.SelectedAt.AddHours 2. then Error "main-route-admission-authority-refused" else
            let combined=
            ...
            combined|>Result.bind(fun(launch,binding,input,inputBytes,workspace,executionReservation)->
                if wire.BudgetExecutionDeadline>launch.RecordedAt.AddMinutes 30.
                   ||wire.BudgetDeliveryDeadline>launch.RecordedAt.AddHours 2. then Error "main-route-admission-deadline-binding-refused" else
                ...
                if not(validHostedRouteShape DateTimeOffset.UtcNow workItemId route) then Error "main-route-admission-route-refused" else
                ...
                let preparation={Snapshot={ProjectId=Id.project wire.ProjectId;WorkItemId=workItemId;WorkflowRevision=revision;CanonicalSha256=wire.CanonicalSha256;BoardMembershipIds=[];CapturedAt=wire.CapturedAt};Budget=budget;
                ...
                if wire.CommandId<>commandId preparation then Error "main-route-admission-command-binding-refused"
                elif not(ReadOnlySpan<byte>(bytes).SequenceEqual(ReadOnlySpan<byte>(encode wire.CommandId preparation))) then Error "main-route-admission-noncanonical-refused"
                else Ok preparation)
