...
type HostedWriterProviderCalls =
    {
        AcquireExternalClaim:
            HostedRoutePlan -> EffectIntent -> CancellationToken -> Task<Result<HostedEffectReadback, string>>
        DispatchRunner:
            HostedRoutePlan -> EffectIntent -> CancellationToken -> Task<Result<HostedEffectReadback, string>>
        StoreCandidate:
            HostedRoutePlan -> EffectIntent -> CancellationToken -> Task<Result<HostedEffectReadback, string>>
        ...
        MergePullRequest:
            HostedRoutePlan -> EffectIntent -> CancellationToken -> Task<Result<HostedEffectReadback, string>>
        ReadNativeDelivery:
            HostedRoutePlan -> EffectIntent -> CancellationToken -> Task<Result<NativeDeliveryReadback, string>>
    }
...
type HostedWriterProviderAdapter private (calls: HostedWriterProviderCalls) =
    ...
    member _.Dispatch(route: HostedRoutePlan, intent: EffectIntent, cancellationToken: CancellationToken) =
        task {
            let expected =
            ...
            let hostedResult (result: Result<HostedEffectReadback, string>) =
                ...
                |> Result.bind (fun readback ->
                    ...
                    then
                        ...
                        Error "provider-readback-is-not-bound-to-selected-hosted-route")
            ...
            if
                expected <> Some(intent.OperationId, intent.ResourceId)
                || intent.Generation <> route.Generation
                || intent.WorkflowRevision <> route.WorkflowRevision
                || not (validSha256 intent.PayloadSha256)
            then
                ...
                | ReleaseExternalClaim
                | CancelRunner
                | InspectExternalOperation -> return Error "effect-kind-is-outside-hosted-writer-capability"
        }
...
module HostedWriterJournal =
    ...
    let recover (store: IJournalStore) workItemId cancellationToken =
        task {
                        ...
                        Ok
                            {
                                ...
                                Sequence =
                                    value.Events |> List.tryLast |> Option.map _.Sequence |> Option.defaultValue 0L
                                State = replay events
    ...
    let decideAndAppend (clock: TimeProvider) (store: IJournalStore) workItemId envelope cancellationToken =
        task {
    ...
    let persistStartupPause (clock: TimeProvider) (store: IJournalStore) workItemId principalId cancellationToken =
        task {
            let! recovered = recover store workItemId cancellationToken
