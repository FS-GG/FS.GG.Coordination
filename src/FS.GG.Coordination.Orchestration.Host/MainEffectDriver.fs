...
type MainEffectDriver
    (
        ...
        principalId: string,
        providers: HostedWriterProviderAdapter,
        reconcile:
            HostedRoutePlan -> EffectIntent -> CancellationToken -> Task<Result<HostedWriterProviderReadback, string>>,
        ?advance:
            HostedRoutePlan
                -> EffectIntent
                -> HostedWriterProviderReadback
                -> CancellationToken
    ...
    let authorityCurrent now (intent: EffectIntent) (state: State) =
        let subscriptionDeadline (budget: SubscriptionExecutionBudget) =
            ...
            | EffectKind.AcquireExternalClaim
            | EffectKind.DispatchRunner -> budget.ExecutionDeadline
        ...
        && ((state.Budget
            ...
            || (state.SubscriptionBudget
                |> Option.exists (fun budget ->
                    ...
                    && now < subscriptionDeadline budget)))
    ...
    let derivedCommandId (operationId: OperationId) stage (revision: WorkflowRevision) =
        ...
        let bytes =
            SHA256.HashData(Encoding.UTF8.GetBytes($"{seed:D}:{stage}:{Id.revisionValue revision}"))

        Id.command (Guid(ReadOnlySpan(bytes, 0, 16)))

    let append state operationId stage command cancellationToken =
        task {
            let now = clock.GetUtcNow()

            let envelope =
                {
                    CommandId = derivedCommandId operationId stage state.Revision
            ...
            let! result =
                HostedWriterJournal.decideAndAppend clock store workItemId envelope cancellationToken
            ...
            return
                ...
                |> Result.bind (fun (decision, sequence) ->
                    ...
                    | ReceiptDisposition.Duplicate -> Ok sequence
                    | _ -> Error decision.Receipt.Detail)
        }

    member this.Drive(operationId: OperationId, cancellationToken: CancellationToken) =
        task {
                ...
                | Some route, Some(OperationState.IntentRecorded intent) ->
                    ...
                    if not (authorityCurrent (clock.GetUtcNow()) intent current.State) then
                        ...
                        | Ok() ->
                            ...
                            | Error failures -> return EffectDriveRefused(sprintf "%A" failures)
                            | Ok latest when not (authorityCurrent (clock.GetUtcNow()) intent latest.State) ->
                                return EffectDriveRefused "effect-authority-not-current"
                            | Ok latest ->
                                let! marked =
                                    append
                                    ...
                                    | Ok next ->
                                        return!
                                            this.RecordObservation(
                                                ...
                                                intent,
                                                next.State,
                                                providers.Dispatch(route, intent, cancellationToken),
                                                cancellationToken
                                            )
                | Some route, Some(OperationState.Dispatching intent) ->
                    ...
                    return!
                        this.RecordObservation(
                            ...
                            intent,
                            current.State,
                            reconcile route intent cancellationToken,
                            cancellationToken
                        )
    ...
    member private _.RecordObservation
        (
            route: HostedRoutePlan,
            intent: EffectIntent,
            state: State,
            pending: Task<Result<HostedWriterProviderReadback, string>>,
            cancellationToken
        ) =
        task {
            ...
            | Ok(HostedEffect readback) ->
                let! appended =
                    append
                        ...
                        intent.OperationId
                        "hosted-readback"
                        (RecordHostedEffectReadback(intent.OperationId, readback))
                        cancellationToken
                ...
                | Ok sequence ->
                    ...
                    return
                        match advanced with
                        | Ok() -> EffectCompleted(intent.OperationId, sequence)
                        | Error reason -> EffectDriveRefused reason
            | Ok(NativeDelivery readback) ->
                let! appended =
                    append
                        ...
                        intent.OperationId
                        "native-readback"
                        (RecordNativeDeliveryReadback(intent.OperationId, readback))
                        cancellationToken
            ...
            | Error reason ->
                let needsReconciliation sequence =
                    if reason.StartsWith("github-", StringComparison.Ordinal) then
                        EffectNeedsExternalReconciliation(intent.OperationId, sequence)
                    else
                        EffectNeedsReconciliation(intent.OperationId, sequence)

                match Map.tryFind intent.OperationId state.Operations with
                | Some(OperationState.NeedsObservation _) ->
                    return needsReconciliation (Id.revisionValue state.Revision)
                | _ ->
                    let! appended =
                        append
                            state
                            intent.OperationId
                            "unknown"
                            (ObserveEffect(intent.OperationId, Unknown reason))
                            cancellationToken

                    return
                        match appended with
                        | Ok sequence -> needsReconciliation sequence
                        | Error appendReason -> EffectDriveRefused appendReason
        }
