        ...
        let bytes =
            SHA256.HashData(
                Encoding.UTF8.GetBytes(
                    $"{WorkItemIdentity.persistenceId workItemId}:{route}:{attempt}:{generationValue}:{stage}"
                )
            )
...
type MainRouteWorkflow
    ...
    let legacyCommandId stage =
        let bytes =
            SHA256.HashData(Encoding.UTF8.GetBytes($"{WorkItemIdentity.persistenceId workItemId}:{stage}"))
    ...
    let commandId (value: MainRoutePreparation) stage =
        MainRouteWorkflowIdentity.commandId
            workItemId
            value.Route.RouteId
            value.Route.AttemptId
            value.Route.Generation
            stage

    let append value stage command token =
        task {
            ...
            | Ok current ->
                ...
                let protocol =
                    match command with
                    | AdmitSubscription _
                    | RecordSubscriptionAccounting _ -> Id.protocolVersion 2 0
                    | _ -> Id.protocolVersion 1 0

                let envelope =
                    {
                        CommandId = commandId value stage
    ...
    let nextIntent (route: HostedRoutePlan) payload kind =
        ...
        | AcquireExternalClaim ->
            Some(
                effect route payload DispatchRunner route.ProcessOperationId (string (Id.attemptValue route.AttemptId))
            )
        | DispatchRunner ->
            Some(
                effect
                    ...
                    (string (Id.candidateValue route.CandidateId))
            )
        ...
        | PublishCandidateBranch ->
            Some(effect route payload CreatePullRequest route.PullRequestOperationId route.BranchRef)
    ...
    let runIf value condition stage command token =
        task {
            if condition then
                return! append value stage command token
            else
                return Ok()
        }

    let validatePausedBinding (value: MainRoutePreparation) token =
        task {
            let! durableRoute =
                executions.ReadRoute(value.LaunchIntent.Key.AssignmentId, value.LaunchIntent.Key.AttemptId, token)

            let! durableAttempt =
                executionJournal.ReadAttempt(
                    value.LaunchIntent.Key.AssignmentId,
                    value.LaunchIntent.Key.AttemptId,
                    token
                )

            let! durableSubscription =
                executions.ReadSubscription(value.ExecutionReservation.ReservationId, token)
            ...
            let! durableJournal =
                workItems.Recover(WorkItemIdentity.persistenceId workItemId, token)
            ...
            let launchBound =
                ...
                |> Option.exists (fun state -> state.Intent = value.LaunchIntent)

            let subscriptionBound =
                durableSubscription
                |> Result.toOption
                |> Option.bind (fun (bytes, _) ->
                    SubscriptionAccountingCodec.decodeReservation bytes |> Result.toOption)
                |> Option.exists ((=) value.ExecutionReservation)

            let inputBound =
                durableInput
                |> Result.toOption
                |> Option.exists (fun bytes ->
                    ReadOnlySpan<byte>(bytes).SequenceEqual(ReadOnlySpan<byte>(value.InputBytes)))

            let workspaceBound =
                durableWorkspace
                |> Result.toOption
                |> Option.exists (fun bytes ->
                    ReadOnlySpan<byte>(bytes).SequenceEqual(ReadOnlySpan<byte>(workspaceBytes)))
            ...
            | Ok recovery ->
                ...
                let failures =
                    [
                        ...
                        "launch-intent", launchBound
                        "subscription", subscriptionBound
                        "input", inputBound
                ...
                return
                    if failures.IsEmpty then
                        ...
                        Error($"main-route-paused-recovery-binding-refused:{failureDetail}")
        }

    member _.Prepare(value: MainRoutePreparation, token: CancellationToken) =
        task {
            let bindingFailures =
                [
                    "work-item", value.Route.WorkItemId = workItemId
                    "route-assignment",
                    Id.operationValue value.Route.ProcessOperationId = value.LaunchIntent.Key.AssignmentId
                    ...
                    value.ExecutionReservation.AssignmentId = value.LaunchIntent.Key.AssignmentId
            ...
            if not bindingFailures.IsEmpty then
                return Error($"main-route-preparation-binding-refused:{bindingFailureDetail}")
            else
                ...
                let recoverable =
                    beforePreparation
                    |> Result.exists (fun current ->
                        current.HostedRoute.IsNone
                ...
                let mutable preflightFailure =
                    match beforePreparation with
                    | Ok _ -> None
                ...
                if recoverable then
                    let priorRoute =
                        { value.Route with
                            BranchRef = value.Route.BranchRef.Replace("refs/heads/fsgg/pilot/", "refs/heads/fsgg/")
                        }

                    let! recoveredRoute =
                        append
                            value
                            "recover-select-route"
                            (RecoverHostedRoute(rejectedCommandId, priorRoute, value.Route))
                            token

                    match recoveredRoute with
                    | Error reason -> preflightFailure <- Some reason
                    | Ok() -> ()
                ...
                let preAdmitted =
                    ...
                    && (prepared
                        |> Result.exists (fun (current: State) ->
                            current.Snapshot = Some value.Snapshot
                            && current.SubscriptionBudget = Some value.Budget
                            && current.Generation = value.Route.Generation
                ...
                let refused =
                    preflightFailure |> Option.defaultValue "main-route-pre-admission-required"
                ...
                let! input, workspace =
                    if preAdmitted then
                        task {
                            let! input =
                                executions.StageInput(
                                    ExecutorWire.encodeInputManifest value.InputManifest,
                                    value.InputBytes,
                                    token
                                )

                            let! workspace =
                                executions.StageWorkspaceManifest(
                                    ExecutorWire.encodeWorkspaceManifest value.WorkspaceManifest,
                                    token
                                )
                        ...
                        Task.FromResult((Error refused, Error refused))
                ...
                | Ok(), Ok _ ->
                    ...
                    if failure.IsNone then
                        let! intentStored =
                            executionJournal.AppendAttempt(
                                ...
                                value.LaunchIntent.Key.AttemptId,
                                0L,
                                LaunchIntentRecorded value.LaunchIntent,
                                token
                            )

                        match intentStored with
                        | AppendConflict -> failure <- Some "main-route-launch-intent-conflict"
                        | Appended
                        | DuplicateEvent -> ()
                    ...
                    if failure.IsNone then
                        let! reserved =
                            executions.ReserveSubscription(
                                SubscriptionAccountingCodec.encodeReservation value.ExecutionReservation,
                                1,
                                1,
                                token
                            )
                    ...
                    if failure.IsSome && subscriptionReservationAcquired then
                        ...
                        try
                            ...
                            let! released =
                                executions.ReleaseSubscription(
                                    value.ExecutionReservation.ReservationId,
                                    value.ExecutionReservation.AttemptId,
                                    value.ExecutionReservation.Generation,
                                    cleanup.Token
                                )
                            ...
                            | SubscriptionReleaseConflict ->
                                failure <- Some($"%s{originalFailure};main-route-subscription-release-conflict")
                        with error ->
                            failure <-
                                Some(
                                    $"%s{originalFailure};main-route-subscription-release-failed:%s{error.GetType().Name}"
                                )
                    ...
                    if failure.IsNone then
                        ...
                        let! result =
                            runIf
                                value
                                (current |> Result.exists (fun state -> not state.ReadbackCurrent))
                                "route-readback"
                                (RecordHostedRouteReadback value.Readback)
                                token
                    ...
                    if failure.IsNone then
                        ...
                        let! result =
                            runIf
                                value
                                (current |> Result.exists (fun state -> state.Control <> ControlState.Running))
                                "resume"
                                Command.Resume
                                token
                    ...
                    if failure.IsNone then
                        ...
                        let! result =
                            runIf
                                value
                                (current |> Result.exists _.Reservation.IsNone)
                                "reserve"
                                (Reserve(
                                    value.Reservation.ReservationId,
                                    value.Reservation.ExpiresAt,
                                    value.Reservation.RequiredClaimIds
                                ))
                                token
                    ...
                    if failure.IsNone then
                        let claim =
                            effect
                                value.Route
                                value.Binding.BindingSha256
                                AcquireExternalClaim
                                value.Route.ClaimOperationId
                                value.Route.ClaimResourceId

                        let! current = state token

                        let shouldRecord =
                            current
                            |> Result.exists (fun state ->
                                not (state.Operations.ContainsKey value.Route.ClaimOperationId))

                        let! result =
                            runIf value shouldRecord "intent-claim" (RecordEffectIntent claim) token
    ...
    member _.ValidatePausedBinding(value: MainRoutePreparation, token: CancellationToken) =
        task {
            let! result = validatePausedBinding value token
            return result |> Result.map ignore
        }

    member _.ReconnectPaused(value: MainRoutePreparation, readback: HostedRouteReadback, token: CancellationToken) =
        task {
            ...
            | Ok current ->
                ...
                if not bound then
                    return Error "main-route-paused-recovery-binding-refused"
                else
                    return!
                        append
                            value
                            $"reconnect-route-readback-{Id.revisionValue current.Revision}"
    ...
    member _.Advance
        (
            value: MainRoutePreparation,
            intent: EffectIntent,
            candidateReceipt: CandidateStorageReceipt option,
        ...
        task {
            ...
            | AcquireExternalClaim ->
                let claim =
                    {
                        ClaimId = value.Route.ClaimResourceId
                        Generation = value.Route.Generation
                        WorkflowRevision = value.Route.WorkflowRevision
                ...
                | Ok() ->
                    let! started =
                        append
                            value
                            "start-attempt"
                            (StartAttempt(value.Route.AttemptId, value.SessionId, value.Runner))
                            token

                    match started, nextIntent value.Route payload intent.Kind with
                    | Error reason, _ -> return Error reason
                    | Ok(), Some next -> return! append value "intent-dispatch" (RecordEffectIntent next) token
            ...
            | StoreCandidate ->
                ...
                | Ok candidate, Some receipt when receipt.CandidateId = candidate.Candidate.CandidateId ->
                    let! recorded =
                        append value "record-candidate" (RecordCandidate(candidate.Candidate, receipt)) token

                    match recorded, nextIntent value.Route payload intent.Kind with
                    | Error reason, _ -> return Error reason
    ...
    member this.RecoverContinuation(value: MainRoutePreparation, token: CancellationToken) =
        task {
            let! current = state token
            ...
            | Ok state ->
                let ordered =
                    [
                        value.Route.ClaimOperationId
                        value.Route.ProcessOperationId
                        value.Route.CandidateOperationId
                        value.Route.BranchOperationId
                        value.Route.PullRequestOperationId
                        value.Route.MergeOperationId
                        value.Route.ReadbackOperationId
                    ]

                let pendingNext index =
                    index = ordered.Length - 1
                    || not (state.Operations.ContainsKey ordered[index + 1])

                let settled: EffectIntent option =
                    ordered
                    |> List.indexed
                    |> List.tryPick (fun (index, id) ->
                        match Map.tryFind id state.Operations with
                        | Some(OperationState.Settled(intent, Applied _)) when pendingNext index -> Some intent
                        | _ -> None)
