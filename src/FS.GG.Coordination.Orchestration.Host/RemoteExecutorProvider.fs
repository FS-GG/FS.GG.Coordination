...
type IExecutorBindingResolver =
    ...
    abstract ResolveSession:
        ProviderSessionReference * CancellationToken ->
            Task<Result<LaunchIntent * ResolvedExecutorRouteBinding, string>>
...
type PostgreSqlExecutorBindingResolver
        ...
        task {
            ...
            | Some stream, Ok bytes ->
                ...
                | Some state, Ok route when
                    ...
                    && route.Generation = state.Intent.Key.Generation
                    && route.PromptDigest = state.Intent.InputDigest
                    ->
                    ...
                    | Ok inputBytes, Ok workspaceBytes ->
                        ...
                        | Ok workspaceManifest when
                            workspaceManifest.InputDigest = state.Intent.InputDigest
                            ...
                            return
                                Ok(
                                    state.Intent,
                                    {
                                        WorkItemPersistenceId = route.WorkItemPersistenceId
                                        RouteOperationId = route.ProcessOperationId
                                        CandidateId = route.CandidateId
                                        ExpectedRevision = stream.Revision
                                        ExecutorBinding = route.ExecutorBinding
    ...
    interface IExecutorBindingResolver with
        member _.ResolveReadiness token =
            resolve readinessAssignment readinessAttempt None token

        member _.Resolve(intent, token) =
            task {
                let! value = resolve intent.Key.AssignmentId intent.Key.AttemptId (Some intent) token in
                return value |> Result.map snd
            }

        member _.ResolveSession(session, token) =
            task {
                let! found =
                    store.FindAttemptBySession(ProviderSessionReference.value session, token)

                match found with
                | Error reason -> return Error reason
                | Ok(assignment, attempt) -> return! resolve assignment attempt None token
            }

[<RequireQualifiedAccess>]
module RemoteCandidatePipeline =
    let store
        (candidateStore: ICandidateStore)
        expectedCandidate
        expectedBaseline
        retainUntil
        (readback: ExecutorTransportReadback)
        cancellationToken
        =
        task {
            let artifactManifest =
                readback.Frames
                |> List.tryFind (fun bytes -> ExecutorWire.parseArtifactManifest bytes |> Result.isOk)

            let artifactChunks =
                readback.Frames
                |> List.filter (fun bytes -> ExecutorWire.parseArtifactContent bytes |> Result.isOk)
            ...
            | Some bytes ->
                ...
                | Error reason -> return Error reason
                | Ok manifest when
                    manifest.CandidateId <> expectedCandidate
                    || manifest.BaselineObjectId <> expectedBaseline
                    ->
                    return Error "executor-artifact-identity-refused"
                | Ok manifest ->
                    ...
                    | None ->
                        let ordered =
                            ...
                            |> List.choose (function
                                | Ok(value, chunk) -> Some(value, chunk)
                                | _ -> None)
                            |> List.sortBy (fun (value, _) -> value.Offset)
                        ...
                        for value, chunk in ordered do
                            if
                                ...
                                || value.BundleSha256 <> manifest.BundleSha256
                                || value.Offset <> offset
                            then
                                valid <- false
                            else
                        ...
                        let finalPresent =
                            ordered |> List.tryLast |> Option.exists (fun (value, _) -> value.Final)

                        if
                            not valid
                            || offset <> manifest.BundleSizeBytes
                            || RunnerWire.sha256 bundle <> manifest.BundleSha256
                            || not finalPresent
                        then
                            return Error "executor-artifact-content-refused"
                        else
                            let candidate =
                                {
                                    CandidateId = Id.candidate manifest.CandidateId
                                    BaselineSha = manifest.BaselineObjectId
                                    HeadSha = manifest.HeadObjectId
                                    TreeSha = manifest.TreeObjectId
                                    ManifestSha256 = manifest.ManifestSha256
                                    ContentSha256 = manifest.BundleSha256
                                    MediaType = "application/vnd.fsgg.runner-candidate+zip"
                                    SizeBytes = manifest.BundleSizeBytes
                                    RetainUntil = retainUntil
                                    Location = ContentAddressedObject($"sha256/{manifest.BundleSha256}")
                                }
                            ...
                            let receipt =
                                match put with
                                | Ok value
                                | Error(Existing value) -> Some value
                                | _ -> None
                            ...
                            | Some receipt ->
                                ...
                                return
                                    match verified with
                                    | Ok value when value.Candidate = candidate && value.Bytes = bundle ->
                                        Ok(candidate, receipt)
                                    | _ -> Error "candidate-storage-readback-refused"
        }

[<Sealed>]
type RemoteExecutorProvider
    (store: IExecutorCommandStore, resolver: IExecutorBindingResolver, transport: IAuthenticatedExecutorTransport) =
    ...
    let observation (response: ExecutorResponse) =
        ...
        | Ok session ->
            let lifecycle =
                ...
                | "starting" -> Some SessionLifecycle.Starting
                | "running" -> Some SessionLifecycle.Running
                | "cancelling" -> Some SessionLifecycle.Cancelling
                | "succeeded" -> Some SessionLifecycle.Succeeded
                | "failed" -> Some SessionLifecycle.Failed
                | "cancelled" -> Some SessionLifecycle.Cancelled
                | "deadline-exceeded" -> Some SessionLifecycle.DeadlineExceeded
                | "outcome-unknown" -> Some SessionLifecycle.OutcomeUnknown
                | _ -> None
                ...
                let usage =
                    ...
                    |> Array.map (fun item ->
                        ...
                        (match item.State with
                         | "observed" -> UsageKnown(item.Value.Value, item.UnitName, item.Provenance)
                         | "not-applicable" -> UsageNotApplicable item.Provenance
                         | _ -> UsageUnknown item.Provenance))
                    |> Map.ofArray

                let cost =
                    match response.InvocationCostState with
                    | "known" ->
                        CostKnown(
                            response.InvocationCostAmount.Value,
                            response.InvocationCostCurrency,
                            response.InvocationCostProvenance
                        )
                    | "not-applicable" -> CostNotApplicable response.InvocationCostProvenance
                    | _ -> CostUnknown response.InvocationCostProvenance
                ...
                let refs (values: ExecutorReference array) : OutputReference list =
                    ...
                    |> Array.map (fun value ->
                        ({
                            ...
                            Digest = nullableText value.Digest
                        }
                        : OutputReference))
                    |> List.ofArray

                let candidate =
                    if response.CandidateId = Guid.Empty then
                        None
                    else
                        Some
                            {
                                CandidateId = response.CandidateId
                                HeadSha = response.CandidateHeadSha
                                TreeSha = response.CandidateTreeSha
                            }
                ...
                Ok
                    {
                        ...
                        Resolved =
                            {
                                Model = nullableText response.ResolvedModel
                                Effort = nullableText response.ResolvedEffort
                            }
                        Lifecycle = lifecycle
                        Output = refs response.Output
                        LifecycleReferences = refs response.LifecycleReferences
                        Usage = { Values = usage; Cost = cost }
                        Candidate = candidate
    ...
    let command
        kind
        (intent: LaunchIntent)
        (binding: ResolvedExecutorRouteBinding)
        session
        artifactDigest
        offset
        length
        ...
        let value =
            {
                ...
                BodySha256 = ""
                Kind = kind
                WorkItemPersistenceId = binding.WorkItemPersistenceId
                RouteOperationId = binding.RouteOperationId
                ...
                WorkspaceManifestSha256 = (RunnerWire.serialize binding.WorkspaceManifest |> RunnerWire.sha256)
                RequestedModel = Option.toObj intent.Requested.Model
                RequestedEffort = Option.toObj intent.Requested.Effort
                InputDigest = intent.InputDigest
                ExecutorBinding = binding.ExecutorBinding
    ...
    let invoke kind intent binding session token =
        task {
            ...
            let! manifestPrepared =
                store.StageWorkspaceManifest(ExecutorWire.encodeWorkspaceManifest binding.WorkspaceManifest, token)
            ...
            | CommandDuplicate _ ->
                ...
                | Ok value ->
                    ...
                    let receipts =
                        value.Frames
                        |> List.choose (fun bytes -> ExecutorWire.parseReceipt bytes |> Result.toOption)

                    let outcome =
                        value.Frames
                        |> List.choose (fun bytes -> ExecutorWire.parseOperationOutcome bytes |> Result.toOption)
                        |> List.tryLast
                        ...
                        |> List.forall (fun manifest ->
                            ...
                            && response
                               |> Option.exists (fun item ->
                                   item.CandidateId = manifest.CandidateId
                                   && item.CandidateHeadSha = manifest.HeadObjectId
                                   && item.CandidateTreeSha = manifest.TreeObjectId))

                    let valid =
                        artifactsBound
                        && receipts |> List.forall (fun item -> bound item.CommandId item.BodySha256)
                        && outcome |> Option.forall (fun item -> bound item.CommandId item.BodySha256)
                        && response |> Option.forall (fun item -> bound item.CommandId item.BodySha256)

                    let terminal = value.Frames |> List.tryLast

                    if not valid || terminal.IsNone then
                        return Error "executor-readback-binding-refused"
                    else
                        observedArtifacts
                        |> List.iter (fun manifest -> artifacts[intent.Key] <- manifest)
    ...
    interface IExecutionProvider with
        member _.ObserveReadiness token =
            task {
                    ...
                    return
                        {
                            Identity =
                                {
                                    Provider = "remote-execution-provider"
                                    AdapterVersion = "1"
                                }
                            Authentication = AuthenticationUnknown reason
                ...
                | Ok(intent, binding) ->
                    ...
                    | Ok response ->
                        let authentication =
                            match response.AuthenticationState with
                            | "authenticated" -> Authenticated response.AuthenticationProvenance
                            | "not-authenticated" -> NotAuthenticated response.AuthenticationProvenance
                            | _ -> AuthenticationUnknown response.AuthenticationProvenance
            ...
            task {
                ...
                | Ok binding ->
                    ...
                    | Ok() ->
                        ...
                        match manifest with
                        | Error reason -> return LaunchRefused reason
                        | Ok digest when digest <> (RunnerWire.serialize binding.WorkspaceManifest |> RunnerWire.sha256) ->
                            return LaunchRefused "executor-workspace-manifest-digest-refused"
                        | Ok _ ->
                            ...
                            | Ok(_, Some response, _) ->
                                ...
                                | Ok value -> return LaunchStarted value
                                | Error reason -> return LaunchAmbiguous reason
                            | Ok(Some outcome, None, _) when outcome.Disposition = "refused" ->
                                return LaunchRefused outcome.Reason
        ...
        member _.Reconcile(intent, token) =
            task {
                let! resolved = resolver.Resolve(intent, token)
                ...
                | Ok binding ->
                    ...
                    | Ok(_, Some response, _) ->
                        match observation response with
                        | Ok value -> return Reconciled value
                        | Error reason -> return ReconcileUnknown reason
        ...
        member _.Observe(session, token) =
            task {
                ...
                | Ok(intent, binding) ->
                    let! invoked =
                        invoke "observe" intent binding (ProviderSessionReference.value session) token

                    return
                        invoked
                        |> Result.bind (fun (_, response, _) ->
                            match response with
                            | Some value -> observation value
                            | None -> Error "executor-observation-missing")
            }

        member _.Cancel(session, token) =
            task {
                let! resolved = resolver.ResolveSession(session, token)
                ...
                | Ok(intent, binding) ->
                    let! invoked =
                        invoke "cancel" intent binding (ProviderSessionReference.value session) token
                    ...
                    | Ok(Some outcome, _, _) ->
                        return
                            ...
                            | "refused" -> CancelRefused outcome.Reason
                            | _ -> CancelUnknown outcome.Reason
                    | Ok(None, _, _) -> return CancelUnknown "executor-cancel-terminal-readback-missing"
            }

    member _.ReadCandidate(intent: LaunchIntent, manifest: ExecutorArtifactManifest, token: CancellationToken) =
        task {
            let! resolved = resolver.Resolve(intent, token)
            ...
            | Ok binding ->
                ...
                while failure.IsNone && offset < manifest.BundleSizeBytes do
                    let length =
                        int (min (int64 ExecutorWire.maximumContentBytes) (manifest.BundleSizeBytes - offset))
                    ...
                    match persisted with
                    | CommandConflict
                    | CommandRefused _ -> failure <- Some "executor-content-command-persistence-refused"
                    | CommandPersisted _
                    | CommandDuplicate _ ->
                        ...
                        | Ok value ->
                            let chunk =
                                value.Frames
                                |> List.choose (fun bytes -> ExecutorWire.parseArtifactContent bytes |> Result.toOption)
                                |> List.tryExactlyOne

                            let outcome =
                                value.Frames
                                |> List.choose (fun bytes ->
                                    ExecutorWire.parseOperationOutcome bytes |> Result.toOption)
                                |> List.tryLast

                            match chunk, outcome with
                            | Some(item, bytes), Some doneValue when
                                item.CommandId = commandValue.CommandId
                                && item.Offset = offset
                                ...
                                let! settled =
                                    store.SettleCommand(
                                        ...
                                        ExecutorWire.encodeOperationOutcome doneValue,
                                        token
                                    )
                                ...
                                | Error reason -> failure <- Some reason
                                | Ok() ->
                                    output.Add(ExecutorWire.encodeArtifactContent item)
                ...
                return
                    ...
                    | None -> Ok { Frames = List.ofSeq output }
        }

    member this.ReadCandidate(intent: LaunchIntent, token: CancellationToken) =
        task {
