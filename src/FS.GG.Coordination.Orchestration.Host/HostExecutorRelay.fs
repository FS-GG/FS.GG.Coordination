...
type HostExecutorRelay(maximumPending: int, maximumAggregateBytes: int) =
    ...
    let bounded (frames: byte array list) =
        ...
        && frames
           |> List.forall (fun value ->
               not (isNull value)
               && value.Length > 0
               && value.Length <= 2 * ExecutorWire.maximumContentBytes)
        && (frames |> List.sumBy (fun value -> int64 value.Length))
    ...
    let sameFrames (left: byte array list) (right: byte array list) =
        ...
        && List.forall2
            (fun (left: byte array) (right: byte array) -> left.AsSpan().SequenceEqual(right.AsSpan()))
            left
            right
    ...
    let remember id requestDigest result resultDigest =
        ...
        while completed.Count > maximumPending do
            completed.Remove(completedOrder.Dequeue()) |> ignore

    member _.Poll(cancellationToken: CancellationToken) =
        task {
    ...
    member _.Complete(commandId: Guid, frames: byte array list) =
        if not (bounded frames) then
            Error "executor-relay-response-bounds-refused"
        else
    ...
    member _.Fail(commandId: Guid, reason: string) =
        if String.IsNullOrWhiteSpace reason || reason.Length > 1024 then
            Error "executor-relay-failure-refused"
        else
            lock syncRoot (fun () ->
                match pending.TryGetValue commandId with
                | true, entry ->
    ...
    interface IAuthenticatedExecutorTransport with
        member _.Exchange(frames, cancellationToken) =
            task {
                if
                    maximumPending < 1
                    || maximumPending > 64
                    ...
                    || not (bounded frames)
                then
                    return Error "executor-relay-request-bounds-refused"
                else
                    ...
                    | Some id ->
                        let entry =
                            {
                                Request = { CommandId = id; Frames = frames }
                                Completion =
                                    TaskCompletionSource<Result<ExecutorTransportReadback, string>>(
                        ...
                        let selected =
                            lock syncRoot (fun () ->
                                ...
                                | true, doneEntry when doneEntry.RequestDigest = digest frames ->
                                    Choice1Of3 doneEntry.Result
                                ...
                                | _ ->
                                    ...
                                    | true, _ -> Choice2Of3 "executor-relay-command-identity-conflict"
                                    | _ when pending.Count >= maximumPending ->
                                        Choice2Of3 "executor-relay-capacity-refused"
                                    | _ ->
                        ...
                        | Choice3Of3 current ->
                            try
                                return! current.Completion.Task.WaitAsync cancellationToken
                            with :? OperationCanceledException ->
                                return Error "executor-relay-response-unknown"
            }
