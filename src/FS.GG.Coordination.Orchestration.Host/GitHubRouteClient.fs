...
type IGitHubRequestExecutor =
    abstract Send: GitHubRequest * CancellationToken -> Task<TransportOutcome>

type IGitCandidatePublisher =
    abstract Publish:
        string * string * string option * string * byte array * CancellationToken -> Task<Result<string, string>>
...
type HttpGitHubRequestExecutor(client: HttpClient, token: string, maximumResponseBytes: int) =
    ...
    let delayUntilAllowed (ct: CancellationToken) =
        task {
    ...
    let updatePacing status (headers: Map<string, string>) =
        ...
        let remaining =
            header "X-RateLimit-Remaining" headers
            |> Option.bind (fun value ->
        ...
        let resetAt =
            header "X-RateLimit-Reset" headers
            |> Option.bind (fun value ->
        ...
        let limited =
            if status = 429 || status = 403 then
                retryAfter |> Option.orElse resetAt |> Option.defaultValue (now.AddSeconds 15.)
            elif remaining |> Option.exists (fun value -> value <= 1) then
                resetAt |> Option.defaultValue (now.AddSeconds 15.)
            elif remaining.IsNone then
                conservative
            else
                now
    ...
    interface IGitHubRequestExecutor with
        member _.Send(request, ct) =
            task {
                try
                    ...
                    try
                        ...
                        | Ok() ->
                            ...
                            use! response =
                                client.SendAsync(message, HttpCompletionOption.ResponseHeadersRead, ct)
                            ...
                            while reading do
                                ...
                                if count = 0 then
                                    reading <- false
                                elif total + count > maximumResponseBytes then
                                    overflow <- true
                                    reading <- false
                                else
                            ...
                            if overflow then
                                ...
                                let headerMap =
                                    Seq.append (response.Headers :> seq<_>) (response.Content.Headers :> seq<_>)
                                    |> Seq.map
                                        (fun
                                            (item: KeyValuePair<string, System.Collections.Generic.IEnumerable<string>>) ->
                                            item.Key, String.concat "," item.Value)
                                    |> Map.ofSeq

                                updatePacing (int response.StatusCode) headerMap

                                let tryInt name =
                                    header name headerMap
                                ...
                                let tryTime name =
                                    ...
                                    |> Option.bind (fun text ->
                                        ...
                                        | true, value -> Some(DateTimeOffset.FromUnixTimeSeconds value)
                                        | _ -> None)
                                    ...
                                    Response
                                        {
                                            ...
                                            ETag = response.Headers.ETag |> Option.ofObj |> Option.map string
                                            RateBudget =
                                                {
                                                    Limit = tryInt "X-RateLimit-Limit"
                                                    Remaining = tryInt "X-RateLimit-Remaining"
                                                    ResetAt = tryTime "X-RateLimit-Reset"
                                                    Cost = None
                                                }
                                        }
                    finally
                ...
                with
                | :? OperationCanceledException -> return TimedOut
                | _ -> return NetworkFailure
            }
...
type GitBundlePublisher(remoteUri: Uri, token: string, maximumOutputBytes: int) =
    let run (cwd: string) (arguments: string list) (cancellationToken: CancellationToken) =
        task {
            let start =
                ProcessStartInfo(
                    "git",
                ...
                task {
                    ...
                    while not doneReading do
                        ...
                        if count = 0 then
                            ...
                            if remaining > 0 then
                                kept.Append(buffer, 0, min count remaining) |> ignore
            ...
            if stdoutOverflow || stderrOverflow then
                return Error "git-output-bounds-refused"
                ...
                return
                    Error(
                        "git-publish-refused:"
                        + (if stderr.Length > 256 then stderr[..255] else stderr).Trim()
                    )
        }

    interface IGitCandidatePublisher with
        member _.Publish(_, branchRef, expectedHead, headSha, bundle, cancellationToken) =
            task {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-candidate-publish-" + Guid.NewGuid().ToString("N"))
                        ...
                        | Ok _ ->
                            ...
                            | Ok _ ->
                                let! observed =
                                    run
                                        repository
                                        [ "ls-remote"; "--refs"; remoteUri.AbsoluteUri; branchRef ]
                                        cancellationToken

                                return
                                    observed
                                    |> Result.bind (fun value ->
                                        let fields = value.Trim().Split('\t') in

                                        if fields.Length = 2 && fields[0] = headSha then
                                            Ok headSha
                                        else
                                            Error "git-publish-readback-refused")
                finally
                    try
...
[<Sealed>]
type GitHubRouteClient
    (executor: IGitHubRequestExecutor, publisher: IGitCandidatePublisher, target: GitHubRouteTarget, clock: TimeProvider)
    =
        ...
        Rest
            {
                ...
                Idempotency =
                    (match key with
                     | Some value -> ReplayWithKey value
                     | None -> ReplaySafe)
            }

    let send
        (methodValue: RestMethod)
        (path: string)
        (body: string option)
        (key: string option)
        (ct: CancellationToken)
        : Task<Result<ResponseEnvelope, string>> =
        task {
            if String.IsNullOrWhiteSpace owner || String.IsNullOrWhiteSpace repo then
                return Error "github-repository-binding-refused"
            else
                let! outcome = executor.Send(request methodValue path body key, ct)

                return
                    match outcome with
                    | Response response when response.StatusCode >= 200 && response.StatusCode < 300 -> Ok response
    ...
    let parse (body: string) =
        ...
        with :? JsonException ->
            Error "github-json-refused"

    let property (name: string) (value: JsonElement) =
        match value.TryGetProperty name with
        | true, item when item.ValueKind = JsonValueKind.String -> Option.ofObj (item.GetString())
        | _ -> None

    let gitId (value: string) =
    ...
    let routineMarker =
        Regex(
            "<!-- fsgg:routine-development/v1 head=(?<head>[a-f0-9]{40}) operation=(?<operation>[a-z-]+) -->",
            RegexOptions.CultureInvariant
        )

    let paginated (headers: Map<string, string>) =
        headers
        |> Map.exists (fun key content ->
            key.Equals("Link", StringComparison.OrdinalIgnoreCase)
            && content.Contains("rel=\"next\"", StringComparison.Ordinal))
    ...
    let readComments (ct: CancellationToken) : Task<Result<(int64 * string * string) list, string>> =
        task {
            let! response =
                send RestMethod.Get $"issues/{target.IssueNumber}/comments?per_page=100" None None ct
            ...
            | Ok value ->
                ...
                | Ok document ->
                    ...
                    if document.RootElement.ValueKind <> JsonValueKind.Array then
                        return Error "github-claim-census-refused"
                    else
                        ...
                        let observed =
                            ...
                            |> Seq.choose (fun (item: JsonElement) ->
                                ...
                                | (true, id), Some body, Some updated when
                                    body.IndexOf("<!-- fsgg:claim", StringComparison.Ordinal) >= 0
                                    ->
                                    ...
                                    | true, time when found.Success ->
                                        ...
                                        let session =
                                            if found.Groups["session"].Success then
                                                found.Groups["session"].Value
                                            else
                                                ""

                                        if time.AddMinutes(float lease) > now then
                                            Some(id.GetInt64(), found.Groups["worker"].Value, session)
                                        else
                                            None
                                    | _ ->
                                        malformed <- true
                        ...
                        return
                            if malformed then
                                Error "github-claim-marker-unparseable"
                            else
                                Ok observed
        }

    let readPull (branchRef: string) (ct: CancellationToken) : Task<Result<PullRequestFacts option, string>> =
        task {
            ...
            let! response =
                send
                    RestMethod.Get
                    $"pulls?state=all&head={query}&base={Uri.EscapeDataString target.BaseRef}&per_page=100"
                    None
                    None
                    ct
            ...
            | Ok value ->
                ...
                | Ok document ->
                    ...
                    if document.RootElement.ValueKind <> JsonValueKind.Array then
                        return Error "github-pull-request-census-refused"
                    else
                        let values = document.RootElement.EnumerateArray() |> Seq.toList

                        if values.Length > 1 then
                            return Error "github-pull-request-identity-ambiguous"
                        else
                            return
                                values
                                |> List.tryHead
                                |> Option.map (fun (item: JsonElement) ->
                                    let baseValue = item.GetProperty("base")

                                    {
                                        Number = item.GetProperty("number").GetInt32()
                                        NodeId = item.GetProperty("node_id").GetString()
                                        State = item.GetProperty("state").GetString()
                                        Merged =
                                            (match item.TryGetProperty "merged_at" with
                                             | true, v -> v.ValueKind = JsonValueKind.String
                                             | _ -> false)
                                        HeadSha = item.GetProperty("head").GetProperty("sha").GetString()
                                        HeadRef = item.GetProperty("head").GetProperty("ref").GetString()
                                        BaseRef = baseValue.GetProperty("ref").GetString()
                                        BaseSha = baseValue.GetProperty("sha").GetString()
                                        BaseRepository =
    ...
    member _.ReadHostedRoute(route: HostedRoutePlan, ct: CancellationToken) =
        task {
            ...
            | Ok repository, Ok issue ->
                ...
                | Ok repositoryDocument, Ok issueDocument ->
                    ...
                    let number =
                        match item.TryGetProperty "number" with
                    ...
                    | Some repositoryNode, Some databaseId, Some issueNode, Some issueNumber when
                        repositoryNode = route.RepositoryNodeId
                        ...
                        let revision =
                            [ repository.ETag; issue.ETag ]
                            |> List.choose id
                            |> String.concat ":"
                            |> fun value -> if String.IsNullOrWhiteSpace value then evidence else value
    ...
    member _.AcquireClaim(claimId: string, operationId: Guid, leaseUntil: DateTimeOffset, ct: CancellationToken) =
        task {
            ...
            if remaining <= TimeSpan.Zero || remaining > TimeSpan.FromHours 2. then
                ...
                match before with
                | Error reason -> return Error reason
                | Ok((_, worker, observed) :: _) when worker = target.Principal && observed = session ->
                    return Ok(claimId, observed)
                ...
                | Ok [] ->
                    ...
                    let body =
                        $"<!-- fsgg:claim worker={target.Principal} lease={lease} renewed={clock.GetUtcNow().Ticks} session={session} -->"

                    let! posted =
                        send
                            RestMethod.Post
                            $"issues/{target.IssueNumber}/comments"
                            (Some(JsonSerializer.Serialize {| body = body |}))
                            (Some session)
                            ct

                    match posted with
                    | Error _ ->
                        let! reconciled = readComments ct in

                        return
                            match reconciled with
                            | Ok((id, worker, found) :: _) when worker = target.Principal && found = session ->
                                Ok(claimId, string id)
                            | Ok _ -> Error "github-claim-outcome-unknown"
                            | Error reason -> Error reason
                    | Ok _ ->
                        let! observed = readComments ct in

                        return
                            match observed with
                            | Ok((id, worker, found) :: _) when worker = target.Principal && found = session ->
                                Ok(claimId, string id)
                            | Ok _ -> Error "github-claim-lost"
                            | Error reason -> Error reason
        }
    ...
    member _.ReadClaim(claimId: string, operationId: Guid, ct: CancellationToken) =
        task {
            let session = operationId.ToString("N")
            ...
            return
                ...
                | Ok((id, worker, found) :: _) when worker = target.Principal && found = session ->
                    Ok(claimId, string id)
                | Ok((_, _, _) :: _) -> Error "github-claim-held-by-competitor"
                | Ok [] -> Error "github-claim-not-observed"
                | Error reason -> Error reason
        }
    ...
    member _.PublishBranch
        ...
        task {
            if not (gitId headSha) || bundle.Length = 0 then
                return Error "github-candidate-publication-binding-refused"
            else
                let! published =
                    publisher.Publish(target.Repository, branchRef, expectedHead, headSha, bundle, ct)
                ...
                | Ok _ ->
                    ...
                    let! response =
                        send RestMethod.Get $"git/ref/heads/{Uri.EscapeDataString name}" None None ct
                    ...
                    | Ok value ->
                        ...
                        | Ok document ->
                            ...
                            let observed =
                                document.RootElement.GetProperty("object").GetProperty("sha").GetString() in

                            return
                                if observed = headSha then
                                    Ok(branchRef, defaultArg value.ETag observed)
                                else
                                    Error "github-branch-publication-readback-refused"
        }

    member _.CreatePullRequest(branchRef: string, headSha: string, operationId: Guid, ct: CancellationToken) =
        task {
            if target.RoutineOperation <> "internal-docs" then
                return Error "github-routine-profile-unsupported"
            else
                ...
                | Ok(Some value) when
                    ...
                    ->
                    return Ok(value.NodeId, value.HeadSha)
                | Ok(Some _) -> return Error "github-pull-request-binding-conflict"
                | Ok None ->
                    let head = branchRef.Replace("refs/heads/", "")

                    let marker =
                        $"<!-- fsgg:routine-development/v1 head={headSha} operation={target.RoutineOperation} -->"
                    ...
                    let! created =
                        send RestMethod.Post "pulls" (Some body) (Some(operationId.ToString "N")) ct

                    let! observed = readPull branchRef ct

                    return
                        match created, observed with
                        | _, Ok(Some value) when
                            value.HeadSha = headSha
                            && value.BaseRef = target.BaseRef
                            && value.BaseRepository = target.Repository
                            ->
                            Ok(value.NodeId, value.HeadSha)
                        | Error _, Ok _ -> Error "github-pull-request-outcome-unknown"
                        | _, Ok _ -> Error "github-pull-request-not-observed"
                        | _, Error reason -> Error reason
        }

    member _.ReadBranch(branchRef: string, headSha: string, ct: CancellationToken) =
        task {
            let name = branchRef.Replace("refs/heads/", "")
        ...
        task {
            ...
            return
                ...
                | Ok(Some value) when
                    ...
                    && value.BaseRepository = target.Repository
                    ->
                    Ok(value.NodeId, value.HeadSha, value.Revision)
                | Ok _ -> Error "github-pull-request-not-observed"
                | Error reason -> Error reason
        }

    member _.CheckProtectedHead(branchRef: string, headSha: string, ct: CancellationToken) =
        task {
            if target.RoutineOperation <> "internal-docs" then
                return Error "github-routine-profile-unsupported"
            else
                ...
                | Ok(Some value) when
                    ...
                    if
                        markers.Count <> 1
                        || markers[0].Groups["head"].Value <> headSha
                        || markers[0].Groups["operation"].Value <> target.RoutineOperation
                    then
                        return Error "github-routine-marker-refused"
                    else
                        ...
                        let nativeEligible =
                            ...
                            | Ok response ->
                                try
                                    ...
                                    let boolean (name: string) (expected: JsonValueKind) =
                                        match root.TryGetProperty name with
                                        | true, item when item.ValueKind = expected -> true
                                        | _ -> false
                                    ...
                                    if
                                        boolean "draft" JsonValueKind.False
                                        && boolean "mergeable" JsonValueKind.True
                                        && (mergeState = Some "clean"
                                            || mergeState = Some "unstable"
                                            || mergeState = Some "has_hooks")
                                        && head = Some headSha
                                        && baseRef = Some target.BaseRef
                                    then
                                        Ok()
                                    else
                                        Error "github-native-mergeability-refused"
                                with :? JsonException ->
                                    Error "github-native-mergeability-refused"
                        ...
                        let! policyResponse =
                            send
                                RestMethod.Get
                                $"contents/.fsgg/routine-development.json?ref={value.BaseSha}"
                                None
                                None
                                ct
                        ...
                        let policy =
                            ...
                            | Ok response ->
                                try
                                    ...
                                    let encoded =
                                        property "content" root |> Option.map (fun text -> text.Replace("\n", ""))
                                    ...
                                    | Some content ->
                                        ...
                                        | Some "fsgg.routine-development-policy/v1", (true, items) when
                                            ...
                                            let allowed =
                                                ...
                                                |> Seq.choose (fun item ->
                                                    if item.ValueKind = JsonValueKind.String then
                                                        Option.ofObj (item.GetString())
                                                    else
                                                        None)
                                                |> Set.ofSeq

                                            if allowed.Contains target.RoutineOperation then
                                                Ok()
                                            else
                                                Error "github-routine-operation-refused"
                                        | _ -> Error "github-routine-policy-refused"
                                with
                                | :? JsonException
                                | :? FormatException -> Error "github-routine-policy-refused"

                        let! protectionOutcome =
                            executor.Send(
                                request
                                    RestMethod.Get
                                    $"branches/{Uri.EscapeDataString target.BaseRef}/protection/required_status_checks"
                                    None
                                    None,
                                ct
                            )
                        ...
                        let required =
                            ...
                            | Response response when response.StatusCode = 404 ->
                                Ok(Set.singleton "routine-eligibility")
                            | Response response when response.StatusCode >= 200 && response.StatusCode < 300 ->
                                ...
                                | Ok document ->
                                    ...
                                    | true, items when items.ValueKind = JsonValueKind.Array ->
                                        items.EnumerateArray()
                                        |> Seq.choose (property "context")
                                        |> Set.ofSeq
                                        |> Set.add "routine-eligibility"
                                        |> Ok
                        ...
                        let! checks =
                            send RestMethod.Get $"commits/{headSha}/check-runs?per_page=100" None None ct
                        ...
                        | _, _, Ok _, Ok response when paginated response.Headers ->
                            return Error "github-check-census-incomplete"
                        | Ok(), Ok(), Ok expected, Ok response ->
                            ...
                            | Ok document ->
                                ...
                                let latest =
                                    ...
                                    | true, items when items.ValueKind = JsonValueKind.Array ->
                                        ...
                                        |> Seq.choose (fun (item: JsonElement) ->
                                            match
                                                property "name" item,
                                                item.TryGetProperty "id",
                                                property "head_sha" item
                                            with
                                            | Some name, (true, id), Some observedHead when
                                                id.ValueKind = JsonValueKind.Number && observedHead = headSha
                                                ->
                                                Some(
                                                    name,
                                                    id.GetInt64(),
                                                    property "status" item,
                                                    property "conclusion" item
                                                )
                                            | _ -> None)
                                        |> Seq.groupBy (fun (name, _, _, _) -> name)
                                        |> Seq.map (fun (name, values) ->
                                            name, values |> Seq.maxBy (fun (_, id, _, _) -> id))
                                        |> Map.ofSeq
                                    | _ -> Map.empty
                                ...
                                let checksGreen =
                                    expected
                                    |> Set.forall (fun name ->
                                        match latest |> Map.tryFind name with
                                        | Some(_, _, Some "completed", Some "success") -> true
                                        | _ -> false)

                                if not checksGreen then
                                    return Error "github-required-checks-not-green"
                                else
    ...
    member this.Merge(branchRef: string, headSha: string, operationId: Guid, ct: CancellationToken) =
        task {
    ...
    member this.MergeAuthorized(branchRef: string, headSha: string, operationId: Guid, ct: CancellationToken) =
        task {
            let! found = readPull branchRef ct
            ...
            | Ok(Some value) when
                ...
                let! _ =
                    send RestMethod.Put $"pulls/{value.Number}/merge" (Some body) (Some(operationId.ToString "N")) ct
