...
[<CLIMutable>]
type ExecutorRelayCompletion =
    {
        Schema: string
        CommandId: Guid
        FramesBase64: string array
        Failure: string
    }
...
module HostRuntime =
    ...
    let private storeOptions
        ...
        : StoreOptions =
        {
            ...
            SupportedSerializerVersions = Set [ EventEnvelope.legacySerializerVersion; EventEnvelope.serializerVersion ]
    ...
    let status (store: HostStore) permitId cancellationToken =
        task {
            ...
            | Error failures ->
                return
                    {
                        Schema = "fsgg.orchestration.host-status/1"
                        Ready = false
                        ...
                        Phase = None
                        Sequence = None
                        ReadbackCurrent = None
                        ActiveAssignments = None
                        UnknownOperations = None
                    ...
                    return
                        {
                            ...
                            Mode = "paused"
                            PermitId = string permitId
                            Phase = Some(string state.Phase)
                            Sequence = Some state.Sequence
                            ReadbackCurrent = Some state.ReadbackCurrent
                            ActiveAssignments = Some state.ActiveAssignments.Count
                            UnknownOperations = Some state.UnknownOperations.Count
                            Findings =
                                if state.Permit.IsSome && not state.ReadbackCurrent then
                                    [ "fresh-reconnect-readback-required" ]
                                else
                                    []
                        }
        }
    ...
    let authorize (expectedToken: string) (authorization: string option) =
        ...
        let expectedBytes, suppliedBytes =
            Encoding.UTF8.GetBytes expectedToken, Encoding.UTF8.GetBytes supplied
    ...
    let applyControl
        ...
        task {
            if
                request.Schema <> "fsgg.orchestration.host-control/1"
                || request.CommandId = Guid.Empty
                || request.PermitId <> configuredPermit
                || request.ExpectedSequence < 0L
                || request.ExpectedGeneration <= 0L
                || request.PrincipalId <> configuredPrincipal
            ...
            then
                ...
                | Ok recovery ->
                    ...
                    | Some permit when
                        ...
                        || permit.PilotOwnerId <> request.PrincipalId
                    ...
                    | Some _ ->
                        ...
                        let probe =
                            PilotJournal.appendRequest request.PermitId now (envelope request command) []
    ...
    let private writeJson (response: HttpListenerResponse) statusCode value cancellationToken =
        task {
            try
    ...
    let private writeEmergency response statusCode value =
        task {
            use deadline = new CancellationTokenSource(TimeSpan.FromMilliseconds 500.)

            try
                do! writeJson response statusCode value deadline.Token
            with _ ->
                try
                    response.Close()
                with _ ->
    ...
    let private readControl (request: HttpListenerRequest) cancellationToken =
        task {
            let mediaType =
            ...
            if
                ...
                || request.ContentLength64 > 4096L
                || not (String.Equals(mediaType, "application/json", StringComparison.OrdinalIgnoreCase))
            then
                ...
                while read < size do
                    let! count =
                        request.InputStream.ReadAsync(Memory(bytes, read, size - read), cancellationToken).AsTask()

                    if count = 0 then read <- size + 1 else read <- read + count

                if read <> size then
                    return Error "truncated-content"
                else
                    try
                        use document =
                        ...
                        if document.RootElement.ValueKind <> JsonValueKind.Object then
                            ...
                            if names.Length <> expected.Count || Set.ofList names <> expected then
    ...
    let private readRunner maximumBytes (request: HttpListenerRequest) cancellationToken =
        task {
            let mediaType =
                if isNull request.ContentType then
                    ""
            ...
            if
                request.ContentLength64 <= 0L
                || request.ContentLength64 > int64 maximumBytes
                || not (String.Equals(mediaType, "application/json", StringComparison.OrdinalIgnoreCase))
            then
                return Error "invalid-runner-content"
            else
                ...
                while offset < bytes.Length do
                    let! read =
                        request.InputStream
                            .ReadAsync(Memory(bytes, offset, bytes.Length - offset), cancellationToken)
                            .AsTask()
                ...
                if offset <> bytes.Length then
                    return Error "truncated-runner-content"
                else
                    return Ok bytes
        }

    let private runnerStore (store: HostStore) =
        {
            RunnerWireStore.WorkItems = store.WorkItems
    ...
    let private handle
        ...
        configuration
        store
        (executorRelay: HostExecutorRelay option)
        (mainAdmission: IMainRouteAdmissionHandler option)
        ...
        task {
            ...
            if request.HttpMethod = "GET" && request.Url.AbsolutePath = "/health/live" then
                ...
                do!
                    writeJson
                        ...
                        200
                        {|
                            schema = "fsgg.orchestration.host-liveness/2"
                            live = true
                ...
                if
                    executorRelay.IsNone
                    || not (
                        configuration.RunnerToken
                        |> Option.exists (fun token -> authorize token (Option.ofObj request.Headers["Authorization"]))
                    )
                then
                    ...
                    | Ok bytes ->
                        try
                            if request.HttpMethod = "POST" && request.Url.AbsolutePath = "/v1/executor/poll" then
                                let value =
                                    JsonSerializer.Deserialize<ExecutorRelayPoll>(ReadOnlySpan bytes, jsonOptions)
                                ...
                                if
                                    ...
                                    || isNull (box value)
                                    || value.Schema <> "fsgg.orchestration.executor-relay-poll/1"
                                    || value.MaximumWaitSeconds < 1
                                    || value.MaximumWaitSeconds > 30
                                then
                                    do!
                                        writeJson
                                            response
                                            400
                                            {|
                                                error = "executor-relay-poll-refused"
                                            |}
                                            cancellationToken
                                else
                                    ...
                                    | None ->
                                        do!
                                            writeJson
                                                ...
                                                204
                                                {|
                                                    schema = "fsgg.orchestration.executor-relay-empty/1"
                                                |}
                                                cancellationToken
                                    | Some pending ->
                                        do!
                                            writeJson
                                                ...
                                                200
                                                {|
                                                    schema = "fsgg.orchestration.executor-relay-request/1"
                                                    commandId = pending.CommandId
                                                    framesBase64 =
                                                        pending.Frames
                                                        |> List.map Convert.ToBase64String
                                                        |> List.toArray
                                                |}
                                                cancellationToken
                            elif
                                request.HttpMethod = "POST"
                                && request.Url.AbsolutePath = "/v1/executor/complete"
                            then
                                let value =
                                    JsonSerializer.Deserialize<ExecutorRelayCompletion>(ReadOnlySpan bytes, jsonOptions)

                                let frames =
                                    if isNull (box value) || isNull value.FramesBase64 then
                                        []
                                    else
                                        value.FramesBase64
                                        |> Array.toList
                                ...
                                let result =
                                    if
                                        ...
                                        || isNull (box value)
                                        || value.Schema <> "fsgg.orchestration.executor-relay-complete/1"
                                        || value.CommandId = Guid.Empty
                                    then
                                        Error "executor-relay-completion-refused"
                                    elif not (String.IsNullOrWhiteSpace value.Failure) then
                                        executorRelay.Value.Fail(value.CommandId, value.Failure)
                                    else
                                        executorRelay.Value.Complete(value.CommandId, frames)
                                ...
                                | Ok() ->
                                    do!
                                        writeJson
                                            ...
                                            200
                                            {|
                                                schema = "fsgg.orchestration.executor-relay-completion/1"
                                                accepted = true
                                            |}
                                            cancellationToken
                                ...
                                do!
                                    writeJson
                                        ...
                                        404
                                        {|
                                            error = "executor-relay-route-not-found"
                                        |}
                                        cancellationToken
                        with :? JsonException ->
                            do!
                                writeJson
                                    ...
                                    400
                                    {|
                                        error = "executor-relay-json-refused"
                                    |}
                                    cancellationToken
            elif
                request.Url.AbsolutePath.StartsWith("/v1/runner/", StringComparison.Ordinal)
                && configuration.RunnerToken.IsSome
            then
                if not (authorize configuration.RunnerToken.Value (Option.ofObj request.Headers["Authorization"])) then
                    do! writeJson response 401 {| error = "unauthorized" |} cancellationToken
                else
                    let maximum =
                        if request.Url.AbsolutePath = "/v1/runner/candidate" then
                            140 * 1024 * 1024
                        else
                            8192
                    ...
                    | Ok bytes ->
                                        ...
                                        return
                                            ...
                                            Result.map
                                                (fun revision ->
                                                    box
                                                        {|
                                                            schema = "fsgg.orchestration.runner-ack-receipt/1"
                                                            accepted = true
                                        ...
                                        return
                                            ...
                                            Result.map
                                                (fun revision ->
                                                    box
                                                        {|
                                                            schema = "fsgg.orchestration.runner-candidate-receipt/1"
                                                            accepted = true
                                                            workflowRevision = revision
                                                        |})
                                                found
                        ...
                        | Error reason ->
                            do! writeJson response (if parsed then 409 else 400) {| error = reason |} cancellationToken
            elif not (authorize configuration.Token (Option.ofObj request.Headers["Authorization"])) then
                do! writeJson response 401 {| error = "unauthorized" |} cancellationToken
            elif
                request.HttpMethod = "POST"
                && (request.Url.AbsolutePath = "/v1/main/admit"
                    || request.Url.AbsolutePath = "/v1/main/recover")
            then
                ...
                | None ->
                    do!
                        writeJson
                            ...
                            503
                            {|
                                error = "main-route-admission-not-configured"
                            |}
                            cancellationToken
                | Some admission ->
                    let! body = readRunner (2 * 1024 * 1024) request cancellationToken
                    ...
                    | Ok bytes ->
                        let! admitted =
                            if request.Url.AbsolutePath = "/v1/main/recover" then
                                admission.RecoverPaused(bytes, cancellationToken)
                            else
                                admission.Admit(bytes, cancellationToken)
                            ...
                            do!
                                writeJson
                                    ...
                                    {|
                                        schema =
                                            (if request.Url.AbsolutePath = "/v1/main/recover" then
                                                 "fsgg.orchestration.main-route-recovery-receipt/1"
                                             else
                                                 "fsgg.orchestration.main-route-admission-receipt/1")
                                        accepted = true
                                    |}
                                    cancellationToken
                        | Error reason -> do! writeJson response 409 {| error = reason |} cancellationToken
                ...
                && (request.Url.AbsolutePath = "/health/ready"
                    || request.Url.AbsolutePath = "/v1/status")
            then
                match mainAdmission with
                | None ->
                ...
                | Some admission ->
                    ...
                    | Error reason ->
                        do!
                            writeJson
                                ...
                                503
                                {|
                                    schema = "fsgg.orchestration.main-host-status/1"
                                    ready = false
                                    dispatchEnabled = false
                                    mode = "unavailable"
                                    findings = [| reason |]
                    ...
                    | Ok value ->
                        do!
                            writeJson
                                response
                                (if value.Ready then 200 else 503)
                                {|
                                    schema = "fsgg.orchestration.main-host-status/1"
                                    ready = value.Ready
                                    ...
                                    findings = List.toArray value.Findings
                                |}
                                cancellationToken
            elif
                request.HttpMethod = "POST"
                && (request.Url.AbsolutePath = "/v1/pause"
            ...
            then
                        ...
                        then
                            do!
                                writeJson
                                    ...
                                    404
                                    {|
                                        error = "legacy-main-control-route-not-supported"
                                    |}
                                    cancellationToken
                        else
                            let! result =
                            ...
                            | Ok sequence ->
                                do!
                                    writeJson
                                        ...
                                        200
                                        {|
                                            schema = "fsgg.orchestration.host-control-receipt/1"
                                            accepted = true
                                            sequence = sequence
                                        |}
                                        cancellationToken
                            | Error reason -> do! writeJson response 409 {| error = reason |} cancellationToken
                    | Some admission ->
                        if control.Schema <> "fsgg.orchestration.host-control/1" then
                            do!
                                writeJson
                                    response
                                    400
                                    {|
                                        error = "invalid-main-control-schema"
                                    |}
                                    cancellationToken
                        elif control.PermitId <> configuration.PermitId then
                            do!
                                writeJson
                                    response
                                    409
                                    {|
                                        error = "control-authority-mismatch"
                                    |}
                                    cancellationToken
                        else
                            let value =
                                {
                                    ...
                                    ExpectedGeneration = control.ExpectedGeneration
                                    PrincipalId = control.PrincipalId
                                    IssuedAt = control.IssuedAt
                                    ExpiresAt = control.ExpiresAt
                                    Reason = control.Reason
                                    Action = request.Url.AbsolutePath.Substring(4)
                                }
                            ...
                            | Ok receipt ->
                                do!
                                    writeJson
                                        ...
                                        200
                                        {|
                                            schema = "fsgg.orchestration.main-host-control-receipt/1"
                                            accepted = true
                                            sequence = receipt.Sequence
                                            action = receipt.Action
                                            requestPersisted = receipt.RequestPersisted
                ...
                do! writeJson response 404 {| error = "not-found" |} cancellationToken
        }
    ...
    let private serveInternal
        ...
        task {
            ...
            use requestSlots =
                new SemaphoreSlim(configuration.MaximumConcurrentRequests, configuration.MaximumConcurrentRequests)
            ...
            try
                do! Task.WhenAll(active)
            with _ ->
                ()
        }

    let serve clock configuration store cancellationToken =
        serveInternal clock configuration store None None cancellationToken

    let serveProduction clock configuration store relay cancellationToken =
        serveInternal clock configuration store (Some relay) None cancellationToken

    let serveMain clock configuration store relay admission cancellationToken =
        serveInternal clock configuration store (Some relay) (Some admission) cancellationToken

    let serveMainLocal clock configuration store admission cancellationToken =
        serveInternal clock configuration store None (Some admission) cancellationToken
