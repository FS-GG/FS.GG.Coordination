...
[<Sealed>]
type LocalExecutorTransport
    (configuration: LocalExecutorConfiguration, ?maximumAggregateBytes: int, ?shutdownTimeout: TimeSpan) =
    ...
    let requireAbsolute name value =
        if String.IsNullOrWhiteSpace value || not (Path.IsPathFullyQualified value) then
            invalidArg name "absolute path required"

    do
        for name, value in
            [
                "runnerExecutable", configuration.RunnerExecutable
        ...
        then
            invalidArg "maximumAggregateBytes" "invalid aggregate bound"
    ...
    let terminate (childProcess: Process) =
        ...
        try
            if
                ...
                && not (childProcess.WaitForExit(int shutdownTimeout.TotalMilliseconds))
            then
    ...
    let start () =
        if disposed then
            raise (ObjectDisposedException(nameof LocalExecutorTransport))
        ...
        if isNull childProcess then
            raise (InvalidOperationException "executor-child-start-refused")
    ...
    let boundedRequest (frames: byte array list) =
        not (List.isEmpty frames)
        && frames.Length <= 64
        && frames
           |> List.forall (fun frame ->
               not (isNull frame)
               && frame.Length > 0
               && frame.Length <= 2 * ExecutorWire.maximumContentBytes)
        && frames |> List.sumBy (fun frame -> int64 frame.Length)
           <= int64 maximumAggregateBytes
    ...
    let terminal bytes =
        Result.isOk (ExecutorWire.parseResponse bytes)
        || Result.isOk (ExecutorWire.parseOperationOutcome bytes)
    ...
    let writeFrame (stream: Stream) (bytes: byte array) token =
        task {
    ...
    let readExact (stream: Stream) (buffer: byte array) token =
        task {
            ...
            while offset < buffer.Length do
                ...
                if count = 0 then
                    raise (EndOfStreamException "executor-child-output-ended")

                offset <- offset + count
        }

    let exchange (frames: byte array list) (token: CancellationToken) =
        task {
            if disposed then
                return Error "executor-child-disposed"
            elif not (boundedRequest frames) then
                return Error "executor-child-request-bounds-refused"
            else
                            ...
                            task {
                                try
                                    ...
                                    for frame in frames do
                                        do! writeFrame childProcess.StandardInput.BaseStream frame token
                                    ...
                                    while not complete do
                                        ...
                                        if
                                            size < 1
                                            || size > 2 * ExecutorWire.maximumContentBytes
                                            || output.Count >= 64
                                            || total > maximumAggregateBytes - size
                                        then
                                        ...
                                        if
                                            not (responseFrameBound expected commandValue.CandidateId baseline bytes)
                                        then
                                            raise (
                                                InvalidDataException "executor-child-stale-or-malformed-frame-refused"
                                            )
                                ...
                                | :? OperationCanceledException ->
                                    discard ()
                                    return Error "executor-child-exchange-cancelled"
                                | :? EndOfStreamException ->
                                    discard ()
                                    return Error "executor-child-exited"
                                | :? InvalidDataException as error ->
                                    discard ()
                                    return Error error.Message
                                | _ ->
                                    discard ()
                                    return Error "executor-child-exchange-refused"
                            }
    ...
    interface IAuthenticatedExecutorTransport with
        member _.Exchange(frames, token) = exchange frames token
