        ...
        |> Array.fold
            (fun state pair ->
                ...
                |> Result.bind (fun values ->
                    if Map.containsKey pair[0] values then
...
let private privateBytes maximumBytes (path: string) =
    try
        if not (Path.IsPathFullyQualified path) then
            ...
            if
                lstat (path, &before) <> 0
                || before.Mode &&& 0xF000u <> 0x8000u
                || before.UserId <> geteuid ()
                || before.Size <= 0L
                || before.Size > int64 maximumBytes
            then
                Error "credential-file-refused"
            else
                ...
                if not (isNull parent.LinkTarget) || parentWritable <> enum 0 || exposed <> enum 0 then
                    Error "credential-file-must-be-owner-only"
                else
                    use stream =
                        new FileStream(
                            path,
                    ...
                    then
                        Error "credential-file-changed-during-open"
                    else
                        ...
                        if offset > maximumBytes then
                            Error "credential-file-size-refused"
                        else
                            Ok bytes[.. offset - 1]
    with _ ->
        Error "credential-file-refused"
...
let private readBoundedRequest path requestPath =
    let maximum =
        if path = "/v1/runner/candidate" then
            140 * 1024 * 1024
        else
            8192
    ...
    try
        ...
        if stream.Length <= 0L || stream.Length > int64 maximum then
            Error "runner-request-size-refused"
        else
            ...
            if offset <> bytes.Length || stream.ReadByte() <> -1 then
                Error "runner-request-changed-during-read"
            else
                Ok bytes
    with _ ->
        Error "runner-request-file-refused"
...
let private readBoundedResponse (response: HttpResponseMessage) =
    ...
    if contentLength.HasValue && contentLength.Value > 8192L then
        Error "runner-response-size-refused"
    else
    ...
    then
        ...
        | Ok values ->
            ...
            then
                ...
                try
                    for key in [ "--workspace-root"; "--input-root"; "--state-root"; "--artifact-root" ] do
                        Directory.CreateDirectory values[key] |> ignore
                    ...
                    0
                with error ->
                    eprintfn "executor-stdio-refused:%s" error.Message
                    3
    elif Array.tryHead arguments <> Some "post" then
        usage ()
    else
        match pairs arguments[1..] with
        | Error reason ->
        ...
        | Ok values ->
            ...
            if
                values.Count <> allowed.Count
                || values |> Map.exists (fun key _ -> not (Set.contains key allowed))
            then
                usage ()
            else
                ...
                | true, endpoint when
                    ...
                    && endpoint.Host = "orchestration.main.internal"
                    && endpoint.Port = 18080
                    && endpoint.UserInfo = ""
                    && endpoint.Query = ""
                    && endpoint.Fragment = ""
                    ...
                    | Ok requestBytes ->
                        match
                            privateBytes 16384 values["--client-cert-file"],
                            privateBytes 16384 values["--client-key-file"],
                            privateBytes 16384 values["--ca-file"]
                        with
                        | Ok certBytes, Ok keyBytes, Ok caBytes ->
                            try
                                match validRequest values["--path"] requestBytes with
                                | Error reason ->
                                    eprintfn "runner-request-refused:%s" reason
                                    2
                                | Ok() ->
                                    let certPem = System.Text.Encoding.UTF8.GetString certBytes
                                    let keyPem = System.Text.Encoding.UTF8.GetString keyBytes

                                    use clientCertificate =
                                        X509Certificate2.CreateFromPem(certPem.AsSpan(), keyPem.AsSpan())
                                    ...
                                    use response =
                                        client
                                            .PostAsync(Uri(endpoint, values["--path"]), content, CancellationToken.None)
                                            .GetAwaiter()
                                            .GetResult()
                                    ...
                                    | Error reason ->
                                        eprintfn "%s" reason
                                        3
                                    | Ok responseBytes when not response.IsSuccessStatusCode ->
                                        Console.Error.Write(System.Text.Encoding.UTF8.GetString responseBytes)
                                        3
                                    | Ok responseBytes ->
                                        let mediaType =
                                        ...
                                        if mediaType <> "application/json" then
                                            eprintfn "runner-response-media-type-refused"
                                            3
                                        else
                                            match validResponse values["--path"] responseBytes with
                                            | Error reason ->
                                                eprintfn "%s" reason
                                                3
                                            | Ok() ->
                                                ...
                                                0
                            with error ->
                                eprintfn "runner-request-refused:%s" error.Message
                                3
                        | _ ->
                            eprintfn "runner-credential-refused"
                            2
                | _ ->
                    eprintfn "runner-endpoint-refused"
                    2
