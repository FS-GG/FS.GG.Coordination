...
type private Supervised =
    {
        ...
        mutable Session: ProviderSessionReference option
        mutable Readiness: ProviderReadiness option
    }

type private LaunchPersistence =
    | NewLaunch
    | ExistingLaunch
    | ConflictingLaunch
...
type ExecutorRuntime(options: ExecutorRuntimeOptions, clock: TimeProvider) =
    ...
    let attemptKey (command: ExecutorCommandV2) =
        command.AssignmentId.ToString("N") + ":" + command.AttemptId.ToString("N")
            ...
            with :? IOException ->
                ...
                try
                    ...
                    then
                        ...
                        match
                            Int64.TryParse(File.ReadAllText path, NumberStyles.None, CultureInfo.InvariantCulture)
                        with
    ...
    let sameAuthority (left: ExecutorCommandV2) (right: ExecutorCommandV2) =
        ...
        && left.MaximumRuntimeSeconds = right.MaximumRuntimeSeconds
        && left.MaximumAttempts = right.MaximumAttempts
        && left.Workspace = right.Workspace
        && left.WorkspaceManifestSha256 = right.WorkspaceManifestSha256
        && left.InputDigest = right.InputDigest
        && left.ExecutorBinding = right.ExecutorBinding
    ...
    let effectiveExpiry (command: ExecutorCommandV2) =
        let runtime =
            try
                command.RecordedAt.AddSeconds(float command.MaximumRuntimeSeconds)
            with _ ->
    ...
    let response
        ...
        (usage: Map<string, UsageValue>)
        (invocationCost: MonetaryCost)
        (broaderCost: MonetaryCost)
        ...
        let invocationState, invocationAmount, invocationCurrency, invocationProvenance =
            costFields invocationCost

        let broaderState, broaderAmount, broaderCurrency, broaderProvenance =
            costFields broaderCost
        ...
        let usageValues =
            ...
            |> Array.map (fun (name, value) ->
                match value with
                | UsageKnown(number, unitName, provenance) ->
                    {
                        Name = name
                        State = "observed"
        ...
        let refs (values: OutputReference list) =
            values
            |> List.map (fun value ->
                {
                    Kind = value.Kind
                    Reference = value.Reference
                    Digest = value.Digest |> Option.toObj
                }
                : ExecutorReference)
            |> List.toArray

        let candidateId, head, tree =
            match candidate with
            | Some value -> value.CandidateId, value.HeadSha, value.TreeSha
            | None -> Guid.Empty, null, null

        {
            ...
            Output = refs output
            LifecycleReferences = refs lifecycleReferences
            Usage = usageValues
            InvocationCostState = invocationState
            InvocationCostAmount = invocationAmount
            InvocationCostCurrency = invocationCurrency
            InvocationCostProvenance = invocationProvenance
            BroaderCostState = broaderState
            BroaderCostAmount = broaderAmount
            BroaderCostCurrency = broaderCurrency
            BroaderCostProvenance = broaderProvenance
    ...
    let unknownUsage reason =
        Map["provider-usage", UsageUnknown reason]

    let operation
        (command: ExecutorCommandV2)
        operation
        disposition
        (session: ProviderSessionReference option)
        reason
        : ExecutorOperationOutcome =
        {
            ...
            ProviderSessionReference = session |> Option.map ProviderSessionReference.value |> Option.toObj
            ObservedAt = clock.GetUtcNow()
            Reason = reason
        }
    ...
    let receipt (command: ExecutorCommandV2) disposition processObserved detail =
        {
            ...
            BodySha256 = command.BodySha256
            Disposition = disposition
            DurableRevision = command.ExpectedRevision
            ProcessCreationObserved = processObserved
            Detail = detail
        }

    let lifecycle (value: SessionLifecycle) =
        match value with
        | Starting -> "starting"
        | Running -> "running"
    ...
    let observationResponse
        (command: ExecutorCommandV2)
        (readiness: ProviderReadiness option)
        ...
        let authenticationState, authenticationProvenance, supportsResume =
            ...
            | Some value ->
                let state, provenance =
                    match value.Authentication with
                    | Authenticated p -> "authenticated", p
                    | NotAuthenticated p -> "not-authenticated", p
                    | AuthenticationUnknown p -> "unknown", p
        ...
        response
            ...
            observation.Provider.AdapterVersion
            authenticationState
            authenticationProvenance
            supportsResume
            (ProviderSessionReference.value observation.Session)
            (lifecycle observation.Lifecycle)
            (command.RequestedModel)
            (command.RequestedEffort)
            (observation.Resolved.Model |> Option.toObj)
            (observation.Resolved.Effort |> Option.toObj)
            observation.Output
            observation.LifecycleReferences
            observation.Usage.Values
            observation.Usage.Cost
            (CostUnknown "broader-monetary-attribution-unavailable")
    ...
    let build (command: ExecutorCommandV2) (manifest: ExecutorWorkspaceManifest) =
        ...
        then
            Error "executor-manifest-command-conflict"
        elif
            sha (ExecutorWire.encodeWorkspaceManifest manifest)
            <> command.WorkspaceManifestSha256
        then
            Error "executor-manifest-digest-mismatch"
            ...
            |> Result.map (fun workspace ->
                let input =
                    DigestInput(options.InputRoot, command.InputDigest, 16L * 1024L * 1024L) :> ICodexExecutionInput
                ...
                let provider =
                    CodexExecution.provider providerOptions input (inspector :> ICodexCandidateInspector) clock
    ...
    let matchesDurableLaunch (command: ExecutorCommandV2) =
        let path =
            Path.Combine(
                ...
                command.AssignmentId.ToString("N"),
                command.AttemptId.ToString("N"),
                command.Generation.ToString(CultureInfo.InvariantCulture),
                "launch-command.json"
            )
        ...
        try
            ...
            if
                ...
                || not (isNull info.LinkTarget)
                || info.Length < 1L
                || info.Length > int64 ExecutorWire.maximumControlBytes
            then
                false
            else
    ...
    let findArtifact candidateId =
        ...
        | _ ->
            let manifestPath =
                Path.Combine(options.ArtifactRoot, candidateId.ToString("N") + ".manifest.json")

            let bundlePath =
                Path.Combine(options.ArtifactRoot, candidateId.ToString("N") + ".bundle")

            if not (File.Exists manifestPath && File.Exists bundlePath) then
                None
            else
                try
                    ...
                    then
                        ...
                        | Ok manifest when manifest.CandidateId = candidateId ->
                            ...
                            if artifacts.Count < 4 then
                                artifacts.TryAdd(candidateId, value) |> ignore
        ...
        | Some artifact when artifact.Manifest.BaselineObjectId = selected.Manifest.BaselineObjectId ->
            try
                ...
                if
                    not info.Exists
                    || not (isNull info.LinkTarget)
                    || info.Length <> artifact.Manifest.BundleSizeBytes
                then
                    None
                else
                    ...
                    if digest = artifact.Manifest.BundleSha256 then
                        Some artifact
                    else
                        None
            with _ ->
    ...
    let ensureReadiness (value: Supervised) =
        task {
                ...
                do!
                    writeFrame
                        ...
                        (ExecutorWire.encodeOperationOutcome (
                            operation
                                command
                                (if command.Kind = "cancel" then "cancel" else "reconcile")
                                "refused"
                                None
                                "executor-generation-stale-or-overlapping"
                        ))
            elif
                command.Kind = "launch"
                && (effectiveExpiry command = DateTimeOffset.MinValue
                    || now >= effectiveExpiry command)
            then
                do!
                    writeFrame
                        output
                        (ExecutorWire.encodeOperationOutcome (
                            operation command "launch" "refused" None "original-execution-budget-expired"
                        ))
                ...
                | None ->
                    do!
                        writeFrame
                            output
                            (ExecutorWire.encodeOperationOutcome (
                                operation command "content-read" "unknown" None "candidate-artifact-not-found"
                            ))
                | Some artifact when artifact.Manifest.BundleSha256 <> command.ArtifactDigest ->
                    do!
                        writeFrame
                            output
                            (ExecutorWire.encodeOperationOutcome (
                                operation command "content-read" "unknown" None "candidate-artifact-digest-conflict"
                            ))
                | Some artifact ->
                    let info = FileInfo artifact.BundlePath
                    ...
                    then
                        do!
                            writeFrame
                                output
                                (ExecutorWire.encodeOperationOutcome (
                                    operation command "content-read" "unknown" None "candidate-artifact-tampered"
                                ))
                    else
                        use stream = File.OpenRead artifact.BundlePath
                        ...
                        if
                            stream.Length <> artifact.Manifest.BundleSizeBytes
                            || observedDigest <> artifact.Manifest.BundleSha256
                        then
                            do!
                                writeFrame
                                    output
                        ...
                        elif command.ContentOffset > stream.Length then
                            do!
                                writeFrame
                                    output
                                    (ExecutorWire.encodeOperationOutcome (
                                        operation
                                            ...
                                            "unknown"
                                            None
                                            "candidate-artifact-offset-refused"
                                    ))
                        else
                            ...
                            let content =
                                {
                                    ...
                                    Offset = command.ContentOffset
                                    Final = stream.Position = stream.Length
                                ...
                                writeFrame
                                    ...
                                    (ExecutorWire.encodeOperationOutcome (
                                        operation
                                            ...
                                            "reconciled"
                                            None
                                            "candidate-artifact-chunk-observed"
                                    ))
            else
                let supervised =
                    match sessions.TryGetValue itemKey with
                    | true, value -> Ok value
                    | _ when command.Kind <> "readiness" && sessions.Count >= 4 ->
                ...
                | Error reason ->
                    do!
                        writeFrame
                            output
                            (ExecutorWire.encodeOperationOutcome (operation command command.Kind "refused" None reason))
                | Ok value ->
                    let selected =
                        if command.Kind = "readiness" then
                            value
                        else
                    ...
                    if not (sameAuthority selected.Command command) then
                            ...
                            writeFrame
                                ...
                                (ExecutorWire.encodeOperationOutcome (
                                    operation
                                        ...
                                        "refused"
                                        None
                                        "executor-command-authority-conflict"
                                ))
                    else
                        ...
                        | "readiness" ->
                            ...
                            let authState, authProvenance =
                                match ready.Authentication with
                                | Authenticated p -> "authenticated", p
                                | NotAuthenticated p -> "not-authenticated", p
                                | AuthenticationUnknown p -> "unknown", p
                            ...
                            let result =
                                response
                                    command
                                    "readiness"
                                    ready.Identity.Provider
                                    ready.Identity.AdapterVersion
                                    authState
                                    authProvenance
                                    ready.SupportsResume
                                    null
                                    (if authState = "authenticated" then "ready" else "not-ready")
                                    command.RequestedModel
                                    command.RequestedEffort
                                    null
                                    null
                                    []
                                    []
                                    (unknownUsage "readiness-no-usage")
                                    (CostUnknown "readiness-no-invocation")
                                    (CostUnknown "broader-monetary-attribution-unavailable")
                                    None
                                    ""
                        ...
                        | "launch" ->
                            let intent =
                                {
                                    ...
                                    Requested =
                                        {
                                            Model = Option.ofObj command.RequestedModel
                                            Effort = Option.ofObj command.RequestedEffort
                                        }
                                    Limits =
                                        {
                                            Deadline = command.Deadline
                                            MaximumRuntime = TimeSpan.FromSeconds(float command.MaximumRuntimeSeconds)
                            ...
                            | ConflictingLaunch ->
                                do!
                                    writeFrame
                                        output
                                        (ExecutorWire.encodeReceipt (
                                            receipt command "conflict" false "executor-launch-identity-conflict"
                                        ))
                                    ...
                                    writeFrame
                                        ...
                                        (ExecutorWire.encodeOperationOutcome (
                                            operation
                                                ...
                                                "refused"
                                                None
                                                "executor-launch-identity-conflict"
                                        ))
                            | ExistingLaunch ->
                                do!
                                    writeFrame
                                        output
                                        (ExecutorWire.encodeReceipt (
                                            receipt command "duplicate" false "launch-intent-already-persisted"
                                        ))
                                    ...
                                    do!
                                        writeFrame
                                            ...
                                            (ExecutorWire.encodeResponse (
                                                observationResponse command selected.Readiness observation
                                            ))
                                | ConfirmedAbsent ->
                                    do!
                                        writeFrame
                                            ...
                                            (ExecutorWire.encodeOperationOutcome (
                                                operation
                                                    ...
                                                    "ambiguous"
                                                    None
                                                    "executor-persisted-launch-local-absence-not-authoritative"
                                            ))
                                | ReconcileUnknown reason ->
                                    do!
                                        writeFrame
                                            output
                                            (ExecutorWire.encodeOperationOutcome (
                                                operation command "launch" "ambiguous" None reason
                                            ))
                            | NewLaunch ->
                                do!
                                    writeFrame
                                        output
                                        (ExecutorWire.encodeReceipt (
                                            receipt command "persisted" false "launch-intent-persisted-before-provider"
                                        ))
                                ...
                                | Authenticated _ ->
                                    ...
                                    | LaunchStarted observation ->
                                        ...
                                        do!
                                            writeFrame
                                                output
                                                (ExecutorWire.encodeReceipt (
                                                    receipt command "observed" true "provider-process-creation-observed"
                                                ))
                                    ...
                                    | LaunchRefused reason ->
                                        do!
                                            writeFrame
                                                output
                                                (ExecutorWire.encodeOperationOutcome (
                                                    operation command "launch" "refused" None reason
                                                ))
                                    | LaunchAmbiguous reason ->
                                        do!
                                            writeFrame
                                                output
                                                (ExecutorWire.encodeOperationOutcome (
                        ...
                        | "observe" ->
                            ...
                            | Ok session, Some artifact when matchesDurableLaunch command ->
                                ...
                                let candidate =
                                    Some
                                        {
                                            CandidateId = artifact.Manifest.CandidateId
                                            HeadSha = artifact.Manifest.HeadObjectId
                                            TreeSha = artifact.Manifest.TreeObjectId
                                        }
                                ...
                                let recovered =
                                    response
                                        ...
                                        "session-observation"
                                        "Codex"
                                        "codex-subscription-exec/1"
                                        "unknown"
                                        "durable-artifact-recovery"
                                        true
                                        (ProviderSessionReference.value session)
                                        "succeeded"
                                        command.RequestedModel
                                        command.RequestedEffort
                                        command.RequestedModel
                                        ...
                                        []
                                        []
                                        (unknownUsage "durable-artifact-recovery")
                                        (CostNotApplicable "codex-chatgpt-subscription-no-per-invocation-price")
                                        (CostUnknown "broader-monetary-attribution-unavailable")
                                        candidate
                                    ...
                                    | Ok observation ->
                                        ...
                                        | Succeeded, Some candidate ->
                                            match
                                                if
                                                    artifacts.Count >= 4
                                                    && not (artifacts.ContainsKey candidate.CandidateId)
                                                then
                                                    Error "executor-artifact-capacity-refused"
                                                else
                                                    selected.Inspector.CreateArtifact(
                                                        candidate,
                                                        ExecutorWire.maximumContentBytes
                                                    )
                                            with
                                            | Ok artifact ->
                                                ...
                                                do!
                                                    writeFrame
                                                        output
                                                        (ExecutorWire.encodeArtifactManifest artifact.Manifest)
                                    ...
                                    writeFrame
                                        ...
                                        (ExecutorWire.encodeOperationOutcome (
                                            operation
                                                ...
                                                "unknown"
                                                None
                                                "executor-local-absence-not-authoritative"
                                        ))
                            | ReconcileUnknown reason ->
                                do!
                                    writeFrame
                                        output
                                        (ExecutorWire.encodeOperationOutcome (
                                            operation command "reconcile" "unknown" None reason
                                        ))
                        | "cancel" ->
                            ...
                            | Error reason ->
                                do!
                                    writeFrame
                                        output
                                        (ExecutorWire.encodeOperationOutcome (
                                            operation command "cancel" "refused" None reason
                                        ))
                            | Ok session ->
                                        ...
                                        writeFrame
                                            ...
                                            (ExecutorWire.encodeOperationOutcome (
                                                operation
                                                    ...
                                                    "accepted"
                                                    (Some session)
                                                    "termination-requested-not-yet-observed"
                                            ))
                                | CancelRefused reason ->
                                    do!
                                        writeFrame
                                            output
                                            (ExecutorWire.encodeOperationOutcome (
                                ...
                                | CancelUnknown reason ->
                                    do!
                                        writeFrame
                                            output
                                            (ExecutorWire.encodeOperationOutcome (
                                                operation command "cancel" "unknown" (Some session) reason
                                            ))
                        | _ ->
                            do!
                                writeFrame
                                    ...
                                    (ExecutorWire.encodeOperationOutcome (
                                        operation
                                            ...
                                            "refused"
                                            None
                                            "executor-command-kind-not-runnable"
                                    ))
        }
    ...
    member _.Run(input: Stream, output: Stream, cancellationToken: CancellationToken) =
        task {
            ...
            while not finished && not cancellationToken.IsCancellationRequested do
                ...
                if finished && got > 0 then
                    raise (EndOfStreamException "executor-frame-header-truncated")

                if not finished then
                    let size = BinaryPrimitives.ReadInt32BigEndian header

                    if size < 1 || size > options.MaximumFrameBytes then
                        raise (InvalidDataException "executor-frame-size-refused")
                    ...
                    | value when value = ExecutorWire.workspaceManifestSchema ->
                        ...
                        | Ok manifest when manifests.Count >= 64 && not (manifests.ContainsKey(sha bytes)) ->
                            raise (InvalidDataException "executor-manifest-capacity-refused")
                    ...
                    | value when value = ExecutorWire.inputManifestSchema ->
                        ...
                        | Ok manifest when
                            inputManifests.Count >= 16
                            && not (inputManifests.ContainsKey manifest.InputDigest)
                            ->
                            raise (InvalidDataException "executor-input-capacity-refused")
                    ...
                    | value when value = ExecutorWire.contentSchema ->
                        ...
                        | Ok(content, payload) ->
                            ...
                            | true, manifest ->
                                if
                                    payload.Length > manifest.ChunkBytes
                                    || content.Offset > manifest.SizeBytes - int64 payload.Length
                                then
                                    raise (InvalidDataException "executor-input-chunk-bounds-refused")
                                ...
                                if File.Exists completed then
                                    ...
                                    if
                                        existing.Length <> manifest.SizeBytes
                                        || (SHA256.HashData(existing) |> Convert.ToHexString |> _.ToLowerInvariant())
                                           <> manifest.InputDigest
                                    then
                                        raise (InvalidDataException "executor-input-existing-conflict")

                                    existing.Position <- content.Offset
                                    let replay = Array.zeroCreate<byte> payload.Length
                                    existing.ReadExactly replay

                                    if replay <> payload then
                                        raise (InvalidDataException "executor-input-replay-conflict")
                                else
                                    if File.Exists temporary && not (isNull (FileInfo(temporary).LinkTarget)) then
                                        raise (InvalidDataException "executor-input-partial-link-refused")
                                    ...
                                    if content.Offset > stream.Length then
                                        raise (InvalidDataException "executor-input-offset-conflict")
                                    elif content.Offset < stream.Length then
                                        if content.Offset + int64 payload.Length > stream.Length then
                                            raise (InvalidDataException "executor-input-replay-overlap-refused")

                                        let existing = Array.zeroCreate<byte> payload.Length
                                        stream.Position <- content.Offset
                                    ...
                                    if content.Final then
                                        if stream.Length <> manifest.SizeBytes then
                                            raise (InvalidDataException "executor-input-size-conflict")
                                        ...
                                        if
                                            ...
                                            <> manifest.InputDigest
                                        then
                                            raise (InvalidDataException "executor-input-digest-mismatch")
                        ...
                        | Ok command ->
                            ...
                            if completed.Length > 0 then
                                ...
                                for item in completed do
                                    running.Remove item |> ignore

                            if running.Count >= 64 then
                                raise (InvalidDataException "executor-active-command-capacity-refused")
