...
module ProviderSessionReference =
    let create value =
        if String.IsNullOrWhiteSpace value || value.Length > 512 then
            Error "provider-session-reference-refused"
        else
            Ok(ProviderSessionReference value)
...
type CancelResult =
    | CancelAccepted
    | CancelRefused of string
    | CancelUnknown of string
...
type IExecutionSessionJournal =
    ...
    abstract member AppendAttempt:
        assignmentId: Guid * attemptId: Guid * int64 * SessionEvent * CancellationToken -> Task<AppendResult>
    ...
    let decode (bytes: byte array) =
        try
            ...
            if document.RootElement.ValueKind <> JsonValueKind.Object then
                Error "execution-launch-object-required"
            else
                let names = document.RootElement.EnumerateObject() |> Seq.map _.Name |> Set.ofSeq

                if names <> properties then
                    Error "execution-launch-shape-refused"
                else
                    let wire = JsonSerializer.Deserialize<LaunchWire>(ReadOnlySpan bytes, options)

                    if isNull (box wire) || wire.Schema <> launchSchema then
                        Error "execution-launch-schema-refused"
                    else
                        Ok
                            {
                                Schema = wire.Schema
                                ...
                                Requested =
                                    {
                                        Model = Option.ofObj wire.RequestedModel
                                        Effort = Option.ofObj wire.RequestedEffort
                                    }
                                Limits =
                                    {
                                        Deadline = wire.Deadline
                                        MaximumRuntime = TimeSpan.FromSeconds(float wire.MaximumRuntimeSeconds)
                                        MaximumAttempts = wire.MaximumAttempts
                                    }
                                RecordedAt = wire.RecordedAt
                            }
        with :? JsonException ->
            Error "execution-launch-json-refused"
...
module SessionState =
    let replay events =
        events
        |> List.fold
            (fun state eventValue ->
                match state, eventValue with
                | None, LaunchIntentRecorded intent ->
                    Some
                        {
                            Intent = intent
                            Observation = None
                ...
                | Some value, LaunchAttemptRecorded(attemptNumber, _) ->
                    Some
                        { value with
                            LaunchAttempts = max value.LaunchAttempts attemptNumber
                            Revision = value.Revision + 1L
                        }
                | Some value, StartObserved observation
                | Some value, ObservationRecorded observation ->
                    Some
                        { value with
                            Observation = Some observation
                            Revision = value.Revision + 1L
                        }
                | Some value, CancelRequested _ ->
                    Some
                        { value with
...
type ExecutionSessionCoordinator(provider: IExecutionProvider, journal: IExecutionSessionJournal, clock: TimeProvider) =
    let validDigest (value: string) =
        value.Length = 64 && value |> Seq.forall Char.IsAsciiHexDigit
    ...
    let validUsageValue =
        ...
        | UsageNotApplicable provenance -> nonEmpty provenance

    let validCost =
        function
        | CostKnown(amount, currency, provenance) -> amount >= 0M && nonEmpty currency && nonEmpty provenance
        | CostUnknown provenance
        | CostNotApplicable provenance -> nonEmpty provenance

    let boundedReferences (observation: SessionObservation) =
        nonEmpty observation.Provider.Provider
        && nonEmpty observation.Provider.AdapterVersion
        && observation.Output.Length <= 64
        && observation.LifecycleReferences.Length <= 64
        && (observation.Output @ observation.LifecycleReferences
            |> List.forall (fun item ->
                nonEmpty item.Reference
                && item.Reference.Length <= 2048
                && nonEmpty item.Kind
                && item.Kind.Length <= 64))
        && observation.Usage.Values.Count <= 64
        && observation.Usage.Values
           |> Map.forall (fun name value -> nonEmpty name && validUsageValue value)
        && validCost observation.Usage.Cost
        && observation.Candidate
           |> Option.forall (fun candidate ->
               nonEmpty candidate.HeadSha
               && candidate.HeadSha.Length <= 128
               && nonEmpty candidate.TreeSha
               && candidate.TreeSha.Length <= 128)

    let append key revision eventValue cancellationToken =
        journal.AppendAttempt(key.AssignmentId, key.AttemptId, revision, eventValue, cancellationToken)

    let expired intent now =
        now >= intent.Limits.Deadline
        || now >= intent.RecordedAt.Add(intent.Limits.MaximumRuntime)

    let readState key cancellationToken =
        task {
    ...
    let launchProvider state cancellationToken =
        task {
            if state.LaunchAttempts >= state.Intent.Limits.MaximumAttempts then
                return SessionRefused "execution-attempt-budget-exhausted"
            else
                ...
                let! recorded =
                    append
                        state.Intent.Key
                        state.Revision
                        (LaunchAttemptRecorded(attemptNumber, clock.GetUtcNow()))
                        cancellationToken
                ...
                | Appended ->
                    ...
                    | LaunchStarted observation ->
                        return! recordObservation attempted (StartObserved observation) observation cancellationToken
    ...
    member _.Launch(intent, cancellationToken) =
        task {
            ...
            if intent.Schema <> ExecutionProtocol.launchSchema then
                return SessionRefused "execution-schema-not-supported"
            elif
                intent.Key.Generation < 0L
                || not (validDigest intent.InputDigest)
                || String.IsNullOrWhiteSpace intent.Workspace
            then
                return SessionRefused "execution-intent-invalid"
            elif
                intent.Limits.MaximumAttempts < 1
                || intent.Limits.MaximumRuntime <= TimeSpan.Zero
                || expired intent now
            then
                return SessionRefused "execution-budget-expired"
            else
                ...
                | Some state ->
                    ...
                    | Reconciled observation ->
                        return! recordObservation state (ObservationRecorded observation) observation cancellationToken
    ...
    member _.Observe(key, cancellationToken) =
        task {
            let! state = readState key cancellationToken

            match state with
            | None -> return SessionRefused "execution-session-not-found"
            | Some value when expired value.Intent (clock.GetUtcNow()) && value.Observation.IsNone ->
                return SessionRefused "execution-budget-expired"
            | Some value when expired value.Intent (clock.GetUtcNow()) && not value.CancelWasRequested ->
                let! persisted =
                    append key value.Revision (CancelRequested(clock.GetUtcNow())) cancellationToken
                ...
                | Appended, None ->
                    return SessionNeedsReconciliation "deadline-cancel-recorded-before-provider-session-observed"
            ...
            | Some value ->
                ...
                | None ->
                    ...
                    match reconciled with
                    | Reconciled observation ->
                        return! recordObservation value (ObservationRecorded observation) observation cancellationToken
    ...
    member _.Cancel(key, cancellationToken) =
        task {
            let! state = readState key cancellationToken
...
type ExecutionSessionCommand =
    | Launch of LaunchIntent
    | Observe of ExecutionKey
    | Reconnect of ExecutionKey
    | Cancel of ExecutionKey
...
type ExecutionSessionActor(coordinator: ExecutionSessionCoordinator) =
    ...
    override this.OnReceive(message) =
        ...
        let operation =
            ...
            | :? ExecutionSessionCommand as command ->
                ...
                | Observe key
                | Reconnect key -> coordinator.Observe(key, CancellationToken.None)
        ...
        operation.ContinueWith(fun (completed: Task<CoordinationResult>) ->
            if completed.IsCompletedSuccessfully then
                ...
                let detail =
                    if isNull completed.Exception then
                        "cancelled"
                    else
    ...
    static member Props(coordinator) =
        Akka.Actor.Props.Create(fun () -> ExecutionSessionActor(coordinator))
