...
module SubscriptionPilot =
    ...
    let settle now runtimeSeconds tokens provenance (reservation:SubscriptionReservation) =
        if reservation.Schema<>reservationSchema || runtimeSeconds<0L || String.IsNullOrWhiteSpace provenance
           || (tokens |> Option.exists(fun value->value<0L)) then Error "subscription-settlement-refused"
        else
