...
module LedgerProtectionCommand =
    let private options (values: string list) =
        let rec loop (state: Map<string, string>) (remaining: string list) =
            ...
            | [] -> Ok state
            | key :: value :: tail when key.StartsWith("--") && not (Map.containsKey key state) ->
                loop (Map.add key value state) tail
            | value :: _ -> Error("unknown or repeated argument: " + value)

        loop Map.empty values

    let private required (name: string) (values: Map<string, string>) =
    ...
    let private planCommand values =
        ...
        | Ok inputPath, Ok authorityPath, Ok keyPath, Ok outputPath ->
            try
                ...
                match
                    LedgerInitializationAdapter.plan DateTimeOffset.UtcNow (readAuthority authorityPath keyPath) input
                with
                | Error findings ->
                    eprintfn "%s" (String.concat "," findings)
                    3
                | Ok plan ->
                    let objects =
                        ...
                        |> List.map (fun value ->
                            {|
                                kind = value.Kind
                                oid = value.Oid
                                bytesBase64 = Convert.ToBase64String value.Bytes
                            |})

                    let envelope =
                        {|
                            schema = "fsgg.github-ledger-initialization-plan/1"
                            inputSha256 = plan.InputSha256
                            authorizationSha256 = plan.AuthorizationSha256
                            ref = plan.Ref
                            tag = plan.Tag
                            expectedRef = "absent"
                            commitOid = plan.Commit.Oid
            ...
            with error ->
                ...
                3
        | _ ->
            eprintfn "plan requires --input --authorization --public-key --output"
            2
    ...
    let private payloadCommand values =
        ...
        | Ok inputPath, Ok outputPath ->
            ...
            with error ->
                ...
                3
        | _ ->
            eprintfn "payload requires --input --output"
            2
    ...
    let private delegated mode values credentialFd =
        ...
        | Ok planPath, Ok transport when File.Exists planPath ->
            ...
            | Some fd when fd < 3 ->
                eprintfn "credential fd must be >=3"
                2
            | _ ->
                ...
                credentialFd
                |> Option.iter (fun fd ->
                    start.ArgumentList.Add "--credential-fd"
                    start.ArgumentList.Add(string fd))

                use child = Process.Start start
                child.WaitForExit()
                child.ExitCode
        | _ ->
            eprintfn "%s requires existing --plan and --transport" mode
            2

    let run arguments =
        match arguments |> Array.toList with
        | "initialize" :: "payload" :: rest ->
            match options rest with
            | Ok values -> payloadCommand values
            | Error error ->
                eprintfn "%s" error
                2
        | "initialize" :: "plan" :: rest ->
            match options rest with
            | Ok values -> planCommand values
            | Error error ->
                eprintfn "%s" error
                2
        | "initialize" :: "apply" :: rest ->
            ...
            | Error error ->
                ...
                2
            | Ok values ->
                match required "--credential-fd" values with
                | Ok value -> delegated "apply" values (Some(Int32.Parse value))
                | Error error ->
                    eprintfn "%s" error
                    2
        | "initialize" :: "verify" :: rest ->
            match options rest with
            | Ok values -> delegated "verify" values None
            | Error error ->
                eprintfn "%s" error
                2
        | _ ->
            eprintfn "ledger-protection initialize <payload|plan|apply|verify>"
            2
