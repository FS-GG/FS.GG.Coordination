...
module ObserverViewCommand =
    ...
    let private readEvents path =
        try
            ...
            elif info.Length > 16L * 1024L * 1024L then
                Error "event file exceeds 16 MiB"
            else
                let lines =
                    File.ReadAllLines path |> Array.filter (String.IsNullOrWhiteSpace >> not)

                if lines.Length > 10_000 then
                    Error "event file exceeds 10000 events"
                else
                    let decoded =
                        lines
                        |> Array.mapi (fun index line ->
                            ...
                            with :? FormatException ->
                                Error $"event {index + 1}: invalid-base64")
                    ...
                    | None ->
                        ...
                        |> Ok
        with exceptionValue ->
            Error $"event file unreadable: {exceptionValue.GetType().Name}"
    ...
    let private json view =
        ...
        for row in view.Rows do
            ...
            value["recordedAt"] <-
                JsonValue.Create(row.RecordedAt.ToUniversalTime().ToString("O", CultureInfo.InvariantCulture))
        ...
        root["sessionId"] <-
            view.SessionId
            |> Option.map (fun value -> JsonValue.Create(value) :> JsonNode)
            |> Option.defaultValue null

        root["sequence"] <- JsonValue.Create(view.Sequence)

        root["observationRevision"] <-
            view.ObservationRevision
            |> Option.map (fun value -> JsonValue.Create(value) :> JsonNode)
            |> Option.defaultValue null
    ...
    let private text view =
        seq {
            ...
            yield
                $"observer authority=unverified-event-export session={session} sequence={view.Sequence} observation={observation}"
