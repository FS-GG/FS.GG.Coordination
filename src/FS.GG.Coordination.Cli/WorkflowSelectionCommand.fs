...
module WorkflowSelectionCommand =
    let private usage () =
        eprintfn
            "workflow-select --inventory FILE --request FILE --expected-inventory-version VERSION --expected-graph-version VERSION --expected-seal SHA256 (--authority FILE | --current-base SHA --current-settings SHA256 --current-queued-head SHA|none) | workflow-select --seal-inventory FILE"
    ...
    let private objectAt name (value: JsonNode) =
        match value with
        | :? JsonObject as result -> result
        | _ -> invalidArg name "object required"

    let private arrayAt name (value: JsonNode) =
        match value with
        | :? JsonArray as result -> result
        | _ -> invalidArg name "array required"
    ...
    let private texts (name: string) (value: JsonObject) =
        arrayAt name value[name] |> Seq.map _.GetValue<string>() |> Seq.toList
    ...
    let private obligations (name: string) (value: JsonObject) =
        texts name value |> List.map parseObligation
    ...
    let private parseRule (value: JsonNode) =
        ...
        {
            Id = text "id" item
            Pattern = text "pattern" item
            Match = text "match" item |> parseMatch
            Roots = obligations "roots" item
        }

    let private rules (name: string) (value: JsonObject) =
        arrayAt name value[name] |> Seq.map parseRule |> Seq.toList
    ...
    let private parseDependency (value: JsonNode) =
        ...
        {
            Source = text "source" item |> parseObligation
            Target = text "target" item |> parseObligation
        }
    ...
    let private loadObject path =
        ...
        while reader.Read() do
            ...
            | JsonTokenType.PropertyName ->
                ...
                | current :: _ ->
                    ...
                    if not (current.Add name) then
                        invalidArg path $"duplicate property '{name}'"
        ...
        if isNull node then
            invalidArg path "JSON document is empty"
    ...
    let private parseInventory path =
...

        {
            ...
            Dependencies =
                arrayAt "dependencies" value["dependencies"]
    ...
    let private parseRequest path =
        ...
        let mergeGroup =
            if isNull value["mergeGroup"] then
                None
            else
    ...
    let private parseAuthority path =
        ...
        | "fsgg.coordination.workflow-selection-authority/1" ->
            ...
            text "baseRevision" value,
            text "currentRevision" value,
            text "baseRevision" value,
            text "settingsSha256" value,
            text "queuedHead" value,
            None
        | "fsgg.coordination.workflow-selection-authority/2" ->
            ...
            text "inventoryBaseRevision" value,
            text "currentRevision" value,
            text "eventBaseRevision" value,
            text "settingsSha256" value,
            text "queuedHead" value,
    ...
    let private refusalCode =
        ...
        | IncompleteInventory -> "incomplete-inventory"
        | IncompleteRequest -> "incomplete-request"
        | InvalidInventory _ -> "invalid-inventory"
        | InventorySealMismatch _ -> "inventory-seal-mismatch"
        | StaleBaseRevision _ -> "stale-base-revision"
        | StaleSettings _ -> "stale-settings"
        | UnknownChangedPath _ -> "unknown-changed-path"
        | UnknownNonFileInput _ -> "unknown-non-file-input"
        | AmbiguousChangedPath _ -> "ambiguous-changed-path"
        | AmbiguousNonFileInput _ -> "ambiguous-non-file-input"
    ...
    let private refusalMessage =
        ...
        | UnknownChangedPath value
        | UnknownNonFileInput value -> value
        | AmbiguousChangedPath(value, ids)
        | AmbiguousNonFileInput(value, ids) ->
            let joined = String.concat "," ids
            $"{value};rules={joined}"
        | InvalidMergeGroup reason -> reason
        | IncompleteInventory
        | IncompleteRequest -> "required input is incomplete"

    let private writeDecision decision =
        use writer =
            new Utf8JsonWriter(Console.OpenStandardOutput(), JsonWriterOptions(Indented = false))

        writer.WriteStartObject()
        writer.WriteString("schema", "fsgg.coordination.workflow-selection-decision/1")
        writer.WriteString("inventoryVersion", decision.InventoryVersion)
        writer.WriteString("graphVersion", decision.GraphVersion)
        writer.WriteString("inventorySeal", decision.InventorySeal)
        writer.WriteStartArray("roots")

        decision.Roots
        |> List.iter (WorkflowSelection.obligationId >> writer.WriteStringValue)

        writer.WriteEndArray()
        writer.WriteStartArray("closure")
        ...
        for child in decision.Children do
            writer.WriteStartObject()
            writer.WriteString("obligation", WorkflowSelection.obligationId child.Obligation)

            match child.Disposition with
            | Selected ->
                writer.WriteString("disposition", "selected")
                writer.WriteNull("reason")
            | NotApplicable reason ->
                writer.WriteString("disposition", "not-applicable")
                writer.WriteString("reason", reason)

            writer.WriteBoolean("provisionExpensiveJob", child.ProvisionExpensiveJob)
        ...
        for aggregate in decision.Aggregates do
            ...
            writer.WriteString("status", aggregate.Status)
            writer.WriteNumber("selectedCount", aggregate.SelectedCount)
            writer.WriteNumber("notApplicableCount", aggregate.NotApplicableCount)
            writer.WriteEndObject()

        writer.WriteEndArray()

        match decision.MergeGroupQueuedHead with
        | Some value -> writer.WriteString("mergeGroupQueuedHead", value)
        | None -> writer.WriteNull("mergeGroupQueuedHead")
    ...
    let private writeRefusals refusals =
        use writer =
            new Utf8JsonWriter(Console.OpenStandardError(), JsonWriterOptions(Indented = false))

        writer.WriteStartObject()
        writer.WriteString("schema", "fsgg.coordination.workflow-selection-refusal/1")
        writer.WriteStartArray("findings")

        for refusal in refusals do
            writer.WriteStartObject()
            writer.WriteString("code", refusalCode refusal)
            writer.WriteString("message", refusalMessage refusal)
            writer.WriteEndObject()
    ...
    let run arguments =
        let rec parse
            ...
            | "--seal-inventory" :: value :: rest ->
                parse
                    ...
                    currentQueuedHead
                    rest
            | "--expected-inventory-version" :: value :: rest ->
                parse
                    inventory
                    request
                    seal
                    ...
                    currentQueuedHead
                    rest
            | "--expected-graph-version" :: value :: rest ->
                parse
                    inventory
                    request
                    seal
            ...
            | "--current-settings" :: value :: rest ->
                parse
                    ...
                    currentQueuedHead
                    rest
            | "--current-queued-head" :: value :: rest ->
                parse
                    inventory
                    request
                    seal
        ...
        | None, None, Some inventoryPath, None, None, None, None, None, None, None ->
            try
                printfn "%s" (parseInventory inventoryPath |> WorkflowSelection.computeInventorySeal)
                0
            with ex ->
          ...
          legacyQueuedHead ->
            try
                ...
                let currentBase, currentRevision, eventBase, currentSettings, currentQueuedHead, contentAuthority =
                    ...
                    | None, Some baseRevision, Some settingsSha256, Some queuedHead ->
                        baseRevision, baseRevision, baseRevision, settingsSha256, queuedHead, None
                ...
                let authorityRefusals =
                    [
                        ...
                        | Some(inventorySha, requestSha, sourceRequestSha) ->
                            if
                                not (Regex.IsMatch(sourceRequestSha, "^[0-9a-f]{64}$", RegexOptions.CultureInvariant))
                            then
                                InvalidMergeGroup "source request authority is malformed"

                            if sha256File inventoryPath <> inventorySha then
                                InvalidInventory "inventory content authority"

                            if sha256File requestPath <> requestSha then
                                IncompleteRequest
                        | None -> ()
                        if
                            expectedInventory <> WorkflowSelection.supportedInventoryVersion
                            || inventory.InventoryVersion <> expectedInventory
                            || request.InventoryVersion <> expectedInventory
                        then
                            UnsupportedInventoryVersion(expectedInventory, request.InventoryVersion)
                        if
                            expectedGraph <> WorkflowSelection.supportedGraphVersion
                            || inventory.GraphVersion <> expectedGraph
                            || request.GraphVersion <> expectedGraph
                        then
                            UnsupportedGraphVersion(expectedGraph, request.GraphVersion)
                        if inventory.Seal <> expectedSeal || request.ExpectedInventorySeal <> expectedSeal then
                            InventorySealMismatch(expectedSeal, inventory.Seal)
                        if request.BaseRevision <> currentBase then
                            StaleBaseRevision(currentBase, request.BaseRevision)
                        if request.SettingsSha256 <> currentSettings then
                            StaleSettings(currentSettings, request.SettingsSha256)
                        match request.MergeGroup with
                        | None when currentQueuedHead <> "none" ->
                            InvalidMergeGroup "current queued-head authority must be none outside merge group"
                        | Some merge when merge.QueuedHead <> currentQueuedHead ->
                            InvalidMergeGroup "queued head authority differs"
                        | Some merge when merge.CurrentQueuedHead <> currentQueuedHead ->
                            InvalidMergeGroup "current queued head authority differs"
                        | Some merge when merge.CurrentBaseRevision <> eventBase ->
                            InvalidMergeGroup "current base authority differs"
                        | Some merge when merge.CurrentSettingsSha256 <> currentSettings ->
                            InvalidMergeGroup "current settings authority differs"
                        | _ -> ()
                    ]

                match authorityRefusals, WorkflowSelection.select inventory request with
                | refusal :: rest, _ ->
                    writeRefusals (refusal :: rest)
                    3
                | [], Ok decision ->
