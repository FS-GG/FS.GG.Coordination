...
if args.Length <> 4 then
    failwith "usage: dotnet fsi eng/probe-migration-pull-request-comments.fsx -- OWNER REPOSITORY EXPECTED_REPOSITORY_ID PULL_REQUEST_NUMBER"
...
let options =
    { ApiBase=Uri "https://api.github.com/"
      ...
      Token=token
      UserAgent="fsgg-coordination-migration-pr-comment-probe"
      Owner=args.[0]
      Repository=args.[1]
...
let read () =
    ...
    let comments =
        match MigrationGitHubRead.readPullRequestComments options pullRequests pullRequestNumber transport with
        | Ok result -> result
        | Error failure -> failwithf "pull request comment stream refused: %A" failure
    issues, pullRequests, comments

let fingerprint (issues: MigrationIssuePopulation, pullRequests: MigrationPullRequestPopulation,
                 comments: MigrationIssueCommentPopulation) =
    let values =
        [ string issues.RepositoryId; string issues.PageCount; string issues.PullRequestCount
          ...
          string comments.SubjectNumber; comments.SubjectNodeId; string comments.PageCount ]
...
if firstDigest <> secondDigest then failwith "two read-only pull request comment passes were not quiescent"

let issues, pullRequests, comments = second
printfn "MIGRATION_PULL_REQUEST_COMMENTS_PROBE_OK repositoryId=%d issuePages=%d issues=%d pullRequestPages=%d pullRequests=%d subject=%d commentPages=%d comments=%d twoPassSha256=%s"
    repositoryId issues.PageCount issues.Issues.Length pullRequests.PageCount
    pullRequests.PullRequests.Length pullRequestNumber comments.PageCount comments.Comments.Length secondDigest
client.Dispose()
