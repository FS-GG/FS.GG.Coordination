...
if
    text "schema" c <> "fsgg.github-event-envelope-evidence/v1"
    || text "unit" c <> "GS2-07.1"
then
    failwith "evidence contract identity differs"

if
    shaFile "evidence/github-substrate-v2/accepted/GS2-06.8.json"
...
if shaFile "src/FS.GG.Coordination.Protocol/Protocol.md" <> text "protocolSha256" c then
    failwith "canonical Quint protocol changed"
...
let source =
    {
        ...
        SourceRevision = text "sourceRevision" c
    }
...
let delivery pos did eid subject revision cause correlation receipt =
    {
        ...
        EventId = eid
        Subject = subject
        SubjectRevision = string revision
        CausationId = cause
...
let d1 =
    delivery 1L "delivery-1" "event-1" "issue-294" 1 "cause-root" "corr-294" "receipt-1"

let d2 =
    delivery 2L "delivery-2" "event-2" "issue-294" 2 "event-1" "corr-294" "receipt-2"
...
let sourceText =
    File.ReadAllText(path "src/FS.GG.Coordination.Qualification.Contracts/GitHubEventEnvelopeQualification.fs")
...
let executeGenerated control =
    ...
    | EventNoNetwork ->
        let detector (v: string) =
            v.Contains("HttpClient") || v.Contains("WebRequest") in

        not (detector sourceText)
    | EventNoQueue ->
        let detector (v: string) =
            v.Contains("enqueue") || v.Contains("dequeue") in

        not (detector sourceText)
    | EventNoMutation ->
        let detector (v: string) =
            v.Contains("Octokit") || v.Contains("PATCH ") || v.Contains("POST ") in
...
let executeIndependent control =
    match control with
    | EventPrerequisites ->
        text "prerequisiteReceiptDigest" c = "c8831d8e3b06f77ae26d23579b738347794a8d08e460c84c5856cbbff50abd0e"
    ...
    | EventCursor ->
        ...
        (compile source [ left ] |> get).Cursor
        <> (compile source [ right ] |> get).Cursor
    | EventOrdering ->
        verify
            baseline.Seal
            { baseline with
    ...
    | EventNoNetwork ->
        let detector (v: string) =
            Regex.IsMatch(v, "httpclient|webrequest|webhook", RegexOptions.IgnoreCase) in

        not (detector sourceText) && detector (sourceText + "\nhttpCLIENT")
    | EventNoQueue ->
        let detector (v: string) =
            Regex.IsMatch(v, "enqueue|dequeue|queueclient", RegexOptions.IgnoreCase) in

        not (detector sourceText) && detector (sourceText + "\nEnQueue")
    | EventNoMutation ->
        let detector (v: string) =
            Regex.IsMatch(v, "octokit|githubclient|\\b(patch|post|put|delete)\\b", RegexOptions.IgnoreCase) in
...
let generated: GitHubEventEnvelopeControlResult list =
    requiredControls
...
let independent: GitHubEventEnvelopeControlResult list =
    requiredControls
    |> List.map (fun control ->
        {
            Control = control
...
let retained relative =
    ...
    let controls =
        root.GetProperty("controls").EnumerateArray()
        |> Seq.map _.GetString()
        |> Seq.toList
...
let generatedIds, generatedCases, generatedContract =
    retained "evidence/github-substrate-v2/gs2-07-1/generated-controls.json"

let independentIds, independentCases, independentContract =
    retained "evidence/github-substrate-v2/gs2-07-1/independent-controls.json"

if
    generatedIds <> ids
    || generatedCases.Length <> ids.Length
...
if
    independentIds <> ids
    || independentCases.Length <> ids.Length
...
printfn
    "GITHUB_EVENT_ENVELOPE_OK deliveries=%d cursor=%d controls=%d seal=%s"
    baseline.Deliveries.Length
    baseline.Cursor.Length
    ids.Length
    baseline.Seal
