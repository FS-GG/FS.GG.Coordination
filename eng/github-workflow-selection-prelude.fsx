...
let texts (name: string) (value: JsonObject) =
    value[name] |> arr |> List.map _.GetValue<string>()
...
let obligation: string -> GitHubWorkflowObligation =
    function
...
let metrics (node: JsonNode) =
    ...
    {
        WorkflowFanOut = integer "workflowFanOut" value
        JobFanOut = integer "jobFanOut" value
        BilledMinutes = integer "billedMinutes" value
        QueueTimeSeconds = integer "queueTimeSeconds" value
        P50Seconds = integer "p50Seconds" value
        P95Seconds = integer "p95Seconds" value
    }
...
let parseSnapshot (path: string) =
        ...
        |> List.map (fun node ->
            ...
            exactProperties
                ...
                [
                    ...
                    "policyJobs"
                    "compositeSteps"
                    "reusableJobContracts"
                    "aggregateOutputs"
                ]
                value

            {
                Workflow = text "workflow" value
                PolicyJobs = texts "policyJobs" value
                CompositeSteps = texts "compositeSteps" value
                ReusableJobContracts = texts "reusableJobContracts" value
                AggregateOutputs = texts "aggregateOutputs" value
            })

    let edges =
        root["dependencyEdges"]
        |> arr
        |> List.map (fun node ->
            let value = obj node
            ...
            {
                Source = obligation (text "source" value)
                Target = obligation (text "target" value)
            })
        ...
        |> List.map (fun node ->
            ...
            let mergeGroup =
                if isNull (value["mergeGroup"]) then
                    ...
                    Some
                        {
                            QueuedHead = text "queuedHead" merge
                            CurrentBase = text "currentBase" merge
                            CurrentSettings = text "currentSettings" merge
                            ObservedBase = text "observedBase" merge
                            ObservedSettings = text "observedSettings" merge
                            Recomputed = boolean "recomputed" merge
                        }

            {
                Id = text "id" value
                ChangedSubjects = texts "changedSubjects" value
                NonFileInputs = texts "nonFileInputs" value
                Roots = obligations "roots" value
                ExpectedClosure = obligations "expectedClosure" value
                Unknown = boolean "unknown" value
                Ambiguous = boolean "ambiguous" value
                Fresh = boolean "fresh" value
    ...
    let childOutcomes =
        ...
        |> List.map (fun node ->
            ...
            let disposition =
                ...
                | "selected" ->
                    if not (isNull value["reason"]) then
                        failwith "selected child carries a reason"
            ...
            {
                Obligation = obligation (text "obligation" value)
    ...
    let performance =
        ...
        |> List.map (fun node ->
            ...
            exactProperties
                "performance"
                [ "repository"; "baseline"; "target"; "selected"; "selectedKind"; "provenance" ]
                value
            ...
            {
                Repository = text "repository" value
                Baseline = metrics value["baseline"]
                Target = metrics value["target"]
                Selected = metrics value["selected"]
                SelectedKind = text "selectedKind" value
                Provenance =
                    {
                        ObservationId = text "observationId" provenance
                        Query = text "query" provenance
                        ...
                        RunSampleCount = integer "runSampleCount" provenance
                        JobSampleCount = integer "jobSampleCount" provenance
                        Aggregation = text "aggregation" provenance
                        Complete = boolean "complete" provenance
                        IndependentRecomputed = boolean "independentRecomputed" provenance
                        ReviewedBy = text "reviewedBy" provenance
                        ReviewedAt = DateTimeOffset.Parse(text "reviewedAt" provenance)
    ...
    let removals =
        ...
        |> List.map (fun node ->
            ...
            {
                Workflow = text "workflow" value
                Obligation = text "obligation" value
...

    {
        ...
        RoadmapRevision = text "roadmapRevision" root
        RoadmapSha256 = text "roadmapSha256" root
        PrerequisiteReceiptDigest = text "prerequisiteReceiptDigest" root
        Complete = boolean "complete" root
        InventoryComplete = boolean "inventoryComplete" root
        NonFileInputInventoryComplete = boolean "nonFileInputInventoryComplete" root
        GraphVersion = text "graphVersion" root
        Workflows = workflows
        Obligations = obligations "obligations" root
        DependencyEdges = edges
        UnconditionalObligations = obligations "unconditionalObligations" root
        ImpactCases = impactCases
        ChildOutcomes = childOutcomes
        RequiredAggregates = texts "requiredAggregates" root
        UnconditionalCore = obligations "unconditionalCore" root
        ObservationSha256 = text "observationSha256" root
        Performance = performance
        Sentinel =
            {
                Scheduled = boolean "scheduled" sentinelNode
                SelectedClosure = obligations "selectedClosure" sentinelNode
                ActualFailures = obligations "actualFailures" sentinelNode
            }
        FleetSelectionEnabled = boolean "fleetSelectionEnabled" root
        RemovalLedgerComplete = boolean "removalLedgerComplete" root
        RemovalLedgerSha256 = text "removalLedgerSha256" root
        Removals = removals
    }

let parseExpectations path =
    JsonNode.Parse(File.ReadAllText path) |> obj

let expectCompileError snapshot =
    GitHubWorkflowSelectionQualification.compile snapshot |> Result.isError

let expectVerifyError seal snapshot =
    GitHubWorkflowSelectionQualification.verify seal snapshot |> Result.isError
