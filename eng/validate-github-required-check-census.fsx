...
let source value =
    if value = "classic" then
        ...
        failwith $"unsupported required-check source {value}"
...
let snapshot =
    {
        ...
        Requirements =
            corpus["requirements"].AsArray()
            |> Seq.map (requirement repository)
            |> List.ofSeq
...
let compile candidate =
    RequiredCheckCensusAdapter.compile asOf (TimeSpan.FromHours 1) candidate

let refused candidate = compile candidate |> Result.isError

let report =
    compile snapshot
    |> Result.defaultWith (failwithf "required-check census baseline refused: %A")

let altered candidate =
    RequiredCheckCensusAdapter.verify report.Seal asOf (TimeSpan.FromHours 1) candidate
    |> Result.isError

let workflowPath =
    report.Entries
    |> List.map _.ProducerWorkflow
    |> List.distinct
    |> List.exactlyOne
...
let workflowSha256 =
    workflowBytes |> SHA256.HashData |> Convert.ToHexString |> _.ToLowerInvariant()

if
    report.Entries
    |> List.exists (fun entry -> entry.ProducerWorkflowSha256 <> workflowSha256)
then
...
let authoritySha256 =
    authorityBytes |> SHA256.HashData |> Convert.ToHexString |> _.ToLowerInvariant()

if snapshot.AuthorityEvidenceSha256 <> authoritySha256 then
    failwith "authority evidence digest differs"

let authorities = JsonNode.Parse(authorityBytes).AsObject()
let classic = authorities["classicProtection"].AsObject()

if
    classic["httpStatus"].GetValue<int>() <> 404
    || classic["requirements"].AsArray().Count <> 0
then
    failwith "classic protection absence evidence differs"

let rulesets = authorities["rulesets"].AsArray()

if rulesets.Count <> 1 then
    failwith "ruleset authority count differs"

let ruleset = rulesets[0].AsObject()

if ruleset["id"].GetValue<int64>() <> 21633423L then
    failwith "ruleset authority differs"
...
let retainedChecks =
    report.Entries
    |> List.map (fun entry -> entry.Context, entry.IntegrationId.Value)

if authoritativeChecks <> retainedChecks then
    failwith "retained census differs from authority evidence"
...
if fsi.CommandLineArgs |> Array.contains "--mint" then
    ...
    let expectedControls =
        ...
        |> List.map GitHubRequiredCheckCensusQualification.controlId

    if texts expectations "controls" <> expectedControls then
        failwith "independent control inventory differs from closed qualification contract"

    if text expectations "repository" <> report.Repository then
        failwith "repository differs"

    if texts expectations "expectedOrder" <> (report.Entries |> List.map _.Context) then
        failwith "stable order differs"

    let expectedInt (name: string) = expectations[name].GetValue<int>()
    let aggregate = report.Aggregate

    if
        aggregate.RequiredCount <> expectedInt "requiredCount"
        ...
        || aggregate.PullRequestUnconditionalCount
           <> expectedInt "pullRequestUnconditionalCount"
        || aggregate.MergeGroupUnconditionalCount
           <> expectedInt "mergeGroupUnconditionalCount"
    then
        failwith "stable aggregate differs"

    if report.Seal <> text expectations "expectedSeal" then
        failwith "census seal differs"

    if
        RequiredCheckCensusAdapter.verify report.Seal asOf (TimeSpan.FromHours 1) snapshot
        <> Ok report
    then
        failwith "exact census replay failed"

    let replaceProducer context transform =
        { snapshot with
            Producers =
                snapshot.Producers
                |> List.map (fun value -> if value.Context = context then transform value else value)
        }
    ...
    let generatedMutation =
        ...
        | SourceBinding ->
            let renamed =
                replaceProducer "compiler-and-tests" (fun value ->
                    { value with
                        Workflow = ".github/workflows/renamed.yml"
                    })
            ...
            && RequiredCheckCensusAdapter.verify report.Seal asOf (TimeSpan.FromHours 1) renamed =
                Error [ AlteredRequiredCheckCensusSeal ]
        | CompleteAuthorities ->
            refused
                { snapshot with
        ...
        | StableOrdering ->
            compile
                { snapshot with
                    Requirements = List.rev snapshot.Requirements
                    Producers = List.rev snapshot.Producers
                }
                =
                Ok report
        | ExactIdentity ->
            let mixed =
                { snapshot.Requirements.Head with
                    IntegrationId = None
                }

            refused
                { snapshot with
                    Requirements = mixed :: snapshot.Requirements
                }
        | AuthorityUnion ->
            let changed =
                { snapshot with
                    Requirements =
                        ...
                        |> List.map (fun value -> { value with Source = Ruleset 99L })
                }

            (report.Entries
             |> List.forall (fun entry -> entry.Sources = [ Ruleset 21633423L ]))
            && altered changed
        | ProvenanceRetention ->
            report.Entries
            |> List.forall (fun entry ->
                entry.Sources = [ Ruleset 21633423L ]
                && entry.ProducerWorkflow = ".github/workflows/bootstrap-qualification.yml")
        | ProducerCompleteness ->
            refused
                { snapshot with
                    Producers = snapshot.Producers.Tail
                }
            ...
            refused
                { snapshot with
                    Requirements =
                        ...
                        :: snapshot.Requirements.Tail
                }
        | Freshness ->
            RequiredCheckCensusAdapter.compile (asOf.AddHours 2) (TimeSpan.FromHours 1) snapshot
            |> Result.isError
        | StableAggregates ->
            not ((string aggregate).Contains("compiler-and-tests"))
            && aggregate.RequiredCount = report.Entries.Length
        | ExactSeal ->
            RequiredCheckCensusAdapter.verify (String.replicate 64 "0") asOf (TimeSpan.FromHours 1) snapshot
            |> Result.isError
        ...
        | NoPlanSurface ->
            not (
                File
                    .ReadAllText(Path.Combine(root, "src/FS.GG.Coordination.GitHub/RequiredCheckCensusAdapter.fsi"))
                    .Contains("val plan")
            )
        | NoApplySurface ->
            not (
    ...
    let independentMutation =
        ...
        | PrerequisiteReceipt ->
            let receipt =
                JsonNode.Parse(
                    File.ReadAllText(Path.Combine(root, "evidence/github-substrate-v2/accepted/GS2-06.1.json"))
                )
        ...
        | SourceBinding ->
            snapshot.SourceRevision = "f3a92488d6c15e1a4592686c6f00c375c62b167d"
            && workflowSha256 = "0b913aab5149d035addd280adbe7ed069dc2df9a25a062add4b46a0aba44bd4a"
        | CompleteAuthorities ->
            snapshot.Complete
            && snapshot.ClassicComplete
            && snapshot.RulesetsComplete
            && snapshot.ProducersComplete
        | StableOrdering ->
            report.Entries |> List.map _.Context =
                [
                    "bootstrap-recovery"
                    "compiler-and-tests"
        ...
        | EventFilters ->
            ...
            |> List.forall (fun value ->
                value.PullRequest.BranchFilters.IsEmpty
                && value.PullRequest.PathFilters.IsEmpty
                && value.PullRequest.ActivityTypes.IsEmpty
                && value.MergeGroup.BranchFilters.IsEmpty
                && value.MergeGroup.PathFilters.IsEmpty
                && value.MergeGroup.ActivityTypes.IsEmpty)
        | JobConditions ->
            snapshot.Producers
            |> List.forall (fun value -> value.Conditional && not value.ContinueOnError)
        | DependencyClosure -> snapshot.Producers |> List.forall _.DependenciesComplete
        | RepositoryBoundary ->
            snapshot.Requirements |> List.forall (_.Repository >> (=) snapshot.Repository)
            && snapshot.Producers |> List.forall (_.Repository >> (=) snapshot.Repository)
        | Freshness -> asOf - snapshot.ObservedAt <= TimeSpan.FromHours 1
        | StableAggregates ->
            aggregate.RequiredCount = 6
            && aggregate.DualSourceCount
               + aggregate.RulesetOnlyCount
               + aggregate.ClassicOnlyCount
                =
    ...
    | Ok() ->
        printfn
            "GITHUB_REQUIRED_CHECK_CENSUS_OK repository=%s required=%d classicOnly=%d rulesetOnly=%d dual=%d controls=%d seal=%s"
            report.Repository
            aggregate.RequiredCount
            aggregate.ClassicOnlyCount
            aggregate.RulesetOnlyCount
