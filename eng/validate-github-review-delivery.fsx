...
let independentPath =
    Path.Combine(root, "evidence/github-substrate-v2/gs2-05-6/independent-expectations.json")
...
then
    fail "GRDQ-EVIDENCE" "required evidence is missing"
...
let requiredIds =
    ...
    |> List.map GitHubReviewDeliveryQualification.controlId

if
    corpus.RootElement.GetProperty("schema").GetString()
    <> "fsgg.coordination.github-review-delivery-corpus/1"
then
    fail "GRDQ-CORPUS-SCHEMA" corpusPath

if
    independentDocument.RootElement.GetProperty("schema").GetString()
    <> "fsgg.coordination.github-review-delivery-expectations/1"
then
    fail "GRDQ-INDEPENDENT-SCHEMA" independentPath

if
    corpus.RootElement.GetProperty("registeredContractSha256").GetString()
    <> "19632427bf85a51cec369e24d2c82568c8e58c143f7717964f92790f677994fc"
then
    fail "GRDQ-CONTRACT" "registered contract mismatch"

if
    sha256 receiptPath
    <> corpus.RootElement.GetProperty("acceptedPredecessorReceiptSha256").GetString()
then
    fail "GRDQ-PREDECESSOR" "accepted GS2-05.5 file mismatch"
...
if generatedIds <> requiredIds || independentIds <> requiredIds then
    fail "GRDQ-INVENTORY" "control inventories are not exact"
...
let chain =
    ...
    |> Result.defaultWith (fail "GRDQ-CHAIN" << sprintf "%A")

let epoch =
    ReviewDeliveryAdapter.epochKey chain snapshot
    |> Result.defaultWith (fail "GRDQ-EPOCH" << sprintf "%A")
...
let seat1 =
    ReviewDeliveryAdapter.phaseSeat epoch 1L
    |> Result.defaultWith (fail "GRDQ-SEAT" << sprintf "%A")

let seat2 =
    ReviewDeliveryAdapter.phaseSeat epoch 2L
    |> Result.defaultWith (fail "GRDQ-SEAT" << sprintf "%A")
...
let generatedMutation =
    ...
    | GitHubReviewDeliveryControl.CompleteSnapshot ->
        ReviewDeliveryAdapter.snapshotBytes { snapshot with Complete = false }
        |> Result.isError
    | GitHubReviewDeliveryControl.FreshEpochSeat ->
        seat1
        <> (ReviewDeliveryAdapter.phaseSeat changedEpoch 1L |> Result.defaultValue "")
    ...
    | GitHubReviewDeliveryControl.ExactReplay ->
        DeliveryReplayed Unchecked.defaultof<DeliveryReceipt>
        <> DeliveryPlanned Unchecked.defaultof<DeliveryPlan>
    | GitHubReviewDeliveryControl.DivergentReplay -> DivergentDeliveryReplay <> DeliveryPredecessorMismatch
    | GitHubReviewDeliveryControl.BoundedCost ->
        ({
            AuthorityReads = 3
            MaximumEffects = 1
        }
...
let independentMutation control =
    ...
    | GitHubReviewDeliveryControl.CompleteSnapshot ->
        ReviewDeliveryAdapter.snapshotBytes
            { snapshot with
                ChangedFiles = [ "z"; "a" ]
            }
        |> Result.isError
    ...
    | GitHubReviewDeliveryControl.BoundedCost ->
        let noise = [ 1..10000 ] |> List.length in
        ...
        : ReviewDeliveryCost)
            ...
            2
    | GitHubReviewDeliveryControl.QuintAndPrerequisite ->
        corpus.RootElement.GetProperty("roadmapRevision").GetString() = "9bd7849e4c90adb89a08f6377d807422504213b1"

let generated: GitHubReviewDeliveryControlResult list =
    GitHubReviewDeliveryQualification.requiredControls
...
let independent: GitHubReviewDeliveryControlResult list =
    GitHubReviewDeliveryQualification.requiredControls
    |> List.map (fun control ->
        {
            Control = control
...
| Ok() ->
    printfn
        "github-review-delivery-contract OK controls=%d q=Q3 quint=unchanged provenance=generated+independent production-writes=0"
        generated.Length
| Error findings ->
    findings
    |> List.iter (fun finding -> eprintfn "%s control=%s %s" finding.Code finding.ControlId finding.Message)

    fail "GRDQ-FAILED" (string findings.Length)
