...
let prestateDocument =
    json "evidence/github-substrate-v2/gs2-07-6/hosted-prestate.json"
...
let checkpointDocument =
    json "evidence/github-substrate-v2/gs2-07-6/durable-checkpoint.json"

let checkpoint = checkpointDocument.RootElement

let checkpointJournalDocument =
    json "evidence/github-substrate-v2/gs2-07-6/checkpoint-journal.json"

let checkpointJournal = checkpointJournalDocument.RootElement

let finalJournalDocument =
    json "evidence/github-substrate-v2/gs2-07-6/final-journal.json"

let finalJournal = finalJournalDocument.RootElement

let refusalDocument =
    json "evidence/github-substrate-v2/gs2-07-6/expired-admission-refusal.json"

let refusal = refusalDocument.RootElement

let handoffDocument =
    json "evidence/github-substrate-v2/gs2-07-6/process-handoff.json"

let handoff = handoffDocument.RootElement

let cleanupReadbackDocument =
    json "evidence/github-substrate-v2/gs2-07-6/cleanup-readback.json"

let cleanupReadback = cleanupReadbackDocument.RootElement

let observedRecordDocument =
    json "evidence/github-substrate-v2/gs2-07-6/authority-observed.json"

let observedRecord = observedRecordDocument.RootElement

let currentRecordDocument =
    json "evidence/github-substrate-v2/gs2-07-6/authority-current.json"
...
let stringsAt (node: JsonElement) (name: string) =
...
let pilotFacts: QueuePilotFacts =
    {
        Repository = stringAt hosted "repository"
        RepositoryId = int64At hosted "repositoryId"
        IsProduction = false
        ProviderCapability = QueueProviderCapability.Supported
        PreVisibility = stringAt settings "visibility"
        RepositorySecretCount = secrets.GetProperty("repositorySecretCount").GetInt32()
        EnvironmentSecretCount = secrets.GetProperty("environmentSecretCount").GetInt32()
        PilotId = stringAt hosted "pilotId"
        CandidateSha = stringAt candidate "sha"
        CurrentCandidateSha = stringAt candidate "currentShaAtReevaluation"
        MergeGroupHeadSha = stringAt recoveryRun "headSha"
        BaseRef = stringAt recovery "baseRef"
        ObservedBaseSha = stringAt recovery "priorBaseSha"
        CurrentBaseSha = stringAt recovery "baseSha"
        ReevaluatedBaseSha = stringAt recovery "baseSha"
        BaseObservationRevision = int64At initialRun "id"
        CurrentBaseObservationRevision = int64At recoveryRun "id"
        OriginalRequiredChecks = stringsAt initial "requiredChecks"
        CurrentRequiredChecks = checks
        CheckResults =
            recovery.GetProperty("jobs").EnumerateArray()
            |> Seq.map (fun job ->
                ({
                    Name = stringAt job "name"
                    HeadSha = stringAt job "headSha"
                    EventName = stringAt recoveryRun "event"
                    Conclusion =
                        if stringAt job "conclusion" = "success" then
                            QueueCheckConclusion.Success
                        else
                            QueueCheckConclusion.Failure
                }
                : QueueCheckResult))
            |> Seq.toList
        ObservedClaimGeneration = int64At observedAuthority "claimGeneration"
        CurrentClaimGeneration = int64At currentAuthority "claimGeneration"
        ObservedReviewDigest = stringAt observedAuthority "reviewDigest"
        CurrentReviewDigest = stringAt currentAuthority "reviewDigest"
        ObservedDependencyDigest = stringAt observedAuthority "dependencyDigest"
        CurrentDependencyDigest = stringAt currentAuthority "dependencyDigest"
        ObservedReleaseObligationsMet = observedAuthority.GetProperty("releaseObligationsMet").GetBoolean()
        CurrentReleaseObligationsMet = currentAuthority.GetProperty("releaseObligationsMet").GetBoolean()
        ObservedSettingsDigest = stringAt observedAuthority "settingsDigest"
        CurrentSettingsDigest = stringAt currentAuthority "settingsDigest"
        AdmittedAtUnixSeconds = int64At recovery "admittedAtUnixSeconds"
        ExpiresAtUnixSeconds = int64At recovery "expiresAtUnixSeconds"
        EvaluatedAtUnixSeconds = int64At recovery "evaluatedAtUnixSeconds"
    }
...
let effectsAt (node: JsonElement) (name: string) =
    ...
    |> Seq.map (fun effect ->
        ({
            OperationId = stringAt effect "operationId"
            Attempt = effect.GetProperty("attempt").GetInt32()
            ResultDigest = stringAt effect "resultDigest"
        }
        : QueueEffect))
    |> Seq.toList
...
let compensationsAt (node: JsonElement) (name: string) =
    ...
    |> Seq.map (fun row ->
        ({
            OperationId = stringAt row "operationId"
            CompensationId = stringAt row "compensationId"
            FinalStateDigest = stringAt row "finalStateDigest"
        }
        : QueueCompensation))
    |> Seq.toList
...
let recoveryFacts =
    {
        ...
        PilotSeal = pilot.Seal
        DurableCheckpointDigest = stringAt initial "durableCheckpointDigest"
        ResumeCheckpointDigest = stringAt initial "resumeCheckpointDigest"
        Interrupted = initial.GetProperty("interrupted").GetBoolean()
        FailedStepInjected = initial.GetProperty("failedStepInjected").GetBoolean()
        AppliedEffects = effects
        RetryEffects = retryEffects
        Compensations = compensations
        DuplicateEffectCount = recovery.GetProperty("duplicateEffectCount").GetInt32()
        TemporaryResourceCount =
            cleanup.GetProperty("temporaryBranchCount").GetInt32()
            + cleanup.GetProperty("queueRefCount").GetInt32()
            + cleanup.GetProperty("temporaryWorkflowFileCount").GetInt32()
        PreVisibility = stringAt settings "visibility"
        FinalVisibility = stringAt cleanup "finalVisibility"
        PreSettingsDigest = stringAt settings "digest"
        FinalSettingsDigest = stringAt cleanup "finalSettingsDigest"
        Recoverable = true
    }
...
let source =
    read "src/FS.GG.Coordination.Qualification.Contracts/GitHubQueueSandbox.fs"
...
let roadmap () =
    text "roadmapRevision" = "7216ec4aae14b17f151a1ed3616eb8a2f4ed2d47"
    && text "roadmapSha256" = "0498209c27cdf75d3c1067dad2c3b88084b03c20f1bd87dcb99192aa43457c36"
...
let hostedRecovery () =
    ...
    let expectedQueueRef =
        $"refs/heads/gh-readonly-queue/{baseName}/pr-{pullRequestNumber}-{recoveryBaseSha}"
    ...
    let authoritySources (row: JsonElement) =
        ...
        && stringAt row "reviewSource" =
            "https://github.com/FS-GG/FS.GG.Coordination/issues/320#issuecomment-5564723174"
    ...
    let effectRows (node: JsonElement) name =
        effectsAt node name
        |> List.map (fun row -> row.OperationId, row.Attempt, row.ResultDigest)

    let compensationRows (node: JsonElement) name =
        compensationsAt node name
        |> List.map (fun row -> row.OperationId, row.CompensationId, row.FinalStateDigest)
    ...
    && int64At initial "expiredObservedAtUnixSeconds"
       >= int64At initial "expiresAtUnixSeconds"
    && stringAt initial "durableCheckpointDigest" = stringAt initial "resumeCheckpointDigest"
    && shaFile "evidence/github-substrate-v2/gs2-07-6/durable-checkpoint.json" =
        stringAt initial "durableCheckpointDigest"
    && shaFile "evidence/github-substrate-v2/gs2-07-6/checkpoint-journal.json" =
        stringAt initial "checkpointJournalDigest"
    ...
    && (initial.GetProperty("jobs").EnumerateArray()
        |> Seq.exists (fun job ->
            stringAt job "name" = "queue-growth"
            && stringAt job "headSha" = stringAt initialRun "headSha"
            && stringAt job "conclusion" = "failure"))
    && (initial.GetProperty("jobs").EnumerateArray()
        |> Seq.exists (fun job ->
            stringAt job "name" = "queue-pilot"
            && stringAt job "headSha" = stringAt initialRun "headSha"
            && stringAt job "conclusion" = "cancelled"))
    && stringAt recoveryRun "event" = "merge_group"
    && stringAt recoveryRun "conclusion" = "success"
    && stringAt proof "fullRef" = expectedQueueRef
    && (recovery.GetProperty("jobs").EnumerateArray()
        |> Seq.map (fun job -> stringAt job "name", stringAt job "headSha", stringAt job "conclusion")
        |> Seq.toList)
        =
        [
            ("queue-growth", stringAt recoveryRun "headSha", "success")
            ("queue-pilot", stringAt recoveryRun "headSha", "success")
        ]
    ...
    && effectRows checkpointJournal "appliedEffects" =
        (effectRows finalJournal "appliedEffects"
         |> List.take (checkpointJournal.GetProperty("appliedEffects").GetArrayLength()))
    ...
    && cleanup.GetProperty("environmentSecretCount").GetInt32() =
        secrets.GetProperty("environmentSecretCount").GetInt32()
    ...
    && stringAt cleanupReadback "finalActiveWorkflowInventoryDigest" =
        stringAt cleanup "finalActiveWorkflowInventoryDigest"
    && sameAuthority observedAuthority observedRecord
    && sameAuthority currentAuthority currentRecord
    && int64At observedAuthority "observedAtUnixSeconds"
       >= int64At initial "admittedAtUnixSeconds"
    && int64At observedAuthority "observedAtUnixSeconds"
       <= int64At checkpoint "sealedAtUnixSeconds"
    && int64At currentAuthority "observedAtUnixSeconds"
       >= int64At refusal "observedAtUnixSeconds"
    && int64At currentAuthority "observedAtUnixSeconds"
       <= int64At recovery "admittedAtUnixSeconds"
    && authoritySources observedAuthority
    && authoritySources currentAuthority
...
let noWriter () =
    let detector (value: string) =
        Regex.IsMatch(
            value,
            "HttpClient|WebRequest|Octokit|GitHubClient|QueueClient|GetEnvironmentVariable",
...
let generated control =
    ...
    | "roadmap" -> roadmap ()
    | "sandbox-identity" ->
        Sandbox.repository = text "repository"
        && Sandbox.repositoryId = integer "repositoryId"
    | "interruption" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                Interrupted = false
            }
        |> has GitHubQueueSandboxFinding.MissingInterruption
    | "failed-step" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                FailedStepInjected = false
            }
        |> has GitHubQueueSandboxFinding.MissingFailedStep
    | "sealed-resume" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                ResumeCheckpointDigest = digest "0"
            }
        |> has GitHubQueueSandboxFinding.UnsealedResume
    | "deterministic-retry" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                RetryEffects = recoveryFacts.RetryEffects.Tail
            }
        |> has GitHubQueueSandboxFinding.RetryDiverged
    | "duplicate-effect" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                DuplicateEffectCount = 1
            }
        |> has GitHubQueueSandboxFinding.DuplicateEffect
    | "compensation" ->
        let changed =
            { recoveryFacts.Compensations.Head with
                OperationId = "wrong-operation"
            } in

        Sandbox.compileRecovery
            { recoveryFacts with
                Compensations = [ changed ]
            }
        |> has GitHubQueueSandboxFinding.CompensationOrderInvalid
    | "rollback" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                FinalVisibility = "public"
            }
        |> has (GitHubQueueSandboxFinding.RollbackMismatch "visibility")
    | "cleanup" ->
        hostedRecovery ()
        && (Sandbox.compileRecovery
                { recoveryFacts with
            ...
            |> has GitHubQueueSandboxFinding.CleanupIncomplete)
    | "authoritative-readback" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                FinalSettingsDigest = digest "0"
            }
        |> has (GitHubQueueSandboxFinding.RollbackMismatch "settings")
    ...
    | "unrecoverable" ->
        ...
        |> has GitHubQueueSandboxFinding.Unrecoverable
    | "no-fleet" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                Repository = "FS-GG/production"
                RepositoryId = 1L
            }
        |> Result.isError
...
let independent control =
    ...
    | "sandbox-identity" ->
        ...
        |> Result.isError
    | "interruption" ->
        not recoveryFacts.Interrupted |> not
        && Sandbox.compileRecovery
            { recoveryFacts with
                Interrupted = false
            }
    ...
    | "sealed-resume" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                DurableCheckpointDigest = "bad"
            }
        |> Result.isError
    | "deterministic-retry" ->
        let changed =
            { recoveryFacts.RetryEffects.Head with
                ResultDigest = digest "9"
            } in

        Sandbox.compileRecovery
            { recoveryFacts with
                RetryEffects = changed :: recoveryFacts.RetryEffects.Tail
            }
        |> Result.isError
    | "duplicate-effect" ->
        Sandbox.compileRecovery
    ...
    | "compensation" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                Compensations = recoveryFacts.Compensations.Tail
            }
        |> Result.isError
    | "rollback" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                PreVisibility = "public"
                FinalVisibility = "public"
            }
        |> Result.isError
    | "cleanup" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                TemporaryResourceCount = 2
            }
        |> Result.isError
    | "authoritative-readback" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                PreSettingsDigest = "bad"
            }
        |> Result.isError
    | "ordering" -> Sandbox.parseRecovery (bytes + "\n") |> Result.isError
    | "seal" ->
        let changed =
            (if baseline.Seal[0] = '0' then "1" else "0") + baseline.Seal.Substring(1) in

        Sandbox.parseRecovery (bytes.Replace(baseline.Seal, changed)) = Error [ GitHubQueueSandboxFinding.AlteredSeal ]
    | "unrecoverable" ->
        Sandbox.compileRecovery
            { recoveryFacts with
                Recoverable = false
        ...
        |> Result.isError
    | "no-fleet" ->
        not (boolean "fleetAuthority")
        && Sandbox.compileRecovery
            { recoveryFacts with
                Repository = "FS-GG/Fleet"
            }
           |> Result.isError
...
let retained relative =
    ...
    root.GetProperty("recoveryControls").EnumerateArray()
    |> Seq.map _.GetString()
    |> Seq.toList,
...
let generatedIds, generatedContract =
    retained "evidence/github-substrate-v2/gs2-07-6/generated-controls.json"

let independentIds, independentContract =
    retained "evidence/github-substrate-v2/gs2-07-6/independent-controls.json"

if
    generatedIds <> expected
    || independentIds <> expected
    || generatedContract = independentContract
then
    failwith "retained recovery controls are stale or not independent"
...
let generatedRows =
    ...
    |> List.map (fun id ->
        {
            ControlId = id
            ControlPassed = generated id
            BaselineGreen = (Sandbox.parseRecovery bytes = Ok baseline)
        })
...
let independentRows =
    ...
    |> List.map (fun id ->
        {
            ControlId = id
            ControlPassed = independent id
            BaselineGreen = (Sandbox.verifyRecovery baseline.Seal baseline = Ok baseline)
        })

match Sandbox.validateControls expected generatedRows independentRows with
| Ok() -> ()
| Error errors -> failwithf "Q6 controls failed: %A" errors

if Sandbox.replayRecovery baseline recoveryFacts <> Ok baseline then
    failwith "deterministic recovery replay diverged"

printfn
    "GITHUB_QUEUE_SANDBOX_RECOVERY_OK disposition=%s controls=%d seal=%s"
