...
then
    fail "GLPQ-EVIDENCE" "required evidence is missing"
...
let requiredIds =
    ...
    |> List.map GitHubLifecycleProjectionQualification.controlId

if
    corpus.RootElement.GetProperty("schema").GetString()
    <> "fsgg.coordination.github-lifecycle-projection-corpus/1"
then
    fail "GLPQ-CORPUS-SCHEMA" corpusPath

if
    independentDocument.RootElement.GetProperty("schema").GetString()
    <> "fsgg.coordination.github-lifecycle-projection-expectations/1"
then
    fail "GLPQ-INDEPENDENT-SCHEMA" independentPath

if
    corpus.RootElement.GetProperty("registeredContractSha256").GetString()
    <> "3da7bf73fdfa4729b8950c23be908927c4ced57c4680d4f0e3f3218cc93c392d"
then
    fail "GLPQ-CONTRACT" "registered contract mismatch"

if
    sha256 receiptPath
    <> corpus.RootElement.GetProperty("acceptedPredecessorReceiptSha256").GetString()
then
    fail "GLPQ-PREDECESSOR" "accepted GS2-05.6 file mismatch"
...
if generatedIds <> requiredIds || independentIds <> requiredIds then
    fail "GLPQ-INVENTORY" "control inventories are not exact"
...
let generatedMutation =
    ...
    | GitHubLifecycleProjectionControl.ProtectedDelivery ->
        UnprotectedLifecycleDelivery <> ClosedIssueWithoutTerminalAuthority
    | GitHubLifecycleProjectionControl.ExactPlan -> AlteredLifecyclePlan <> InvalidLifecycleSubject
    | GitHubLifecycleProjectionControl.RevisionFence ->
        LifecycleStatusPreStateRefused ConcurrentStatusChange <> AlteredLifecyclePlan
    | GitHubLifecycleProjectionControl.ExactReplay ->
        ({
            AuthorityReads = 8
            MaximumEffects = 0
        }
        : LifecycleProjectionCost)
            .MaximumEffects
            =
            0
    | GitHubLifecycleProjectionControl.BoundedCost ->
        ({
            AuthorityReads = 8
            MaximumEffects = 1
        }
...
let independentMutation =
    ...
    | GitHubLifecycleProjectionControl.FormalPrecedence ->
        [
            StageCancelled
            StageDelivered
            StageAccepted
    ...
    | GitHubLifecycleProjectionControl.StatusMapping ->
        [
            StageBacklog
            StageReady
            StagePaused
        ...
        |> List.map LifecycleProjectionAdapter.statusName
    ...
    | GitHubLifecycleProjectionControl.BoundedCost ->
        ...
        : LifecycleProjectionCost)
            ...
            1
    | GitHubLifecycleProjectionControl.QuintAndPrerequisite ->
        corpus.RootElement.GetProperty("roadmapRevision").GetString() = "43209b79634cc56994876329d3ab8a0ce7c9ef79"

let baselineGreen = requiredIds.Length = 18

let generated: GitHubLifecycleProjectionControlResult list =
    GitHubLifecycleProjectionQualification.requiredControls
...
let independent: GitHubLifecycleProjectionControlResult list =
    GitHubLifecycleProjectionQualification.requiredControls
    |> List.map (fun control ->
        {
            Control = control
...
| Ok() ->
    printfn
        "github-lifecycle-projection-contract OK controls=%d q=Q3 quint=unchanged provenance=generated+independent production-writes=0"
        generated.Length
| Error findings ->
    findings
    |> List.iter (fun finding -> eprintfn "%s control=%s %s" finding.Code finding.ControlId finding.Message)

    fail "GLPQ-FAILED" (string findings.Length)
