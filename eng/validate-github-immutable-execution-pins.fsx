...
let referenceKind =
    ...
    | value -> failwith $"unsupported reference kind {value}"
...
let workflow (node: JsonNode) =
...

    {
        ...
        References =
            value["references"].AsArray()
            |> Seq.map (fun item ->
                let reference = item.AsObject()

                {
                    WorkflowPath = path
                    TargetRepository = text reference "repository"
                    TargetPath = optionalText reference "path"
                    Revision = text reference "revision"
                    Kind = referenceKind (text reference "kind")
                })
            |> List.ofSeq
    }
...
let publication (node: JsonNode) =
...

    {
        ...
        WorkflowCall = value["workflowCall"].GetValue<bool>()
    }
...
let updater (node: JsonNode) =
...

    {
        ...
        Automated = value["automated"].GetValue<bool>()
        PullRequestOnly = value["pullRequestOnly"].GetValue<bool>()
        DirectPush = value["directPush"].GetValue<bool>()
        PolicyRepository = text value "policyRepository"
        PolicyRevision = text value "policyRevision"
        PolicyPath = text value "policyPath"
...
let snapshot =
    {
        ...
        Repository = text corpus "repository"
        ...
        UpdaterConfigurations =
            corpus["updaterConfigurations"].AsArray()
            |> Seq.map updaterConfiguration
            |> List.ofSeq
...
let compile candidate =
    GitHubImmutableExecutionPinsQualification.compile candidate

let refused candidate = compile candidate |> Result.isError

let report =
    compile snapshot
    |> Result.defaultWith (failwithf "immutable execution pins baseline refused: %A")

if fsi.CommandLineArgs |> Array.contains "--mint" then
    printfn "%s" report.Seal
else
    let extensionPath =
        Path.Combine(root, "evidence/github-substrate-v2/gs2-06-7/execution-pins-extension.json")

    if not (File.Exists extensionPath) then
        failwith "current execution-pin extension is absent"
    ...
    let exactProperties context expected (value: JsonObject) =
        ...
        if actual <> Set.ofList expected then
            failwith $"{context} property inventory differs"

    exactProperties "execution-pin extension" [ "schema"; "complete"; "documents" ] extension

    if
        text extension "schema" <> "fsgg.coordination.execution-pins-extension/1"
        || not (extension["complete"].GetValue<bool>())
    then
    ...
    for document in extensionDocuments do
        exactProperties
            "execution-pin document"
            [ "kind"; "path"; "sha256"; "externalReferences"; "localReferences" ]
            document

        if text document "kind" <> "workflow" && text document "kind" <> "action" then
            failwith "execution-pin document kind differs"

    let extensionPaths =
        extensionDocuments |> List.map (fun document -> text document "path")

    if extensionPaths.Length <> (extensionPaths |> Set.ofList |> Set.count) then
        failwith "duplicate execution-pin extension document"
    ...
    let declaredWorkflows =
        ...
        @ (extensionDocuments
           |> List.filter (fun document -> text document "kind" = "workflow")
           |> List.map (fun document -> text document "path"))
        |> Set.ofList

    if trackedWorkflows <> declaredWorkflows then
        failwith
            $"workflow inventory is incomplete: tracked={trackedWorkflows.Count} declared={declaredWorkflows.Count}"

    for workflow in snapshot.Workflows do
        let path = Path.Combine(root, workflow.Path)

        if sha256File path <> workflow.Sha256 then
            failwith $"workflow digest differs: {workflow.Path}"

    for document in extensionDocuments do
        let path = text document "path"

        if sha256File (Path.Combine(root, path)) <> text document "sha256" then
            failwith $"execution-pin extension digest differs: {path}"
    ...
    let uses =
        Regex("(?m)^\\s*(?:-\\s*)?uses:\\s*([^\\s#]+)", RegexOptions.CultureInvariant)
        ...
        [
            for workflow in snapshot.Workflows do
                for matched in uses.Matches(File.ReadAllText(Path.Combine(root, workflow.Path))) do
                    ...
                    | Ok(kind, repository, path, revision) -> yield workflow.Path, kind, repository, path, revision
    ...
    let declaredReferences =
        ...
        |> List.collect (fun workflow ->
            ...
            |> List.map (fun reference ->
                workflow.Path, reference.Kind, reference.TargetRepository, reference.TargetPath, reference.Revision))
        |> Set.ofList

    if observedReferences <> declaredReferences then
        failwith
            $"execution reference inventory differs: observed={observedReferences.Count} declared={declaredReferences.Count}"
    ...
    for document in extensionDocuments do
        ...
        let observedExternal =
            observed
            |> Set.filter (fun target -> not (target.StartsWith("./", StringComparison.Ordinal)))

        let observedLocal =
            observed
            |> Set.filter (fun target -> target.StartsWith("./", StringComparison.Ordinal))
    ...
    let trackedPaths =
        ...
        for argument in [ "-C"; root; "ls-files"; "-z" ] do
        ...
        if child.ExitCode <> 0 then
            failwith $"tracked updater inventory failed: {error}"
    ...
    let dependabotPaths =
        Set.ofList [ ".github/dependabot.yml"; ".github/dependabot.yaml" ]

    let packageRenovate =
        Regex("(?s)\\\"renovate\\\"\\s*:", RegexOptions.CultureInvariant)
    ...
    if
        configuredPaths <> Set.union officialRenovatePaths dependabotPaths
        || officialRenovatePaths
           <> (texts expectations "officialRenovatePaths" |> Set.ofList)
        || invocationSelectors <> texts expectations "invocationSelectors"
    then
        failwith "independent updater discovery contract differs"

    let branchAutomerge =
        Regex(
            "(?i)(automergeType[\\\"']?\\s*[:=]\\s*[\\\"']?(?:branch|direct)|RENOVATE_AUTOMERGE_TYPE\\s*[:=]\\s*[\\\"']?(?:branch|direct)|--automerge-type(?:=|\\s+)(?:branch|direct))",
    ...
    let renovateCommand =
        Regex(
            "(?im)(?:^|[;&|`(\\\"'\\s,])(?:[^\\\"'\\s,;()]+@npm:)?(?:npm:)?(?:(?:file:)?[^\\\"'\\s,;()]*[\\\\/])?renovate(?:\\.(?:cmd|exe))?(?:[@#:][^\\\"'\\s,;()]+)?(?:[\\\"'\\s,;)]|$)",
            RegexOptions.CultureInvariant
        )
    ...
    let observedUpdaterConfigurations =
        ...
        |> Seq.choose (fun path ->
            ...
            if configuredPaths.Contains path && dependabotPaths.Contains path then
                Some(configurationTuple path "dependabot")
            elif configuredPaths.Contains path then
                Some(configurationTuple path "renovate")
            elif path = "package.json" && packageRenovate.IsMatch(File.ReadAllText fullPath) then
                Some(configurationTuple path "renovate")
    ...
    let declaredUpdaterConfigurations =
        ...
        |> List.map (fun configuration ->
            ...
            configuration.PullRequestOnly,
            configuration.DirectPush)
        |> Set.ofList
    ...
    let receipt =
        JsonNode
            .Parse(File.ReadAllText(Path.Combine(root, "evidence/github-substrate-v2/accepted/GS2-06.3.json")))
            .AsObject()

    if text receipt "digest" <> snapshot.PrerequisiteReceiptDigest then
        failwith "accepted GS2-06.3 receipt differs"

    if
        text corpus "roadmapRevision" <> "7ab43852609563265291eec2b4010a829582d447"
        || text corpus "roadmapSha256"
           <> "9c8c87581bc0e7d1e9aac6d2691fdbf5f4e3db531c45879b1acc5b37669f0112"
    then
        failwith "accepted roadmap binding differs"

    let acceptedUpdater = snapshot.Updaters |> List.exactlyOne

    if
        acceptedUpdater.PolicyRepository <> "FS-GG/.github"
        || acceptedUpdater.PolicyRevision <> "7ab43852609563265291eec2b4010a829582d447"
        || acceptedUpdater.PolicyPath <> "renovate.json"
        || acceptedUpdater.PolicySha256
           <> "fb9c4ec557a849a553881dbc9ac75ef6f1e98d7f6a43efeefeca67d4f9ec36fb"
        || not acceptedUpdater.PullRequestOnly
        || acceptedUpdater.DirectPush
    then
    ...
    if
        ...
        || report.WorkflowCount <> expectations["workflowCount"].GetValue<int>()
        || report.ReferenceCount <> expectations["referenceCount"].GetValue<int>()
    then
        failwith "baseline inventory differs"

    if
        observedReferences
        |> Seq.map (fun (_, kind, repository, _, revision) -> kind, repository, revision)
        |> Set.ofSeq
        |> Set.count
        <> expectations["distinctActionCount"].GetValue<int>()
    then
        failwith "distinct action inventory differs"

    if
        report.PublicationCount <> expectations["publicationCount"].GetValue<int>()
        || report.UpdaterConfigurationCount
           <> expectations["updaterConfigurationCount"].GetValue<int>()
        || report.AutomatedUpdater <> text expectations "automatedUpdater"
        || report.Managers <> texts expectations "managers"
    then
        failwith "publication or updater result differs"

    if report.Seal <> text expectations "expectedSeal" then
        failwith "baseline seal differs"

    if
        GitHubImmutableExecutionPinsQualification.verify report.Seal snapshot
        <> Ok report
    then
    ...
    let expectedControls =
        ...
        |> List.map GitHubImmutableExecutionPinsQualification.controlId
    ...
    let reusable =
        {
            WorkflowPath = snapshot.Workflows.Head.Path
            TargetRepository = "FS-GG/.github"
            TargetPath = Some ".github/workflows/reusable.yml"
            Revision = sha
    ...
    let generatedMutation =
        ...
        | ThirdPartyActionPins ->
            ...
            refused
                { snapshot with
                    Workflows =
                        { head with
                            References = changed :: head.References.Tail
                        }
                        :: snapshot.Workflows.Tail
                }
        | ReusableWorkflowPins ->
            ...
            refused
                { snapshot with
                    Workflows =
                        { head with
                            References = changed :: head.References
                        }
                        :: snapshot.Workflows.Tail
        ...
        | LocalExecutionReferenceRejection ->
            ...
            | _ -> false
        | WorkflowDigestBinding ->
            refused
                { snapshot with
                    Workflows =
                        { snapshot.Workflows.Head with
                            Sha256 = "changed"
                        }
                        :: snapshot.Workflows.Tail
                }
        ...
        | PublicationWorkflowCall ->
            refused
                { snapshot with
                    Publications = [ { published with WorkflowCall = false } ]
                }
        | StablePinOrdering ->
            compile
                { snapshot with
                    Workflows = List.rev snapshot.Workflows
                }
                =
                Ok report
        | UpdaterConfigurationInventory ->
            refused
                { snapshot with
                    UpdaterConfigurations = [ dependabotConfiguration; unsafeRenovateConfiguration ]
                }
        | RenovateSoleUpdater ->
            refused
                { snapshot with
                    Updaters =
                        { snapshot.Updaters.Head with
                            Name = "dependabot"
                        }
                        :: snapshot.Updaters
                }
        | RenovatePullRequestOnly ->
            refused
                { snapshot with
                    Updaters =
                        [
        ...
        | RenovateOwnership ->
            refused
                { snapshot with
                    RequiredManagers = [ "github-actions"; "regex" ]
                }
        | ExactPinsSeal ->
            GitHubImmutableExecutionPinsQualification.verify (String.replicate 64 "0") snapshot
            |> Result.isError
        ...
        | NoWorkflowPublicationSurface ->
            let surface =
                File.ReadAllText(
                    Path.Combine(
                        root,
                        "src/FS.GG.Coordination.Qualification.Contracts/GitHubImmutableExecutionPinsQualification.fsi"
                    )
                )
            ...
            |> List.forall (surface.Contains >> not)

    let independentMutation control =
        match control with
        | ImmutablePinsPrerequisite ->
            snapshot.PrerequisiteReceiptDigest = "eec15747e2e5c1cf0ae91fbf370eb82a3e6ea88d6fe3c0f2f738a556e63e5063"
        ...
        | ThirdPartyActionPins ->
            observedReferences
            |> Seq.filter (fun (_, kind, _, _, _) -> kind = ThirdPartyAction)
            |> Seq.forall (fun (_, _, _, _, revision) -> revision.Length = 40 && revision |> Seq.forall Uri.IsHexDigit)
        | ReusableWorkflowPins ->
            observedReferences
            |> Seq.filter (fun (_, kind, _, _, _) -> kind = ReusableWorkflow)
            |> Seq.forall (fun (_, _, _, path, revision) -> path.IsSome && revision.Length = 40)
        | LocalExecutionReferenceRejection ->
            snapshot.Workflows
            |> List.collect (fun workflow ->
        ...
        | WorkflowDigestBinding ->
            snapshot.Workflows
            |> List.forall (fun workflow -> sha256File (Path.Combine(root, workflow.Path)) = workflow.Sha256)
        | PublicationIdentity
        | PublicationContent
        | PublicationWorkflowCall ->
            snapshot.Publications.IsEmpty
            && observedReferences
               |> Seq.exists (fun (_, kind, _, _, _) -> kind = ReusableWorkflow)
               |> not
        | StablePinOrdering ->
            compile
                { snapshot with
        ...
        | UpdaterConfigurationInventory ->
            ...
            && observedUpdaterConfigurations
               |> Seq.forall (fun (_, _, authority, pullRequestOnly, directPush) ->
                   authority = "renovate" && pullRequestOnly && not directPush)
        ...
        | RenovateOwnership ->
            snapshot.Updaters.Head.OwnedManagers = [ "github-actions" ]
            && snapshot.RequiredManagers = [ "github-actions" ]
        | ExactPinsSeal -> report.Seal = text expectations "expectedSeal"
        | ExactPinsReplay -> GitHubImmutableExecutionPinsQualification.verify report.Seal snapshot = Ok report
        | QuintPinsUnchanged -> generatedMutation QuintPinsUnchanged
        | NoPinsMutationSurface
        | NoWorkflowPublicationSurface -> generatedMutation control
    ...
    | Ok() ->
        printfn
            "GITHUB_IMMUTABLE_EXECUTION_PINS_OK workflows=%d references=%d publications=%d updaterConfigurations=%d updater=%s controls=%d seal=%s"
            report.WorkflowCount
            report.ReferenceCount
            report.PublicationCount
            report.UpdaterConfigurationCount
            report.AutomatedUpdater
