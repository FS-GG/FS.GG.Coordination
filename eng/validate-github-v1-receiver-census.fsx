...
let evidenceRoot =
    Path.Combine(root, "evidence/github-substrate-v2/gs2-08-3/receiver-source")

let bytes name =
    File.ReadAllBytes(Path.Combine(evidenceRoot, name))

let parse (value: byte array) =
    JsonNode.Parse(ReadOnlySpan<byte>(value)).AsObject()
...
let routes =
    ...
    |> Seq.map (fun value ->
...

        {
            ...
            Callsites = texts row "callsites"
            EffectClass = text row "effectClass"
            RemoteEffects = texts row "remoteEffects"
            CredentialBoundary = text row "credentialBoundary"
            LaterDisposition = text row "laterDisposition"
        })
    |> List.ofSeq
...
let dependencies =
    ...
    |> Seq.map (fun value ->
...

        {
            ...
            ResolvedRevision = optionalText row "resolvedRevision"
            CalleeSha256 = optionalText row "calleeSha256"
        })
    |> List.ofSeq
...
let tools =
    ...
    |> Seq.map (fun value ->
...

        {
            ...
            Tree = text row "tree"
            OptionsSha256 = text row "optionsSha256"
            ProjectSha256 = text row "projectSha256"
        })
    |> List.ofSeq
...
let snapshot =
    {
        ...
        Installed = claims["installed"].GetValue<bool>()
        Fenced = claims["fenced"].GetValue<bool>()
        Accepted = claims["accepted"].GetValue<bool>()
        TelemetryLocalOnly =
            text telemetry "classification" = "local-only"
            && not (telemetry["sourceRead"].GetValue<bool>())
            && not (telemetry["privateCorpusRead"].GetValue<bool>())
    }

if text binding "schema" <> "fsgg.v1-receiver-census-source-binding/1" then
    failwith "receiver source binding schema differs"

if text census "schema" <> "fsgg.v1-writer-receiver-census/1" then
    failwith "producer receiver census schema differs"

if text binding "producerRepository" <> "FS-GG/.github" then
    failwith "producer repository differs"

if
    text binding "producerCandidateRevision"
    <> "7bb93466dc3f7e0f70d419137e34e2a76c7356fa"
    || snapshot.ProducerRevision <> "3719b6cfc6f2d766ad56f930b56f025e20c4b2cc"
    || snapshot.ProducerTree <> "d20205d921f5ce128385d2a9ba6361c47d3f9a49"
then
    failwith "producer candidate, landed revision, or identical tree differs"

if
    sha256 censusBytes <> snapshot.CensusSha256
    || sha256 manifestsBytes <> snapshot.SourceManifestsSha256
    || sha256 blobsBytes <> snapshot.SourceBlobsSha256
then
    failwith "retained producer evidence byte digest differs"

if
    snapshot.AcceptedEpochReceiptSha256
...
let count (effect: string) =
    routes |> List.filter (_.EffectClass >> (=) effect) |> List.length

let effectCounts = expected["effectCounts"].AsObject()

if
    receivers.Length <> expectedInt "receiverCount"
    || sources.Length <> expectedInt "sourceIdentityCount"
    || census["sourceBlobDigests"].AsArray().Count
       <> expectedInt "deduplicatedBlobCount"
    || routes.Length <> expectedInt "routeCount"
    || dependencies.Length <> expectedInt "dependencyCount"
    || tools.Length <> expectedInt "toolSourceCount"
then
    failwith "independent receiver census counts differ"

for property in effectCounts do
    if count property.Key <> property.Value.GetValue<int>() then
        failwith $"independent effect count differs: {property.Key}"

let refused candidate =
    GitHubV1ReceiverCensusQualification.validateSnapshot candidate |> Result.isError
...
let generatedMutation =
    ...
    | ManifestByteBinding ->
        ...
        && refused
            { snapshot with
                SourceManifestsSha256 = "invalid"
            }
    | SourceBlobByteBinding ->
        sha256 blobsBytes = snapshot.SourceBlobsSha256
        && refused
            { snapshot with
                SourceBlobsSha256 = "invalid"
            }
    | CompleteReceiverRoster ->
        refused
            { snapshot with
                Receivers = snapshot.Receivers.Tail
            }
    ...
    | CompleteRoutePopulation ->
        refused
            { snapshot with
                Routes = snapshot.Routes.Tail
            }
    | CompleteDependencyPopulation ->
        refused
            { snapshot with
                Dependencies = snapshot.Dependencies.Tail
            }
    | StableUniqueOrdering ->
        refused
            { snapshot with
                Sources = List.rev snapshot.Sources
            }
    ...
    | RouteEffectClassification ->
        refused
            { snapshot with
                Routes =
                    { snapshot.Routes.Head with
                        EffectClass = "unknown"
                    }
                    :: snapshot.Routes.Tail
            }
    ...
    | LegacyToolCorrespondence ->
        refused
            { snapshot with
                InstalledTools = snapshot.InstalledTools.Tail
            }
...
let independentMutation =
    ...
    | SourceIdentityBinding ->
        sources
        |> List.forall (fun row -> row.Sha256.Length = 64 && row.BlobSha1.Length = 40)
    | RouteEffectClassification ->
...
let actualControls =
    ...
    |> List.map GitHubV1ReceiverCensusQualification.controlId

if actualControls <> expectedControls then
    failwith "independent receiver control inventory differs"
...
match
    GitHubV1ReceiverCensusQualification.validateControls (results generatedMutation) (results independentMutation)
with
| Error findings -> failwithf "receiver census controls failed: %A" findings
| Ok() ->
    printfn
        "GITHUB_V1_RECEIVER_CENSUS_OK producer=%s receivers=%d sources=%d blobs=%d routes=%d dependencies=%d controls=%d census=%s"
