...
let expect path expected actual =
    if expected <> actual then
        fail "DR-BINDING" $"%s{path}: expected %A{expected}; observed %A{actual}"
...
let validateBytes root (bytes: byte array) =
    ...
    let originalBytes =
        File.ReadAllBytes(Path.Combine(root, "evidence/github-substrate-v2", str "path" original))
    ...
    let previousBytes =
        File.ReadAllBytes(Path.Combine(root, "evidence/github-substrate-v2", str "path" previous))
    ...
    let repair268 =
        File.ReadAllBytes(Path.Combine(root, "evidence/github-substrate-v2/repair-receipts/GS2-06.7-repair-268.json"))
    ...
    expect
        "review.digest"
        "26055bbd0438ba2b40482592b18b413c2975ecae033bc683bb88c3f25852a799"
        (str "decisionDigest" review)
    ...
    expect
        "review.acceptanceDigest"
        "317751fcbef4fcf86247c0add1bc2778239170fb5b14b2f0aa4f38f46918b549"
        (str "hostAcceptanceDigest" review)
    ...
    let validateRuns name head expected =
        ...
        let actual =
            runs
            |> List.map (fun run -> str "workflow" run, number "runId" run)
            |> Set.ofList
    ...
    let inventorySha =
        "25ec1d81d2a5c5b9de6cf2411f26c288a365e6ea1e2f9eac40ec3449322d291a"
    ...
    let inventoryBytes =
        readGitObject root merge "evidence/github-substrate-v2/gs2-06-7/runtime-inventory.json"
    ...
    let reviewed =
        JsonNode
            .Parse(readGitObject root merge "evidence/github-substrate-v2/gs2-06-7/runtime-reviewed-authority.json")
            .AsObject()
    ...
    let observationBytes =
        readGitObject root merge (Path.Combine("evidence/github-substrate-v2", str "path" observations))
    ...
    expect
        "roadmap.manifest"
        "d69cf02083aafa750e069aa1781964ed13ce6ab76b9a8bab25e937736cba7b08"
        (str "manifestSha256" roadmap)
...
let validate root =
    ...
    let index =
        JsonNode.Parse(File.ReadAllBytes(Path.Combine(evidenceRoot, "index.json"))).AsObject()

    let entries =
        arr "entries" index
        |> Seq.map _.AsObject()
        |> Seq.filter (fun entry -> str "id" entry = "repair-GS2-06.7-276" || str "path" entry = relative)
        |> Seq.toList
...
let selfTest root (baseline: byte array) =
    ...
    let reject name mutate =
        ...
        try
            ...
            fail "DR-SELF-TEST" $"%s{name} was accepted"
    ...
    try
        ...
        fail "DR-SELF-TEST" "duplicate member was accepted"
    with error when not (error.Message.StartsWith("DR-SELF-TEST", StringComparison.Ordinal)) ->
        ()
...
printfn
    "GS2_06_7_DURABLE_AUTHORITY_REPAIR_RECEIPT_OK repairId=GS2-06.7-repair-276 previous=fa0d1e78ac9528d1793d43e850bfc5479628ee242f0407c49d65d81cc74063da merge=48a3880c695111df360fbe0efd8bf35071ce8194 distance=3 controls=%d"
    (if selfTestRequested then 13 else 0)
