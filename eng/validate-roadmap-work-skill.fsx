    ...
    if not (File.Exists path) then
        ...
        for token in required do
            if not (text.Contains(token, StringComparison.Ordinal)) then
                fail "required-token" token

        for token in forbidden do
            if text.Contains(token, StringComparison.OrdinalIgnoreCase) then
                fail "forbidden-authority" token

        if bytes.Length > 12000 then
            fail "size" "SKILL.md exceeds 12,000 UTF-8 bytes"

        if Environment.ExitCode = 0 then
            printfn "ROADMAP_WORK_SKILL_OK bytes=%d" bytes.Length
