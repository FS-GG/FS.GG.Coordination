...
if fanout.GetProperty("logicalPartition").GetInt32() <> 1 then failwith "formal logical partition must remain one"
if fanout.GetProperty("maxConcurrentExecutions").GetInt32() <> 6 then failwith "formal fanout must share the six-execution bound"
let semanticShards = fanout.GetProperty("semanticShards").EnumerateArray() |> Seq.map _.GetString() |> Seq.toList
let quintPlan = JsonDocument.Parse(File.ReadAllBytes(Path.Combine(root, "eng/quint-qualification.json")))
let performanceShard = fanout.GetProperty("performanceShard").GetString()
let expectedSemantic =
    "base" :: (quintPlan.RootElement.GetProperty("formalTests").EnumerateArray() |> Seq.map (fun item -> item.GetProperty("id").GetString()) |> Seq.filter ((<>) performanceShard) |> Seq.toList)
if semanticShards.Head <> "base" || Set.ofList semanticShards <> Set.ofList expectedSemantic || semanticShards.Length <> (semanticShards |> List.distinct |> List.length) then
    failwith "formal fanout must exactly retain base and every non-performance formal scenario"
if fanout.GetProperty("performanceShard").GetString() <> "epoch" then failwith "formal performance shard must remain epoch"
...
for required in [ "group: optimistic-coherent-${{ inputs.candidate_sha || github.event.pull_request.head.sha || github.event.merge_group.head_sha || github.sha }}"; "cancel-in-progress: false"; "fail-fast: false"; "max-parallel: 6"; "cron: '17 3 * * *'"; "prepare:"; "needs: [prepare, classify-reuse]"; "run-partition:"; "formal-aggregate:"; "aggregate:"; "shard: base"; "shard: epoch" ] do
...
for shard in semanticShards @ [ performanceShard ] do
    ...
    if text.Split(token, StringSplitOptions.None).Length <> 2 then failwith $"workflow projection must schedule formal shard exactly once: %s{shard}"
for partition in [ 0; 2; 3; 4; 5 ] do
    let token = $"- {{ kind: partition, partition: %d{partition} }}"
    if text.Split(token, StringSplitOptions.None).Length <> 2 then failwith $"workflow projection must schedule nonformal partition exactly once: %d{partition}"
if text.Contains("kind: partition, partition: 1") then failwith "workflow must reserve logical partition one for formal aggregation"
