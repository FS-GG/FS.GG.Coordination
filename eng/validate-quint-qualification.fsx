...
let strings (node: JsonNode) (name: string) =
    node[name].AsArray() |> Seq.map _.GetValue<string>() |> Seq.toList

let objects (node: JsonNode) (name: string) =
    node[name].AsArray() |> Seq.map _.AsObject() |> Seq.toList
...
let unique code label values =
    let duplicates =
        values |> List.countBy id |> List.filter (fun (_, count) -> count <> 1)

    if List.isEmpty duplicates then
        Ok()
    else
        fail code ($"%s{label}=%A{duplicates}")

let exact code label expected actual =
    if expected = actual then
        Ok()
    else
        fail code ($"%s{label}; expected=%A{expected}; actual=%A{actual}")
...
let validateDocument root (document: JsonObject) =
    try
        ...
        let sourcePath =
            Path.Combine(root, sourceRelative.Replace('/', Path.DirectorySeparatorChar))
        ...
        let selectionImports =
            modules
            |> List.map (fun item -> text item "id", strings item "selectionImports")
            |> Map.ofList
        ...
        let declaredDefinition name =
            Regex.IsMatch(
                sourceText,
                $"(?m)^\\s*(pure\\s+)?(val|def|run|action|temporal)\\s+%s{Regex.Escape name}(\\s|\\(|=|:)"
            )
        ...
        let actualCanonicalImports main =
            ...
            if not moduleMatch.Success then
                ...
                Regex.Matches(
                    ...
                    "(?m)^\\s*import\\s+([A-Za-z][A-Za-z0-9_]*)\\.([A-Za-z][A-Za-z0-9_]*|\\*)"
                )
                |> Seq.cast<Match>
        ...
        let rec closure visiting moduleId =
            if Set.contains moduleId visiting then
                fail "QQ-MODULE-CYCLE" moduleId
            elif not (Map.containsKey moduleId selectionImports) then
                fail "QQ-MODULE-UNKNOWN" moduleId
            else
                ...
                |> List.fold
                    (fun state dependency ->
                        state
                        |> bind (fun found ->
                            closure (Set.add moduleId visiting) dependency |> Result.map (Set.union found)))
                    (Ok Set.empty)
        ...
        let classificationCheck kind declaredNames rows =
            ...
            |> bind (fun () ->
                exact
                    ...
                    kind
                    (Set declaredNames)
                    (rows |> List.map (fun (name, _, _) -> name) |> Set.ofList))
            |> bind (fun () ->
                rows
                |> List.tryFind (fun (_, classification, owner) ->
                    not (Set.contains classification allowedClasses)
                    || not (Map.containsKey owner selectionImports))
                |> function
                    | None -> Ok()
                    | Some row -> fail "QQ-CLASSIFICATION-SHAPE" ($"%A{row}"))
        ...
        let rootCheck (item: JsonObject) =
            ...
            |> bind (fun () ->
                ...
                if sourceText.Contains($"module %s{main} {{", StringComparison.Ordinal) then
                    Ok()
                else
                    fail "QQ-ROOT-MAIN" ($"%s{id}:%s{main}"))
            |> bind (fun () ->
                [ "positive"; "adversarial"; "invalid" ]
                |> List.tryFind (fun field -> not (sourceText.Contains(text item field, StringComparison.Ordinal)))
                |> function
                    | None -> Ok()
                    | Some field -> fail "QQ-ROOT-WITNESS" ($"%s{id}:%s{field}"))
            |> bind (fun () ->
                let budget = item["budget"].AsObject()
                let actualDepth = dependencyDepth owner

                let fields =
                    [ "depth"; "states"; "samples"; "elapsedMs"; "peakMiB"; "artifactBytes" ]

                if fields |> List.exists (fun field -> number budget field <= 0) then
                    fail "QQ-BUDGET" id
                elif number budget "depth" < actualDepth then
                    fail "QQ-BUDGET-DEPTH" id
                else
                    Ok())
            |> bind (fun () ->
                ...
                let expectedWitnesses =
                    Set [ text item "positive"; text item "adversarial"; text item "invalid" ]

                if actualFields <> requiredAdmission then
                    fail "QQ-ROOT-ADMISSION-FIELDS" id
                elif text admission "owner" <> owner || text admission "root" <> text item "main" then
                    fail "QQ-ROOT-ADMISSION-IDENTITY" id
                elif
                    strings admission "imports" |> Set.ofList
                    <> (closure Set.empty owner |> Result.defaultValue Set.empty)
                then
                    fail "QQ-ROOT-ADMISSION-IMPORTS" id
                elif strings admission "witnesses" |> Set.ofList <> expectedWitnesses then
                    fail "QQ-ROOT-ADMISSION-WITNESSES" id
                elif
                    strings admission "bounds" |> Set.ofList
                    <> Set [ "depth"; "states"; "samples"; "elapsedMs"; "peakMiB"; "artifactBytes" ]
                then
                    fail "QQ-ROOT-ADMISSION-BOUNDS" id
                elif strings admission "invariants" |> List.exists (declaredDefinition >> not) then
                    fail "QQ-ROOT-ADMISSION-INVARIANT" id
                elif strings admission "witnesses" |> List.exists (declaredDefinition >> not) then
                    fail "QQ-ROOT-ADMISSION-WITNESS" id
                elif
                    (strings admission "independentOracles"
                     |> Set.ofList
                     |> fun actual -> not (Set.isSubset actual requiredOracles))
                then
                    fail "QQ-ROOT-ADMISSION-ORACLE" id
                elif
                    (strings admission "projections"
                     |> Set.ofList
                     |> fun actual -> not (Set.isSubset actual knownProjections))
                then
                    fail "QQ-ROOT-ADMISSION-PROJECTION" id
                elif not (Set.contains (text admission "ciImpact") allowedCiImpact) then
                    fail "QQ-ROOT-ADMISSION-CI" id
                elif text admission "budgetEffect" <> "within-calibrated-envelope" then
                    fail "QQ-ROOT-ADMISSION-BUDGET-EFFECT" id
                else
                    Ok())
        ...
        let formalTestCheck (item: JsonObject) =
            ...
            if fields <> requiredFormalFields then
                fail "QQ-FORMAL-FIELDS" id
            elif not (Set.contains (text item "main") allowedFormalMains) then
                fail "QQ-FORMAL-MAIN" id
            elif text item "backend" <> "tlc" then
                fail "QQ-FORMAL-BACKEND" id
            elif
                text item "counterexample"
                <> $"work/96-gs2-03-5-native-quint-formal-tests/counterexamples/%s{id}.itf.json"
            then
                fail "QQ-FORMAL-COUNTEREXAMPLE" id
            elif
                text item "counterexampleTrace"
                <> $"work/96-gs2-03-5-native-quint-formal-tests/counterexamples/%s{id}.quint-trace.json"
            then
                fail "QQ-FORMAL-COUNTEREXAMPLE-TRACE" id
            elif
                text item "counterexampleManifest"
                <> $"work/96-gs2-03-5-native-quint-formal-tests/counterexamples/%s{id}.manifest.json"
            then
                fail "QQ-FORMAL-COUNTEREXAMPLE-MANIFEST" id
            elif
                executableFields
                |> List.exists (fun field -> not (declaredDefinition (text item field)))
            then
        ...
        if text document "schema" <> "fsgg.coordination.quint-qualification/1" then
            fail "QQ-SCHEMA" "unsupported"
        elif text document "sourceSha256" <> sha256 sourceBytes then
            fail "QQ-SOURCE" "digest"
        else
            ...
            |> bind (fun () ->
                ...
                |> function
                    | None -> Ok()
                    | Some(owner, dependency) -> fail "QQ-MODULE-IMPORT" ($"%s{owner}:%s{dependency}"))
            |> bind (fun () ->
                modules
                |> List.fold
                    (fun state item ->
                        state
                        |> bind (fun () -> closure Set.empty (text item "id") |> Result.map ignore))
                    (Ok()))
            |> bind (fun () -> classificationCheck "state" (declared "^\\s*var\\s+([A-Za-z][A-Za-z0-9_]*)") state)
            |> bind (fun () ->
                classificationCheck "actions" (declared "^\\s*action\\s+([A-Za-z][A-Za-z0-9_]*)") actions)
            ...
            |> bind (fun () ->
                roots
                |> List.fold (fun state item -> state |> bind (fun () -> rootCheck item)) (Ok()))
            ...
            |> bind (fun () ->
                formalTests
                |> List.fold (fun state item -> state |> bind (fun () -> formalTestCheck item)) (Ok()))
            |> bind (fun () ->
                exact "QQ-ORACLE-INVENTORY" "oracles" requiredOracles (strings document "oracleIds" |> Set.ofList))
            |> bind (fun () ->
                if
                    text selection "pullRequest" <> "reverse-dependency-closure"
                    || text selection "protectedPolicy" <> "full-inventory"
                then
                    ...
                    exact
                        "QQ-PROTECTED-MODES"
                        "protected"
                        (Set [ "main"; "acceptance"; "freeze"; "release" ])
                        (strings selection "protected" |> Set.ofList))
            |> bind (fun () ->
                exact
                    "QQ-ADMISSION"
                    "required"
                    requiredAdmission
                    (strings (document["admission"]) "required" |> Set.ofList))
            |> bind (fun () ->
                roots
    ...
    with error ->
        fail "QQ-MALFORMED" error.Message

let oracleTests =
    Map
        [
            "claim-exclusion", "oracleClaimExclusion"
...
let reverseSelection (document: JsonObject) changed =
    ...
    let imports =
        modules
        |> List.map (fun item -> text item "id", strings item "selectionImports" |> Set.ofList)
        |> Map.ofList
    ...
    let rec expand selected =
        let next =
            imports
            |> Map.toSeq
            |> Seq.choose (fun (id, dependencies) ->
                if Set.isEmpty (Set.intersect dependencies selected) then
                    None
                else
                    Some id)
            |> Set.ofSeq
    ...
    |> List.choose (fun root ->
        if Set.contains (text root "module") affected then
            Some(text root "id")
        else
            None)
    |> Set.ofList
...
let rootsForOracles (document: JsonObject) oracleIds =
    let modules =
        ...
        |> List.choose (fun root ->
            ...
            if Set.isEmpty (Set.intersect admitted oracleIds) then
                None
            else
                Some(text root "module"))
        |> Set.ofList
    ...
    let candidates =
        [
            if not (String.IsNullOrWhiteSpace explicit) then
                yield explicit
            if not (String.IsNullOrWhiteSpace cache) then
                yield Path.Combine(cache, "objects", expected)
        ]

    candidates
    |> List.tryFind (fun path -> File.Exists path && sha256 (File.ReadAllBytes path) = expected)
...
let extractQuintSource (markdown: string) =
    ...
    for line in lines do
        if
            not inside
            && (line.StartsWith("```quint ", StringComparison.Ordinal) || line = "```quint-test")
        then
            inside <- true
...
let typecheckProposalSource sourcePath =
    ...
    | Some quint, Some source ->
        let temporary =
            Path.Combine(Path.GetTempPath(), $"fsgg-quint-proposal-{Guid.NewGuid():N}.qnt")
        ...
        try
            ...
            if child.ExitCode = 0 then
                ...
                fail "QQ-PROPOSAL-TYPECHECK" (sha256 (Encoding.UTF8.GetBytes(output + "\n" + error)))
        finally
...
let validateProposalCompilerReceipt root expectedReceiptPath sourceSha (proposal: JsonObject) =
    try
        ...
        if Path.GetFullPath receiptPath <> Path.GetFullPath expectedReceiptPath then
            fail "QQ-PROPOSAL-COMPILER-RECEIPT" "non-canonical"
        elif not (File.Exists receiptPath) then
            fail "QQ-PROPOSAL-COMPILER-RECEIPT" "missing"
        else
            ...
            if text proposal "compilerReceiptSha256" <> sha256 receiptBytes then
                fail "QQ-PROPOSAL-COMPILER-RECEIPT" "digest"
            elif actualFields <> expectedFields then
                fail "QQ-PROPOSAL-COMPILER-RECEIPT" "shape"
            elif text receipt "schema" <> "fsgg.quint.observed-compilation-receipt/v2" then
                fail "QQ-PROPOSAL-COMPILER-RECEIPT" "schema"
            elif text receipt "sourceSha256" <> sourceSha then
                fail "QQ-PROPOSAL-COMPILER-RECEIPT" "source"
            elif
                text receipt "toolchainSha256"
                <> "79b32dacc5bb150e23c4017eef16f3f688cde062441583d5ea1ffa5cc9e62486"
            then
                fail "QQ-PROPOSAL-COMPILER-RECEIPT" "toolchain"
            elif strings receipt "processSteps" |> Set.ofList <> Set [ "extract"; "typecheck" ] then
                fail "QQ-PROPOSAL-COMPILER-RECEIPT" "typecheck"
            elif
                [
                    "fenceManifestSha256"
                    "generatedModulesSha256"
                ...
                |> List.exists (fun field -> not (Regex.IsMatch(text receipt field, "^[0-9a-f]{64}$")))
            then
                fail "QQ-PROPOSAL-COMPILER-RECEIPT" "fingerprint"
            else
                Ok()
    with error ->
        fail "QQ-PROPOSAL-COMPILER-RECEIPT" error.Message

let validateProposal root (document: JsonObject) (proposal: JsonObject) =
    try
        let modules =
            objects document "modules"
            |> List.map (fun item -> text item "id")
            |> Set.ofList
        ...
        let namesAreValid (values: string list) =
            ...
            && values
               |> List.forall (fun value -> Regex.IsMatch(value, "^[A-Za-z][A-Za-z0-9_]*$"))
        ...
        let canonicalSourcePath =
            Path.GetFullPath(Path.Combine(root, text document "source"))

        let canonicalCompilerReceiptPath =
            Path.GetFullPath(Path.Combine(root, text document "compilerReceipt"))
        ...
        let proposedDefinition name =
            Regex.IsMatch(
                proposedSource,
                $"(?m)^\\s*(pure\\s+)?(val|def|run|action)\\s+%s{Regex.Escape name}(\\s|\\(|=)"
            )

        let proposedRoot = text proposal "root"
        let proposalTypecheck = lazy (typecheckProposalSource sourcePath)

        if actualFields <> requiredFields then
            fail "QQ-PROPOSAL-FIELDS" ($"%A{actualFields}")
        elif text proposal "schema" <> "fsgg.coordination.quint-proposal/1" then
            fail "QQ-PROPOSAL-SCHEMA" "unsupported"
        elif
            not (Regex.IsMatch(text proposal "owner", "^[a-z][a-z0-9-]*$"))
            || Set.contains (text proposal "owner") modules
        then
            fail "QQ-PROPOSAL-OWNER" "invalid-or-existing"
        elif Path.GetFullPath sourcePath <> canonicalSourcePath then
            fail "QQ-PROPOSAL-SOURCE" "canonical-source-required"
        elif not sourceExists then
            fail "QQ-PROPOSAL-SOURCE" sourcePath
        elif
            not (Regex.IsMatch(text proposal "behaviorSha256", "^[0-9a-f]{64}$"))
            || text proposal "behaviorSha256" <> sha256 (File.ReadAllBytes sourcePath)
        then
            fail "QQ-PROPOSAL-BEHAVIOR" "unbound"
        elif not (Regex.IsMatch(text proposal "compilerReceiptSha256", "^[0-9a-f]{64}$")) then
            fail "QQ-PROPOSAL-COMPILER-RECEIPT" "digest-shape"
        elif
            validateProposalCompilerReceipt root canonicalCompilerReceiptPath (text proposal "behaviorSha256") proposal
            |> Result.isError
        then
            validateProposalCompilerReceipt root canonicalCompilerReceiptPath (text proposal "behaviorSha256") proposal
        elif proposalTypecheck.Value |> Result.isError then
            proposalTypecheck.Value
        elif
            (identifiers "imports"
             |> Set.ofList
             |> fun actual -> Set.isEmpty actual || not (Set.isSubset actual modules))
        then
            fail "QQ-PROPOSAL-IMPORTS" "empty-or-unknown"
        elif
            not (namesAreValid (identifiers "invariants"))
            || not (namesAreValid (identifiers "witnesses"))
        then
            fail "QQ-PROPOSAL-EXECUTABLES" "invalid"
        elif
            (identifiers "invariants" @ identifiers "witnesses"
             |> List.exists (proposedDefinition >> not))
        then
            fail "QQ-PROPOSAL-EXECUTABLES" "missing-from-source"
        elif
            (identifiers "independentOracles"
             |> Set.ofList
             |> fun actual -> Set.isEmpty actual || not (Set.isSubset actual oracleIds))
        then
            fail "QQ-PROPOSAL-ORACLES" "unknown"
        elif
            not (Regex.IsMatch(proposedRoot, "^[A-Za-z][A-Za-z0-9_]*$"))
            || not (Regex.IsMatch(proposedSource, $"(?m)^module\\s+%s{Regex.Escape proposedRoot}\\s*\\{{"))
        then
            fail "QQ-PROPOSAL-ROOT" "invalid-or-missing"
        elif (bounds |> Seq.map (fun property -> property.Key) |> Set.ofSeq) <> boundFields then
            fail "QQ-PROPOSAL-BOUNDS" "fields"
        elif boundFields |> Set.exists (fun field -> number bounds field <= 0) then
            fail "QQ-PROPOSAL-BOUNDS" "non-positive"
        elif number bounds "depth" <= (identifiers "imports").Length then
            fail "QQ-PROPOSAL-BOUNDS" "insufficient-depth"
        elif
            (identifiers "projections"
             |> Set.ofList
             |> fun actual -> Set.isEmpty actual || not (Set.isSubset actual projections))
        then
            fail "QQ-PROPOSAL-PROJECTIONS" "unknown"
        elif not (Set.contains (text proposal "ciImpact") (Set [ "bounded-state-root"; "bounded-test-root" ])) then
            fail "QQ-PROPOSAL-CI" "unsupported"
        elif text proposal "budgetEffect" <> "within-calibrated-envelope" then
            fail "QQ-PROPOSAL-BUDGET" "unsupported"
        else
            Ok()
    with error ->
        fail "QQ-PROPOSAL-MALFORMED" error.Message
...
let validateReuseReceipt root configBytes (document: JsonObject) selected (receipt: JsonObject) =
    try
        ...
        if actualFields <> expectedFields then
            fail "QQ-REUSE-FIELDS" ($"%A{actualFields}")
        elif text receipt "schema" <> "fsgg.coordination.quint-reuse/1" then
            fail "QQ-REUSE-SCHEMA" "unsupported"
        elif text receipt "sourceSha256" <> text document "sourceSha256" then
            fail "QQ-REUSE-SOURCE" "stale"
        elif text receipt "configurationSha256" <> sha256 configBytes then
            fail "QQ-REUSE-CONFIGURATION" "stale"
        elif text receipt "baselineSha256" <> sha256 (File.ReadAllBytes baselinePath) then
            fail "QQ-REUSE-BASELINE" "stale"
        elif text receipt "backendIdentity" <> "quint-rust-apalache" then
            fail "QQ-REUSE-BACKEND" "unsupported"
        elif
            text receipt "toolchainIdentity"
            <> "79b32dacc5bb150e23c4017eef16f3f688cde062441583d5ea1ffa5cc9e62486"
        then
            fail "QQ-REUSE-TOOLCHAIN" "unsupported"
        elif strings receipt "selectedRoots" |> Set.ofList <> selected then
            fail "QQ-REUSE-ROOTS" "selection-mismatch"
        else
            Ok()
    with error ->
        fail "QQ-REUSE-MALFORMED" error.Message
...
let validateBaseline root configBytes (document: JsonObject) =
    try
        ...
        let roots =
            objects document "roots"
            |> List.map (fun item -> text item "id", item)
            |> Map.ofList

        let formalTests =
            objects document "formalTests"
            |> List.map (fun item -> text item "id", item)
            |> Map.ofList
        ...
        then
            ...
            let rootResult =
                ...
                |> List.tryPick (fun item ->
                    ...
                    if
                        fields |> List.exists (fun field -> number item field < 0)
                        || number item "sampleCount" = 0
                    then
                        Some("QQ-BASELINE-METRIC", id)
                    elif number item "dependencyDepth" > number budget "depth" then
                        Some("QQ-BASELINE-DEPTH", id)
                    elif
                        number item "stateCount" > number budget "states"
                    ...
                    then
                        Some("QQ-BASELINE-BUDGET", id)
                    else
                        None)
                |> function
                    | None -> Ok()
                    | Some(code, id) -> fail code id
            ...
            |> bind (fun () ->
                ...
                |> function
                    ...
                    | Some(code, id) -> fail code id)
    with error ->
        fail "QQ-BASELINE-MALFORMED" error.Message

let clone (node: JsonObject) =
    JsonNode.Parse(node.ToJsonString()).AsObject()

let runSelfTests root original =
    let firstObject (collection: string) (value: JsonObject) =
        ((value[collection].AsArray())[0]).AsObject()

    let firstArrayField collection (field: string) value =
        ((firstObject collection value)[field]).AsArray()

    let firstObjectField collection (field: string) value =
        ((firstObject collection value)[field]).AsObject()

    let nestedArray (owner: string) (field: string) (value: JsonObject) =
        (value[owner].AsObject()[field]).AsArray()
    ...
    let mutations: (string * (JsonObject -> unit)) list =
        [
            ...
            "cycle",
            fun value -> (firstArrayField "modules" "selectionImports" value).Add(JsonValue.Create("qualification"))
            ...
            fun value -> (firstObject "roots" value).["admission"].["owner"] <- JsonValue.Create("wrong-owner")
            "root-admission-oracle",
            fun value -> ((firstObject "roots" value).["admission"].["independentOracles"]).AsArray().RemoveAt(0)
            "root-admission-witness",
            fun value -> ((firstObject "roots" value).["admission"].["witnesses"]).AsArray().RemoveAt(0)
            "root-admission-invariant",
            fun value ->
                ((firstObject "roots" value).["admission"].["invariants"])[0] <- JsonValue.Create("missingInvariant")
            "root-admission-projection",
            fun value ->
                ((firstObject "roots" value).["admission"].["projections"])[0] <- JsonValue.Create("unknown-projection")
            "root-admission-ci",
            fun value -> (firstObject "roots" value).["admission"].["ciImpact"] <- JsonValue.Create("unbounded")
            "root-admission-budget-effect",
            fun value -> (firstObject "roots" value).["admission"].["budgetEffect"] <- JsonValue.Create("unknown")
            "formal-coverage", fun value -> value["formalTests"].AsArray().RemoveAt(0)
            "formal-main",
            fun value -> (firstObject "formalTests" value)["main"] <- JsonValue.Create("MissingFormalRoot")
            "formal-temporal",
            fun value -> (firstObject "formalTests" value)["temporal"] <- JsonValue.Create("missingTemporal")
            "formal-invalid",
            fun value -> (firstObject "formalTests" value)["invalid"] <- JsonValue.Create("missingInvalid")
    ...
    | Some _ ->
        ...
        let forgedSource =
            Path.Combine(Path.GetTempPath(), $"fsgg-forged-proposal-{Guid.NewGuid():N}.md")
...
let rec parse
    ...
    | "--mode" :: value :: tail ->
        parse
            ...
            planOut
            tail
    | "--changed-module" :: value :: tail ->
        parse
            selfTest
            root
            config
            mode
            (Set.add value changed)
            changedPaths
            changedSurfaces
            protectedMode
            reuseSource
    ...
    | "--protected-mode" :: value :: tail ->
        parse
            ...
            planOut
            tail
    | "--reuse-source-sha256" :: value :: tail ->
        parse
            selfTest
            root
            config
...
let configPath =
    if Path.IsPathRooted config then
        config
    else
...
| Ok() ->
    ...
    | Ok() ->
        ...
        let allRoots =
            objects document "roots" |> List.map (fun item -> text item "id") |> Set.ofList
            ...
            |> Set.fold
                (fun selected path ->
                    ...
                    if not (File.Exists absolute || Directory.Exists absolute) then
                        ...
                        normalized = text document "source"
                        ...
                        || normalized.Contains("qualify-canonical-quint", StringComparison.Ordinal)
                    then
                        Set.union selected allRoots
                    else
                        let matched =
                            moduleIds
                            |> Set.filter (fun moduleId -> normalized.Contains(moduleId, StringComparison.Ordinal))

                        if Set.isEmpty matched then
                            eprintfn "QQ-SELECTION-PATH: %s" path
                            exit 1

                        Set.union selected (reverseSelection document matched))
                Set.empty

        let rootsFromSurfaces =
            changedSurfaces
            |> Set.fold
                (fun selected surface ->
                    ...
                    | [| kind; owner |] when kind = "bound" || kind = "budget" ->
                        let ownerModule =
                            if Set.contains owner moduleIds then
                                owner
                            elif Map.containsKey owner rootsById then
                                text rootsById[owner] "module"
                            else
                                eprintfn "QQ-SELECTION-SURFACE: %s" surface
                                exit 1
        ...
        let selected =
            ...
            | "reuse" when
                reuseSource = Some(text document "sourceSha256")
                && Option.isSome reuseReceiptPath
                && not (Set.isEmpty changedSelection)
                ->
                changedSelection
            | "future-behavior" when Option.isSome proposal -> allRoots
            | _ ->
                eprintfn
                    "QQ-SELECTION-INPUT: mode=%s changed=%A paths=%A surfaces=%A protected=%s"
                    mode
                    changed
                    changedPaths
                    changedSurfaces
        ...
        | Some path ->
            ...
            output["changedModules"] <-
                JsonArray(
                    changed
                    |> Set.toArray
                    |> Array.map JsonValue.Create
                    |> Array.map (fun value -> value :> JsonNode)
                )
            ...
            reuseSource
            |> Option.iter (fun value -> output["reuseSourceSha256"] <- JsonValue.Create(value))

            reuseReceipt
            |> Option.iter (fun value ->
                output["reuseReceiptSha256"] <- JsonValue.Create(sha256 (Encoding.UTF8.GetBytes(value.ToJsonString()))))

            proposal
            |> Option.iter (fun value ->
                output["proposalSha256"] <- JsonValue.Create(sha256 (Encoding.UTF8.GetBytes(value.ToJsonString()))))
            ...
            output["formalTests"] <-
                JsonArray(
                    objects document "formalTests"
                    |> List.map (fun item -> text item "id")
                    |> List.map JsonValue.Create
                    |> List.map (fun value -> value :> JsonNode)
                    |> List.toArray
                )
        ...
        printfn
            ...
            (objects document "formalTests").Length
            oracleTests.Count
            mutationCount
            (objects document "formalTests").Length
            (sha256 bytes)
