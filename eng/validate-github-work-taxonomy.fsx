...
let expectationsPath =
    atRoot "evidence/github-substrate-v2/gs2-05-1/independent-expectations.json"

let modelPath = atRoot "evidence/github-substrate-v2/gs2-05-1/workTaxonomy.quint"

let testModelPath =
    atRoot "evidence/github-substrate-v2/gs2-05-1/workTaxonomy_test.quint"

let qualificationPath =
    atRoot "evidence/github-substrate-v2/gs2-05-1/qualification.json"
...
let corpusBytes =
    readBound corpusPath "c89d6acbbb8b47ad928eb3ed29e8044d2e33ee66d165dd5d6243c8e42d597ae8" "WTX-CORPUS"

let expectationBytes =
    readBound expectationsPath "0ead0c765ec062ed3f30bd7ddb43287176b3614e83eda5cb8410d2a95a5fea37" "WTX-EXPECTATIONS"
...
if
    ...
    <> "fsgg.github-substrate-v2.work-taxonomy-corpus/1"
then
    fail "WTX-CORPUS-SCHEMA" corpusPath

if
    expectations.GetProperty("schema").GetString()
    <> "fsgg.github-substrate-v2.work-taxonomy-expectations/1"
then
    fail "WTX-EXPECTATIONS-SCHEMA" expectationsPath
...
let expectedClassInventory =
    [ "<null>"; "capability"; "hardening"; "defect"; "decision" ]

let expectedKindInventory = [ "<null>"; "work"; "anchor"; "register"; "directive" ]

let expectedNativeInventory =
    [ "Epic"; "Feature"; "Task"; "Bug"; "Decision"; "Register"; "Directive" ]

if
    strings (expectations.GetProperty("requiredLegacyClasses"))
    <> expectedClassInventory
then
    fail "WTX-EXPECTATIONS-CLASS" "independent class inventory drifted"

if
    strings (expectations.GetProperty("requiredLegacyKinds"))
    <> expectedKindInventory
then
    fail "WTX-EXPECTATIONS-KIND" "independent kind inventory drifted"

if
    strings (expectations.GetProperty("requiredNativeTypes"))
    <> expectedNativeInventory
then
    fail "WTX-EXPECTATIONS-NATIVE" "independent native inventory drifted"
...
let actualMapping =
    ...
    |> Seq.map (fun item ->
        item.GetProperty("signal").GetString(),
        item.GetProperty("target").GetString(),
        item.GetProperty("lifecycle").GetString())
    |> Seq.toList

if actualMapping <> expectedMapping then
    fail "WTX-EXPECTATIONS-MAPPING" $"actual={actualMapping}"

let optionalString (item: JsonElement) (name: string) =
    let mutable value = Unchecked.defaultof<JsonElement>

    if not (item.TryGetProperty(name, &value)) || value.ValueKind = JsonValueKind.Null then
        None
    elif value.ValueKind = JsonValueKind.String then
        Some(value.GetString())
    else
        fail "WTX-CORPUS-TYPE" name

let requiredString (item: JsonElement) (name: string) =
    optionalString item name
    |> Option.defaultWith (fun () -> fail "WTX-CORPUS-FIELD" name)
...
if (actualIds |> Set.ofList |> Set.count) <> actualIds.Length then
    fail "WTX-CORPUS-DUPLICATE" "case ids are not unique"
...
if Set.ofList omissionCandidate = Set.ofList requiredIds then
    fail "WTX-OMISSION-INVERSION" "removing one case stayed green"
...
| Ok dispositions ->
    ...
    if stableIds <> List.sort stableIds then
        fail "WTX-PLAN-ORDER" "dispositions are not canonical"
    ...
    let reversed =
        ...
        |> List.rev
        |> WorkTaxonomy.plan
        |> Result.defaultWith (fail "WTX-PLAN-SHUFFLE" << string)

    if
        WorkTaxonomy.canonicalPlanBytes dispositions
        <> WorkTaxonomy.canonicalPlanBytes reversed
    then
        fail "WTX-PLAN-BYTES" "input order changed canonical plan bytes"

    if
        dispositions
        |> List.exists (fun disposition ->
            disposition.StableRowId.StartsWith("native-", StringComparison.Ordinal)
            && not disposition.NoOp)
    then
        fail "WTX-NOOP" "already-native case was not a no-op"
...
let independentRefusals =
    [
        ...
        "WTX-MISSING-STABLE-ID"
        { baseObservation with
            RepositoryScope = ""
        },
        "WTX-MISSING-REPOSITORY-SCOPE"
        { baseObservation with Revision = "" }, "WTX-MISSING-REVISION"
        { baseObservation with
            LegacyClass = None
        },
        "WTX-MISSING-CLASSIFICATION"
        { baseObservation with
            LegacyClass = Some "unknown"
        },
        "WTX-UNKNOWN-CLASS:unknown"
        { baseObservation with
            LegacyKind = Some "unknown"
        },
        "WTX-UNKNOWN-KIND:unknown"
        { baseObservation with
            NativeIssueType = Some "Incident"
            LegacyClass = None
        },
        "WTX-UNSUPPORTED-NATIVE:incident"
        { baseObservation with
            NativeIssueType = Some "Feature"
            LegacyClass = Some "defect"
        },
        "WTX-CONTRADICTORY"
        { baseObservation with
            LegacyClass = Some "defect"
            LegacyKind = Some "anchor"
        },
        "WTX-AMBIGUOUS"
        { baseObservation with
            NativeIssueType = Some "Feature"
...
let requiredRefusalFamilies =
    strings (expectations.GetProperty("requiredRefusalCodes"))

let observedRefusalCodes =
    (independentRefusals |> List.map snd) @ [ "WTX-DUPLICATE-STABLE-ID" ]

for family in requiredRefusalFamilies do
    if
        observedRefusalCodes
...
| Error failures when
    failures
    |> List.forall (fun refusal -> refusal.Diagnostics |> List.contains WorkTaxonomyDiagnostic.DuplicateStableRowId)
    ->
    ()
...
let quintScratch =
    Path.Combine(Path.GetTempPath(), $"fsgg-gs2-05-1-{Guid.NewGuid():N}")
...
run "quint" [ "typecheck"; stagedTestModelPath ] "WTX-QUINT-TEST-TYPECHECK"
|> ignore

run "quint" [ "test"; stagedTestModelPath; "--main=workTaxonomy_test" ] "WTX-QUINT-TEST"
|> ignore
...
if not (File.Exists qualificationPath) then
    fail "WTX-QUALIFICATION-MISSING" qualificationPath
...
if
    ...
    <> "fsgg.github-substrate-v2.work-taxonomy-qualification/1"
then
    fail "WTX-QUALIFICATION-SCHEMA" qualificationPath

if
    qualification.GetProperty("unitContractSha256").GetString()
    <> "ed9ae9d198d6eaaf89030f85d214a0a359333598be7ceb3597c2c4aeb629ef28"
then
    fail "WTX-QUALIFICATION-CONTRACT" "registration drift"

if
    qualification.GetProperty("gateCommandSha256").GetString()
    <> "c10ddf0ee6bb9328e09fceae4d0deebcc688478c1b2ae80c3fc3b4fbc766ef7f"
then
    fail "WTX-QUALIFICATION-COMMAND" "registered command drift"
...
let artifactPaths =
    artifactRows |> List.map (fun item -> item.GetProperty("path").GetString())

if artifactPaths <> expectedArtifactPaths then
    fail "WTX-QUALIFICATION-ARTIFACTS" $"actual={artifactPaths}"
...
for artifact in artifactRows do
    ...
    if actual <> expected then
        fail "WTX-QUALIFICATION-DIGEST" $"path={relative} expected={expected} actual={actual}"

printfn
    "github-work-taxonomy-contract OK cases=%d accepted=%d refused=%d q=Q2 network=offline inversions=%d"
    cases.Length
    accepted.Length
    refused
    (independentRefusals.Length + 2)
