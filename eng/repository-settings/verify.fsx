...
let loadCanonical code path =
    ...
    if bytes <> expected then
        fail code $"{path} is not compact canonical JSON with one trailing newline"

    JsonDocument.Parse(bytes)

let canonicalText (value: JsonElement) =
    canonicalBytes value |> Encoding.UTF8.GetString

let requireEqual code name (expected: JsonElement) (actual: JsonElement) =
    if canonicalText expected <> canonicalText actual then
        fail code $"{name} differs from desired state"
...
let requireString (code: string) (name: string) (expected: string) (value: JsonElement) =
    ...
    if actual <> expected then
        fail code $"{name} expected={expected} actual={actual}"

let hashPattern = Regex("^[0-9a-f]{64}$", RegexOptions.CultureInvariant)

let requireHash (code: string) (name: string) (value: string) =
    if not (hashPattern.IsMatch value) then
        fail code $"{name} must be lowercase SHA-256"
...
let validate (desiredPath: string) (preStatePath: string) (receiptPath: string) =
    ...
    exactProperties
        ...
        (desired.GetProperty("actions"))

    exactProperties
        "RS-DESIRED-REPOSITORY-SHAPE"
        [
            "allowAutoMerge"
            "allowMergeCommit"
            "allowRebaseMerge"
    ...
    exactProperties
        "RS-DESIRED-CODE-SECURITY-SHAPE"
        [ "associationStatus"; "configuration" ]
        (desired.GetProperty("codeSecurityConfiguration"))

    exactProperties
        "RS-DESIRED-CODE-SECURITY-CONFIGURATION-SHAPE"
        [
            "advancedSecurity"
    ...
    let desiredPreStateRules =
        desired.GetProperty("rulesets").EnumerateArray() |> Seq.toArray

    let actualPreStateRules =
        preState.GetProperty("rulesets").EnumerateArray() |> Seq.toArray

    if desiredPreStateRules.Length <> actualPreStateRules.Length then
        fail "RS-PRESTATE-RULESET-COUNT" "pre-state ruleset count differs"
    ...
    for index in 0 .. desiredPreStateRules.Length - 1 do
        ...
        if actual.GetProperty("id").GetInt64() <= 0L then
            fail "RS-PRESTATE-RULESET-ID" "pre-state ruleset id must be positive"

        for property in semanticProperties do
            requireEqual
                "RS-PRESTATE-RULESET-MISMATCH"
                property
                (wanted.GetProperty(property))
                (actual.GetProperty(property))

    let preStateOperations =
        preState.GetProperty("operations").EnumerateArray() |> Seq.toArray

    if preStateOperations.Length <> 15 then
        fail "RS-PRESTATE-OPERATIONS" "pre-state must bind all 15 authoritative operations"

    let mutable preStateNames = Set.empty

    for operation in preStateOperations do
        exactProperties
            "RS-PRESTATE-OPERATION-SHAPE"
            [ "httpStatus"; "method"; "name"; "path"; "responseSha256"; "status" ]
            operation

        let name = operation.GetProperty("name").GetString()

        if String.IsNullOrWhiteSpace(name) || preStateNames.Contains(name) then
            fail "RS-PRESTATE-OPERATIONS" "pre-state operation names must be unique and nonempty"
    ...
    if preStateNames <> expectedPreStateNames then
        fail "RS-PRESTATE-OPERATIONS" "pre-state authoritative operation set must be exact"
    ...
    if
        not (
            DateTimeOffset.TryParseExact(
                preState.GetProperty("observedAt").GetString(),
    ...
    if desiredRules.Length <> receiptRules.Length then
        fail "RS-RULESET-COUNT" "ruleset count differs"
    ...
    for index in 0 .. desiredRules.Length - 1 do
        ...
        if actual.GetProperty("id").GetInt64() <= 0L then
            fail "RS-RULESET-ID" "ruleset id must be positive"
    ...
    if desiredDigest <> sha256 desiredBytes then
        fail "RS-DESIRED-DIGEST" "desired contract digest mismatch"
    ...
    if operations.Length = 0 then
        fail "RS-OPERATIONS" "at least one authoritative operation is required"
    ...
    for operation in operations do
        ...
        if String.IsNullOrWhiteSpace(name) || operationNames.Contains(name) then
            fail "RS-OPERATIONS" "operation names must be unique and nonempty"
        ...
        if name = "organization-actions-permissions" then
            ...
            if httpStatus <> 403 then
                fail "RS-OPERATION-STATUS" "organization Actions permissions must bind the observed 403"
        else
            requireString "RS-OPERATION-STATUS" "status" "verified" operation

            if httpStatus < 200 || httpStatus > 299 then
                fail "RS-OPERATION-STATUS" $"operation {name} was not successful"

        if
            String.IsNullOrWhiteSpace(operation.GetProperty("method").GetString())
            || String.IsNullOrWhiteSpace(operation.GetProperty("path").GetString())
        then
    ...
    if operationNames <> requiredOps then
        fail "RS-OPERATIONS" "authoritative response operation set must be exact"
    ...
    let staticOperations =
        [
            ...
            "private-vulnerability-reporting",
            ("GET", "/repos/FS-GG/FS.GG.Coordination/private-vulnerability-reporting", 200)
    ...
    for name, (expectedMethod, expectedPath, expectedStatus) in staticOperations do
        let operation =
            operations
            |> Array.find (fun item -> item.GetProperty("name").GetString() = name)

        if
            operation.GetProperty("method").GetString() <> expectedMethod
            || operation.GetProperty("path").GetString() <> expectedPath
            || operation.GetProperty("httpStatus").GetInt32() <> expectedStatus
        then
            fail "RS-OPERATION-CONTRACT" $"{name} must be {expectedMethod} {expectedPath} status={expectedStatus}"

    let dependabotOperation =
        operations
        |> Array.find (fun item -> item.GetProperty("name").GetString() = "dependabot-alerts")

    if
        dependabotOperation.GetProperty("responseSha256").GetString()
    ...
    for ruleset in receiptRules do
        let name =
            if ruleset.GetProperty("target").GetString() = "branch" then
                "main-ruleset"
            else
                "release-tag-ruleset"
        ...
        if
            operation.GetProperty("method").GetString() <> "GET"
            || operation.GetProperty("path").GetString() <> expectedPath
            || operation.GetProperty("httpStatus").GetInt32() <> 200
        then
            fail "RS-RULESET-RESPONSE" $"{name} must bind GET {expectedPath} status=200"

    if String.IsNullOrWhiteSpace(receipt.GetProperty("repair").GetString()) then
        fail "RS-REPAIR" "rollback or forward-repair guidance is required"
    ...
    if statedDigest <> computedDigest then
        fail "RS-RECEIPT-DIGEST" "receipt self-digest mismatch"
