...
let validateRoot root =
    ...
    if
        not (Directory.Exists(Path.Combine(full, ".git")))
        && not (File.Exists(Path.Combine(full, ".git")))
    then
...
let execute root =
    ...
    let forbiddenOverrides =
        [
            "FSGG_BOOTSTRAP_PACKAGE_OVERRIDE"
            "FSGG_BOOTSTRAP_RECOVERY_COMMAND"
            "RESTORESOURCES"
    ...
    let sourceStatus =
        run source "git" [ "status"; "--porcelain=v1"; "--untracked-files=all" ] []

    if sourceStatus <> "" then
        fail "BR-SOURCE-DIRTY" "candidate worktree must be clean"
    ...
    let scratch =
        Path.Combine(Path.GetTempPath(), $"fsgg-bootstrap-recovery-{Guid.NewGuid():N}")
    ...
    try
        ...
        run source "git" [ "clone"; "--no-local"; "--no-checkout"; "--quiet"; source; clone ] environment
        |> ignore

        run clone "git" [ "checkout"; "--detach"; "--quiet"; candidate ] environment
        |> ignore

        if run clone "git" [ "rev-parse"; "HEAD" ] environment <> candidate then
            fail "BR-CLONE-REVISION" candidate

        if
            run clone "git" [ "status"; "--porcelain=v1"; "--untracked-files=all" ] environment
            <> ""
        then
        ...
        let package =
            Path.Combine(feed, "FS.GG.Coordination.Protocol.0.0.0-bootstrap.nupkg")

        if not (File.Exists package) then
            fail "BR-PACKAGE" package

        Directory.CreateDirectory consumer |> ignore

        for file in [ "Bootstrap.Consumer.fsproj"; "Program.fs" ] do
            File.Copy(
                Path.Combine(clone, "tests/fixtures/bootstrap-package-consumer", file),
                Path.Combine(consumer, file)
            )
        ...
        if observed <> "FS.GG.Coordination.Protocol:1" then
            fail "BR-EXECUTE" observed
