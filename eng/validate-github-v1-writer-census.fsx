...
let readBytes name =
    File.ReadAllBytes(Path.Combine(evidenceRoot, name))

let parse (bytes: byte array) =
    JsonNode.Parse(ReadOnlySpan<byte>(bytes)).AsObject()
    ...
    |> Seq.map (fun value ->
...

        {
            ...
            NonWriterJustification =
                if isNull row["nonWriterJustification"] then
                    None
                else
                    Some(text row "nonWriterJustification")
        })
    |> List.ofSeq

if text census "schema" <> "fsgg.v1-writer-census/1" then
    failwith "producer census schema differs"

if text contract "schema" <> "fsgg.coord.commands/1" then
    failwith "candidate command contract schema differs"

if sha256 censusBytes <> text binding "censusSha256" then
    failwith "producer census byte digest differs"

if sha256 contractBytes <> text binding "commandContractSha256" then
    failwith "candidate command-contract byte digest differs"

if
    text binding "acceptedEpochReceiptDigest"
    <> "49c70359ebfbc00331ba90c7c5b100a292efa4cc95a5dfa8007867ceceec5c31"
then
    failwith "accepted GS2-08.1 receipt binding differs"

if text binding "sourceBaseRevision" <> "95de1c77674b9dd8d7a9ce568d1ee175a7797e5e" then
    failwith "historical Q0 source base differs"

if
    text binding "q0EvidenceSha256"
    <> "ef07c245e4ab3dd0b97d997a32940cdd70485070bfedc9c44e5b4ed2422078c5"
then
    failwith "historical Q0 evidence binding differs"

if
    text binding "q0CorpusSha256"
...
if contractCommands <> censusPairs then
    failwith "candidate-built command contract differs from the producer census"
...
let count (writes: string) =
    censusCommands |> List.filter (_.Writes >> (=) writes) |> List.length

if
    censusCommands.Length <> expectedCount "commandRootCount"
    ...
    || sources.Length <> expectedCount "sourceIdentityCount"
then
    failwith "independent census counts differ"

if
    binding["trackedExecutableCount"].GetValue<int>()
    <> expectedCount "trackedExecutableCount"
then
    failwith "tracked executable population count differs"

let names writes =
    censusCommands |> List.filter (_.Writes >> (=) writes) |> List.map _.Name

if
    names "always" <> texts expected "alwaysCommands"
    || names "conditional" <> texts expected "conditionalCommands"
    || names "never" <> texts expected "neverCommands"
then
    failwith "independently authored exact command write classification differs"

let sourceByPath =
    sources |> List.map (fun value -> value.Path, value) |> Map.ofList
...
let refused candidate =
    GitHubV1WriterCensusQualification.validateSnapshot candidate |> Result.isError
...
let generatedMutation =
    ...
    | AcceptedEpochPrerequisite ->
        refused
            { snapshot with
                AcceptedEpochReceiptSha256 = "invalid"
            }
    | RoadmapBinding ->
        refused
            { snapshot with
                RoadmapRevision = "main"
                RoadmapSha256 = "invalid"
            }
    | HistoricalQ0Binding ->
        refused
            { snapshot with
                SourceBaseRevision = "main"
                Q0EvidenceSha256 = "invalid"
            }
    | ProducerSourceBinding ->
        refused
            { snapshot with
                ProducerRevision = "main"
            }
    | CensusByteBinding ->
        sha256 censusBytes = snapshot.CensusSha256
        && refused
            { snapshot with
                CensusSha256 = "invalid"
            }
    | CandidateCommandContract ->
        contractCommands = censusPairs
        && contractCommands @ [ ("unknown", "always") ] <> censusPairs
    ...
    | CompleteCommandRoots ->
        refused
            { snapshot with
                Commands = snapshot.Commands.Tail
            }
    | CompleteSourcePopulation ->
        refused
            { snapshot with
                Sources = snapshot.Sources.Tail
            }
    | StableUniqueOrdering ->
        refused
            { snapshot with
                Commands = List.rev snapshot.Commands
            }
    | ExactWriteClassification ->
        refused
            { snapshot with
                Commands =
                    { snapshot.Commands.Head with
                        Writes = "never"
                    }
                    :: snapshot.Commands.Tail
            }
    | SourceIdentity ->
        refused
            { snapshot with
                Sources =
                    { snapshot.Sources.Head with
                        Sha256 = "main"
                    }
                    :: snapshot.Sources.Tail
            }
    | SinkDisposition ->
        refused
            { snapshot with
    ...
    | UnknownCommandRefusal ->
        refused
            { snapshot with
                Commands = snapshot.Commands @ [ { Name = "unknown"; Writes = "always" } ]
            }
    | DynamicWriterRefusal ->
        sourceByPath.ContainsKey mandatoryPath
        && refused
            { snapshot with
                Sources = snapshot.Sources |> List.filter (_.Path >> (<>) mandatoryPath)
            }
    | NoFenceClaim -> scope.Contains("no installed ledger") && scope.Contains("effect fence")
    | NoReceiverClaim ->
        scope.Contains("no installed ledger")
        && scope.Contains("receiver")
        && scope.Contains("publication")

let independentMutation =
    function
    | SchemaBinding ->
        text census "schema" = "fsgg.v1-writer-census/1"
        && text contract "schema" = "fsgg.coord.commands/1"
    | AcceptedEpochPrerequisite ->
        snapshot.AcceptedEpochReceiptSha256 = "49c70359ebfbc00331ba90c7c5b100a292efa4cc95a5dfa8007867ceceec5c31"
    | RoadmapBinding -> snapshot.RoadmapSha256 = "20f4f2bcdcd6e2bfdd787edc66efbe30a21289f7ddf2030a8b1b3ae2150b7c52"
    | HistoricalQ0Binding ->
        snapshot.Q0EvidenceSha256 = "ef07c245e4ab3dd0b97d997a32940cdd70485070bfedc9c44e5b4ed2422078c5"
        && snapshot.Q0CorpusSha256 = "5c94fa3ee60e02b7fbee80918b45e5e2046a152a2342f6b88044ac169c1dc67b"
    | ProducerSourceBinding ->
        snapshot.ProducerTree = "6c6b084df2fab729034828183b9ba4a373501d9e"
        && snapshot.ProducerRevision.Length = 40
    | CensusByteBinding -> sha256 censusBytes = "589ab3d20bbdfb78d4863e9fd11bc48dbbd0b222d47b635a7fe13fec02239812"
    | CandidateCommandContract ->
        sha256 contractBytes = "780b3a8779b9e91358b2200f8d696de54b4e76341f0e3ddf2962d5b0ddd8a441"
        && contractCommands = censusPairs
    | CheckerBinding ->
        snapshot.CheckerSha256 = "2b90d16f7d4b1a4ea1f9afd78eed306655f32f60470b20370d9e895f3c999619"
        && snapshot.FixtureRunnerSha256 = "11aa3f00627dfa9fe10a2c5994659deb82f93a93abcc4c3a41a01ad389fac754"
    ...
    | SinkDisposition -> expected["mandatorySources"].AsObject().Count = 23
    | UnknownCommandRefusal -> generatedMutation UnknownCommandRefusal
    | DynamicWriterRefusal ->
        sourceByPath[mandatoryPath].Disposition = "remote-writer"
        && sourceByPath[mandatoryPath].SinkKinds = [ "dynamic-process" ]
...
let expectedControls =
    ...
    |> List.map GitHubV1WriterCensusQualification.controlId

if texts expected "controls" <> expectedControls then
...
| Ok() ->
    printfn
        "GITHUB_V1_WRITER_CENSUS_OK producer=%s roots=%d always=%d conditional=%d never=%d sources=%d controls=%d census=%s contract=%s"
        snapshot.ProducerRevision
        censusCommands.Length
        (count "always")
        (count "conditional")
        (count "never")
        sources.Length
        expectedControls.Length
        snapshot.CensusSha256
        snapshot.CommandContractSha256
