...
let staleDocument =
    json "evidence/github-substrate-v2/gs2-07-6/stale-green-refusal.json"

let stale = staleDocument.RootElement

let cleanupDocument =
    json "evidence/github-substrate-v2/gs2-07-6/burst-cleanup.json"
...
let hints =
    ...
    |> Seq.map (fun row ->
...

        {
            ...
            HintId = stringAt row "hintId"
            Sequence = intAt row "sequence"
            SupersedesHintId =
                if prior.ValueKind = JsonValueKind.Null then
                    None
                else
                    Some(prior.GetString())
        })
    |> Seq.toList
...
let decisions =
    ...
    |> List.map (fun row ->
        {
            Subject = stringAt row "subject"
            PriorHeadSha = stringAt row "priorHeadSha"
            HeadSha = stringAt row "headSha"
            AuthorizationHeadSha = stringAt row "authorizationHeadSha"
            PriorBaseSha = stringAt row "priorBaseSha"
            BaseSha = stringAt row "baseSha"
            PriorRequiredChecks = stringsAt row "priorRequiredChecks"
            RequiredChecks = stringsAt row "requiredChecks"
            SuccessfulChecks = stringsAt row "successfulChecks"
            WorkMilliseconds = int64At row "workMilliseconds"
            WaitingMilliseconds = int64At row "waitingMilliseconds"
            Delivered = row.GetProperty("delivered").GetBoolean()
        })
...
let facts =
    {
        Hints = hints
        Decisions = decisions
        MaxHintsPerSubject = intAt burst "maxHintsPerSubject"
        InFlightEffectCount = intAt burst "inFlightEffectCount"
        CancelledInFlightEffectCount = intAt burst "cancelledInFlightEffectCount"
        WorkMilliseconds = int64At burst "workMilliseconds"
        WaitingMilliseconds = int64At burst "waitingMilliseconds"
    }

let get =
    function
    | Ok value -> value
    | Error errors -> failwithf "routine burst refused: %A" errors
...
let editIds =
    providerEdits.GetProperty("nodes").EnumerateArray()
    |> Seq.map (fun row -> stringAt row "id")
    |> Seq.toList
...
let hosted () =
    ...
    && (facts.Hints
        |> List.filter (fun h -> h.Subject = decisions.Head.Subject)
        |> List.length)
        =
        4
    && providerEdits.GetProperty("totalCount").GetInt32() >= 4
    && editIds.Length >= 4
    && editIds.Length = (editIds |> List.distinct |> List.length)
...
if not (hosted ()) then
    failwith "retained routine-burst evidence is not cross-bound"
...
if
    not (
        ...
        |> has GitHubQueueSandboxFinding.InFlightEffectCancelled
    )
then
    failwith "cancellation mutant passed"
...
if
    not (
        ...
        |> has GitHubQueueSandboxFinding.DistinctSubjectLost
    )
then
    failwith "lost-subject mutant passed"
...
if
    not (
        ...
        |> has (GitHubQueueSandboxFinding.MissingRequiredContext primaryFacts.Subject)
    )
then
    failwith "missing-context mutant passed"
    ...
    not (
        Sandbox.compileBurst
            { facts with
                Decisions =
                    { primaryFacts with
                        AuthorizationHeadSha = primaryFacts.PriorHeadSha
                    }
                    :: decisions.Tail
            }
        |> has (GitHubQueueSandboxFinding.StaleGreenAuthorization primaryFacts.Subject)
    )
then
    failwith "stale-green mutant passed"

if Sandbox.verifyBurst receipt.Seal receipt <> Ok receipt then
    failwith "burst seal replay failed"

printfn
    "GITHUB_QUEUE_ROUTINE_BURST_OK subjects=%d hints=%d work_ms=%d waiting_ms=%d seal=%s"
    receipt.Subjects.Length
    receipt.HintCount
    receipt.WorkMilliseconds
    receipt.WaitingMilliseconds
    receipt.Seal
