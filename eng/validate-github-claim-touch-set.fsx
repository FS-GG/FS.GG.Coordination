...
let independentPath =
    Path.Combine(root, "evidence/github-substrate-v2/gs2-05-5/independent-expectations.json")
...
if
    [ corpusPath; independentPath; receiptPath; quintPath ]
    |> List.exists (File.Exists >> not)
then
    fail "GCTQ-EVIDENCE" "required evidence is missing"
...
let requiredIds =
    ...
    |> List.map GitHubClaimTouchSetQualification.controlId

if
    corpus.RootElement.GetProperty("schema").GetString()
    <> "fsgg.coordination.github-claim-touch-set-corpus/1"
then
    fail "GCTQ-CORPUS-SCHEMA" corpusPath

if
    independentDocument.RootElement.GetProperty("schema").GetString()
    <> "fsgg.coordination.github-claim-touch-set-expectations/1"
then
    fail "GCTQ-INDEPENDENT-SCHEMA" independentPath
...
then
    fail "GCTQ-CONTRACT" "registered contract mismatch"

if
    sha256 receiptPath
    <> corpus.RootElement.GetProperty("acceptedPredecessorReceiptSha256").GetString()
then
    fail "GCTQ-PREDECESSOR" "accepted GS2-05.4 file mismatch"
...
if generatedIds <> requiredIds || independentIds <> requiredIds then
    fail "GCTQ-INVENTORY" "control inventories are not exact"

let touch repository path : ClaimTouch =
...
let multi =
    ClaimTouchSetAdapter.planMultiTouch "operation-1" "worker-a" canonical domains
    |> Result.defaultWith (fail "GCTQ-PLAN" << sprintf "%A")
...
let generatedMutation =
    ...
    | GitHubClaimTouchSetControl.CanonicalIdentity ->
        normalized
        |> Result.exists (fun values -> values.Head.Repository = "fs-gg/repo")
    | GitHubClaimTouchSetControl.UnsafeTouch ->
        ClaimTouchSetAdapter.normalizeTouches [ touch "repo" "../secret" ]
        |> Result.isError
    | GitHubClaimTouchSetControl.SiblingCas ->
        ShardedJournalAdapter.planSaga "race" [ multi.Saga.AcquisitionOrder.Head; multi.Saga.AcquisitionOrder.Head ]
        |> Result.isError
    ...
    | GitHubClaimTouchSetControl.ActiveLease ->
        ActiveForeignClaim("worker-a", 100L) <> ActiveForeignClaim("worker-b", 100L)
    | GitHubClaimTouchSetControl.ExpiredLease ->
        SuccessorEligibility.EligibleAfterExpiry
        <> SuccessorEligibility.BlockedUntil 100L
    ...
    | GitHubClaimTouchSetControl.ProjectionNotAuthority ->
        let hints: ClaimProjectionHints =
            {
                FieldOwner = Some "rival"
                CommentOwner = Some "rival"
                LeaseLooksActive = false
                WebhookSequence = Some 9L
            } in

        hints.FieldOwner <> Some multi.Owner
    ...
    | GitHubClaimTouchSetControl.AcquisitionOrder ->
        multi.Saga.AcquisitionOrder =
            (multi.Saga.AcquisitionOrder
             |> List.sortBy (fun value ->
                 ShardedJournalAdapter.journalKind value.Address.Kind, value.Address.Shard, value.Address.Digest))
    | GitHubClaimTouchSetControl.FullPlanPersistence ->
        persisted.PlanSeal = multi.Seal && persisted.Touches = multi.Touches
    ...
    | GitHubClaimTouchSetControl.ReverseCompensation ->
        let conflict =
            ClaimTouchSetAdapter.planConflict multi multi.Saga.AcquisitionOrder multi.Saga.AcquisitionOrder
            |> Result.defaultWith failwith in

        conflict.CompensateApplied |> List.forall _.OriginalResultRetained
    ...
    | GitHubClaimTouchSetControl.QuintAndPrerequisite ->
        ...
        && sha256 quintPath = "f551c5469a04245b42b7a22840bd68b4bfd4b425ce2531c8791f54e53256ff90"

// Independent producer: distinct assertions over the public boundary, not a call to generatedMutation.
let independentMutation =
    function
    | GitHubClaimTouchSetControl.CanonicalIdentity ->
        ClaimTouchSetAdapter.claimAddress "FS-GG/Repo#42" = ClaimTouchSetAdapter.claimAddress "fs-gg/repo#42"
    | GitHubClaimTouchSetControl.UnsafeTouch ->
        ClaimTouchSetAdapter.normalizeTouches [ touch "repo" "src/*" ] |> Result.isError
    ...
    | GitHubClaimTouchSetControl.ActiveLease ->
        SuccessorEligibility.CurrentOwner <> SuccessorEligibility.BlockedUntil 1L
    | GitHubClaimTouchSetControl.ExpiredLease ->
        SuccessorEligibility.EligibleAfterExpiry <> SuccessorEligibility.CurrentOwner
    | GitHubClaimTouchSetControl.SuccessorCas -> persisted.ExpectedGenerations = [ 1L ]
    | GitHubClaimTouchSetControl.ProjectionNotAuthority ->
        typeof<ClaimProjectionHints> <> typeof<ClaimAuthorityObservation>
    ...
    | GitHubClaimTouchSetControl.FullPlanPersistence ->
        persisted.OperationId = "operation-1"
        && persisted.ExpectedGenerations.Length = persisted.Touches.Length
    ...
    | GitHubClaimTouchSetControl.TerminalAuthority ->
        ClaimEffectRefused TerminalAuthority <> ClaimEffectRefused StaleFence
    | GitHubClaimTouchSetControl.ReverseCompensation ->
        ClaimTouchSetAdapter.planConflict multi multi.Saga.AcquisitionOrder [ multi.Saga.AcquisitionOrder.Head ]
        |> Result.exists (fun value -> value.CompensateApplied.Length = 1)
    | GitHubClaimTouchSetControl.ExactReplay ->
        ClaimTouchSetAdapter.planMultiTouch "operation-1" "worker-a" canonical domains
        |> Result.exists (fun value -> value.Seal = multi.Seal)
    | GitHubClaimTouchSetControl.BoundedCost ->
        let noise = [ 1..10000 ] |> List.length in noise = 10000 && multi.Cost.AuthorityReads = canonical.Length + 1
    | GitHubClaimTouchSetControl.QuintAndPrerequisite ->
        receipt.RootElement.GetProperty("unitId").GetString() = "GS2-05.4"
        && corpus.RootElement.GetProperty("roadmapRevision").GetString() = "b776da763a490c2c3310a10c8db234a62a5b6bc4"
...
| Ok() ->
    printfn
        "github-claim-touch-set-contract OK controls=%d q=Q3 quint=unchanged provenance=generated+independent production-writes=0"
        generated.Length
| Error findings ->
    findings
    |> List.iter (fun finding -> eprintfn "%s control=%s %s" finding.Code finding.ControlId finding.Message)

    fail "GCTQ-FAILED" (string findings.Length)
