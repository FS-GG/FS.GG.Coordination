#!/usr/bin/env python3
"""Prepare and verify the immutable Linux-x64 orchestration-host candidate."""
...
SCHEMA = "fsgg.coordination.orchestration-host-candidate/1"
PREPARED_SCHEMA = "fsgg.coordination.orchestration-host-prepared/1"
VERIFICATION_SCHEMA = "fsgg.coordination.orchestration-host-verification/1"
PROVENANCE_TYPE = "https://slsa.dev/provenance/v1"
REPOSITORY = "FS-GG/FS.GG.Coordination"
PROJECT = "src/FS.GG.Coordination.Orchestration.Host/FS.GG.Coordination.Orchestration.Host.fsproj"
LOCK = "src/FS.GG.Coordination.Orchestration.Host/packages.lock.json"
WORKFLOW = ".github/workflows/orchestration-host-candidate.yml"
SDK = "10.0.400"
RUNTIME = "10.0.11"
FSC_SHA256 = "3e82a7fb4fb386f645b538dd56b73e02eb77d12f453915c5ae029421343d5d18"
RID = "linux-x64"
PAYLOAD = "fsgg-coord-orchestration-host"
ROOT = "fsgg-coord-orchestration-host-linux-x64"
ARCHIVE_PREFIX = "fsgg-coord-orchestration-host-linux-x64-"
LIMIT = 100 * 1024 * 1024
...
def run(cwd: Path, arguments: list[str], env: dict[str, str] | None = None, expected: int = 0) -> str:
    ...
    require(process.returncode == expected, "OHC-COMMAND", f"exit={process.returncode} command={arguments!r}\n{process.stdout}")
    return process.stdout.strip()


def exact_candidate(repo: Path, candidate: str, protected_ref: str) -> tuple[str, str]:
    require(bool(SHA.fullmatch(candidate)), "OHC-CANDIDATE", "expected lowercase 40-character SHA")
    actual = run(repo, ["git", "rev-parse", candidate])
    require(actual == candidate, "OHC-CANDIDATE", "revision did not resolve byte-identically")
    run(repo, ["git", "merge-base", "--is-ancestor", candidate, protected_ref])
    tree = run(repo, ["git", "rev-parse", candidate + "^{tree}"])
    require(bool(SHA.fullmatch(tree)), "OHC-TREE", "source tree is invalid")
...
def toolchain(repo: Path) -> dict[str, str]:
    ...
    require(sdk == SDK, "OHC-SDK", f"expected {SDK}, observed {sdk}")
    runtimes = run(repo, ["dotnet", "--list-runtimes"])
    require(f"Microsoft.NETCore.App {RUNTIME} " in runtimes, "OHC-RUNTIME", f"runtime {RUNTIME} is absent")
    rows = [line for line in run(repo, ["dotnet", "--list-sdks"]).splitlines() if line.startswith(SDK + " [")]
    require(len(rows) == 1 and rows[0].endswith("]"), "OHC-SDK", "pinned SDK location is ambiguous")
    sdk_root = Path(rows[0][len(SDK) + 2 : -1]) / SDK
    compiler = sdk_root / "FSharp" / "fsc.dll"
    require(compiler.is_file(), "OHC-FSC", "pinned F# compiler is absent")
    observed = digest_file(compiler)
    require(observed == FSC_SHA256, "OHC-FSC", "pinned F# compiler digest differs")
...
def project_contract(source: Path) -> str:
    ...
    require("net10.0/linux-x64" in lock_value.get("dependencies", {}), "OHC-LOCK", "host lock omits net10.0/linux-x64")
...
def safe_project(repo: Path, candidate: str, source: Path) -> str:
    ...
    with zipfile.ZipFile(archive) as zipped:
        ...
        require(bool(entries), "OHC-SOURCE", "tracked projection is empty")
        for entry in entries:
            path = Path(entry.filename)
            require(not path.is_absolute() and ".." not in path.parts, "OHC-SOURCE", f"unsafe path {entry.filename}")
        zipped.extractall(source)
    archive.unlink()
    require((source / PROJECT).is_file(), "OHC-SOURCE", "tracked projection omits host project")
...
def validate_archive(path: Path, expected_manifest: bytes | None = None) -> tuple[dict, bytes]:
    require(path.is_file() and path.stat().st_size <= LIMIT, "OHC-ARCHIVE-SIZE", "archive is absent or exceeds 100 MiB")
    with zipfile.ZipFile(path) as zipped:
        infos = zipped.infolist()
        names = [info.filename for info in infos]
        expected = [f"{ROOT}/{PAYLOAD}", f"{ROOT}/manifest.json"]
        require(names == expected, "OHC-LAYOUT", f"archive layout differs: {names!r}")
        require(all(info.date_time == FIXED_TIME for info in infos), "OHC-TIMESTAMP", "archive timestamp differs")
        modes = [(info.external_attr >> 16) & 0xFFFF for info in infos]
        require(modes == [stat.S_IFREG | 0o500, stat.S_IFREG | 0o400], "OHC-MODE", f"archive modes differ: {modes!r}")
        binary = zipped.read(expected[0])
        manifest_bytes = zipped.read(expected[1])
    require(len(binary) <= LIMIT, "OHC-PAYLOAD-SIZE", "payload exceeds 100 MiB")
    require(binary[:4] == b"\x7fELF" and binary[4:6] == b"\x02\x01" and binary[18:20] == b"\x3e\x00", "OHC-ELF", "payload is not Linux x86-64 ELF")
    ...
    if expected_manifest is not None:
        require(manifest_bytes == expected_manifest, "OHC-MANIFEST", "embedded manifest differs")
    return manifest, binary


def usage_readback(binary: bytes) -> str:
    with tempfile.TemporaryDirectory(prefix="o2-host-usage-") as temporary:
...
def build_once(repo: Path, candidate: str, tree: str, commit_time: str, identity: dict[str, str], root: Path) -> tuple[Path, bytes, dict]:
    ...
    require(files == [PAYLOAD], "OHC-SINGLE-FILE", f"publish output differs: {files!r}")
...
def prepare(args: argparse.Namespace) -> None:
    ...
    require(not output.exists() or not any(output.iterdir()), "OHC-OUTPUT", "output must be absent or empty")
    output.mkdir(parents=True, exist_ok=True)
    tree, commit_time = exact_candidate(repo, args.candidate, args.protected_ref)
    identity = toolchain(repo)
    fixed = Path(tempfile.gettempdir()) / f"fsgg-orchestration-host-candidate-{args.candidate}"
    ...
    try:
        ...
        if fixed.exists():
            shutil.rmtree(fixed)
    require(products[0][0] == products[1][0], "OHC-REPRODUCIBILITY", "double-build archives differ")
    require(products[0][1] == products[1][1], "OHC-REPRODUCIBILITY", "double-build manifests differ")
    require(products[0][2] == products[1][2], "OHC-REPRODUCIBILITY", "double-build readback differs")
    ...
    provenance = {
        ...
        "predicate": {"buildDefinition": {"buildType": f"https://github.com/{REPOSITORY}/orchestration-host-candidate/v1", "externalParameters": {"candidate": args.candidate, "sourceTree": tree, "rid": RID}, "resolvedDependencies": [{"uri": LOCK, "digest": {"sha256": manifest["lock"]["sha256"]}}]}, "runDetails": {"builder": {"id": f"https://github.com/{REPOSITORY}/{WORKFLOW}"}}},
    }
    ...
    print(f"ORCHESTRATION_HOST_PREPARED candidate={args.candidate} archiveSha256={prepared['archive']['sha256']} bytes={prepared['archive']['bytes']}")


def verify_prepared(path: Path) -> dict:
    receipt = json.loads(path.read_text())
    require(receipt.get("schema") == PREPARED_SCHEMA, "OHC-PREPARED", "prepared schema differs")
    candidate = receipt.get("candidate", "")
    require(bool(SHA.fullmatch(candidate)), "OHC-PREPARED", "prepared candidate differs")
    root = path.parent
    archive = receipt.get("archive", {})
    archive_path = root / archive.get("file", "")
    require(archive_path.name == f"{ARCHIVE_PREFIX}{candidate}.zip", "OHC-PREPARED", "archive name differs")
    require(archive_path.stat().st_size == archive.get("bytes") and digest_file(archive_path) == archive.get("sha256"), "OHC-PREPARED", "archive bytes differ")
    manifest_path = root / "manifest.json"
    require(digest_file(manifest_path) == receipt.get("manifestSha256"), "OHC-PREPARED", "manifest digest differs")
    ...
    require(canonical_bytes(provenance) == provenance_path.read_bytes(), "OHC-PROVENANCE", "provenance is not canonical")
    require(provenance.get("_type") == "https://in-toto.io/Statement/v1" and provenance.get("predicateType") == PROVENANCE_TYPE, "OHC-PROVENANCE", "provenance type differs")
    expected_subject = [{"name": archive["file"], "digest": {"sha256": archive["sha256"]}}]
    require(provenance.get("subject") == expected_subject, "OHC-PROVENANCE", "provenance subject differs")
    definition = provenance.get("predicate", {}).get("buildDefinition", {})
    require(definition.get("externalParameters") == {"candidate": candidate, "sourceTree": receipt["sourceTree"], "rid": RID}, "OHC-PROVENANCE", "provenance source binding differs")
    dependencies = definition.get("resolvedDependencies", [])
    require(dependencies == [{"uri": LOCK, "digest": {"sha256": manifest["lock"]["sha256"]}}], "OHC-PROVENANCE", "provenance lock binding differs")
    require(receipt.get("stages") == ["protected-main-bound", "tracked-source-projected", "locked-rid-restored", "published-twice", "archives-byte-identical", "prepared-verified"], "OHC-PREPARED", "prepared stages differ")
    return receipt
...
def verify_served(args: argparse.Namespace) -> None:
    ...
    require(served.name == archive["file"], "OHC-SERVED", "served filename differs")
    require(served.stat().st_size == archive["bytes"] and digest_file(served) == archive["sha256"], "OHC-SERVED", "served archive bytes differ")
...
def validate_artifact_identity(candidate: str, artifact_id_value: str, artifact_name: str, artifact_url: str, artifact_digest: str) -> dict:
    ...
    require(artifact_id.isdigit() and int(artifact_id) > 0, "OHC-ARTIFACT", "artifact id is invalid")
    expected_name = f"orchestration-host-linux-x64-{candidate}"
    require(artifact_name == expected_name, "OHC-ARTIFACT", "artifact name differs")
    require(bool(re.fullmatch(r"[0-9a-f]{64}", artifact_digest)), "OHC-ARTIFACT", "outer artifact digest is invalid")
    ...
    require(parsed_url.scheme == "https" and parsed_url.netloc == "github.com" and route_ok and not parsed_url.query and not parsed_url.fragment, "OHC-ARTIFACT", "artifact URL differs")
...
def validate_workflow(repo: Path) -> None:
    ...
    require(text.count("  workflow_dispatch:\n") == 1, "OHC-WORKFLOW", "manual trigger differs")
    for trigger in ("push", "pull_request", "schedule", "repository_dispatch"):
        require(not re.search(rf"^  {trigger}:\s*$", text, re.MULTILINE), "OHC-WORKFLOW", f"forbidden trigger {trigger}")
    for exact in (
        ...
        "name: orchestration-host-verification-${{ github.sha }}",
    ):
        require(text.count(exact) == 1, "OHC-WORKFLOW", f"missing or ambiguous workflow binding: {exact}")
    require(text.count("name: orchestration-host-linux-x64-${{ github.sha }}") == 2, "OHC-WORKFLOW", "candidate upload/download identity differs")
    require(text.count("${{ inputs.expected_sha }}") == 1, "OHC-WORKFLOW", "dispatch input escapes inert comparison")
    require(text.count("${{ github.sha }}") == 9, "OHC-WORKFLOW", "trusted revision use differs")
    require(text.count("retention-days: 90") == 2, "OHC-WORKFLOW", "retention binding differs")
    require(text.count("uses: actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a") == 2, "OHC-WORKFLOW", "upload action count differs")
    require(text.count("uses: actions/download-artifact@3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c") == 1, "OHC-WORKFLOW", "download action count differs")
    for forbidden in ("gh release", "git tag", "nuget push", "packages: write", "contents: write", "continue-on-error"):
        require(forbidden not in text, "OHC-WORKFLOW", f"forbidden workflow capability: {forbidden}")


def self_test(repo: Path) -> None:
    validate_workflow(repo)
    with tempfile.TemporaryDirectory(prefix="o2-host-candidate-test-") as temporary:
        root = Path(temporary)
        binary = root / PAYLOAD
        ...
        try:
            ...
            mutations += 1
        require(mutations == 6, "OHC-SELF-TEST", f"archive mutation refusals differ: {mutations}")
        candidate = "a" * 40
        artifact_name = f"orchestration-host-linux-x64-{candidate}"
...
def main() -> None:
    ...
    elif args.command == "verify": verify_prepared(Path(args.prepared).resolve()); print("ORCHESTRATION_HOST_PREPARED_OK")
