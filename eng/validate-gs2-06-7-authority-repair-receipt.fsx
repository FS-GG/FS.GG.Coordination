...
let exact path expected (value: JsonElement) =
    ...
    if actual <> expected then
        fail "AR-SHAPE" $"%s{path}: expected %s{expectedText}; observed %s{actualText}"

let expect path expected actual =
    if actual <> expected then
        fail "AR-BINDING" $"%s{path}: expected %A{expected}; observed %A{actual}"

let noDuplicateMembers (bytes: byte array) =
    let mutable reader =
        Utf8JsonReader(ReadOnlySpan<byte>(bytes), JsonReaderOptions(CommentHandling = JsonCommentHandling.Disallow))
    ...
    while reader.Read() do
        ...
        | JsonTokenType.PropertyName ->
            if objects.Count = 0 || not (objects.Peek().Add(reader.GetString())) then
                fail "AR-DUPLICATE-MEMBER" "duplicate JSON member"
...
let readGitObject root revision relative =
    ...
    for argument in [ "show"; $"%s{revision}:%s{relative}" ] do
        start.ArgumentList.Add argument
...
let validateBytes root (bytes: byte array) =
    ...
    expect
        "previous.sha256"
        "37d36961589dbcd5db2b1d9deab09932dc9204fedaddb618eea5be6dfddbfd27"
        (string "sha256" previous)

    expect
        "previous.digest"
        "f0360e50a1c94262d8c1b83a12871276c8b1c58e6efceba14b49926dca00d45d"
        (string "digest" previous)

    expect
        "previous.implementationMerge"
        "286bde7afd607ac8e62a4ca71f6f82d363c052b4"
        (string "implementationMerge" previous)

    let previousBytes =
        File.ReadAllBytes(Path.Combine(root, "evidence/github-substrate-v2", string "path" previous))
    ...
    expect
        "previous.currentMerge"
        (string "implementationMerge" previous)
        (previousDocument.RootElement |> property "implementation" |> string "mergeSha")
    ...
    exact
        ...
        [
            ...
            "decisionDigest"
            "waitCommentId"
            "hostAcceptanceCommentId"
            "hostAcceptanceDigest"
        ]
        review
    ...
    expect
        "review.decisionUrl"
        "https://github.com/FS-GG/FS.GG.Coordination/pull/273#issuecomment-5521926908"
        (string "decisionUrl" review)

    expect
        "review.decisionDigest"
        "604e7c479baee9985e7d62df89bd3ed4732b079bd41ca118bc3a3120a6daaa54"
        (string "decisionDigest" review)
    ...
    expect
        "review.hostAcceptanceDigest"
        "ec7c842c2b1f09eaf6bcf8e4a49a99bd64ca35136dbd97d4b7fe05b866ce1e9c"
        (string "hostAcceptanceDigest" review)
    ...
    let validateRuns path expectedHead expected =
        ...
        let identities =
            runs
            |> List.map (fun run -> string "workflow" run, number "runId" run)
            |> Set.ofList
    ...
    validateRuns
        "exactHeadChecks"
        "0705405ff76eaf5cf34b627017a40240396616f9"
        (set [ "Bootstrap qualification", 33725627516L; "CodeQL", 33725618051L ])
    ...
    for name in [ "buildWarnings"; "buildErrors"; "unitFailed"; "architectureFailed" ] do
        expect $"qualification.%s{name}" 0L (number name qualification)

    for name, expected in
        [
            "unitPassed", 189L
            "architecturePassed", 466L
            "q3Controls", 23L
            "q7Controls", 12L
            "repositories", 10L
        ] do
        expect $"qualification.%s{name}" expected (number name qualification)

    expect "qualification.fleetEnabled" false (boolean "fleetEnabled" qualification)

    expect
        "qualification.seal"
        "34cc3b592c9a67d26814cea8768b1864071ff261204d56d42ef8d38bcfbe8ad2"
        (string "seal" qualification)
    ...
    expect
        ...
        (string "settingsSha256" authority)
    ...
    expect
        "authority.closure"
        [ "policy"; "coordination" ]
        (array "selectionClosure" authority |> List.map _.GetString())
    ...
    use request =
        JsonDocument.Parse(
            readGitObject root merge "evidence/github-substrate-v2/gs2-06-7/runtime-request-sentinel.json"
        )

    expect
        "authority.request.settings"
        (string "settingsSha256" authority)
        (string "settingsSha256" request.RootElement)

    use settings =
        JsonDocument.Parse(readGitObject root merge "eng/repository-settings/receipt.json")

    expect
        "authority.reviewed.settings"
        (string "settingsSha256" authority)
        (string "desiredSha256" settings.RootElement)
    ...
    let observationBytes =
        readGitObject root merge (Path.Combine("evidence/github-substrate-v2", string "path" observations))
    ...
    for field, expected in [ "repositories", 10L; "runs", 80L; "jobs", 305L ] do
        expect $"observations.%s{field}" expected (number field observations)
    ...
    for field, relative in
        [
            "analysisSha256", "readiness/262-workflow-selection/analysis.json"
            "workModelSha256", "readiness/262-workflow-selection/work-model.json"
            "verifySha256", "readiness/262-workflow-selection/verify.json"
            "shipVerdictSha256", "readiness/262-workflow-selection/ship-verdict.json"
        ] do
    ...
    for name in
        ...
        expect $"boundaries.%s{name}" false (boolean name boundaries)
...
let validate root =
    ...
    if JsonNode.Parse(bytes).ToJsonString() + "\n" <> Encoding.UTF8.GetString bytes then
        fail "AR-CANONICAL" receiptRelative

    validateBytes root bytes

    use index =
        JsonDocument.Parse(File.ReadAllBytes(Path.Combine(evidenceRoot, "index.json")))

    let entries =
        array "entries" index.RootElement
        |> List.filter (fun entry ->
            string "id" entry = "repair-GS2-06.7-272"
            || string "path" entry = receiptRelative)

    if entries.Length <> 1 then
        fail "AR-INDEX" "repair identity/path must be unique"
...
let selfTest root (baseline: byte array) =
    ...
    let reject name mutate =
        ...
        try
            validateBytes root (reseal node)
            fail "AR-SELF-TEST" $"%s{name} was accepted"
    ...
    let duplicate =
        Encoding.UTF8
            .GetString(baseline)
            .Replace("\"unitId\":\"GS2-06.7\",", "\"unitId\":\"GS2-06.7\",\"unitId\":\"GS2-06.7\",")
        |> utf8

    try
        validateBytes root duplicate
        fail "AR-SELF-TEST" "duplicate-member was accepted"
    with error when not (error.Message.StartsWith("AR-SELF-TEST", StringComparison.Ordinal)) ->
        ()
...
if selfTestRequested then
    selfTest root baseline

printfn
    "GS2_06_7_AUTHORITY_REPAIR_RECEIPT_OK repairId=GS2-06.7-repair-272 previous=37d36961589dbcd5db2b1d9deab09932dc9204fedaddb618eea5be6dfddbfd27 merge=588e1a4bcceeef1cc5a110c924aa52636f263b07 controls=%d"
    (if selfTestRequested then 9 else 0)
