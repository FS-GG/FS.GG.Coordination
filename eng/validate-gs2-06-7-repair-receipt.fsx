...
let exact (path: string) (expected: string list) (value: JsonElement) =
    ...
    if actual <> expected then
        fail "RR-SHAPE" $"%s{path}: expected %s{expectedText}; observed %s{actualText}"

let expect path expected actual =
    if actual <> expected then
        fail "RR-BINDING" $"%s{path}: expected %A{expected}; observed %A{actual}"
    ...
    while reader.Read() do
        ...
        | JsonTokenType.PropertyName ->
            if objects.Count = 0 || not (objects.Peek().Add(reader.GetString())) then
                fail "RR-DUPLICATE-MEMBER" "duplicate JSON member"
...
let validateBytes root (bytes: byte array) =
    ...
    expect
        "merge.tree"
        (string "candidateTreeSha" implementation)
        (runGit root [ "show"; "-s"; "--format=%T"; string "mergeSha" implementation ])
    ...
    expect
        ...
        "f6be2102b687b199853db43b5168addb170cf13684e97ae4ce714b021673fa30"
    ...
    expect
        ...
        "e2aa1ff5e147c063d93224459194b87422ae4bf288301862fa344f9d86ccc548"
    ...
    validateRuns
        "exactHeadChecks"
        "2da51ef3a77ac826e16141418232a46d786388c6"
        (set [ "Bootstrap qualification", 33716739336L; "CodeQL", 33716737131L ])

    validateRuns
        "protectedMainChecks"
        "286bde7afd607ac8e62a4ca71f6f82d363c052b4"
        (set [ "Bootstrap qualification", 33717657519L; "CodeQL", 33717657228L ])
    ...
    expect
        "qualification.seal"
        "1a7b8201130401a703c9f65812c23412330b9092e8ffe050b945e3a8d11de868"
    ...
    let observationBytes =
        readGitObject
            root
            implementationMerge
            (Path.Combine("evidence/github-substrate-v2", string "path" observations))
    ...
    expect
        "observations.runs"
        (repositories
         |> List.sumBy (fun repository -> array "runs" repository |> List.length)
         |> int64)
        (number "runs" observations)

    expect
        "observations.jobs"
        (repositories
         |> List.sumBy (fun repository ->
             array "runs" repository
             |> List.sumBy (fun run -> array "jobs" run |> List.length))
         |> int64)
        (number "jobs" observations)
...
let validate root =
    ...
    if JsonNode.Parse(bytes).ToJsonString() + "\n" <> Encoding.UTF8.GetString bytes then
        fail "RR-CANONICAL" receiptRelative

    validateBytes root bytes

    use policy =
        JsonDocument.Parse(File.ReadAllBytes(Path.Combine(evidenceRoot, "storage-policy.json")))

    let categories =
        array "categories" policy.RootElement
        |> List.filter (fun category -> string "name" category = "repair-receipts")

    if categories.Length <> 1 then
        fail "RR-CATEGORY" "repair category must be unique"
    ...
    let entries =
        ...
        |> List.filter (fun entry ->
            string "id" entry = "repair-GS2-06.7-268"
            || string "path" entry = receiptRelative)

    if entries.Length <> 1 then
        fail "RR-INDEX" "repair identity/path must be unique"
...
let selfTest (root: string) (baseline: byte array) =
    ...
    let reject (name: string) (mutate: JsonObject -> unit) =
        ...
        try
            validateBytes root candidate
            fail "RR-SELF-TEST" $"%s{name} was accepted"
    ...
    reject "qualification-omission" (fun node ->
        node["postMergeQualification"].AsObject().Remove("q7Controls") |> ignore)
    ...
    try
        ...
        fail "RR-SELF-TEST" "duplicate-member was accepted"
    with error when not (error.Message.StartsWith("RR-SELF-TEST", StringComparison.Ordinal)) ->
        ()
...
printfn
    "GS2_06_7_REPAIR_RECEIPT_OK repairId=GS2-06.7-repair-268 original=9a98a13213c9a6934b362a6cb75dc3b523800205961e76cd4de984157733dc0b merge=286bde7afd607ac8e62a4ca71f6f82d363c052b4 controls=%d"
    (if selfTestRequested then 7 else 0)
