...
let q0ManifestRelative =
    "work/2953-gh-modernization-m0-invariants/q0-corpus-originals.json"
...
let addString (target: JsonObject) (name: string) (value: string) =
    target.Add(name, JsonValue.Create value)

let addInt (target: JsonObject) (name: string) (value: int) =
    target.Add(name, JsonValue.Create value)
...
if sourceHead <> q0Revision then
    fail "FC-Q0-REVISION" $"expected {q0Revision}; observed {sourceHead}"
...
if sha256 q0ManifestBytes <> q0ManifestSha256 then
    fail "FC-Q0-MANIFEST" q0ManifestRelative

if sha256 q0EvidenceBytes <> q0EvidenceSha256 then
    fail "FC-Q0-EVIDENCE" q0EvidenceRelative
...
if stringProperty "sourceCommit" manifestRoot <> sourceCommit then
    fail "FC-SOURCE-COMMIT" "Q0 source commit differs"

let sourceEntries =
    manifestRoot.GetProperty("entries").EnumerateArray() |> Seq.toArray

let evidenceEntries =
    evidenceRootNode.GetProperty("corpus").EnumerateArray() |> Seq.toArray

if sourceEntries.Length <> 21 || evidenceEntries.Length <> 21 then
    fail "FC-COUNT" "Q0 corpus must contain exactly 21 rows"
...
for ordinal, sourceEntry in sourceEntries |> Array.indexed do
    ...
    if stringProperty "sourceRef" sourceEntry <> stringProperty "source" evidenceEntry then
        fail "FC-SOURCE-REF" id

    if
        stringProperty "sha256" sourceEntry
        <> stringProperty "originalBytesSha256" evidenceEntry
    then
        fail "FC-SOURCE-SHA" id

    let sourcePath = stringProperty "path" sourceEntry
    let expectedBlob = stringProperty "gitBlobSha1" sourceEntry

    let resolvedBlob =
        runGitText sourceRoot [ "rev-parse"; $"{sourceCommit}:{sourcePath}" ]

    if resolvedBlob <> expectedBlob then
        fail "FC-SOURCE-BLOB" id

    let payload = runGitBytes sourceRoot [ "cat-file"; "blob"; expectedBlob ]

    if payload.Length <> intProperty "byteLength" sourceEntry then
        fail "FC-SOURCE-LENGTH" id

    if sha256 payload <> stringProperty "sha256" sourceEntry then
        fail "FC-SOURCE-SHA" id

    if gitBlobSha1 payload <> expectedBlob then
        fail "FC-SOURCE-BLOB" id
    ...
    let expectedBehavior =
        JsonNode.Parse(evidenceEntry.GetProperty("expected").GetRawText())

    let expectedDecision =
        evidenceEntry.GetProperty("expected").GetProperty("decisionClass").GetString()
...
printfn
    "FROZEN_CORPUS_IMPORTED cases=%d observed=2 unobserved=19 q0=%s source=%s"
