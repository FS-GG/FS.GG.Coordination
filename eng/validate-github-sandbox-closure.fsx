...
let receipt =
    { unsignedReceipt with
        ResultDigest = GitHubSandboxClosure.resultDigest unsignedReceipt
    }

let sign (value: SandboxClosureReceipt) =
    { value with
        ResultDigest = GitHubSandboxClosure.resultDigest { value with ResultDigest = "" }
    }

let baselineGreen () =
    GitHubSandboxClosure.validateReceipt now plan receipt |> Result.isOk
...
let planRed changed =
    GitHubSandboxClosure.validatePlan now changed |> Result.isError

let receiptRed changed =
    GitHubSandboxClosure.validateReceipt now plan (sign changed) |> Result.isError

let replaceAt index replacement values =
    values
    |> List.mapi (fun current value -> if current = index then replacement value else value)
    ...
    |> List.map (fun control ->
        let red =
            ...
            | Control.ProductionTarget ->
                planRed
                    { plan with
                        Targets = { plan.Targets with Production = true }
                    }
            | Control.ProductionCredential ->
                planRed
                    { plan with
                        Authority =
                            { plan.Authority with
                                Credential = ProductionCapableHuman
                            }
                    }
            | Control.Quota ->
                planRed
            ...
            | Control.StaleFence ->
                receiptRed
                    { receipt with
                        Effects = effects |> replaceAt 0 (fun value -> { value with Disposition = Refused })
                    }
            | Control.ResponseUnknown ->
                receiptRed
                    { receipt with
                        Effects = effects |> replaceAt 1 (fun value -> { value with Disposition = Ambiguous })
                    }
            | Control.PartialCleanup ->
                receiptRed
                    { receipt with
                        Cleanup = cleanup |> replaceAt 0 (fun value -> { value with Disposition = Residual })
                    }
            | Control.ReceiptSubstitution ->
                receiptRed
                    { receipt with
                        CandidateSha = String.replicate 64 "b"
                    }
            | Control.WarmReuse ->
                receiptRed
                    { receipt with
                        ChildGates =
                            children
                            |> replaceAt 1 (fun value ->
                                { value with
                                    ProcessId = children.Head.ProcessId
                                })
                    }
            | Control.OmittedAdapter ->
                receiptRed
                    { receipt with
                        ChildGates = children |> List.tail
                    }
    ...
    |> List.map (fun control ->
        let red =
            ...
            | Control.ProductionIdentity ->
                planRed
                    { plan with
                        Authority = { plan.Authority with ActorId = "" }
                    }
            | Control.ProductionTarget ->
                planRed
                    { plan with
            ...
            | Control.StaleFence ->
                receiptRed
                    { receipt with
                        Effects =
                            effects
                            |> replaceAt 2 (fun value ->
                                { value with
                                    AuthoritativeRevision = None
                                })
                    }
            | Control.ResponseUnknown ->
                receiptRed
                    { receipt with
                        Effects = effects |> replaceAt 3 (fun value -> { value with Disposition = Unavailable })
                    }
            | Control.PartialCleanup ->
                receiptRed
                    { receipt with
                        Cleanup = cleanup |> List.tail
                    }
            | Control.ReceiptSubstitution ->
                receiptRed
                    { receipt with
                ...
                receiptRed
                    { receipt with
                        ChildGates =
                            ...
                            |> replaceAt 2 (fun value ->
                                { value with
                                    RunNonce = "prior-run-nonce-0000"
                                })
                    }
            | Control.OmittedAdapter ->
                receiptRed
                    { receipt with
                        ChildGates = children |> List.take 7
                    }
...
| Ok() ->
    printfn
        "github-sandbox-closure-contract OK controls=%d q=Q4 network=offline provenance=synthetic baseline=green"
        generated.Length
| Error findings ->
    findings
